...
class PerformanceProfiler:
    ...
    def record_metric(self, metric: PerformanceMetric) -> None: with self._lock:
            self.metrics[metric.operation].append(metric)

    def get_stats(self, operation: str) -> Optional[PerformanceStats]: with self._lock:
    ...
    def get_all_stats(self) -> Dict[str, PerformanceStats]: with self._lock:
            return {operation: self.get_stats(operation) for operation in self.metrics.keys() if self.get_stats(operation) is not None}

    def get_slowest_operations(self, limit: int=10) -> List[PerformanceStats]: all_stats = self.get_all_stats()
        return sorted(all_stats.values(), key=lambda s: s.avg_duration_ms, reverse=True)[:limit]

    def get_most_frequent_operations(self, limit: int=10) -> List[PerformanceStats]: all_stats = self.get_all_stats()
        return sorted(all_stats.values(), key=lambda s: s.count, reverse=True)[:limit]

    def record_error(self, operation: str) -> None: with self._lock:
            self.error_counts[operation] += 1

    def clear_metrics(self, operation: Optional[str]=None) -> None: with self._lock:
            if operation:
    ...
    @staticmethod
    def _percentile(data: List[float], percentile: float) -> float: if not data:
            return 0.0
    ...
    @contextmanager
    def profile_operation(self, operation: str, context: Optional[Dict[str, Any]]=None) -> Generator[Any, None, None]: start_time = time.perf_counter()
    ...
    @asynccontextmanager
    async def profile_async_operation(self, operation: str, context: Optional[Dict[str, Any]]=None) -> AsyncGenerator[Any, None]: start_time = time.perf_counter()
        start_memory = self._process.memory_info().rss / 1024 / 1024
        start_cpu = self._process.cpu_percent()
        try:
            yield
...
class APIPerformanceMonitor:
        ...
        def decorator(func) -> Any:
            ...
            if asyncio.iscoroutinefunction(func):
                ...
                async def async_wrapper(*args, **kwargs) -> Dict[str, Any]:
                    operation = f'{method} {endpoint}' context = {'endpoint': endpoint, 'method': method, 'args_count': len(args), 'kwargs_count': len(kwargs)}
    ...
    def _update_endpoint_stats(self, endpoint: str, method: str): key = f'{method} {endpoint}'
    ...
    def get_endpoint_performance_report(self) -> Dict[str, Any]: all_stats = self.profiler.get_all_stats()
...
class DatabasePerformanceMonitor:
        ...
        def decorator(func) -> Any:
            ...
            if asyncio.iscoroutinefunction(func):
                ...
                async def async_wrapper(*args, **kwargs) -> Any:
                    operation = f'db.{query_type}.{table}' context = {'query_type': query_type, 'table': table, 'database': 'postgresql'}
                    async with self.profiler.profile_async_operation(operation, context):
    ...
    def get_slow_queries_report(self, threshold_ms: float=100.0) -> List[Dict[str, Any]]: all_stats = self.profiler.get_all_stats()
        slow_queries = []

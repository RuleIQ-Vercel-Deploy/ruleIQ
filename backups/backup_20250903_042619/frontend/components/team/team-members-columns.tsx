      ...
      return (
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col">
            <span className="text-oxford-blue font-medium">{name}</span>
            <span className="text-oxford-blue/70 text-sm">{email}</span>
          </div>
        </div>
      );
    },
    ...
    cell: ({ row }) => {
      ...
      return (
        <Select defaultValue={role}>
          <SelectTrigger className="bg-eggshell-white/50 border-oxford-blue/30 h-9 w-40 focus:ring-gold">
            <SelectValue>
          ...
          <SelectContent className="bg-oxford-blue text-eggshell-white border-gold/50">
            {roles.map((r) => (
              <SelectItem
                key={r.value}
                value={r.value}
                className="focus:bg-gold/20 data-[highlighted]:bg-gold/20"
              >
                <div className="flex items-center gap-2">
    ...
    cell: ({ row }) => {
      ...
      return (
        <DropdownMenu>
          ...
          <DropdownMenuContent
            align="end"
            className="bg-eggshell-white text-oxford-blue border-oxford-blue/20"
          >
            ...
            <DropdownMenuItem className="text-error focus:bg-error/10 focus:text-error">

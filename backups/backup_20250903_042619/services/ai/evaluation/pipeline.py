...
class EvaluationPipeline:
    ...
    async def execute_task(self, task: EvaluationTask) ->EvaluationResult:
        ...
        try:
            if task.task_type not in self.evaluators:
                raise ValueError(
                    f'No evaluator registered for type: {task.task_type}')
            evaluator = self.evaluators[task.task_type]
            result_metrics = await self._execute_with_retry(evaluator, task,
                self.config.retry_attempts)
            task.complete(result_metrics)
            result = EvaluationResult(task_id=task.task_id, success=True,
                metrics=result_metrics, duration=(datetime.now(timezone.utc
            ...
            error_msg = (
                f'Task {task.task_id} exceeded timeout of {self.config.timeout_seconds}s',
                )
            task.fail(error_msg)
            return EvaluationResult(task_id=task.task_id, success=False,
                error=error_msg, duration=(datetime.now(timezone.utc) -
                start_time).total_seconds())
    ...
    async def _execute_with_retry(self, evaluator: Callable, task:
        EvaluationTask, attempts: int) ->Dict[str, Any]:
        """Execute evaluator with retry logic."""
        last_error = None
        for attempt in range(attempts):
            try:
                result = await asyncio.wait_for(evaluator(task), timeout=
                ...
                if attempt < attempts - 1:
                    logger.warning('Attempt %s failed for task %s: %s' % (
                        attempt + 1, task.task_id, e))
                    await asyncio.sleep(2 ** attempt)
        raise last_error

    async def execute_parallel(self, tasks: List[EvaluationTask]) ->List[
        EvaluationResult]:
        ...
        results = await asyncio.gather(*[_execute_with_semaphore(task) for
            task in tasks], return_exceptions=True)
        final_results = []
        for i, result in enumerate(results):
            if isinstance(result, Exception):
                final_results.append(EvaluationResult(task_id=tasks[i].
                    task_id, success=False, error=str(result)))
            else:
...
class MetricAggregator:
    ...
    def aggregate(self, results: List[EvaluationResult], weighted: bool=False
        ) ->Dict[str, Any]:
        ...
        aggregated = {'_success_rate': len(successful) / len(results) if
            results else 0, '_total_results': len(results),
            '_successful_results': len(successful), '_failed_results': len(
            failed)}
        if not successful:
            return aggregated
        metric_values = {}
        ...
        for metric, values in metric_values.items():
            ...
            aggregated[metric] = {'mean': float(np.mean(values_array)),
                'std': float(np.std(values_array)), 'min': float(np.min(
                values_array)), 'max': float(np.max(values_array)),
                'median': float(np.median(values_array)), 'count': len(values)}
        ...
        for metric, baseline_value in baseline.items():
            if metric in current:
                ...
                if change < -self.threshold:
                    regressions.append({'metric': metric, 'baseline':
                        baseline_value, 'current': current_value, 'change':
                        change, 'threshold': self.threshold})
        return regressions

    def analyze_trend(self, history: List[Dict[str, Any]], metric: str) ->Dict[
        str, Any]:
...
class BaselineComparator:
    ...
    def compare(self, baseline: Dict[str, Dict[str, float]], current: Dict[
        str, Dict[str, float]], test_significance: bool=False,
        ...
        for metric in baseline:
            ...
            if baseline_mean != 0:
                change_pct = (current_mean - baseline_mean
                    ) / baseline_mean * 100
            else:
                change_pct = float('inf') if current_mean != 0 else 0
            comparison[metric] = {'baseline': baseline_mean, 'current':
                current_mean, 'change': current_mean - baseline_mean,
                'change_pct': change_pct, 'improved': current_mean >
                baseline_mean}
            if test_significance:
                p_value = self._test_significance(baseline_stats, current_stats,
                    )
                comparison[metric]['p_value'] = p_value
                comparison[metric]['significant'
                    ] = p_value < 1 - confidence_level
            if (confidence_level and 'std' in current_stats and 'n' in
                current_stats):
                ci_lower, ci_upper = self._confidence_interval(current_mean -
                    baseline_mean, baseline_stats.get('std', 0),
                    current_stats['std'], baseline_stats.get('n', 1),
                    current_stats['n'], confidence_level)
                comparison[metric]['ci_lower'] = ci_lower
                comparison[metric]['ci_upper'] = ci_upper
        return comparison

    def _test_significance(self, baseline: Dict[str, float], current: Dict[
        str, float]) ->float:
        ...
        se = np.sqrt(baseline_std ** 2 / baseline_n + current_std ** 2 /
            current_n)
        if se == 0:
            return 1.0
        t_stat = (current_mean - baseline_mean) / se
        df = (baseline_std ** 2 / baseline_n + current_std ** 2 / current_n
            ) ** 2 / ((baseline_std ** 2 / baseline_n) ** 2 / (baseline_n -
            1) + (current_std ** 2 / current_n) ** 2 / (current_n - 1))
        p_value = 2 * (1 - stats.t.cdf(abs(t_stat), df))
        return float(p_value)

    def _confidence_interval(self, diff: float, std1: float, std2: float,
        n1: int, n2: int, confidence: float) ->Tuple[float, float]:
...
class PipelineOrchestrator:
    ...
    async def run_evaluation(self, tasks: List[EvaluationTask]) ->Dict[str, Any
        ...
        evaluation_result = {'timestamp': datetime.now(timezone.utc).
            isoformat(), 'summary': {'total_tasks': len(tasks),
            'successful_tasks': sum(1 for r in results if r.success),
            'failed_tasks': sum(1 for r in results if not r.success),
            'duration': (datetime.now(timezone.utc) - start_time).
            total_seconds()}, 'metrics': aggregated, 'details': results}
        if self.baseline:
            evaluation_result['comparison'] = self.comparator.compare(self.
                baseline, aggregated, test_significance=True)
            current_metrics = {k: v['mean'] for k, v in aggregated.items() if
                isinstance(v, dict) and 'mean' in v}
            baseline_metrics = {k: v['mean'] for k, v in self.baseline.
                items() if isinstance(v, dict) and 'mean' in v}
            regressions = self.regression_detector.detect(baseline_metrics,
                current_metrics)
            evaluation_result['regressions'] = regressions
        return evaluation_result

    async def schedule_evaluation(self, tasks: List[EvaluationTask],
        interval_seconds: float, max_runs: Optional[int]=None) ->None:

...
class GapAnalysisTool(BaseTool): 
    ...
    def get_function_schema(self) ->Dict[str, Any]: return {'name': 'extract_compliance_gaps', 'description':
            ...
            'items': {'type': 'object', 'properties': {'section': {'type':
            'string', 'description':
            "Compliance framework section (e.g., 'GDPR Article 25', 'ISO 27001 A.5.1')"
            }, 'severity': {'type': 'string', 'enum': ['low', 'medium',
            'high', 'critical'], 'description': 'Severity level of the gap'
            }, 'description': {'type': 'string', 'description':
            'Detailed description of the compliance gap'}, 'impact': {
            'type': 'string', 'description':
            'Business impact of not addressing this gap'}, 'current_state':
            {'type': 'string', 'description':
            'Current state of compliance for this requirement'},
            'target_state': {'type': 'string', 'description':
            'Required state to achieve compliance'}}, 'required': [
            'section', 'severity', 'description', 'impact', 'current_state',
            'target_state']}}, 'overall_risk_level': {'type': 'string',
            'enum': ['low', 'medium', 'high', 'critical'], 'description':
            'Overall risk level based on identified gaps'},
            'priority_order': {'type': 'array', 'items': {'type': 'string'},
            'description':
            'Recommended order for addressing gaps (section identifiers)'},
            'estimated_effort': {'type': 'string', 'description':
            'Estimated total effort to address all gaps'}}, 'required': [
    ...
    async def execute(self, parameters: Dict[str, Any], context: Optional[
        Dict[str, Any]]=None) ->ToolResult: try:
            ...
            for i, gap_data in enumerate(gaps_data):
                gap = ComplianceGap(id=f'gap_{i + 1}', section=gap_data.get
                    ('section', ''), severity=gap_data.get('severity',
                    'medium'), description=gap_data.get('description', ''),
                    impact=gap_data.get('impact', ''), current_state=
                    gap_data.get('current_state', ''), target_state=
                    gap_data.get('target_state', ''), priority=self.
                    _calculate_priority(gap_data.get('severity', 'medium'), i))
                processed_gaps.append(gap.to_dict())
            processed_gaps.sort(key=lambda x: x['priority'], reverse=True)
            result_data = {'gaps': processed_gaps, 'gap_count': len(
                processed_gaps), 'severity_breakdown': self.
                _analyze_severity_breakdown(processed_gaps),
                'overall_risk_level': overall_risk, 'priority_order':
                priority_order, 'estimated_effort': estimated_effort,
            ...
            return ToolResult(success=True, data=result_data, metadata={
                'tool_type': 'gap_analysis', 'gaps_processed': len(
                processed_gaps), 'risk_level': overall_risk})
        except Exception as e:
            logger.error('Gap analysis failed: %s' % e)
            return ToolResult(success=False, error=
                f'Gap analysis execution failed: {e!s}')

    def _calculate_priority(self, severity: str, index: int) ->int: severity_scores = {'critical': 100, 'high': 75, 'medium': 50, 'low': 25
            }
    ...
    def _generate_gap_recommendations(self, gaps: List[Dict[str, Any]]) ->List[
        ...
        if any('GDPR' in section for section in sections):
            recommendations.append(
                'Focus on GDPR compliance as regulatory priority')
...
class RecommendationGenerationTool(BaseTool): 
    ...
    def get_function_schema(self) ->Dict[str, Any]: return {'name': 'generate_compliance_recommendations',
            ...
            'array', 'description': 'List of compliance recommendations',
            'items': {'type': 'object', 'properties': {'title': {'type':
            'string', 'description': 'Title of the recommendation'},
            'description': {'type': 'string', 'description':
            'Detailed description of what needs to be implemented'},
            'priority': {'type': 'string', 'enum': ['low', 'medium', 'high',
            'critical'], 'description': 'Priority level for implementation'
            }, 'implementation_effort': {'type': 'string', 'description':
            "Estimated effort (e.g., '2-4 weeks', '1-2 days')"},
            'cost_impact': {'type': 'string', 'enum': ['low', 'medium',
            'high'], 'description': 'Expected cost impact'}, 'timeline': {
            'type': 'string', 'description':
            'Recommended implementation timeline'}, 'dependencies': {'type':
            'array', 'items': {'type': 'string'}, 'description':
            'Dependencies that must be addressed first'},
            'resources_required': {'type': 'array', 'items': {'type':
            'string'}, 'description': 'Resources needed for implementation'
            }}, 'required': ['title', 'description', 'priority',
            'implementation_effort', 'cost_impact', 'timeline']}},
            'implementation_roadmap': {'type': 'object', 'properties': {
            'phase_1': {'type': 'array', 'items': {'type': 'string'},
            'description': 'Immediate actions (0-3 months)'}, 'phase_2': {
            'type': 'array', 'items': {'type': 'string'}, 'description':
            'Short-term actions (3-6 months)'}, 'phase_3': {'type': 'array',
            'items': {'type': 'string'}, 'description':
            'Long-term actions (6+ months)'}}, 'required': ['phase_1',
            'phase_2', 'phase_3']}, 'budget_estimate': {'type': 'object',
            'properties': {'low_estimate': {'type': 'string'},
            'high_estimate': {'type': 'string'}, 'key_cost_drivers': {
            'type': 'array', 'items': {'type': 'string'}}}, 'required': [
            'low_estimate', 'high_estimate', 'key_cost_drivers']}},
            'required': ['recommendations', 'implementation_roadmap',
            'budget_estimate']}}
    ...
    async def execute(self, parameters: Dict[str, Any], context: Optional[
        Dict[str, Any]]=None) ->ToolResult: try:
            ...
            for i, rec_data in enumerate(recommendations_data):
                recommendation = ComplianceRecommendation(id=f'rec_{i + 1}',
                    title=rec_data.get('title', ''), description=rec_data.
                    get('description', ''), priority=rec_data.get(
                    'priority', 'medium'), implementation_effort=rec_data.
                    get('implementation_effort', 'Unknown'), cost_impact=
                    rec_data.get('cost_impact', 'medium'), timeline=
                    rec_data.get('timeline', 'TBD'), dependencies=rec_data.
                    get('dependencies', []), resources_required=rec_data.
                    get('resources_required', []))
                processed_recommendations.append(recommendation.to_dict())
            priority_order = {'critical': 4, 'high': 3, 'medium': 2, 'low': 1}
            processed_recommendations.sort(key=lambda x: priority_order.get
                (x['priority'], 2), reverse=True)
            result_data = {'recommendations': processed_recommendations,
                'recommendation_count': len(processed_recommendations),
                'priority_breakdown': self._analyze_priority_breakdown(
                processed_recommendations), 'implementation_roadmap':
                roadmap, 'budget_estimate': budget, 'analysis_timestamp':
                datetime.now().isoformat(), 'quick_wins': self.
                _identify_quick_wins(processed_recommendations),
            ...
            return ToolResult(success=True, data=result_data, metadata={
                ...
                'high_priority_count': sum(1 for r in
                processed_recommendations if r['priority'] in ['critical',
                'high'])})
        except Exception as e:
    ...
    def _identify_quick_wins(self, recommendations: List[Dict[str, Any]]
        ...
        for rec in recommendations:
            ...
            if any(term in effort for term in ['day', 'week', 'quick', 'easy']
                ) and cost == 'low':
                quick_wins.append({'title': rec['title'], 'effort': rec[
    ...
    def _identify_major_initiatives(self, recommendations: List[Dict[str, Any]]
        ...
        for rec in recommendations:
            ...
            if any(term in effort for term in ['month', 'quarter', 'major',
                'significant']) or priority == 'critical':
                major_initiatives.append({'title': rec['title'], 'priority':

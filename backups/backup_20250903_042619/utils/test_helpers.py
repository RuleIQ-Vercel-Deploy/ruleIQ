...
async def async_with_timeout(coro, timeout: float): Execute an async operation with a timeout.
...
def sync_with_timeout(func: Callable, timeout: float, *args, **kwargs): Execute a synchronous function with a timeout.
...
def with_db_cleanup(async_func): Decorator to ensure database connections are cleaned up.
...
def with_redis_cleanup(async_func): Decorator to ensure Redis connections are cleaned up.
...
async def cleanup_all_connections(connections: Dict[str, Any]): Clean up multiple connections.
...
def with_test_timeout(seconds: int): Decorator to add timeout to test functions.
    ...
    def decorator(test_func):
        ...
        if asyncio.iscoroutinefunction(test_func):
            ...
            async def async_wrapper(*args, **kwargs):
                return await async_with_timeout( test_func(*args, **kwargs),
                    timeout=seconds,
                )
...
# Smart mock creation
def create_smart_mock_ai_client(): Create a mock AI client that returns context-specific responses.
...
def create_smart_mock_openai(): Create a mock OpenAI client with intelligent responses.
...
def get_redis_config() -> Dict[str, Any]: Get Redis configuration from environment.
...
def get_postgres_config() -> Dict[str, Any]: Get PostgreSQL configuration from environment.
...
@asynccontextmanager
async def isolated_test_context(): Provide an isolated context for test execution.
...
@contextmanager
def timeout_context(seconds: float): Context manager for timeout handling.

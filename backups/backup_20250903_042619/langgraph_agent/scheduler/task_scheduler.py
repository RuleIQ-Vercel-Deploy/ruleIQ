...
class TaskScheduler:
    ...
    def _setup_checkpointer(self):
        ...
        try:
            ...
            if 'neon.tech' not in db_url:
                logger.warning('Database URL does not appear to be a Neon database. Using standard PostgreSQL connection.')
            conn = psycopg.connect(db_url, autocommit=True, row_factory=dict_row, options='-c statement_timeout=30s -c idle_session_timeout=300s')
    ...
    def _build_graph(self):
        ...
        workflow.add_conditional_edges('scheduler', self._route_task, {'evidence': 'evidence_collector', 'compliance': 'compliance_updater', 'reports': 'report_generator', 'notifications': 'notification_sender', 'cleanup': 'cleanup', 'error': 'error_handler', 'end': END})
        for node in ['evidence_collector', 'compliance_updater', 'report_generator', 'notification_sender', 'cleanup']:
            workflow.add_conditional_edges(node, self._check_task_result, {'scheduler': 'scheduler', 'error': 'error_handler', 'end': END})
        workflow.add_conditional_edges('error_handler', lambda state: 'scheduler' if state.get('retry_count', 0) < 3 else END, {'scheduler': 'scheduler', END: END})
    ...
    def _register_default_tasks(self):
        ...
        default_tasks = [ScheduledTask(task_id='daily-evidence-collection', name='Collect All Integration Evidence', node_name='evidence_collector', schedule='0 2 * * *', priority=TaskPriority.HIGH, queue='evidence'), ScheduledTask(task_id='process-pending-evidence', name='Process Pending Evidence', node_name='evidence_collector', schedule='0 * * * *', priority=TaskPriority.MEDIUM, queue='evidence'), ScheduledTask(task_id='update-compliance-scores', name='Update All Compliance Scores', node_name='compliance_updater', schedule='0 3 * * 1', priority=TaskPriority.HIGH, queue='compliance'), ScheduledTask(task_id='check-evidence-expiry', name='Check Evidence Expiry', node_name='evidence_collector', schedule='0 1 * * *', priority=TaskPriority.MEDIUM, queue='evidence'), ScheduledTask(task_id='sync-integration-status', name='Sync Integration Status', node_name='evidence_collector', schedule='0 */6 * * *', priority=TaskPriority.LOW, queue='evidence'), ScheduledTask(task_id='generate-compliance-reports', name='Generate Daily Compliance Reports', node_name='report_generator', schedule='0 4 * * *', priority=TaskPriority.MEDIUM, queue='compliance'), ScheduledTask(task_id='cleanup-old-reports', name='Cleanup Old Reports', node_name='cleanup', schedule='0 5 * * 0', priority=TaskPriority.BACKGROUND, queue='reports'), ScheduledTask(task_id='send-report-summaries', name='Send Monthly Report Summaries', node_name='notification_sender', schedule='0 6 1 * *', priority=TaskPriority.LOW, queue='reports')]
    ...
    async def _scheduler_node(self, state: EnhancedComplianceState) -> Dict[str, Any]:
        ...
        if next_task:
            ...
            return {'current_task': next_task.task_id, 'task_metadata': {'task_id': next_task.task_id, 'name': next_task.name, 'node': next_task.node_name, 'queue': next_task.queue, 'priority': next_task.priority.value}}
    ...
    async def _evidence_collector_node(self, state: EnhancedComplianceState) -> Dict[str, Any]:
        ...
        try:
            ...
            return {'task_result': 'success', 'evidence_collected': True, 'node_execution_times': {'evidence_collector': datetime.now().isoformat()}}
        except Exception as e:
            logger.error(f'Evidence collection failed: {e}')
            if task_id in self.tasks:
                self.tasks[task_id].status = TaskStatus.FAILED
                self.tasks[task_id].retry_count += 1
            return {'task_result': 'error', 'error': str(e), 'error_count': state.get('error_count', 0) + 1}
    ...
    async def _compliance_updater_node(self, state: EnhancedComplianceState) -> Dict[str, Any]:
        ...
        try:
            ...
            if task_id in self.tasks:
                self.tasks[task_id].status = TaskStatus.COMPLETED
            return {'task_result': 'success', 'compliance_updated': True, 'node_execution_times': {'compliance_updater': datetime.now().isoformat()}}
        except Exception as e:
    ...
    async def _report_generator_node(self, state: EnhancedComplianceState) -> Dict[str, Any]:
        ...
        try:
            ...
            if task_id in self.tasks:
                self.tasks[task_id].status = TaskStatus.COMPLETED
            return {'task_result': 'success', 'report_generated': True, 'node_execution_times': {'report_generator': datetime.now().isoformat()}}
        except Exception as e:
    ...
    async def _notification_sender_node(self, state: EnhancedComplianceState) -> Dict[str, Any]:
        ...
        try:
            ...
            if task_id in self.tasks:
                self.tasks[task_id].status = TaskStatus.COMPLETED
            return {'task_result': 'success', 'notification_sent': True, 'node_execution_times': {'notification_sender': datetime.now().isoformat()}}
        except Exception as e:
    ...
    async def _cleanup_node(self, state: EnhancedComplianceState) -> Dict[str, Any]:
        ...
        try:
            ...
            if task_id in self.tasks:
                self.tasks[task_id].status = TaskStatus.COMPLETED
            return {'task_result': 'success', 'cleanup_completed': True, 'node_execution_times': {'cleanup': datetime.now().isoformat()}}
        except Exception as e:
    ...
    def health_check(self) -> Dict[str, Any]:
        ...
        return {'status': 'healthy', 'total_tasks': len(self.tasks), 'pending_tasks': sum((1 for t in self.tasks.values() if t.status == TaskStatus.PENDING)), 'running_tasks': sum((1 for t in self.tasks.values() if t.status == TaskStatus.RUNNING)), 'failed_tasks': sum((1 for t in self.tasks.values() if t.status == TaskStatus.FAILED)), 'checkpointer_status': 'connected' if self.checkpointer else 'disconnected'}
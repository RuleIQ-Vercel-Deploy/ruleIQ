...
class InputValidators:
    ...
    @staticmethod
    def sanitize_html(value: str) ->str: allowed_tags = ['b', 'i', 'u', 'em', 'strong', 'p', 'br', 'ul',
            'li', 'ol']
    ...
    @staticmethod
    def validate_safe_string(value: str, max_length: int=255) ->str: if len(value) > max_length:
            raise ValueError(f'String exceeds maximum length of {max_length}')
        cleaned = bleach.clean(value, tags=[], strip=True)
        return cleaned.strip()

    @staticmethod
    def validate_company_name(value: str) ->str: if not value or len(value.strip()) < 2:
    ...
    @staticmethod
    def validate_url(url: str) ->str: url_regex = re.compile(
    ...
    @staticmethod
    def validate_employee_count(value: int) ->int: if value < 1:

...
class ReportParameters(BaseModel): frameworks: Optional[List[str]] = Field(None, description='List of compliance frameworks to include.')
...
class ScheduleConfig(BaseModel): day_of_week: Optional[int] = Field(None, ge=0, le=6, description='Day of the week for weekly reports (0=Monday, 6=Sunday).')
...
class UpdateScheduleRequest(BaseModel):
    frequency: Optional[ReportFrequency] = None recipients: Optional[List[str]] = None
...
class ScheduleListResponse(BaseModel):
    schedules: List[ScheduleResponse] total: int
...
class ReportHistoryResponse(BaseModel):
    reports: List[ReportHistoryItem] total: int
    page: int
    per_page: int

...
async def login(request: Request, login_data: LoginRequest, db: Session=
    Depends(get_db)) ->Dict[str, Any]: ip_address = request.client.host if request.client else 'unknown'
    user_agent = request.headers.get('user-agent', 'unknown')
...
async def refresh_token(refresh_request: dict, db: Session=Depends(get_db)
    ) ->Dict[str, Any]: from api.dependencies.auth import decode_token
...
async def get_current_user(db: Session=Depends(get_db), token: str=Depends(
    oauth2_scheme)) ->Dict[str, Any]: from api.dependencies.auth import decode_token
...
async def assign_default_role(db: Session=Depends(get_db), token: str=
    Depends(oauth2_scheme)) ->Dict[str, Any]: Assign the business_user role to the current authenticated user.

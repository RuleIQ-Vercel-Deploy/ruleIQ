...
class RedisRateLimiter:
    ...
    async def _get_redis_client(self) ->redis.Redis: if self.redis_client is None:
            self.redis_client = redis.from_url(self.redis_url,
                decode_responses=True, socket_connect_timeout=5,
                socket_timeout=5, retry_on_timeout=True)
        return self.redis_client

    async def check_rate_limit(self, identifier: str) ->Tuple[bool, int]: Check if request is within rate limit.
    ...
    async def get_remaining_requests(self, identifier: str) ->int: try:
            redis_client = await self._get_redis_client()
            key = f'{self.key_prefix}:{identifier}'
            current_time = int(time.time())
            window_start = current_time - self.window_seconds
    ...
    async def reset_limit(self, identifier: str) ->bool: try:
            redis_client = await self._get_redis_client()
            key = f'{self.key_prefix}:{identifier}'
    ...
    async def close(self) ->None: if self.redis_client:
...
async def redis_rate_limit_middleware(request: Request, call_next) ->Any: skip_paths = ['/docs', '/redoc', '/openapi.json', '/health', '/ready']

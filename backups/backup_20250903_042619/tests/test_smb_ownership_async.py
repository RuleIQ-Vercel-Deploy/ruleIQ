...
@pytest.fixture
async def test_user(setup_db): async with TestingSessionLocal() as db:
...
@pytest.fixture
async def other_user(setup_db): async with TestingSessionLocal() as db:
...
@pytest.fixture
def auth_headers(test_user): token = create_access_token({'sub': str(test_user.id)})
    return {'Authorization': f'Bearer {token}'}


@pytest.fixture
def other_auth_headers(other_user): token = create_access_token({'sub': str(other_user.id)})
    return {'Authorization': f'Bearer {token}'}
...
class TestSMBOwnership:
    ...
    async def test_create_business_profile(self, client, auth_headers): profile_data = {'company_name': 'Small Business LLC', 'industry':
    ...
    async def test_get_own_profile(self, client, auth_headers): profile_data = {'company_name': 'My Small Business', 'industry':
    ...
    async def test_no_auth_rejected(self, client): response = await client.get('/api/v1/business-profiles/')
        assert response.status_code == HTTP_UNAUTHORIZED
        response = await client.post('/api/v1/business-profiles/', json={
            'company_name': 'Test'})
        assert response.status_code == HTTP_UNAUTHORIZED

    async def test_expired_token_rejected(self, client, test_user): expired_token = create_access_token({'sub': str(test_user.id)},

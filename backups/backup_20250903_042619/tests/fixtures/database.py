...
def setup_test_environment(): # Set test environment variables
...
@pytest.fixture(scope="session")
def test_db_engine(): manager = get_test_db_manager()
...
@pytest.fixture(scope="session")
def SessionLocal(test_db_engine): return scoped_session(
        sessionmaker(
            autocommit=False,
            autoflush=False,
            bind=test_db_engine
...
@pytest.fixture
def db_session(SessionLocal, test_db_engine) -> Generator[Session, None, None]: Provide a transactional database session for tests.
...
@pytest.fixture
async def async_db_session() -> AsyncGenerator[AsyncSession, None]: Provide an async database session for async tests.
    """
    manager = get_test_db_manager()
...
@pytest.fixture
def clean_db(db_session): Ensure a clean database state for tests that need it.
...
@pytest.fixture
def sample_user(db_session): from database import User
...
@pytest.fixture
def sample_business_profile(db_session, sample_user): from database import BusinessProfile
...
@pytest.fixture
def authenticated_user(db_session, sample_user): from utils.auth import create_access_token
...
@pytest.fixture
def mock_redis_client(monkeypatch): from unittest.mock import MagicMock

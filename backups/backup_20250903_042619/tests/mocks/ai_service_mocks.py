...
class MockComplianceAssistant:
    ...
    def __init__(self, db_session=None, fail_rate: float = 0.0, delay_ms: int = 100): Initialize mock with configurable failure rate and response delay
    ...
    async def _simulate_delay(self): if self.delay_ms > 0:
            await asyncio.sleep(self.delay_ms / 1000)

    def _should_fail(self) -> bool: import random
    ...
    ) -> Dict[str, Any]: await self._simulate_delay()
        self.call_count += 1
    ...
    def _get_related_topics(self, framework_id: str) -> List[str]: topics_map = {
            "gdpr": [
                "Data Protection",
                "Privacy Rights",
                "Consent Management",
    ...
    def _get_follow_up_suggestions(self, question_text: str) -> List[str]: question_lower = question_text.lower()
    ...
    def _get_source_references(self, framework_id: str) -> List[str]: references_map = {
    ...
    def _identify_gaps(self, answers: Dict, framework_id: str) -> List[Dict]: gaps = []
    ...
    def _assess_risk_level(self, completion: float, answers: Dict) -> Dict: risk_score = max(0, 100 - completion)
    ...
    def _generate_insights(self, answers: Dict, framework_id: str) -> List[str]: insights = [
    ...
    def _suggest_evidence(self, answers: Dict, framework_id: str) -> List[Dict]: evidence = [
            {
    ...
    def _create_implementation_plan(self, timeline: str) -> Dict: weeks_map = {"urgent": 2, "standard": 8, "extended": 16}
    ...
    def _define_success_metrics(self, gaps: List[Dict]) -> List[Dict]: return [
...
class MockAIServiceFailures:
    ...
    @staticmethod
    def timeout_service(): mock = MockComplianceAssistant(fail_rate=1.0)
    ...
    @staticmethod
    def quota_exceeded_service(): mock = MockComplianceAssistant(fail_rate=1.0)
    ...
    @staticmethod
    def content_filter_service(): mock = MockComplianceAssistant(fail_rate=1.0)

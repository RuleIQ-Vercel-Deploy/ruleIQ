...
class TestAPIResponseTimes:
    ...
    def _get_test_data_for_endpoint(self, endpoint: str) ->dict: test_data_map = {'/api/assessments': {'session_type':
...
class TestAIGenerationPerformance:
    ...
    def _get_ai_test_data(self, endpoint: str) ->dict: ai_test_data = {'/api/policies/generate': {'framework_id':
...
class TestConcurrentUserLoad:
    ...
    def test_memory_usage_under_load(self, client, authenticated_headers): process = psutil.Process()
...
class TestSoakTesting:
    ...
    def test_extended_operation_stability(self, client, authenticated_headers): start_time = time.time()
        test_duration = 300
        error_count = 0
        request_count = 0
        response_times = []
    ...
    def test_memory_leak_detection(self, client, authenticated_headers): process = psutil.Process()
        memory_samples = []
        for _i in range(20):
            for _ in range(10):
                client.get('/api/frameworks', headers=authenticated_headers)
...
class TestResourceUtilization:
    ...
    def test_cpu_usage_under_load(self, client, authenticated_headers): process = psutil.Process()
        cpu_samples = []
    ...
    def test_disk_io_efficiency(self, client, authenticated_headers): process = psutil.Process()
...
class TestScalabilityLimits:
    ...
    def test_large_data_set_handling(self, client, authenticated_headers): large_profile = {'company_name': 'Large Corp', 'industry':
    ...
    def test_pagination_performance(self, client, authenticated_headers): page_sizes = [10, 50, 100]

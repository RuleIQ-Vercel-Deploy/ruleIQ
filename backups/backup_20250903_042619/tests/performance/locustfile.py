...
class AuthenticatedUser(HttpUser): 
    def on_start(self): self.user_data = {'email': f'loadtest-{uuid4()}@example.com',
...
class UserOnboardingFlow(SequentialTaskSet): 
    ...
    @task
    def create_business_profile(self): profile_data = {'company_name': random.choice(SAMPLE_COMPANIES),
    ...
    @task
    def start_assessment(self): if not self.business_profile_id:
            return
    ...
    @task
    def complete_assessment(self): if not self.assessment_id:
            return
    ...
    @task
    def get_recommendations(self): if not self.business_profile_id:
            return
...
class ComplianceUser(AuthenticatedUser): wait_time = between(1, 3)
    tasks = [UserOnboardingFlow]

    @task(3)
    def create_evidence_item(self): evidence_types = ['document', 'screenshot', 'configuration',
            'audit_log']
        evidence_data = {'title':
    ...
    @task(2)
    def search_evidence(self): search_terms = ['security', 'policy', 'procedure', 'control', 'audit']
    ...
    @task(1)
    def get_dashboard(self): self.client.get('/api/users/dashboard', headers=self.headers)

    @task(1)
    def get_compliance_status(self): self.client.get('/api/compliance/status', headers=self.headers)


class AIAssistantUser(AuthenticatedUser): wait_time = between(2, 5)

    @task(5)
    def ask_compliance_question(self): question = random.choice(SAMPLE_QUESTIONS)
    ...
    @task(2)
    def get_chat_history(self): if hasattr(self, 'conversation_id'):
            self.client.get(
    ...
    @task(1)
    def start_new_conversation(self): conversation_data = {'title':
...
class ReportingUser(AuthenticatedUser): wait_time = between(3, 8)

    @task(3)
    def generate_compliance_report(self): report_data = {'report_type': random.choice(['gap_analysis',
    ...
    def check_report_status(self, task_id): max_checks = 10
    ...
    @task(2)
    def get_analytics_dashboard(self): params = {'time_range': random.choice(['7d', '30d', '90d', '1y']),
    ...
    @task(1)
    def export_data(self): export_data = {'data_type': random.choice(['evidence',
...
class StressTestUser(AuthenticatedUser): wait_time = between(0.1, 0.5)

    @task(10)
    def rapid_api_calls(self): endpoints = ['/api/users/profile', '/api/business-profiles',
            '/api/evidence/stats', '/api/frameworks', '/api/compliance/status']
        endpoint = random.choice(endpoints)
        self.client.get(endpoint, headers=self.headers)

    @task(5)
    def concurrent_evidence_creation(self): evidence_data = {'title':
    ...
    @task(3)
    def bulk_operations(self): bulk_data = {'evidence_ids': [str(uuid4()) for _ in range(10)],
            'status': 'reviewed', 'reason': 'Bulk stress test update'}
        self.client.post('/api/evidence/bulk-update', json=bulk_data,
            headers=self.headers)


class LightLoad(AuthenticatedUser): weight = 3
    tasks = [ComplianceUser, AIAssistantUser]


class MediumLoad(AuthenticatedUser): weight = 2
    tasks = [ComplianceUser, AIAssistantUser, ReportingUser]


class HeavyLoad(AuthenticatedUser): weight = 1
...
def on_test_start(environment, **kwargs): logger.info('Starting ComplianceGPT Performance Tests')
...
def on_test_stop(environment, **kwargs): logger.info('Performance test completed')
...
class ChatWebSocketUser(HttpUser): 
    ...
    @task
    def websocket_chat_session(self): try:
            import websocket
...
class DatabaseStressUser(AuthenticatedUser): wait_time = between(0.5, 2)

    @task(5)
    def complex_evidence_search(self): search_params = {'q': 'security policy governance', 'evidence_type':
    ...
    @task(3)
    def aggregate_analytics(self): analytics_params = {'metrics': ['compliance_trends',
    ...
    @task(2)
    def concurrent_updates(self): for _ in range(5):
            evidence_data = {'title':
...
class PeakTrafficScenario(AuthenticatedUser): 
    ...
    @task(10)
    def morning_rush_pattern(self): self.client.get('/api/users/dashboard', headers=self.headers)
        self.client.get('/api/evidence?page=1&page_size=10', headers=self.
            headers)
        self.client.get('/api/compliance/status', headers=self.headers)

    @task(5)
    def deadline_crunch_pattern(self): report_data = {'report_type': 'readiness_assessment', 'frameworks':

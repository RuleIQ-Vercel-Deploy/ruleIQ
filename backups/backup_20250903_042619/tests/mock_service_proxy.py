...
class ServiceProxy:
    ...
    def get_test_postgres_url(self, original_url=None): cfg = self.test_config['postgres']
        return f"postgresql://{cfg['user']}:{cfg['password']}@{cfg['host']}:{cfg['port']}/{cfg['database']}"

    def get_test_redis_url(self, original_url=None): cfg = self.test_config['redis']
        return f"redis://{cfg['host']}:{cfg['port']}/{cfg['db']}"

    def patch_postgres_connections(self): test_url = self.get_test_postgres_url()
    ...
    def patch_redis_connections(self): cfg = self.test_config['redis']
    ...
    def patch_opentelemetry(self): try:
    ...
    def patch_environment_variables(self): test_env = {
    ...
    def activate(self): self.patch_environment_variables()
    ...
    def _patch_database_module(self): try:
            import database
    ...
    def _patch_cache_module(self): try:
            import config.cache
...
def setup_test_services(): Setup function to be called at the start of test sessions.
...
@contextmanager
def test_service_context(): Context manager for test services.
    """
    service_proxy.activate()
    try:
        yield service_proxy

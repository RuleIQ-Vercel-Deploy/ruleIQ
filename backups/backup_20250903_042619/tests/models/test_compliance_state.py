...
class TestComplianceStateInitialization:
    ...
    def test_minimal_state_creation(self):
        ...
        minimal_data = {'case_id': 'test-case-001', 'actor': 'PolicyAuthor',
            'objective': 'Generate compliance policy', 'trace_id': str(uuid4())
            }
    ...
    def test_full_state_creation(self):
        ...
        full_data = {'case_id': 'test-case-002', 'actor':
            'EvidenceCollector', 'objective':
            'Collect evidence for ISO 27001', 'context': {'org_profile': {
            'name': 'TestOrg', 'industry': 'Finance'}, 'framework':
            'ISO 27001', 'obligations': ['Data Protection',
            'Access Control']}, 'memory': {'episodic': ['event-001',
            'event-002'], 'semantic': {'policies': ['policy-001'],
            'evidence': ['evidence-001']}}, 'evidence': [], 'decisions': [],
            'cost_tracker': {'total_tokens': 0, 'prompt_tokens': 0,
            'completion_tokens': 0, 'estimated_cost': 0.0, 'model': 'gpt-4'
            }, 'trace_id': str(uuid4()), 'workflow_status': 'pending',
            'node_execution_times': {}, 'retry_count': 0, 'error_count': 0}
...
class TestActorValidation:
    ...
    def test_valid_actor_types(self):
        ...
        valid_actors = ['PolicyAuthor', 'EvidenceCollector', 'RegWatch',
            'FilingScheduler']
        for actor in valid_actors:
            data = {'case_id': f'test-{actor}', 'actor': actor, 'objective':
                f'Test {actor}', 'trace_id': str(uuid4())}
...
class TestEvidenceAccumulation:
    ...
    def test_evidence_initialization_empty(self):
        ...
        data = {'case_id': 'test-evidence-001', 'actor':
            'EvidenceCollector', 'objective': 'Test evidence', 'trace_id':
            str(uuid4())}
        state = ComplianceState(**data)
        assert state.evidence == []
    ...
    def test_evidence_accumulation_preserves_existing(self):
        ...
        initial_evidence = [{'id': 'ev-001', 'type': 'document', 'content':
            'Policy doc'}, {'id': 'ev-002', 'type': 'screenshot', 'content':
            'Dashboard'}]
        new_evidence = [{'id': 'ev-003', 'type': 'log', 'content': 'Audit log'}
            ]
        data = {'case_id': 'test-evidence-002', 'actor':
            'EvidenceCollector', 'objective': 'Test accumulation',
            'evidence': initial_evidence, 'trace_id': str(uuid4())}
        state = ComplianceState(**data)
    ...
    def test_evidence_item_validation(self):
        ...
        data = {'case_id': 'test-evidence-003', 'actor':
            ...
            invalid_evidence, 'trace_id': str(uuid4())}
...
class TestCostTracking:
    ...
    def test_cost_tracker_initialization(self):
        ...
        data = {'case_id': 'test-cost-001', 'actor': 'PolicyAuthor',
            'objective': 'Test cost', 'trace_id': str(uuid4())}
        state = ComplianceState(**data)
    ...
    def test_cost_tracker_update(self):
        ...
        initial_cost = {'total_tokens': 100, 'prompt_tokens': 60,
            'completion_tokens': 40, 'estimated_cost': 0.01, 'model': 'gpt-4'}
        data = {'case_id': 'test-cost-002', 'actor': 'PolicyAuthor',
            'objective': 'Test cost update', 'cost_tracker': initial_cost,
            'trace_id': str(uuid4())}
        state = ComplianceState(**data)
    ...
    def test_cost_accumulation(self):
        ...
        initial = CostSnapshot(total_tokens=100, prompt_tokens=60,
            completion_tokens=40, estimated_cost=0.001)
        additional = CostSnapshot(total_tokens=50, prompt_tokens=30,
            completion_tokens=20, estimated_cost=0.0005)
...
class TestMemoryPersistence:
    ...
    def test_memory_initialization(self):
        ...
        data = {'case_id': 'test-memory-001', 'actor': 'PolicyAuthor',
            'objective': 'Test memory', 'memory': memory_data, 'trace_id':
            str(uuid4())}
        state = ComplianceState(**data)
    ...
    def test_episodic_memory_append(self):
        ...
        data = {'case_id': 'test-memory-002', 'actor': 'PolicyAuthor',
            'objective': 'Test episodic', 'memory': memory_data, 'trace_id':
            str(uuid4())}
        state = ComplianceState(**data)
    ...
    def test_semantic_memory_update(self):
        ...
        data = {'case_id': 'test-memory-003', 'actor': 'PolicyAuthor',
            'objective': 'Test semantic', 'memory': memory_data, 'trace_id':
            str(uuid4())}
        state = ComplianceState(**data)
...
class TestDecisionTracking:
    ...
    def test_decisions_initialization(self):
        ...
        data = {'case_id': 'test-decision-001', 'actor': 'PolicyAuthor',
            'objective': 'Test decisions', 'trace_id': str(uuid4())}
        state = ComplianceState(**data)
        assert state.decisions == []
    ...
    def test_decision_structure(self):
        ...
        decisions = [{'id': 'dec-001', 'timestamp': datetime.now().
            isoformat(), 'actor': 'PolicyAuthor', 'action':
            'approve_policy', 'reasoning': 'Policy meets all requirements',
            'confidence': 0.95, 'alternatives': ['reject', 'request_review']}]
        data = {'case_id': 'test-decision-002', 'actor': 'PolicyAuthor',
            'objective': 'Test decision structure', 'decisions': decisions,
            'trace_id': str(uuid4())}
        state = ComplianceState(**data)
    ...
    def test_decision_accumulation(self):
        ...
        data = {'case_id': 'test-decision-003', 'actor': 'PolicyAuthor',
            'objective': 'Test accumulation', 'decisions':
            initial_decisions, 'trace_id': str(uuid4())}
        state = ComplianceState(**data)
        new_decision = Decision(id='dec-002', timestamp=datetime.now().
            isoformat(), actor='PolicyAuthor', action='approve_policy')
...
class TestStateSerialization:
    ...
    def test_state_to_json(self):
        ...
        data = {'case_id': 'test-serial-001', 'actor': 'PolicyAuthor',
            'objective': 'Test serialization', 'trace_id': str(uuid4()),
            'context': {'framework': 'ISO 27001'}, 'evidence': [{'id':
            'ev-001', 'type': 'doc'}], 'decisions': []}
        state = ComplianceState(**data)
    ...
    def test_state_from_json(self):
        ...
        json_data = {'case_id': 'test-serial-002', 'actor':
            'EvidenceCollector', 'objective': 'Test deserialization',
            'trace_id': str(uuid4()), 'context': {}, 'memory': {'episodic':
            [], 'semantic': {}}, 'evidence': [], 'decisions': [],
            'cost_tracker': {'total_tokens': 0, 'prompt_tokens': 0,
            'completion_tokens': 0, 'estimated_cost': 0.0, 'model': 'gpt-4'
            }, 'workflow_status': 'pending', 'node_execution_times': {},
    ...
    def test_datetime_serialization(self):
        ...
        data = {'case_id': 'test-serial-003', 'actor': 'PolicyAuthor',
            'objective': 'Test datetime', 'trace_id': str(uuid4()),
...
class TestTraceIdGeneration:
    ...
    def test_trace_id_uniqueness(self):
        ...
        trace_id = str(uuid4())
        data1 = {'case_id': 'test-trace-003', 'actor': 'PolicyAuthor',
            'objective': 'Test uniqueness', 'trace_id': trace_id}
        data2 = {'case_id': 'test-trace-004', 'actor': 'EvidenceCollector',
            'objective': 'Test uniqueness 2', 'trace_id': trace_id}
...
class TestContextValidation:
    ...
    def test_context_structure(self):
        ...
        context = {'org_profile': {'name': 'TestCorp', 'industry':
            'Technology', 'size': 'medium', 'location': 'US'}, 'framework':
            'ISO 27001', 'obligations': ['Data Protection',
            'Access Control', 'Incident Management']}
        data = {'case_id': 'test-context-001', 'actor': 'PolicyAuthor',
            'objective': 'Test context', 'context': context, 'trace_id':
            str(uuid4())}
        state = ComplianceState(**data)
    ...
    def test_context_optional(self):
        ...
        data = {'case_id': 'test-context-002', 'actor': 'PolicyAuthor',
            'objective': 'Test optional context', 'trace_id': str(uuid4())}
        state = ComplianceState(**data)
    ...
    def test_context_framework_validation(self):
        ...
        data = {'case_id': 'test-context-003', 'actor': 'PolicyAuthor',
...
class TestStateTransitions:
    ...
    def test_workflow_status_values(self):
        ...
        for status in valid_statuses:
            data = {'case_id': f'test-status-{status}', 'actor':
                'PolicyAuthor', 'objective': 'Test status', 'trace_id': str
                (uuid4()), 'workflow_status': status}
            state = ComplianceState(**data)
    ...
    def test_state_transition_tracking(self):
        ...
        data = {'case_id': 'test-transition-001', 'actor': 'PolicyAuthor',
            'objective': 'Test transitions', 'trace_id': str(uuid4()),
            'workflow_status': 'pending', 'state_history': []}
        state = ComplianceState(**data)
        old_status = state.workflow_status
        state.workflow_status = 'in_progress'
        state.state_history.append({'from': old_status, 'to': state.
            workflow_status, 'timestamp': datetime.now().isoformat(),
            'actor': state.actor})
...
class TestPerformanceMetrics:
    ...
    def test_node_execution_times(self):
        ...
        exec_times = {'policy_generator': 1234, 'evidence_collector': 5678,
            'validator': 910}
        data = {'case_id': 'test-perf-001', 'actor': 'PolicyAuthor',
            'objective': 'Test performance', 'trace_id': str(uuid4()),
    ...
    def test_retry_and_error_counts(self):
        ...
        data = {'case_id': 'test-perf-002', 'actor': 'PolicyAuthor',
            'objective': 'Test counters', 'trace_id': str(uuid4()),
            'retry_count': 2, 'error_count': 1}
        state = ComplianceState(**data)
    ...
    def test_counter_defaults(self):
        ...
        data = {'case_id': 'test-perf-003', 'actor': 'PolicyAuthor',
            'objective': 'Test defaults', 'trace_id': str(uuid4())}
        state = ComplianceState(**data)
...
class TestStateValidation:
    ...
    def test_type_coercion(self):
        ...
        data = {'case_id': 'test-val-002', 'actor': 'PolicyAuthor',
            'objective': 'Test coercion', 'trace_id': str(uuid4()),
            'retry_count': '5', 'error_count': '2'}
        state = ComplianceState(**data)
...
class TestLangGraphIntegration:
    ...
    def test_state_with_langgraph_typeddict(self):
        ...
        state = ComplianceState(case_id='test-123', actor='PolicyAuthor',
            objective='Test TypedDict compatibility', trace_id=str(uuid.
            uuid4()))
    ...
    def test_state_reducer_functions(self):
        ...
        from langgraph_agent.graph.reducers import accumulate_evidence, merge_decisions, update_cost_tracker
        existing_evidence = [{'id': 'ev1', 'source': 'source1', 'content':
            'content1', 'confidence': 0.9, 'timestamp': datetime.now(
            timezone.utc).isoformat()}]
        new_evidence = [{'id': 'ev2', 'source': 'source2', 'content':
            'content2', 'confidence': 0.85, 'timestamp': datetime.now(
            timezone.utc).isoformat()}]
        combined = accumulate_evidence(existing_evidence, new_evidence)
        assert len(combined) == 2
        existing_decisions = [{'id': 'dec1', 'type': 'compliance_check',
            'reasoning': 'test', 'confidence': 0.9, 'timestamp': datetime.
            now(timezone.utc).isoformat()}]
        new_decisions = [{'id': 'dec2', 'type': 'risk_assessment',
            'reasoning': 'test2', 'confidence': 0.8, 'timestamp': datetime.
            now(timezone.utc).isoformat()}]

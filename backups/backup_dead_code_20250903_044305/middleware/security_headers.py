...
class SecurityHeadersMiddleware:
    ...
    async def __call__(self, scope, receive, send): ASGI middleware interface
        ...
        async def send_wrapper(message): nonlocal response_status
    ...
    def _add_basic_security_headers(self, response: Response) -> None: # Prevent MIME type sniffing
    ...
    def _add_csp_header(self, response: Response, nonce: Optional[str] = None) -> None: Add Content Security Policy header
    ...
    def _add_cors_headers(self, request: Request, response: Response) -> None: Add CORS headers based on configuration
    ...
    def _add_advanced_security_headers(self, response: Response) -> None: # Expect-CT for Certificate Transparency
    ...
    def _generate_nonce(self) -> str: return secrets.token_urlsafe(16)

    def update_csp_directive(self, directive: str, values: List[str]) -> None: Update a specific CSP directive
    ...
    def add_allowed_origin(self, origin: str) -> None: Add an allowed CORS origin
    ...
    def remove_allowed_origin(self, origin: str) -> None: Remove an allowed CORS origin
    ...
    def get_security_report(self) -> Dict[str, Any]: Get a report of current security header configuration
...
class CSPViolationHandler:
    ...
    def __init__(self, storage_backend: Optional[Any] = None): Initialize CSP violation handler
    ...
    async def handle_violation(self, request: Request) -> JSONResponse: Handle CSP violation report
    ...
    async def _persist_violation(self, violation: Dict[str, Any]) -> None: if self.storage_backend:
            # Implementation depends on storage backend
            pass

    def _log_violation(self, violation: Dict[str, Any]) -> None: import logging
    ...
    def get_violations_summary(self) -> Dict[str, Any]: Get summary of CSP violations

...
def fix_magic_values() -> Any: files_to_fix = [
...
def fix_long_lines() -> Any: result = subprocess.run(['ruff', 'check', '.', '--select=E501'], 
...
def fix_bare_excepts() -> Any: result = subprocess.run(['grep', '-r', 'except:', '.', '--include=*.py'], 
                          capture_output=True, text=True, cwd='/home/omar/Documents/ruleIQ/api')

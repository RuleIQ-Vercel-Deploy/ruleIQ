...
@pytest.fixture
async def test_user(test_db): async with TestingSessionLocal() as db:
...
@pytest.fixture
async def other_user(test_db): async with TestingSessionLocal() as db:
...
class TestSMBOwnership:
    ...
    def test_get_own_profile(self, client, auth_headers, test_user): profile_data = {'company_name': 'My Small Business', 'industry':
    ...
    def test_no_auth_rejected(self, client): response = client.get('/api/v1/business-profiles/')
        assert response.status_code == HTTP_UNAUTHORIZED
        response = client.post('/api/v1/business-profiles/', json={
            'company_name': 'Test'})
        assert response.status_code == HTTP_UNAUTHORIZED

    def test_expired_token_rejected(self, client, test_user): expired_token = create_access_token({'sub': test_user.email},

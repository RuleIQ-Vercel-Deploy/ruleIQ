...
@pytest.fixture
def mock_db_session(): session = AsyncMock()
...
@pytest.fixture
def mock_processor(): processor = Mock()
...
@pytest.fixture
def mock_duplicate_detector(): detector = Mock()
    detector.is_duplicate = AsyncMock(return_value=False)
    return detector


@pytest.fixture
def sample_evidence_data(): return {
...
class TestEvidenceStateManagement:
    ...
    async def test_evidence_state_initialization(self, sample_state): assert sample_state.actor == "EvidenceCollector"
    ...
    async def test_state_transition_tracking(self, sample_state): transitions = []
    ...
    async def test_error_tracking_in_state(self, sample_state): # Initial state
        assert sample_state.error_count == 0
        assert sample_state.retry_count == 0
...
class TestEvidenceValidationAndScoring:
    ...
    async def test_evidence_scoring_thresholds(self, mock_processor): test_cases = [
    ...
    async def test_evidence_confidence_calculation(self, sample_evidence_data): # Base confidence from source
        source_confidence = {
            "integration_api": 0.9,
            "manual_upload": 0.7,
            "automated_scan": 0.8,
...
class TestRetryAndFallbackMechanisms:
    ...
    async def test_retry_with_exponential_backoff(self): retry_delays = []
        max_retries = 5
        base_delay = 1
    ...
    async def test_retry_with_jitter(self): import random
    ...
    async def test_fallback_to_cache(self, sample_evidence_data): cache = {}
    ...
    async def test_circuit_breaker_pattern(self): 
        class CircuitBreaker:
            def __init__(self, failure_threshold=3, recovery_timeout=60): self.failure_count = 0
...
class TestEvidenceAggregationAndDeduplication:
    ...
    async def test_evidence_aggregation_by_type(self, sample_evidence_data): evidence_list = [
    ...
    async def test_evidence_deduplication_by_hash(self, sample_evidence_data): import hashlib
    ...
    async def test_evidence_merging_strategy(self): evidence_groups = {
            "policy_compliance": [
...
class TestEvidenceCollectionWorkflows:
    ...
    async def test_batch_evidence_collection_workflow(self, sample_state): sources = ["api_1", "api_2", "database_1"]
        collected_evidence = []
    ...
    async def test_evidence_lifecycle_management(self, sample_evidence_data): evidence = {
            **sample_evidence_data,
            "id": str(uuid4()),
...
class TestErrorHandlingAndRecovery:
    ...
    async def test_graceful_degradation(self, sample_evidence_data): results = {
    ...
    async def test_error_recovery_with_checkpoint(self, sample_state): # Save checkpoint before risky operation
        checkpoint = {
...
class TestIntegrationWithLangGraph:
    ...
    async def test_node_registration_in_graph(self): from langgraph.graph import StateGraph
    ...
    async def test_state_updates_through_node(self, sample_state, sample_evidence_data): initial_evidence_count = len(sample_state.evidence_items)
...
class TestPerformanceAndOptimization:
    ...
    async def test_batch_processing_performance(self): import time

        async def process_single(item):
            await asyncio.sleep(0.1)  # Simulate processing return f"processed_{item}"
    ...
    async def test_caching_reduces_database_calls(self, mock_db_session): cache = {}
        db_call_count = 0

        async def get_evidence_with_cache(evidence_id):
            nonlocal db_call_count
            """Get Evidence With Cache"""
    ...
    async def test_memory_efficient_streaming(self): 
        async def evidence_stream(batch_size=100): total_items = 1000
...
class TestMonitoringAndObservability:
    ...
    async def test_execution_time_tracking(self, sample_state): import time
    ...
    async def test_metrics_collection(self): metrics = {
    ...
    async def test_audit_trail_generation(self, sample_state, sample_evidence_data): audit_trail = []
...
class TestEndToEndIntegration:
    ...
    async def test_complete_evidence_collection_pipeline(self): # Setup
    ...
    async def test_parallel_evidence_collection_from_multiple_sources(self): sources = ["slack", "teams", "email", "gdrive"]

        async def collect_from_integration(source):
            # Simulate API call with varying delays
            """Collect From Integration"""

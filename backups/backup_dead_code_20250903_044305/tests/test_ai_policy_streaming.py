...
class TestPolicyStreamingService:
    ...
    @pytest.fixture
    def policy_generator(self): with patch("services.ai.google_client.GoogleAIClient"), patch(
    ...
    @pytest.fixture
    def sample_request(self): return PolicyGenerationRequest(
            framework_id="gdpr",
    ...
    @pytest.fixture
    def sample_framework(self): framework = Mock(spec=ComplianceFramework)
        framework.id = "gdpr"
        framework.name = "GDPR"
    ...
    @pytest.mark.asyncio
    async def test_stream_with_google_success(self, policy_generator, sample_request): # Mock Google's generate_content_async
        mock_response = Mock()
    ...
    @pytest.mark.asyncio
    async def test_stream_with_openai_success(self, policy_generator, sample_request): # Mock OpenAI's streaming response
    ...
    async def test_stream_error_handling(
        ...
        async def mock_google_error(*args, **kwargs):
            yield PolicyStreamingChunk( chunk_id="1", content="Partial content", chunk_type="content",
            )
    ...
    async def test_stream_with_circuit_breaker_open(
        ...
        with patch("services.ai.policy_generator.google_breaker") as mock_breaker:
            ...
            async def mock_openai_stream(*args, **kwargs):
                yield PolicyStreamingChunk( chunk_id="1",
...
class TestPolicyStreamingAPI:
    ...
    @pytest.fixture
    def client(self): from main import app

        return TestClient(app)

    @pytest.fixture
    def auth_headers(self): return {"Authorization": "Bearer test_token"}

    @pytest.fixture
    def mock_auth(self): with patch("api.dependencies.auth.get_current_user") as mock:
            mock.return_value = Mock(id="test_user", email="test@example.com")
            yield mock

    @pytest.mark.asyncio
    async def test_stream_endpoint_sse_format(self, client, auth_headers, mock_auth): # Mock the PolicyGenerator
        with patch("api.routers.ai_policy.PolicyGenerator") as MockGenerator:
            mock_generator = MockGenerator.return_value

            async def mock_stream(*args, **kwargs):
                yield PolicyStreamingChunk( chunk_id="1", content='{"test": "metadata"}', chunk_type="metadata",
                )
                yield PolicyStreamingChunk(
    ...
    @pytest.mark.asyncio
    async def test_stream_endpoint_headers(self, client, auth_headers, mock_auth): with patch("api.routers.ai_policy.PolicyGenerator") as MockGenerator:
            mock_generator = MockGenerator.return_value

            async def mock_stream(*args, **kwargs):
                yield PolicyStreamingChunk( chunk_id="1", content="Test", chunk_type="content",
                )
    ...
    @pytest.mark.asyncio
    async def test_stream_endpoint_authentication(self, client): response = client.post(
            "/api/ai-policy/generate-policy/stream",
            json={
                "framework_id": "gdpr",
                "business_context": {
    ...
    @pytest.mark.asyncio
    async def test_stream_endpoint_rate_limiting(self, client, auth_headers, mock_auth): with patch("api.routers.ai_policy.RateLimiter") as MockRateLimiter:
...
class TestPolicyStreamingIntegration:
    ...
    async def test_end_to_end_streaming_google(self): import os
    ...
    @pytest.mark.asyncio
    @pytest.mark.integration
    async def test_end_to_end_streaming_openai(self): import os
...
class TestPolicyStreamingPerformance:
    ...
    async def test_streaming_latency(
        ...
        async def mock_slow_stream(*args, **kwargs):
            yield PolicyStreamingChunk( chunk_id="1", content="First chunk", chunk_type="content",
            )

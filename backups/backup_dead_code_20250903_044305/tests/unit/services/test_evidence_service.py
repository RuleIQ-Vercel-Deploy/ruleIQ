...
class TestEvidenceService:
    ...
    async def test_create_evidence_item_success(self, fast_db_session, sample_user):
        ...
        evidence_data = {'title': 'Security Policy', 'description':
            ...
            'source': 'manual', 'framework_mappings': ['ISO27001.A.5.1.1']}
        with patch('services.evidence_service.EvidenceService.create_evidence'
            ) as mock_create:
            mock_create.return_value = {**evidence_data, 'id': str(uuid4()),
                'user_id': str(sample_user.id), 'status': 'valid',
                'created_at': datetime.now(timezone.utc), 'quality_score': 85.0
                }
            result = await EvidenceService.create_evidence(sample_user.id,
                evidence_data)
    ...
    async def test_create_evidence_item_validation_error(self, fast_db_session,
        ...
        with patch('services.evidence_service.EvidenceService.create_evidence'
            ...
            mock_create.side_effect = ValidationAPIError(
    ...
    def test_validate_evidence_quality_high_score(self, fast_db_session):
        ...
        evidence_data = {'title': 'Comprehensive Security Policy',
            'description':
            'Detailed security policy covering all ISO 27001 requirements',
            'evidence_type': 'document', 'file_content':
            'Detailed policy content with specific procedures...',
            'metadata': {'creation_date': '2024-01-01', 'author':
            'Chief Security Officer', 'version': '2.1', 'approval_date':
            '2024-01-15'}}
        with patch(
            'services.evidence_service.EvidenceService.validate_evidence_quality'
            ) as mock_validate:
            mock_validate.return_value = {'quality_score': 92,
                'validation_results': {'completeness': 'excellent',
                'relevance': 'high', 'timeliness': 'current',
    ...
    def test_validate_evidence_quality_low_score(self, fast_db_session):
        ...
        poor_evidence_data = {'title': 'Policy', 'description':
            ...
            'Brief policy.', 'metadata': {'creation_date': '2020-01-01'}}
        with patch(
            'services.evidence_service.EvidenceService.validate_evidence_quality'
            ) as mock_validate:
            mock_validate.return_value = {'quality_score': 35,
                'validation_results': {'completeness': 'poor', 'relevance':
                'medium', 'timeliness': 'outdated', 'authenticity':
                'unverified'}, 'issues': ['Title too vague',
                'Content too brief', 'Evidence is outdated'],
                'recommendations': ['Provide more specific title',
                'Add detailed content',
                'Update evidence with current information']}
            result = EvidenceService.validate_evidence_quality(
                poor_evidence_data)
    ...
    def test_identify_evidence_requirements_success(self, fast_db_session):
        ...
        with patch(
            ...
            mock_identify.return_value = [{'control_id': control_ids[0],
                ...
                'Written policy documenting information security procedures',
                'automation_possible': False, 'collection_method': 'manual',
                'required_artifacts': ['policy_document', 'approval_record'
                ], 'estimated_effort': '2-4 hours'}, {'control_id':
                control_ids[1], 'evidence_type': 'log', 'title':
                'Access Control Logs', 'description':
                'System logs showing access control implementation',
                'automation_possible': True, 'collection_method':
                'automated', 'required_artifacts': ['access_logs',
                'audit_trail'], 'estimated_effort': 'automated'}]
    ...
    def test_configure_automation_success(self, fast_db_session):
        ...
        automation_config = {'source_type': 'google_workspace', 'endpoint':
            ...
            'collection_frequency': 'daily', 'credentials_id':
            'gws_creds_001', 'data_mapping': {'user_list':
        ...
        with patch(
            ...
            mock_configure.return_value = {'configuration_successful': True,
                ...
                'successful', 'estimated_data_points': 150,
    ...
    def test_configure_automation_connection_failure(self, fast_db_session):
        ...
        evidence_id = uuid4()
        automation_config = {'source_type': 'google_workspace', 'endpoint':
            'https://invalid-endpoint.com', 'collection_frequency': 'daily',
            'credentials_id': 'invalid_creds'}
        with patch(
            'services.evidence_service.EvidenceService.configure_automation'
            ) as mock_configure:
            mock_configure.return_value = {'configuration_successful': 
                False, 'automation_enabled': False, 'test_connection':
                'failed', 'error_message':
                'Invalid credentials or endpoint unreachable',
                'suggested_actions': ['Verify credentials are valid',
                'Check endpoint URL',
    ...
    def test_get_user_evidence_items_success(self, fast_db_session, sample_user):
        ...
        with patch('services.evidence_service.get_user_evidence_items'
            ...
            mock_get.return_value = [{'id': str(uuid4()), 'title':
                'Security Policy', 'evidence_type': 'document', 'status':
                'valid', 'quality_score': 85.0, 'created_at': datetime.now(
                timezone.utc)}, {'id': str(uuid4()), 'title':
                'User Access Logs', 'evidence_type': 'log', 'status':
                'valid', 'quality_score': 92.0, 'created_at': datetime.now(
                timezone.utc)}]
    ...
    def test_update_evidence_status_success(self, fast_db_session):
        ...
        with patch('services.evidence_service.EvidenceService.update_status'
            ) as mock_update:
            mock_update.return_value = {'evidence_id': str(evidence_id),
                'old_status': 'valid', 'new_status': new_status, 'reason':
                reason, 'updated_at': datetime.now(timezone.utc),
                'renewal_required': True, 'renewal_due_date': datetime.now(
                timezone.utc) + timedelta(days=30)}
            result = EvidenceService.update_status(evidence_id, new_status,
                reason)
    ...
    def test_search_evidence_by_framework(self, fast_db_session, sample_user):
        ...
        with patch(
            ...
            mock_search.return_value = [{'id': str(uuid4()), 'title':
                'Access Control Policy', 'framework_mappings': [
                'ISO27001.A.9.1.1', 'ISO27001.A.9.1.2'], 'evidence_type':
                'document', 'status': 'valid', 'quality_score': 88.0}, {
                'id': str(uuid4()), 'title': 'Incident Response Procedure',
                'framework_mappings': ['ISO27001.A.16.1.1'],
                'evidence_type': 'document', 'status': 'valid',
                'quality_score': 91.0}]
            result = EvidenceService.search_by_framework(sample_user.id,
                framework, search_filters)
            assert len(result) == 2
            assert all(framework in str(item['framework_mappings']) for
                item in result)
    ...
    def test_delete_evidence_item_success(self, fast_db_session, sample_user):
        ...
        with patch('services.evidence_service.EvidenceService.delete_evidence'
            ...
            mock_delete.return_value = {'deleted': True, 'evidence_id': str
                (evidence_id), 'user_id': str(sample_user.id),
    ...
    def test_delete_evidence_item_not_found(self, fast_db_session, sample_user):
        ...
        with patch('services.evidence_service.EvidenceService.delete_evidence'
            ...
            mock_delete.side_effect = NotFoundAPIError('Evidence item',
                evidence_id)
    ...
    def test_bulk_update_evidence_status(self, fast_db_session, sample_user):
        ...
        with patch(
            ...
            mock_bulk_update.return_value = {'updated_count': 5,
                'failed_count': 0, 'total_requested': 5, 'success_rate': 
                100.0, 'updated_evidence_ids': evidence_ids,
                'failed_evidence_ids': []}
            result = EvidenceService.bulk_update_status(evidence_ids,
    ...
    def test_get_evidence_statistics(self, fast_db_session, sample_user):
        ...
        with patch('services.evidence_service.EvidenceService.get_statistics'
            ...
            mock_stats.return_value = {'total_evidence_items': 42,
                'by_status': {'valid': 35, 'expired': 5, 'pending': 2},
                'by_type': {'document': 25, 'log': 12, 'screenshot': 3,
                'configuration': 2}, 'by_framework': {'ISO27001': 28,
                'GDPR': 20, 'SOC2': 15}, 'average_quality_score': 84.7,
                'automation_coverage': 65.0, 'last_updated': datetime.now(
                timezone.utc)}
...
class TestEvidenceValidation:
    ...
    def test_validate_evidence_type_document(self):
        ...
        evidence_data = {'evidence_type': 'document', 'file_content':
            'Policy document content...', 'metadata': {'document_type':
            'policy'}}
        with patch(
            'services.evidence_service.EvidenceService.validate_evidence_type'
            ) as mock_validate:
            mock_validate.return_value = {'valid': True,
                'type_specific_score': 85, 'requirements_met': [
                'has_content', 'has_metadata'], 'requirements_missing': []}
    ...
    def test_validate_evidence_type_log(self):
        ...
        evidence_data = {'evidence_type': 'log', 'log_entries': [{
            'timestamp': '2024-01-01T10:00:00Z', 'event': 'user_login',
            'user': 'john@example.com'}, {'timestamp':
            '2024-01-01T10:05:00Z', 'event': 'file_access', 'user':
            'john@example.com'}], 'metadata': {'log_source': 'application',
            'retention_period': '90_days'}}
        with patch(
            'services.evidence_service.EvidenceService.validate_evidence_type'
            ) as mock_validate:
            mock_validate.return_value = {'valid': True,
                'type_specific_score': 92, 'requirements_met': [
                'has_entries', 'has_timestamps', 'has_metadata'],
                'requirements_missing': [], 'log_analysis': {'entry_count':
                2, 'time_span': '5 minutes', 'event_types': ['user_login',
                'file_access']}}
            result = EvidenceService.validate_evidence_type(evidence_data)
            assert result['valid'] is True

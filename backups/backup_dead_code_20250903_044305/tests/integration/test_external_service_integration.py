...
class TestAIServiceIntegration:
    ...
    @pytest.fixture
    async def async_client(self): async with httpx.AsyncClient(app=app, base_url="http://test") as client:
            yield client

    @pytest.fixture
    async def auth_headers(self, async_db): auth_manager = TestAuthManager()
    ...
    async def test_ai_service_timeout_handling(
        ...
        with patch(
            ...
            async def slow_response(*args, **kwargs):
                await asyncio.sleep(2)  # Longer than circuit breaker timeout return {"compliance_score": 0.7}
...
class TestDatabaseIntegration:
    ...
    async def async_client(self):
        async with httpx.AsyncClient(app=app, base_url="http://test") as client: yield client
    ...
    async def test_database_connection_pool_behavior(
        ...
        async def create_assessment(i: int):
            assessment_data = { "name": f"Concurrent Assessment {i}",
                "framework": "GDPR",
            }

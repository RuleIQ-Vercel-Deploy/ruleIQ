...
def compliance_state_to_dict(state: ComplianceState) -> Dict[str, Any]: return {
...
def dict_to_compliance_state(data: Dict[str, Any]) -> ComplianceState: # Prepare the data for ComplianceState
    prepared_data = {
...
class TestStateWithLangGraph:
    ...
    async def test_state_as_graph_state(self): # Define a simple graph with our state dict
    ...
    @pytest.mark.integration
    async def test_state_with_conditional_edges(self): workflow = StateGraph(ComplianceStateDict)

        # Define nodes
        async def check_evidence(state: Dict[str, Any]) -> Dict[str, Any]:
            state["workflow_status"] = "checking_evidence" return state
    ...
    @pytest.mark.integration
    async def test_parallel_node_execution(self): workflow = StateGraph(ComplianceStateDict)
...
class TestReducerIntegration:
    ...
    @pytest.mark.integration
    def test_evidence_accumulation_reducer(self): existing = [
    ...
    @pytest.mark.integration
    def test_decision_merge_reducer(self): existing = [
    ...
    @pytest.mark.integration
    def test_cost_tracker_reducer(self): from langgraph_agent.models.compliance_state import CostSnapshot
    ...
    @pytest.mark.integration
    def test_memory_append_reducer(self): existing = {"episodic": ["event1"], "semantic": {"concepts": ["concept1"]}}
...
class TestStateWithPostgreSQL:
    ...
    @pytest.fixture
    def postgres_url(self): return os.getenv(
            "DATABASE_URL",
    ...
    def test_state_checkpointing(self, postgres_checkpointer): # Use the checkpointer from fixture
    ...
    @pytest.mark.integration
    @pytest.mark.requires_db
    def test_state_recovery(self, postgres_checkpointer): # Use the checkpointer from fixture
        checkpointer = postgres_checkpointer
...
class TestComplexWorkflow:
    ...
    @pytest.mark.integration
    @pytest.mark.asyncio
    async def test_full_compliance_workflow(self): workflow = StateGraph(ComplianceStateDict)

...
def sqlite_engine(): In-memory SQLite engine for unit tests.
...
def fast_db_session(sqlite_engine) -> Generator[Session, None, None]: Fast database session using transaction rollback.
...
@pytest.fixture(scope="session")
def postgres_engine(): PostgreSQL engine for integration tests.
...
@pytest.fixture(scope="function")
def integration_db_session(postgres_engine) -> Generator[Session, None, None]: PostgreSQL session for integration tests with transaction rollback.
    """
...
@pytest.fixture(scope="session")
def cached_test_data() -> Dict[str, Any]: Pre-computed test data cached for entire test session.
...
def _generate_test_users(): return [
        {
            "id": f"user_{i}",
...
@lru_cache(maxsize=128)
def _generate_compliance_frameworks(): frameworks = ["GDPR", "CCPA", "HIPAA", "SOC2", "ISO27001"]
    return [
        {
            "id": f"framework_{i}",
            "name": framework,
...
@lru_cache(maxsize=128)
def _generate_ai_responses(): return [
        {
...
@lru_cache(maxsize=128)
def _generate_business_profiles(): industries = ["Technology", "Healthcare", "Finance", "Retail", "Manufacturing"]
    return [
        {
            "id": f"business_{i}",
...
@lru_cache(maxsize=128)
def _generate_regulations(): return [
        {
            "id": f"reg_{i}",
...
@pytest.fixture(scope="function")
def mock_ai_client(): Mock AI client for unit tests.
...
@pytest.fixture(scope="session")
def redis_mock(): Mock Redis client for unit tests.
...
def measure_test_duration(request): Automatically measure test execution time.
...
def configure_test_environment(): Configure optimal test environment settings.
    """
...
@contextmanager
def temporary_database(): Context manager for temporary test database.
...
# Performance helpers
def get_slowest_tests(n: int = 10) -> list: sorted_tests = sorted(
...
def print_performance_summary(): if not test_durations:
        return

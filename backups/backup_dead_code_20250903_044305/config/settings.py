...
def parse_list_from_string(v: Union[str, list]) ->list: if isinstance(v, list):
        return v
    if isinstance(v, str):
...
class Environment(str, Enum): DEVELOPMENT = 'development'
    STAGING = 'staging'
    PRODUCTION = 'production'
    TESTING = 'testing'


class LogLevel(str, Enum): DEBUG = 'DEBUG'
    INFO = 'INFO'
    WARNING = 'WARNING'
    ERROR = 'ERROR'
    CRITICAL = 'CRITICAL'
...
class Settings(BaseSettings): 
    ...
    @classmethod
    def parse_list_fields(cls, v: Union[str, List[str]]) ->List[str]: if isinstance(v, list):
            return v
        if isinstance(v, str):
            if v.startswith('[') and v.endswith(']'):
                try:
    ...
    @classmethod
    def validate_database_url(cls, v: str) ->str: if not v or not v.startswith(('postgresql://', 'postgresql+asyncpg://', 'sqlite:///')):
    ...
    @classmethod
    def validate_redis_url(cls, v: str) ->str: if not v or not v.startswith('redis://'):
            # In testing, provide a default
            if os.getenv('TESTING', '').lower() == 'true':
    ...
    @classmethod
    def validate_jwt_secret(cls, v: str) ->str: if not v or len(v) < 32:
            # In testing, provide a default
            if os.getenv('TESTING', '').lower() == 'true':
    ...
    @classmethod
    def validate_google_api_key(cls, v: str) ->str: if v and not v.startswith(('AIza', 'test-')):
            logger.warning(
    ...
    @classmethod
    def validate_max_file_size(cls, v: int) ->int: if v > DEFAULT_LIMIT:
            raise ValueError('Max file size cannot exceed 100MB')
        return v

    @field_validator('bcrypt_rounds')
    @classmethod
    def validate_bcrypt_rounds(cls, v: int) ->int: if v < 10 or v > 16:
            raise ValueError('Bcrypt rounds must be between 10 and 16')
        return v

    def get_secrets_vault_health(self) ->Dict[str, Union[str, bool]]: 🔐 Get SecretsVault health status for monitoring
    ...
    def log_vault_status(self) ->None: if self.is_testing:
    ...
    @property
    def is_secrets_vault_healthy(self) ->bool: if self.is_testing:
            return False
...
def get_settings() ->Settings: return _get_or_create_settings()


def reload_settings() ->Settings: return _create_new_settings()


def _get_or_create_settings() ->Settings: global _settings
...
def _create_new_settings() ->Settings: global _settings
    _settings = Settings()
    return _settings

...
class CollectorRegistry:
    ...
    def __init__(self): self.collectors = []

    def register(self, collector) -> None: self.collectors.append(collector)

    def unregister(self, collector) -> None: if collector in self.collectors:
            self.collectors.remove(collector)

    def collect(self) -> Generator[Any, None, None]: for collector in self.collectors:
            yield from collector.collect()

class PrometheusFormatter:
    """Formats metrics in Prometheus text format."""

    def __init__(self): self.metric_families: Dict[str, Dict[str, Any]] = {}
...
class LabelSanitizer:
    ...
    def __init__(self): self.metric_name_re = re.compile('^[a-zA-Z_:][a-zA-Z0-9_:]*$')
...
class MetricTypeMapper:
    ...
    def __init__(self): self.otel_to_prometheus_map = {'Counter': 'counter', 'UpDownCounter': 'gauge', 'Histogram': 'histogram', 'ObservableCounter': 'counter', 'ObservableUpDownCounter': 'gauge', 'ObservableGauge': 'gauge'}
    ...
    def otel_to_prometheus(self, otel_type: str) -> str: return self.map_otel_to_prometheus(otel_type)
...
class MetricsHTTPServer:
    ...
    def _run_server(self): self.server.serve_forever()

    async def stop(self) -> None: if self.server:
...
class PrometheusMetricsExporter:
    ...
    def update_metrics(self, updates: List[Tuple[str, float, Dict[str, str]]]) -> None:
        ...
        Args:
            updates: List of (metric_name, value, labels) tuples for metric_name, value, labels in updates:
    ...
    def add_collector(self, collector) -> None:
        ...
        Args:
            collector: Collector instance with collect() method self._collectors.append(collector)
    ...
    async def start(self) -> None: self.server.set_metrics_callback(self.collect)
        await self.server.start()

    async def shutdown(self) -> None: await self.server.shutdown()
...
class PrometheusMetricsExporter:
    ...
    def start_http_server(self) -> None: self.http_server.start()

    def stop_http_server(self) -> None: self.http_server.stop()
    ...
    def register_histogram(self, name: str, description: str='', labels: List[str]=None, buckets=None) -> Any: if self.registry and hasattr(self.registry, '__module__'):
    ...
    async def start(self) -> None: await self.http_server.start()

    async def shutdown(self) -> None: await self.http_server.shutdown()
    ...
    def start_http_server_sync(self) -> None: import asyncio
        asyncio.run(self.start())

    async def start_http_server(self) -> None: await self.http_server.start()
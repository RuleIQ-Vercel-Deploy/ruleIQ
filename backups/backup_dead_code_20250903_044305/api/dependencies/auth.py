...
def get_jwt_secret_key() ->str: if settings.is_production:
...
async def blacklist_token(token: str, reason: str='logout', **kwargs) ->None: from .token_blacklist import blacklist_token as enhanced_blacklist_token
    ttl = ACCESS_TOKEN_EXPIRE_MINUTES * 60
    await enhanced_blacklist_token(token, reason=reason, ttl=ttl, **kwargs)


async def is_token_blacklisted(token: str) ->bool: from .token_blacklist import is_token_blacklisted as enhanced_is_blacklisted
    return await enhanced_is_blacklisted(token)


def validate_password(password: str) ->tuple[bool, str]: if len(password) < MIN_PASSWORD_LENGTH:
...
def validate_token_expiry(payload: Dict) ->None: exp = payload.get('exp')
    if not exp:
...
def decode_token(token: str) ->Optional[Dict]: try:
...
async def get_current_user(token: Optional[str]=Depends(oauth2_scheme), db:
    AsyncSession=Depends(get_async_db)) ->Optional[User]: if token is None:
        return None
...
async def get_current_user_from_refresh_token(token: Optional[str]=Depends(
    oauth2_scheme), db: AsyncSession=Depends(get_async_db)) ->Optional[User]: if token is None:
...
def require_auth(func) ->Any: Decorator to require authentication for a route.
...
async def get_api_key_auth(request: Request, x_api_key: str=Header(None,
    alias='X-API-Key'), db: AsyncSession=Depends(get_async_db)) ->dict: Dependency for API key authentication.

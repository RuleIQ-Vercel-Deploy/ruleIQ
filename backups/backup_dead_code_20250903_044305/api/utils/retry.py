...
class RetryManager:
    ...
    def calculate_delay(self, attempt: int) ->float: delay = self.config.base_delay * self.config.exponential_base ** (
            attempt - 1)
    ...
    def should_retry(self, exception: Exception, attempt: int) ->bool: if attempt >= self.config.max_attempts:
            return False
    ...
    def execute_sync(self, func: Callable[..., T], *args, **kwargs) ->T: last_exception = None
        for attempt in range(1, self.config.max_attempts + 1):
            try:

...
class FrameworkCategory(str, Enum): DATA_PROTECTION = 'Data Protection'
    FINANCIAL_SERVICES = 'Financial Services'
    HEALTHCARE = 'Healthcare'
    CYBERSECURITY = 'Cybersecurity'
    GENERAL = 'General'

class GeographicRegion(str, Enum): UK = 'UK'
    ENGLAND = 'England'
    SCOTLAND = 'Scotland'
    WALES = 'Wales'
...
class UKFrameworkSchema(BaseModel): name: str = Field(..., min_length=1, max_length=100)
    ...
    def validate_uk_scope(cls, v) -> Any: uk_regions = {GeographicRegion.UK, GeographicRegion.ENGLAND, GeographicRegion.SCOTLAND, GeographicRegion.WALES, GeographicRegion.NORTHERN_IRELAND}
...
class FrameworkResponse(BaseModel): id: str
    name: str
    display_name: str
    description: str
    category: str
...
class FrameworkListResponse(BaseModel): frameworks: List[FrameworkResponse]
    total_count: int
    filtered_count: int
    region: Optional[str] = None
    category: Optional[str] = None

class FrameworkLoadRequest(BaseModel): frameworks: List[UKFrameworkSchema]
...
class FrameworkLoadResponse(BaseModel): success: bool
    loaded_count: int
    skipped_count: int
    error_count: int
...
class FrameworkQueryParams(BaseModel): region: Optional[str] = Field(None, description='Filter by geographic region')

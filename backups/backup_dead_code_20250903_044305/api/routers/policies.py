...
async def generate_policy(request: PolicyGenerateRequest, current_user:
    User=Depends(get_current_active_user), db: AsyncSession=Depends( get_async_db)) ->Dict[str, Any]:
...
async def list_policies(current_user: User=Depends(get_current_active_user),
    db: AsyncSession=Depends(get_async_db)) ->Dict[str, Any]: policies = await get_user_policies(db, current_user.id)
    return {'policies': policies}


@router.get('/{id}', response_model=GeneratedPolicyResponse)
async def get_policy(policy_id: UUID, current_user: User=Depends(
    get_current_active_user), db: AsyncSession=Depends(get_async_db)) ->Any: policy = await get_policy_by_id(db, policy_id, current_user.id)
    if not policy:
...
async def update_policy_status(policy_id: UUID, status_update: dict,
    ...
    Depends(get_async_db)) ->Dict[str, Any]: policy = await get_policy_by_id(db, policy_id, current_user.id)
    if not policy:
        raise HTTPException(status_code=HTTP_NOT_FOUND, detail=
            'Policy not found')
...
async def approve_policy(policy_id: UUID, current_user: User=Depends(
    get_current_active_user)) ->Dict[str, Any]: return {'message': 'Policy approved', 'policy_id': policy_id}


@router.post('/{id}/regenerate-section')
async def regenerate_policy_section(policy_id: UUID, section_data: dict,
    current_user: User=Depends(get_current_active_user), db: AsyncSession=
    Depends(get_async_db)) ->Dict[str, Any]: section_name = section_data.get('section', 'introduction')
...
async def clone_policy(policy_id: UUID, clone_data: dict, current_user:
    ...
    get_async_db)) ->Dict[str, Any]: new_name = clone_data.get('name', 'Cloned Policy')

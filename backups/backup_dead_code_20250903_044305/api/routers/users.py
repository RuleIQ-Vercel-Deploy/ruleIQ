...
async def get_user_profile(current_user: User=Depends(get_current_active_user)) -> Any: return current_user

async def get_user_dashboard(current_user: User=Depends(get_current_active_user), db: AsyncSession=Depends(get_async_db)) -> Dict[str, Any]: from sqlalchemy import select
...
async def deactivate_account(current_user: User=Depends(get_current_active_user), db: Session=Depends(get_db)) -> Dict[str, Any]:
    return {'message': 'Account deactivation needs to be implemented via Stack Auth admin API'}    """Deactivate Account"""

...
async def list_plans(current_user: User=Depends(get_current_active_user),
    db: AsyncSession=Depends(get_async_db)) ->Dict[str, Any]: plans = await list_implementation_plans(db, current_user)
    return {'plans': plans}


@router.get('/plans/{id}', response_model=ImplementationPlanResponse)
async def get_plan(id: UUID, current_user: User=Depends(
    get_current_active_user), db: AsyncSession=Depends(get_async_db)) ->Any: plan = await get_implementation_plan(db, current_user, id)
    if not plan:
...
async def update_task(plan_id: UUID, task_id: str, task_update:
    ImplementationTaskUpdate, current_user: User=Depends( get_current_active_user), db: AsyncSession=Depends(get_async_db)) ->Dict[
    str, Any]:
...
async def get_implementation_recommendations(framework: str=None,
    ...
    Depends(get_async_db)) ->Dict[str, Any]: from datetime import datetime, timezone
...
async def get_implementation_resources(frameworkId: str, current_user: User
    ...
    ) ->Dict[str, Any]: from datetime import datetime
...
async def get_implementation_plan_analytics(planId: UUID, current_user:
    ...
    get_async_db)) ->Dict[str, Any]: from datetime import datetime, timedelta

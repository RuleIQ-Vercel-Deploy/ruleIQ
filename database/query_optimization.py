...
class QueryOptimizer:
    ...
    async def get_evidence_with_relations(self, user_id: str, limit: int = 100) -> List[Dict[str, Any]]:
        """
        ...
        return [
            {
                ...
                "framework": (
                    {"id": str(item.framework.id), "name": item.framework.name, "category": item.framework.category}
                    if item.framework
                    else None
                ),
            }
    ...
    async def get_business_profile_with_counts(self, user_id: str) -> Optional[Dict[str, Any]]:
        ...
        query = (
            ...
            .outerjoin(AssessmentSession, AssessmentSession.business_profile_id == BusinessProfile.id)
    ...
    async def search_evidence_efficiently(
        ...
        base_query = (
            select(EvidenceItem)
            .options(selectinload(EvidenceItem.business_profile), selectinload(EvidenceItem.framework))
            .where(EvidenceItem.user_id == user_id)
        )
        conditions = []
        if query:
            conditions.append(
                or_(EvidenceItem.evidence_name.ilike(f"%{query}%"), EvidenceItem.description.ilike(f"%{query}%"))
            )
        if status:
        ...
        return [
            {
                ...
                "created_at": item.created_at.isoformat() if item.created_at else None,
                "business_profile": (
                    {"id": str(item.business_profile.id), "company_name": item.business_profile.company_name}
                    if item.business_profile
                    else None
                ),
                "framework": {"id": str(item.framework.id), "name": item.framework.name} if item.framework else None,
            }
            for item in evidence_items
        ]
    ...
    async def analyze_query(self, query: str, params: dict = None) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"execution_time": 0, "error": str(e), "index_suggestions": ["Unable to analyze query"]}
    ...
    async def get_slow_queries(self, threshold_ms: float = 100) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.warning("Could not get slow queries (pg_stat_statements may not be enabled): %s" % e)
            return []
...
class BatchQueryOptimizer:
    ...
    async def batch_delete_evidence(self, evidence_ids: List[str]) -> int:
        ...
        query = (
            delete(EvidenceItem).where(EvidenceItem.id.in_(evidence_ids)).execution_options(synchronize_session=False)
        )
        result = await self.db.execute(query)
        await self.db.commit()
        return result.rowcount

...
Please copy env.template to .env.local and configure the variables."""
    ...
    def get_database_urls() -> tuple[str, str, str]:
        ...
        # Normalize postgres:// to postgresql:// (handle legacy Heroku format)
        if db_url.startswith("postgres://"):
            db_url = db_url.replace("postgres://", "postgresql://", 1)
            logger.info("Normalized legacy postgres:// scheme to postgresql://")
    ...
    def get_engine_kwargs(is_async: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            if is_async:
                ...
                db_url = os.getenv("DATABASE_URL", "")
                connect_args = {"server_settings": {"jit": "off", "application_name": "ruleIQ_backend"}}
    ...
    if _ASYNC_ENGINE is None:
        try:
            ...
            _ASYNC_SESSION_LOCAL = async_sessionmaker(
                bind=_ASYNC_ENGINE, class_=AsyncSession, expire_on_commit=False, autocommit=False, autoflush=False
            )
            logger.info(
...
def get_engine_info() -> Dict[str, Any]:
    ...
    info = {"sync_engine_initialized": _ENGINE is not None, "async_engine_initialized": _ASYNC_ENGINE is not None}
    if _ASYNC_ENGINE:

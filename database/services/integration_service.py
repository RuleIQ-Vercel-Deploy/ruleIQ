...
class IntegrationService:
    ...
    async def store_integration_config(self, user_id: str, provider: str,
        credentials: APICredentials, health_info: Dict[str, Any],
        ...
        try:
            ...
            if provider not in ['aws', 'okta', 'google_workspace',
                'microsoft_365']:
                raise ValueError(f'Invalid provider: {provider}')
            if not credentials or not isinstance(credentials, APICredentials):
                raise ValueError(
                    'credentials must be an APICredentials instance')
            ...
            async with self.db.begin():
                stmt = select(Integration).where(and_(Integration.user_id ==
                    user_id, Integration.provider == provider))
                result = await self.db.execute(stmt)
                existing_integration = result.scalar_one_or_none()
                if existing_integration:
                    existing_integration.encrypted_credentials = (
                        encrypted_creds)
                    ...
                    logger.info('Updated integration %s for user %s' % (
                        provider, user_id))
                else:
                    integration = Integration(user_id=user_id, provider=
                        provider, encrypted_credentials=encrypted_creds,
                        health_status=health_info, configuration_metadata=
                        configuration_metadata or {}, is_active=True)
                    self.db.add(integration)
                    logger.info('Created new integration %s for user %s' %
                        (provider, user_id))
                await self.db.flush()
                await self._log_health_check(integration.id, health_info)
            await self.db.refresh(integration)
            await self._create_audit_log(user_id=user_id, integration_id=
                integration.id, action='create' if not existing_integration
                 else 'update', resource_type='integration', resource_id=
                str(integration.id), details={'provider': provider,
                'health_status': health_info.get('status')})
            return integration
        except Exception as e:
            logger.error('Failed to store integration config for %s: %s' %
                (provider, e))
            raise

    async def get_user_integrations(self, user_id: str, active_only: bool=
        True, include_health: bool=False) ->List[Integration]:
        """
        ...
        try:
            ...
            if include_health:
                stmt = stmt.options(selectinload(Integration.
                    evidence_collections))
            stmt = stmt.order_by(Integration.created_at.desc())
            result = await self.db.execute(stmt)
            integrations = result.scalars().all()
            logger.debug('Retrieved %s integrations for user %s' % (len(
    ...
    async def get_integration_by_id(self, integration_id: str, user_id:
        Optional[str]=None) ->Optional[Integration]:
        """
    ...
    async def decrypt_integration_credentials(self, integration: Integration
        ) ->APICredentials:
        """
        ...
        """
        try:
            decrypted_creds = self.encryption.decrypt_credentials(integration
                .encrypted_credentials)
            api_credentials = APICredentials(provider=integration.provider,
                auth_type=AuthType(decrypted_creds.get('auth_type',
                'api_key')), credentials=decrypted_creds, region=
                decrypted_creds.get('region'))
            logger.debug('Decrypted credentials for integration %s' %
                integration.id)
            return api_credentials
        except CredentialDecryptionError as e:
            logger.error(
                'Failed to decrypt credentials for integration %s: %s' % (
                integration.id, e))
            raise
        except Exception as e:
    ...
    async def update_integration_health(self, integration_id: str,
        health_data: Dict[str, Any], user_id: Optional[str]=None) ->bool:
        """
        ...
        try:
            integration = await self.get_integration_by_id(integration_id,
                user_id)
            if not integration:
                logger.warning('Integration %s not found for health update' %
                    integration_id)
                return False
    ...
    async def delete_integration(self, integration_id: str, user_id: str
        ) ->bool:
        """
        ...
        try:
            integration = await self.get_integration_by_id(integration_id,
                user_id)
            if not integration:
                logger.warning('Integration %s not found for deletion' %
                    integration_id)
                return False
            integration.is_active = False
            integration.updated_at = datetime.now(timezone.utc)
            await self._create_audit_log(user_id=user_id, integration_id=
                integration_id, action='delete', resource_type=
                'integration', resource_id=integration_id, details={
    ...
    async def _log_health_check(self, integration_id: str, health_data:
        Dict[str, Any]) ->None:
        """Log health check result"""
        try:
            health_log = IntegrationHealthLog(integration_id=integration_id,
                status=health_data.get('status', 'unknown'), response_time=
                health_data.get('response_time'), error_details=health_data
                .get('error'), health_data=health_data)
            self.db.add(health_log)
        except Exception as e:
            logger.warning('Failed to log health check: %s' % e)

    async def _create_audit_log(self, user_id: str, action: str,
        resource_type: str, resource_id: str, integration_id: Optional[str]
        =None, collection_id: Optional[str]=None, evidence_item_id:
        Optional[str]=None, details: Optional[Dict[str, Any]]=None,
        ip_address: Optional[str]=None, user_agent: Optional[str]=None,
        request_id: Optional[str]=None) ->None:
        """Create audit log entry"""
        try:
            audit_log = EvidenceAuditLog(user_id=user_id, integration_id=
                integration_id, collection_id=collection_id,
                evidence_item_id=evidence_item_id, action=action,
                resource_type=resource_type, resource_id=resource_id,
                details=details or {}, ip_address=ip_address, user_agent=
                user_agent, request_id=request_id)
...
class EvidenceCollectionService:
    ...
    async def create_evidence_collection(self, integration_id: str, user_id:
        str, framework_id: str, evidence_types_requested: List[str],
        business_profile: Dict[str, Any], collection_mode: str='immediate'
        ) ->EvidenceCollection:
        """
        ...
        """
        try:
            collection = EvidenceCollection(integration_id=integration_id,
                user_id=user_id, framework_id=framework_id,
                evidence_types_requested=evidence_types_requested,
                business_profile=business_profile, collection_mode=
                collection_mode, status='pending')
    ...
    async def update_collection_status(self, collection_id: str, status:
        str, progress_percentage: Optional[int]=None, current_activity:
        Optional[str]=None, errors: Optional[List[str]]=None, quality_score:
        Optional[Dict[str, float]]=None) ->bool:
        """Update evidence collection status"""
        try:
            stmt = select(EvidenceCollection).where(EvidenceCollection.id ==
                collection_id)
            result = await self.db.execute(stmt)
    ...
    async def store_evidence_item(self, collection_id: str, evidence_type:
        str, source_system: str, resource_id: str, resource_name: str,
        evidence_data: Dict[str, Any], compliance_controls: List[str],
        quality_score: Dict[str, Any], collected_at: datetime,
        data_classification: str='internal') ->IntegrationEvidenceItem:
        """Store individual evidence item"""
        try:
            data_json = json.dumps(evidence_data, sort_keys=True,
                separators=(',', ':'))
            checksum = hashlib.sha256(data_json.encode()).hexdigest()
            evidence_item = IntegrationEvidenceItem(collection_id=
                collection_id, evidence_type=evidence_type, source_system=
                source_system, resource_id=resource_id, resource_name=
                resource_name, evidence_data=evidence_data,
                compliance_controls=compliance_controls, quality_score=
                quality_score, collected_at=collected_at,
                data_classification=data_classification, checksum=checksum)
    ...
    async def get_collection_status(self, collection_id: str, user_id:
        Optional[str]=None) ->Optional[EvidenceCollection]:
    ...
    async def get_collection_evidence(self, collection_id: str,
        evidence_type: Optional[str]=None, page: int=1, page_size: int=100
        ...
        try:
            ...
            if evidence_type:
                stmt = stmt.where(IntegrationEvidenceItem.evidence_type ==
                    evidence_type)
...
async def decrypt_integration_credentials(integration: Integration
    ...
    try:
        decrypted_creds = encryption.decrypt_credentials(integration.
            encrypted_credentials)
        return APICredentials(provider=integration.provider, auth_type=
            AuthType(decrypted_creds.get('auth_type', 'api_key')),
            credentials=decrypted_creds, region=decrypted_creds.get('region'))
    except Exception as e:
...
async def update_integration_health(integration_id: str, health_data: Dict[
    ...
    service = IntegrationService(db)
    return await service.update_integration_health(integration_id,
        health_data, user_id)

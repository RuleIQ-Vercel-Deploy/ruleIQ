...
def process_connections(
    connections: List[Dict[str, Any]], stats: Dict[str, int]
) -> List[Dict[str, Any]]:
...
def update_html_file(input_file: str, output_file: str):
    ...
    # Process connections
    stats = {"frontend_updated": 0, "backend_updated": 0, "removed": 0, "unchanged": 0}

    updated_connections = process_connections(connections, stats)

    # Count unchanged
    stats["unchanged"] = (
        len(updated_connections) - stats["frontend_updated"] - stats["backend_updated"]
    )
    ...
    updated_html = re.sub(
        r"(<h1>.*?</h1>\s*<p>.*?</p>)", r"\1" + update_notice, updated_html
    )

...
class SupersedesRelationshipBuilder:
    ...
    async def _create_gdpr_supersessions(self, session):
        ...
        async for record in result:
            count += 1
            logger.info(
                f"GDPR supersession: {record['new_reg']} -> {record['old_reg']}"
            )
            ...
            if len(regs) > 1:
                ...
                for i in range(len(sorted_regs) - 1):
                    ...
                    if await result.single():
                        count += 1
                        logger.info(
                            f"Version supersession: {newer['id']} -> {older['id']}"
                        )
    ...
    async def _create_brexit_supersessions(self, session):
        ...
        async for record in result:
            count += 1
            logger.info(
                f"Brexit supersession: {record['uk_reg']} -> {record['eu_reg']}"
            )
        ...
        if len(pci_regs) > 1:
            for i in range(len(pci_regs) - 1):
                ...
                if await result.single():
                    count += 1
                    logger.info(
                        f"PCI DSS supersession: {pci_regs[i+1]} -> {pci_regs[i]}"
                    )

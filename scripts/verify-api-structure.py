        ...
        for endpoint in paths:
            ...
            for param in params:
                if param not in ['id', 'user_id', 'framework_id', 'assessment_id']:
                    issues['inconsistent_params'].append(f"{router}: {endpoint} - param: {{{param}}}")
    ...
    for i, (router1, endpoint1) in enumerate(all_paths):
        for router2, endpoint2 in all_paths[i+1:]:
            ...
            if method1 == method2:
                ...
                if 'generate' in norm_path1 and 'generate' in norm_path2 and router1 != router2:
                    duplicates.append(f"{router1}: {endpoint1} <-> {router2}: {endpoint2}")
...
def main():
    ...
    for router, paths in endpoints.items():
        if 'polic' in router.lower() or 'ai' in router.lower():
            policy_related = [p for p in paths if 'polic' in p.lower() or 'generate' in p.lower()]
            if policy_related:

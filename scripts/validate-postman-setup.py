...
def validate_environment(filename):
    ...
    try:
        ...
        required_vars = ['base_url', 'api_version', 'test_user_email', 'test_user_password']
...
def main():
    ...
    if success:
        ...
        print(f"  ✅ Authentication: {'Configured' if result['has_auth'] else 'Not configured'}")
    else:
        print(f"  ❌ {result}")
    ...
    if success:
        ...
        print(f"  ✅ Test Credentials: {'Correct' if result['has_credentials'] else 'Incorrect'}")
    else:
        print(f"  ❌ {result}")
    ...
    try:
        import requests
        login_data = {
            'username': 'test@ruleiq.dev',
            'password': 'TestPassword123!'
        }
        response = requests.post('http://localhost:8000/api/v1/auth/token', data=login_data, timeout=5)
        if response.status_code == 200:

...
async def test_neon_connection():
    ...
    is_local = parsed.hostname in ['localhost', '127.0.0.1', 'db', 'postgres'] if parsed.hostname else False
    ...
    try:
        ...
        if "connection refused" in str(e).lower():
            ...
            print("   Please ensure DATABASE_URL in .env.local points to your Neon cloud database.")
    ...
    try:
        ...
        for i in range(10):
            async def query_task(task_id):
                async with engine.begin() as conn:
                    result = await conn.execute(text(f"SELECT {task_id} as id, NOW() as time"))

            ...
            for pattern in patterns:
                if pattern in func_section:
                    ...
                    if auth_type == 'rate_limit':
                        ...
                        perm_match = re.search(r'require_permissions?\(["\']([^"\']+)["\']', func_section)
                        if perm_match:
    ...
    def generate_report(self, endpoints: List[Dict]) -> Dict:
        ...
        report = {
            'summary': {
                ...
                'authenticated_endpoints': len([e for e in endpoints if e['authentication']['required']]),
                'public_endpoints': len([e for e in endpoints if not e['authentication']['required']]),
                'jwt_endpoints': len([e for e in endpoints if e['authentication']['type'] == 'jwt']),
                'stack_auth_endpoints': len([e for e in endpoints if e['authentication']['type'] == 'stack_auth']),
                'rate_limited_endpoints': len([e for e in endpoints if e['authentication']['rate_limited']]),
                'rbac_protected_endpoints': len([e for e in endpoints if e['authentication']['rbac_permissions']])
            },
            'endpoints_by_file': {},
        ...
        for endpoint in endpoints:
            ...
            if not auth['required'] and any(sensitive in endpoint['path'].lower() 
...
def main():
    ...
    print(f"📄 API documentation saved to: API_ENDPOINTS_DOCUMENTATION.md")

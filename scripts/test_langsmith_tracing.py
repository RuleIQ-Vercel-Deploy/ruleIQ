...
async def test_langsmith_tracing() -> Optional[bool]:
    ...
    try:
        ...
        async with AsyncSessionLocal() as db:
            ...
            result = await agent.start_assessment(
                session_id=test_session_id, lead_id=test_lead_id, initial_context=test_context
            )
            ...
            if result.get("messages"):
                ...
                result2 = await agent.process_user_response(
                    session_id=test_session_id, user_response=test_response, confidence="high"
                )

...
class ComprehensiveAPITester:
    ...
    def test_endpoint_group(
        self, group_name: str, endpoints: List[Tuple]
    ) -> Dict[str, Any]:
    ...
    def _test_single_endpoint(
        self, method: str, endpoint: str, expected_codes: List[int], data: Dict = None
    ) -> Dict[str, Any]:
    ...
    def _calculate_group_summary(self, results: List[Dict]) -> Dict[str, Any]:
        ...
        avg_response_time = (
            round(sum(valid_times) / len(valid_times)) if valid_times else 0
        )

        # Count by status code ranges
        status_codes = {
            "2xx": sum(
                1 for r in results if r["status_code"] and 200 <= r["status_code"] < 300
            ),
            "3xx": sum(
            ...
            "5xx": sum(
                1 for r in results if r["status_code"] and 500 <= r["status_code"] < 600
            ),
    ...
    def run_comprehensive_tests(self) -> List[Dict]:
        ...
        all_results.append(
            self.test_endpoint_group("Authentication & Users", auth_endpoints)
        )
        ...
        business_endpoints = [
            ...
            (
                "GET",
                "/api/v1/business-profiles/current",
                [200, 404],
        ...
        all_results.append(
            self.test_endpoint_group("Business Profiles", business_endpoints)
        )
        ...
        framework_endpoints = [
            ...
            (
                "GET",
                "/api/v1/compliance-frameworks/gdpr",
                [200, 404],
                "Get GDPR framework",
            ),
            (
                ...
                "/api/v1/frameworks",
                [200, 404],
                "Alternative frameworks endpoint",
            ),
            ("GET", "/api/v1/frameworks/gdpr", [200, 404], "Get GDPR via frameworks"),
        ]
        all_results.append(
            self.test_endpoint_group("Compliance Frameworks", framework_endpoints)
        )
        ...
        all_results.append(
            self.test_endpoint_group("Assessments & Questions", assessment_endpoints)
        )
        ...
        ai_endpoints = [
            ...
            (
                "POST",
                "/api/v1/ai-assessments/start",
                [200, 400, 422, 500],
                "Start AI assessment",
            ),
            (
                "GET",
                "/api/v1/ai-assessments/status",
                [200, 404, 500],
        ...
        all_results.append(
            self.test_endpoint_group("Evidence & Implementation", evidence_endpoints)
        )
        ...
        all_results.append(
            self.test_endpoint_group("Health & Status", health_endpoints)
        )
    ...
    def generate_comprehensive_report(self, test_groups: List[Dict]) -> Dict[str, Any]:
        ...
        overall_success_rate = (
            round((total_successful / total_tests) * 100, 1) if total_tests > 0 else 0
        )
        ...
        return {
            ...
            "group_summaries": [
                {"group_name": group["group_name"], "summary": group["summary"]}
                for group in test_groups
            ],

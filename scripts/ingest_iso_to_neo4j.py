...
class ISOComplianceNeo4jIngestion:
    ...
    def __init__(self):
        ...
        neo4j_uri = (
            os.popen("doppler secrets get NEO4J_URI --plain 2>/dev/null").read().strip()
        )
        neo4j_user = (
            os.popen("doppler secrets get NEO4J_USER --plain 2>/dev/null")
            .read()
            .strip()
        )
        neo4j_password = (
            os.popen("doppler secrets get NEO4J_PASSWORD --plain 2>/dev/null")
            .read()
            .strip()
        )
            ...
            for framework_name, framework_data in manifest["frameworks"].items():
                ...
                for obligation in framework_data["obligations"]:
                    ...
                    if clause_num not in clauses:
                        clauses[clause_num] = {"title": clause_title, "controls": []}
        ...
        with self.driver.session() as session:
            ...
            relationships = [
                ...
                (
                    ...
                    "ISO_9001_2015",
                    "COMPLEMENTS",
                    "Information security in quality management",
                ),
                (
                    "ISO_27001_2022",
                    "ISO_22301_2019",
            ...
            for source, target, rel_type, description in relationships:
                ...
                session.run(
                    rel_query, source=source, target=target, description=description
                )

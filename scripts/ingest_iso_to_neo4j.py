...
class ISOComplianceNeo4jIngestion:
    ...
    def ingest_manifest(self, manifest_path: Path):
        ...
        with self.driver.session() as session:
            # Create ISO Compliance Meta-Framework node
            ...
            session.run(meta_framework_query,
                       created_at=manifest.get('created_at', datetime.now().isoformat()),
                       version=manifest.get('version', '1.0'),
                       total_obligations=manifest.get('total_obligations', 0))
            
            print("Created ISO Standards Suite meta-framework node")
            
            # Process each ISO framework
            for framework_name, framework_data in manifest['frameworks'].items():
                ...
                session.run(framework_query,
                           framework_id=framework_name.replace(':', '_').replace(' ', '_'),
                           name=framework_name,
                           title=framework_data['title'],
                           version=framework_data['version'],
                           control_count=framework_data['control_count'],
                           mandatory_count=framework_data['mandatory_count'])
                
                # Link Framework to Meta-Framework
                ...
                session.run(link_framework_query,
                           framework_id=framework_name.replace(':', '_').replace(' ', '_'))
                
                print(f"Created ISO Framework node: {framework_name}")
                
                # Create clause structure
                clauses = {}
                ...
                for clause_num, clause_data in clauses.items():
                    clause_id = f"{framework_name.replace(':', '_').replace(' ', '_')}-Clause-{clause_num}"
                    ...
                    session.run(clause_query,
                               ...
                               control_count=len(clause_data['controls']))
                    
                    # Link Clause to Framework
                    link_clause_query = """
                        MATCH (f:ISOFramework {framework_id: $framework_id})
                    ...
                    session.run(link_clause_query,
                               framework_id=framework_name.replace(':', '_').replace(' ', '_'),
                               clause_id=clause_id)
                    
                    # Create Control/Obligation nodes
                    for control in clause_data['controls']:
                        ...
                        # Create Obligation node (for consistency with UK model)
                        obligation_query = """
                        ...
                        session.run(link_control_query,
                                   clause_id=clause_id,
                                   control_id=control['obligation_id'])
                        ...
                        session.run(link_obligation_query,
                                   control_id=control['obligation_id'],
                                   obligation_id=control['obligation_id'])
            
            print(f"Created {manifest['total_obligations']} ISO control/obligation nodes")
    
    def create_cross_framework_relationships(self):
        """Create relationships between related ISO frameworks."""
        with self.driver.session() as session:
            # Link related management systems
            relationships = [
                # ISO 27001 relates to all others for information security aspects
                ("ISO_27001_2022", "ISO_9001_2015", "COMPLEMENTS", "Information security in quality management"),
                ("ISO_27001_2022", "ISO_22301_2019", "COMPLEMENTS", "Information security in business continuity"),
                ("ISO_27001_2022", "ISO_14001_2015", "COMPLEMENTS", "Information security in environmental management"),
                ("ISO_27001_2022", "ISO_45001_2018", "COMPLEMENTS", "Information security in OH&S management"),
                
                # ISO 9001 is foundational for other management systems
                ("ISO_9001_2015", "ISO_22301_2019", "SUPPORTS", "Quality foundation for business continuity"),
                ("ISO_9001_2015", "ISO_14001_2015", "SUPPORTS", "Quality foundation for environmental management"),
                ("ISO_9001_2015", "ISO_45001_2018", "SUPPORTS", "Quality foundation for OH&S management"),
                
                # Business continuity relates to safety and environment
                ("ISO_22301_2019", "ISO_45001_2018", "COMPLEMENTS", "Business continuity for safety incidents"),
                ("ISO_22301_2019", "ISO_14001_2015", "COMPLEMENTS", "Business continuity for environmental incidents"),
                
                # Environmental and OH&S are closely related
                ("ISO_14001_2015", "ISO_45001_2018", "INTEGRATES_WITH", "Environmental and occupational health integration")
            ]
            ...
            print(f"Created {len(relationships)} cross-framework relationships")
    
    def create_common_controls(self):
        """Identify and link common controls across ISO standards."""
        with self.driver.session() as session:
            # Common control themes across ISO standards
            common_themes = [
                {
                    'theme': 'Leadership and Commitment',
                    'controls': ['5.1', '5.2', '5.3'],
                    'description': 'Top management leadership requirements'
                },
                {
                    'theme': 'Risk Management',
                    'controls': ['6.1', '6.1.1', '6.1.2', '8.2', '8.3'],
                    'description': 'Risk assessment and treatment'
                },
                {
                    'theme': 'Document Control',
                    'controls': ['7.5'],
                    'description': 'Documented information management'
                },
                {
                    'theme': 'Internal Audit',
                    'controls': ['9.2'],
                    'description': 'Internal audit requirements'
                },
                {
                    'theme': 'Management Review',
                    'controls': ['9.3'],
                    'description': 'Management review requirements'
                },
                {
                    'theme': 'Continual Improvement',
                    'controls': ['10.1', '10.2', '10.3'],
                    'description': 'Improvement and corrective action'
                }
            ]
            
            for theme_data in common_themes:
                # Create CommonTheme node
                theme_query = """
                ...
                session.run(theme_query,
                           name=theme_data['theme'],
                           description=theme_data['description'])
                
                # Link controls to common themes
                for control_pattern in theme_data['controls']:
                    ...
                    session.run(link_theme_query,
                               theme=theme_data['theme'],
                               control_pattern=control_pattern)
            
            print(f"Created {len(common_themes)} common control themes")
    
    def link_iso_to_uk_regulations(self):
        """Create relationships between ISO standards and UK regulations."""
        with self.driver.session() as session:
            # Map ISO standards to relevant UK regulations
            mappings = [
                # ISO 27001 maps to GDPR/DPA
                {
                    'iso': 'ISO_27001_2022',
                    'uk_pattern': 'GDPR',
                    'relationship': 'SUPPORTS_COMPLIANCE',
                    'description': 'ISO 27001 supports GDPR compliance'
                },
                # ISO 22301 maps to operational resilience requirements
                {
                    'iso': 'ISO_22301_2019',
                    'uk_pattern': 'FCA',
                    'relationship': 'SUPPORTS_COMPLIANCE',
                    'description': 'ISO 22301 supports FCA operational resilience'
                },
                # ISO 9001 supports quality in financial services
                {
                    'iso': 'ISO_9001_2015',
                    'uk_pattern': 'FCA',
                    'relationship': 'ENHANCES_COMPLIANCE',
                    'description': 'ISO 9001 enhances FCA compliance quality'
                },
                # ISO 45001 maps to HSWA
                {
                    'iso': 'ISO_45001_2018',
                    'uk_pattern': 'HSWA',
                    'relationship': 'SUPPORTS_COMPLIANCE',
                    'description': 'ISO 45001 supports health and safety compliance'
                },
                # ISO 14001 supports environmental regulations
                {
                    'iso': 'ISO_14001_2015',
                    'uk_pattern': 'Environment',
                    'relationship': 'SUPPORTS_COMPLIANCE',
                    'description': 'ISO 14001 supports environmental compliance'
                }
            ]
            ...
            for mapping in mappings:
                ...
                try:
                    session.run(map_query,
                               iso_id=mapping['iso'],
                               uk_pattern=mapping['uk_pattern'],
                               description=mapping['description'])
                except:
                    # UK regulations might not exist yet
                    pass
            
            print("Created ISO to UK regulation mappings")
    ...
    def generate_statistics(self):
        ...
        with self.driver.session() as session:
            ...
            print("\n" + "="*80)
            print("ISO FRAMEWORK NEO4J INGESTION COMPLETE")
            print("="*80)
            print(f"Meta-Frameworks: {result['meta_frameworks']}")
            print(f"ISO Frameworks: {result['frameworks']}")
            print(f"Clauses: {result['clauses']}")
            print(f"Controls: {result['controls']}")
            print(f"Obligations: {result['obligations']}")
            print(f"Common Themes: {result['themes']}")
...
def main():
    ...
    try:
        ...
        # Ingest manifest
        manifest_path = Path('/home/omar/Documents/ruleIQ/data/manifests/iso_compliance_manifest.json')
        ...
        # Create common control themes
        ingestion.create_common_controls()
        
        # Link to UK regulations if they exist

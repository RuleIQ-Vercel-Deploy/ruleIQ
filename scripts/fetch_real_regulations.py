...
class RegulationDataFetcher:
    ...
    async def fetch_and_update_regulation(self, reg_info: Dict[str, str], client: RegulationAPIClient) -> Dict[str, Any]:
        ...
        if not result['success']:
            ...
            return {'id': reg_info['id'], 'success': False, 'error': result.get('error')}
        ...
        async with self.driver.session() as session:
            ...
            if result.get('enforcement'):
                ...
                powers=result['enforcement'].get('powers', [])[:3])  # Limit to 3
        
        logger.info(f"✅ Updated {reg_info['id']} with {len(result['requirements'])} requirements, {len(result['penalties'])} penalties")

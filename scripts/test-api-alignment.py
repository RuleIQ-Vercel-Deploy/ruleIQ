...
class APIAlignmentTester:
    ...
    async def run_tests(self):
        ...
        print(f"Timestamp: {colored(datetime.now().strftime('%Y-%m-%d %H:%M:%S'), Colors.YELLOW)}")
        print(f"{colored('=' * 60, Colors.CYAN)}\n")
        ...
        endpoints = [
            ...
            ("POST", "/api/v1/freemium-assessment", "Create freemium assessment", False),
            ("GET", "/api/v1/freemium-assessment/questions", "Get freemium questions", False),
            ...
            ("GET", "/api/v1/ai/optimization/circuit-breaker/status", "Circuit breaker status", True),
            ("POST", "/api/v1/ai/optimization/circuit-breaker/reset", "Reset circuit breaker", True),
            ...
            ("GET", "/api/v1/frameworks/{id}/requirements", "Get framework requirements", True),
            ...
            ("GET", "/api/v1/evidence-collection/requirements", "Get requirements", True),
            
            # Foundation Evidence (2 endpoints)
            ("GET", "/api/v1/foundation/evidence", "List foundation evidence", True),
            ("POST", "/api/v1/foundation/evidence/collect", "Collect foundation evidence", True),
            ...
            ("GET", "/api/v1/compliance/recommendations", "Compliance recommendations", True),
            
            # UK Compliance (3 endpoints)
            ("GET", "/api/v1/uk-compliance/gdpr/requirements", "GDPR requirements", True),
            ("GET", "/api/v1/uk-compliance/companies-house/filing", "Companies House filing", True),
            ("GET", "/api/v1/uk-compliance/employment/regulations", "Employment regulations", True),
            ...
            ("POST", "/api/v1/agentic-rag/query-with-validation", "Query with validation", True),
        ]
        ...
        async with aiohttp.ClientSession() as session:
            ...
            if not self.auth_token:
                print(f"{colored('⚠️  No auth token available - auth endpoints will show as 401', Colors.YELLOW)}\n")
            ...
            for method, path, description, auth_required in endpoints:
                ...
                # Print result
                print(f"{colored(status_symbol, status_color)} {method:6} {path:50} "
    ...
    def print_summary(self):
        ...
        print(f"Failed: {colored(f'{failed} ({100-pass_rate:.1f}%)', Colors.RED if failed > 0 else Colors.GREEN)}")
        
        if pass_rate == 100:
            print(f"\n{colored('🎉 PERFECT ALIGNMENT!', Colors.GREEN + Colors.BOLD)}")
            print(f"{colored('All API endpoints are properly connected and working!', Colors.GREEN)}")
        elif pass_rate >= 95:
            print(f"\n{colored('✅ EXCELLENT ALIGNMENT', Colors.GREEN)}")
            print(f"{colored('Nearly all endpoints are working correctly.', Colors.GREEN)}")
        elif pass_rate >= 90:
        ...
        async with aiohttp.ClientSession() as session:
            try:
                async with session.get(f"{BASE_URL}/health", timeout=TIMEOUT) as response:
                    if response.status != 200:
                        print(f"{colored('⚠️  Server health check failed', Colors.YELLOW)}")
            except:
                print(f"{colored('❌ Server is not running!', Colors.RED)}")
                print(f"Please start the server with: {colored('python main.py', Colors.YELLOW)}")
                return

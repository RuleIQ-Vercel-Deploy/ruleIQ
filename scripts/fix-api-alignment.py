...
class APIAlignmentFixer:
    ...
    def extract_backend_endpoints(self, filepath: Path, prefix: str) -> List[Dict]:
        ...
        for method, path in endpoints:
            full_path = (
                f"{prefix}{path}" if not path.startswith("/") else f"{prefix}{path}"
            )
            result.append(
                {
                    "method": method.upper(),
    ...
    def extract_frontend_endpoints(self, filepath: Path) -> List[Dict]:
        ...
        for method, endpoint in matches:
            ...
            actual_endpoint = (
                endpoint if endpoint.startswith("/api") else f"/api/v1{endpoint}"
            )
        ...
        for service, endpoints in self.frontend_endpoints.items():
            for endpoint in endpoints:
                ...
                for backend_key, backend_endpoint in all_backend.items():
                    if backend_key.startswith(method) and self.paths_match(
                        path, backend_endpoint["full_path"]
                    ):
    ...
    def generate_fixes(self, mismatches: Dict):
        ...
        fixes = {"backend_fixes": [], "frontend_fixes": [], "new_endpoints_needed": []}
    ...
    def generate_report(self, mismatches: Dict, fixes: Dict):
        ...
        report = {
            ...
            .strip(),
            "summary": {
                "total_frontend_endpoints": sum(
                    len(e) for e in self.frontend_endpoints.values()
                ),
                "total_backend_endpoints": sum(
                    len(d["endpoints"]) for d in self.backend_endpoints.values()
                ),

...
class APIAlignmentFixer:
    ...
    def fix_parameter_naming(self, file_path: Path) -> List[str]:
        ...
        if '{id}' in content:
            content = re.sub(r'def \w+\([^)]*\bplan_id\b', 
                           lambda m: m.group(0).replace('plan_id', 'id'), content)
            content = re.sub(r'def \w+\([^)]*\btask_id\b',
                           lambda m: m.group(0).replace('task_id', 'id'), content)
            content = re.sub(r'def \w+\([^)]*\balertId\b',
                           lambda m: m.group(0).replace('alertId', 'id'), content)
            content = re.sub(r'def \w+\([^)]*\bprofileId\b',
                           lambda m: m.group(0).replace('profileId', 'id'), content)
            content = re.sub(r'def \w+\([^)]*\breportId\b',
                           lambda m: m.group(0).replace('reportId', 'id'), content)
    ...
    def remove_duplicate_endpoints(self, file_path: Path, router_name: str) -> List[str]:
        ...
        for i, line in enumerate(lines):
            ...
            route_match = re.match(r'^@\w+\.(get|post|put|patch|delete)\([\'"`]([^\'"`]+)[\'"`]', line)
            ...
            if route_match:
                ...
                for dup_path, keep_in_router in self.duplicate_handlers.items():
                    if full_path == dup_path and router_name != keep_in_router:
                        should_keep = False
                        changes.append(f"  Removed duplicate {method} {full_path} (kept in {keep_in_router})")

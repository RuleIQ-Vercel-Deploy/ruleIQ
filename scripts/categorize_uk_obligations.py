...
REGULATION_PATTERNS = {
    ...
    "HSWA": [r"health.?safety", r"hswa", r"workplace.?safety", r"occupational.?health"],
    "PECR": [
...
def main():
    ...
    if len(selected_obligations) < 108:
        ...
        for reg_name, reg_data in sorted(
            regulation_obligations.items(), key=lambda x: x[1]["count"], reverse=True
        ):
            additional = reg_data["obligations"][
                target_per_regulation : target_per_regulation + remaining
            ]

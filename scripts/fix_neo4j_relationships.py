...
class Neo4jRelationshipFixer:
    ...
    async def _add_generic_business_triggers(self):
        ...
        async with self.driver.session() as session:
            ...
            logger.info(f"Found {len(regulations)} regulations without business_triggers")
            ...
            logger.info(f"✅ Added generic business_triggers to {len(regulations)} regulations")
    
    def _generate_business_triggers(self, tags: List[str], title: str) -> Dict[str, Any]:
    ...
    async def create_dependency_relationships(self):
        ...
        async with self.driver.session() as session:
            ...
            dependencies = [
                ...
                ("gdpr-general", "gdpr-article-32", "Article 32 depends on general GDPR"),
                
                # PCI DSS dependencies
                ("pci-dss-general", "pci-dss-requirement-1", "Requirement 1 depends on PCI DSS"),
                ("pci-dss-general", "pci-dss-requirement-2", "Requirement 2 depends on PCI DSS"),
                
                # SOC 2 dependencies
                ("soc2-general", "soc2-security", "Security principle depends on SOC 2"),
                ("soc2-general", "soc2-availability", "Availability principle depends on SOC 2"),
            ]
            ...
            for parent_id, child_id, description in dependencies:
                ...
                async for record in result:
                    created += 1
                    logger.info(f"Created DEPENDS_ON: {record['child']} -> {record['parent']}")
    ...
    async def create_equivalence_relationships(self):
        ...
        async with self.driver.session() as session:
            ...
            equivalences = [
                ...
                ("gdpr", "uk-data-protection", "privacy", "Similar data protection laws"),
                
                # Security framework equivalences
                ("iso27001", "soc2-security", "security", "Security management frameworks"),
            ...
            for reg1_pattern, reg2_pattern, tag, description in equivalences:
                ...
                async for record in result:
                    created += 1
                    logger.info(f"Created EQUIVALENT_TO: {record['reg1']} <-> {record['reg2']}")
    ...
    async def create_supersedes_relationships(self):
        ...
        async with self.driver.session() as session:
            ...
            supersessions = [
                ...
                ("gdpr-general", "data-protection-directive-95", "GDPR replaced 1995 directive"),
                ...
                ("uk-gdpr", "gdpr-general", "UK GDPR supersedes EU GDPR post-Brexit", "UK"),
            ]
            ...
            for new_pattern, old_pattern, description, *conditions in supersessions:
                ...
                async for record in result:
                    created += 1
                    logger.info(f"Created SUPERSEDES: {record['new_reg']} -> {record['old_reg']}")
    ...
    async def create_control_dependencies(self):
        ...
        async with self.driver.session() as session:
            ...
            control_deps = [
                ("encryption_at_rest", "key_management", "Encryption requires key management"),
                ("access_control", "authentication", "Access control requires authentication"),
                ("audit_logging", "log_management", "Audit logs require log management"),
                ("incident_response", "monitoring", "Incident response requires monitoring"),
            ...
            for control1, control2, description in control_deps:
                ...
                async for record in result:
                    created += 1
                    logger.info(f"Control dependency: {record['control1']} -> {record['control2']}")

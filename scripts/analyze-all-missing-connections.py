    ...
    for conn in connections:
        if conn.get("status") == "missing" and conn.get("frontendDetails"):
            ...
            endpoint_info = {
                ...
                "full_path": (
                    f"/api/v1{details.get('path', '')}"
                    if not details.get("path", "").startswith("/api")
    ...
    for conn in connections:
        if conn.get("status") == "connected":
            if conn.get("frontendDetails") and conn.get("backend"):
                ...
                patterns["working_services"].add(
                    frontend.get("file", "").replace(".ts", "")
                )
...
def generate_fix_plan(missing_by_service, patterns, endpoints_to_create):
    ...
    plan = {
        "summary": {
            "total_missing": sum(
                len(endpoints) for endpoints in missing_by_service.values()
            ),
    ...
    for router, endpoints in sorted(
        by_router.items(), key=lambda x: len(x[1]), reverse=True
    ):

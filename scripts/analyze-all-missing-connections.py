...
def analyze_missing_endpoints(connections):
    ...
    for conn in connections:
        if conn.get('status') == 'missing' and conn.get('frontendDetails'):
            ...
            endpoint_info = {
                ...
                'full_path': f"/api/v1{details.get('path', '')}" if not details.get('path', '').startswith('/api') else details.get('path', '')
            }
...
def analyze_connected_patterns(connections):
    """Analyze patterns in successfully connected endpoints."""
    
    patterns = {
        'path_patterns': [],
        'working_services': set(),
        'common_prefixes': set()
    }
    ...
    for conn in connections:
        if conn.get('status') == 'connected':
            if conn.get('frontendDetails') and conn.get('backend'):
                ...
                patterns['working_services'].add(frontend.get('file', '').replace('.ts', ''))
                
                # Analyze path relationship
                f_path = frontend.get('path', '')
                b_path = backend.get('path', '')
                
                if f_path and b_path:
                    if b_path.startswith('/api/v1'):
                        prefix = b_path[:7]  # '/api/v1'
                        patterns['common_prefixes'].add(prefix)
                    
                    patterns['path_patterns'].append({
                        'frontend': f_path,
                        'backend': b_path,
                        'service': frontend.get('file', '')
                    })
    
    return patterns

def generate_backend_endpoints(missing_by_service):
    ...
    for service, endpoints in missing_by_service.items():
        for endpoint in endpoints:
            # Determine the router file based on service name
            router_map = {
                'ai-self-review.service': 'ai_assessments',
                ...
                'useSurvey': 'surveys',
                'useBusinessProfile': 'business_profiles',
                'useAssessments': 'assessments',
                'useEvidence': 'evidence',
                'useTeam': 'team',
                'useUsers': 'users',
                'useWorkflow': 'workflows',
            }
            
            # Get router name
            router_name = router_map.get(service, service.replace('.service', '').replace('-', '_'))
            ...
            endpoints_to_create.append({
                ...
                'full_path': f"/api/v1/{path_with_params}",
                'frontend_line': endpoint['line']
            })
    
    return endpoints_to_create

def generate_fix_plan(missing_by_service, patterns, endpoints_to_create):
    """Generate a comprehensive fix plan."""
    
    plan = {
        'summary': {
            'total_missing': sum(len(endpoints) for endpoints in missing_by_service.values()),
            'services_affected': len(missing_by_service),
            'endpoints_to_create': len(endpoints_to_create)
        },
        'by_service': {},
        'implementation_order': []
    }
    
    # Group by router for implementation
    by_router = defaultdict(list)
    for endpoint in endpoints_to_create:
        by_router[endpoint['router']].append(endpoint)
    
    # Create implementation order (prioritize by number of endpoints)
    for router, endpoints in sorted(by_router.items(), key=lambda x: len(x[1]), reverse=True):
        plan['by_service'][router] = {
            'file': f"api/routers/{router}.py",
            'endpoints': endpoints,
            'count': len(endpoints)
        }
...
def generate_documentation(plan, missing_by_service):
    """Generate comprehensive documentation."""
    
    doc = []
    doc.append("# API Connection Fix Implementation Plan")
    doc.append(f"\n## Summary")
    doc.append(f"- **Total Missing Endpoints**: {plan['summary']['total_missing']}")
    doc.append(f"- **Services Affected**: {plan['summary']['services_affected']}")
    doc.append(f"- **Backend Endpoints to Create**: {plan['summary']['endpoints_to_create']}")
    ...
    for router in plan['implementation_order']:
        ...
        for endpoint in router_info['endpoints']:
            doc.append(f"| {endpoint['method']} | `{endpoint['path']}` | {endpoint['service']} |")
...
def main():
    ...
    print("=" * 80)
    print("COMPREHENSIVE API CONNECTION ANALYSIS")
    print("=" * 80)
    
    # Extract connections
    connections = extract_connections()
    if not connections:
        print("ERROR: Could not extract connections")
        return
    ...
    # Generate backend endpoints needed
    ...
    print("-" * 40)
    for service, endpoints in sorted(missing_by_service.items()):
        print(f"{service:30} : {len(endpoints):3} endpoints")
    
    print(f"\n🔧 BACKEND ROUTERS TO UPDATE:")
    print("-" * 40)
    for router in plan['implementation_order']:
        router_info = plan['by_service'][router]
        print(f"{router:30} : {router_info['count']:3} endpoints")
    
    print(f"\n✅ WORKING PATTERNS FOUND:")
    print("-" * 40)
    print(f"Working services: {', '.join(patterns['working_services'])}")
    print(f"Common prefixes: {', '.join(patterns['common_prefixes'])}")
    ...
    with open('api-fix-plan.json', 'w') as f:
        json.dump({
            'missing_by_service': {k: v for k, v in missing_by_service.items()},
            'patterns': {
                'working_services': list(patterns['working_services']),
                'common_prefixes': list(patterns['common_prefixes']),
                'path_patterns': patterns['path_patterns']
            },
            'plan': plan
        }, f, indent=2)
    
    print(f"📊 Detailed plan saved to: api-fix-plan.json")

...
class SprintManager:
    ...
    def init_sprint(self, sprint_data: Dict[str, Any]) -> Sprint:
        ...
        sprint = Sprint(
            id=sprint_data.get("id", f"sprint_{datetime.datetime.now().strftime('%Y%m%d')}"),
            name=sprint_data.get("name", "New Sprint"),
            goal=sprint_data.get("goal", "Deliver high-priority features"),
            start_date=datetime.datetime.strptime(sprint_data.get("start_date"), "%Y-%m-%d").date(),
    ...
    def generate_sprint_stories(self, sprint_id: str, roadmap_context: Dict[str, Any]) -> List[UserStory]:
        ...
        design_story = UserStory(
            ...
            acceptance_criteria=[
                AcceptanceCriteria("All purple and cyan colors replaced with teal variants"),

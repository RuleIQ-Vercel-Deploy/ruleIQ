...
class DopplerSecretsVerifier:
    ...
    def verify_secret_access(self, secret_name: str) -> Dict:
        ...
        success, value = self.run_doppler_command(
            ["secrets", "get", secret_name, "--plain"]
        )
    ...
    def validate_secret_format(self, secret_name: str, value: str) -> Tuple[bool, Dict]:
        ...
        # Key/Token validation
            ...
            pattern in secret_name
            for pattern in ["_KEY", "_TOKEN", "_SECRET", "PASSWORD"]
        ):
            ...
            validation["contains_special_chars"] = bool(
    ...
    def test_runtime_injection(self) -> Dict:
        ...
        test_result["fallback_mechanism"] = os.path.exists(".env") or os.path.exists(
            ".env.local"
        )
...
"""
        ...
        for secret in critical_secrets:
            result = next(
                (r for r in self.verification_results if r["secret_name"] == secret),
                None,
            )
        ...
        format_issues = [
            r["secret_name"] for r in self.verification_results if not r["format_valid"]
        ]
        if format_issues:
            report += (

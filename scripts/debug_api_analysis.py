...
class APIRouteAnalyzer:
    ...
    def _get_expected_endpoints(self) -> List[Dict[str, str]]:
        ...
        return [
            ...
            {
                "path": "/api/v1/assessments/readiness",
                "method": "GET",
                "router": "assessments",
            },
            # Business profiles
            ...
            {
                "path": "/api/v1/ai/assessments",
                "method": "POST",
                "router": "ai_assessments",
            },
            {"path": "/api/v1/ai/health", "method": "GET", "router": "ai_assessments"},
            {
                "path": "/api/v1/ai/policies/generate",
                "method": "POST",
                "router": "ai_policy",
            },
            {
                "path": "/api/v1/ai/policies/templates",
                "method": "GET",
                "router": "ai_policy",
            },
            {
    ...
    async def analyze_routes(self) -> APIAnalysisResult:
        ...
        recommendations.extend(
            self._generate_recommendations(endpoints, router_mapping)
        )
    ...
    async def _test_endpoint(self, path: str, method: str, router: str) -> APIEndpoint:
        ...
        try:
            ...
            if method == "GET":
                ...
                return APIEndpoint(
                    path, method, router, False, error="Method not tested"
                )
            ...
            if response.status_code == 404:
                return APIEndpoint(
                    path, method, router, False, response.status_code, response_time_ms
                )
    ...
    def _identify_issues(
        self, endpoints: List[APIEndpoint], actual_paths: set
    ) -> List[str]:
        ...
        if missing_endpoints:
            ...
            for endpoint in missing_endpoints:
                ...
                similar_paths = [
                    p for p in actual_paths if endpoint.path.split("/")[-1] in p
                ]
                if similar_paths:
                    issues.append(
        ...
        error_endpoints = [
            e for e in endpoints if e.exists and e.error and "Server error" in e.error
        ]
        if error_endpoints:
        ...
        slow_endpoints = [
            e for e in endpoints if e.response_time_ms and e.response_time_ms > 2000
        ]
        if slow_endpoints:
            issues.append(
        ...
        ai_endpoints = [
            e for e in endpoints if "/api/v1/ai/" in e.path and not e.exists
        ]
        if ai_endpoints:
            issues.append(
    ...
    def _generate_recommendations(
        self, endpoints: List[APIEndpoint], router_mapping: Dict[str, List[str]]
    ) -> List[str]:
        ...
        auth_errors = [
            e
            for e in endpoints
            if e.exists and e.error and ("401" in e.error or "403" in e.error)
        ]
        if auth_errors:
            recommendations.append(
        ...
        validation_errors = [
            e for e in endpoints if e.exists and e.error and "422" in e.error
        ]
        ...
        slow_endpoints = [
            e for e in endpoints if e.response_time_ms and e.response_time_ms > 1000
        ]
        if slow_endpoints:
            recommendations.append(
    ...
    for router, endpoints in result.router_mapping.items():
        working_count = len(
            [
                ...
                if e.router == router and e.exists and not e.error
            ]
        )

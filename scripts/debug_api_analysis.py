...
class APIRouteAnalyzer:
    ...
    async def _test_endpoint(self, path: str, method: str, router: str) -> APIEndpoint:
        ...
        try:
            ...
            return APIEndpoint(
                path, method, router, exists, response.status_code, response_time_ms, error
            )
    ...
    def _get_test_data_for_endpoint(self, path: str) -> Dict[str, Any]:
        ...
        test_data_map = {
            "/api/v1/auth/register": {"email": "test@example.com", "password": "testpass123"},
            "/api/v1/auth/login": {"email": "test@example.com", "password": "testpass123"},
            "/api/v1/assessments": {
                "assessment_type": "compliance_readiness",
                "company_name": "Test Company",
            },
            "/api/v1/business-profiles": {"company_name": "Test Company", "industry": "Technology"},
            "/api/v1/policies/generate": {
                "policy_type": "privacy_policy",
                "company_name": "Test Company",
            },
            "/api/v1/ai/assessments": {"assessment_type": "compliance_check", "context": "test"},
    ...
    def _generate_recommendations(
        ...
        auth_errors = [
            e for e in endpoints if e.exists and e.error and ("401" in e.error or "403" in e.error)
        ]
        if auth_errors:
            recommendations.append(
...
def print_analysis_report(result: APIAnalysisResult) -> None:
    ...
    for router, endpoints in result.router_mapping.items():
        working_count = len(
            [e for e in result.endpoints if e.router == router and e.exists and not e.error]
        )
    ...
    if working:
        ...
        for endpoint in working:
            response_time = (
                f" ({endpoint.response_time_ms:.0f}ms)" if endpoint.response_time_ms else ""
            )

...
class IntegrationTestRunner:
    ...
    async def run_test_suite(
        ...
        try:
            ...
            return False, {"suite": suite_name, "success": False, "error": str(e)}

    async def run_all_suites(
        self, selected_suites: List[str] = None, additional_args: List[str] = None
    ) -> Dict:
    ...
    def generate_report(self, results: Dict) -> str:
        ...
        slow_suites = [
            (name, dur) for name, dur in suite_durations if dur > 300
        ]  # 5+ minutes
        if slow_suites:
...
async def main():
    ...
    try:
        if args.suite == "all":
            ...
            success, suite_result = await runner.run_test_suite(
                args.suite, additional_args
            )

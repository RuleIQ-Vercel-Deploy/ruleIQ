...
def should_remove_request(request: Dict[str, Any]) -> bool:
    ...
    if isinstance(url, dict) and "raw" in url:
        ...
        for removed_path in ENDPOINT_UPDATES["removed"]:
            if removed_path in raw_url and removed_path not in ENDPOINT_UPDATES["updated"]:
                print(f"  Removing deprecated endpoint: {removed_path}")
                return True
    
    return False

def process_items(items: List[Dict[str, Any]], stats: Dict[str, int]) -> List[Dict[str, Any]]:
...
def update_collection(input_file: str, output_file: str):
    ...
    collection["info"]["name"] = f"RuleIQ API Collection - Consolidated ({datetime.now().strftime('%Y-%m-%d')})"

...
SYSTEMD_UNIT=${SYSTEMD_UNIT:-/etc/systemd/system/ruleiq-api.service}
...
sudo mkdir -p "${APP_DIR}" "${VENV_DIR}" "${ENV_DIR}" /var/log/ruleiq
sudo chown -R "${APP_USER}:${APP_GROUP}" "$(dirname "${APP_DIR}")" "${ENV_DIR}" /var/log/ruleiq

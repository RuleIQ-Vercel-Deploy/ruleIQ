...
def main() -> None:
    ...
    test_endpoint_with_details(
        "GET", "/api/v1/health", description="API v1 health (previously failing)"
    )
    ...
    success, response = test_endpoint_with_details(
        "POST", "/api/v1/auth/register", register_data, description="New user registration"
    )
    ...
    if not success:
        ...
        success, response = test_endpoint_with_details(
            "POST", "/api/v1/auth/login", login_data, description="Existing user login"
        )
    ...
    if success and response:
        try:
            ...
            if token:
                ...
                test_endpoint_with_details(
                    "GET", "/api/v1/assessments/", headers=auth_headers, description="Assessments"
                )
                test_endpoint_with_details(
                    "GET", "/api/v1/frameworks/", headers=auth_headers, description="Frameworks"
                )

                print("\n🤖 AI Endpoints Analysis (404 errors expected if router issues remain):")
                test_endpoint_with_details(
                    "GET", "/api/v1/ai/policies", headers=auth_headers, description="AI policies"
                )
                test_endpoint_with_details(
                    "GET", "/api/v1/ai/cost", headers=auth_headers, description="AI cost monitoring"
                )

                print("\n💬 Chat & Freemium Analysis:")
                test_endpoint_with_details(
                    "GET", "/api/v1/chat/", headers=auth_headers, description="Chat endpoint"
                )
                test_endpoint_with_details(
                    "POST",
                    "/api/v1/freemium/capture-lead",
                    {"email": "test@example.com", "consent": True},
                    description="Freemium endpoint",
                )

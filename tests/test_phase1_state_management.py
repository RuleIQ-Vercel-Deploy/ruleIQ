...
class TestStateReducers:
    ...
    def test_accumulate_errors_with_limit(self):
        ...
        # With default max_errors=10
        accumulated = accumulate_errors(existing, new)
        
        assert len(accumulated) == 10
        assert accumulated[0]["error"] == "error_1"  # Oldest errors removed (keeps from index 1)
        assert accumulated[-1]["error"] == "error_11"  # Newest error retained
    ...
    def test_merge_compliance_data(self):
        ...
        new = {
            "frameworks": ["GDPR", "SOX"],  # GDPR duplicate
            "obligations": [
                {"id": "ob2", "title": "Updated Obligation 2"},  # Update
                {"id": "ob3", "title": "Obligation 3"}  # New
            ],
            "evidence": [
                {"hash": "hash1", "data": "evidence1"},  # Duplicate
                {"hash": "hash2", "data": "evidence2"}  # New
            ]
        }
        
        merged = merge_compliance_data(existing, new)
        
        # Check framework deduplication
        assert len(merged["frameworks"]) == 3
        assert set(merged["frameworks"]) == {"GDPR", "HIPAA", "SOX"}
        
        # Check obligation merging
        assert len(merged["obligations"]) == 3
...
class TestStateTransition:
    ...
    def test_validate_transition_limits(self):
        ...
        # Reset turn count but exceed error limit
        state["turn_count"] = 2
        state["error_count"] = 3
        assert validator.validate_transition(state, "node1", "node2") is False
    ...
    def test_record_transition(self):
        ...
        # Record transition with latency
...
class TestStateAggregator:
    ...
    def test_get_compliance_summary(self):
        ...
        assert summary["frameworks_identified"] == 3
...
class TestEnhancedComplianceGraph:
    ...
    async def test_error_handling(self):
        ...
        # Create a state that will trigger errors
        state = create_enhanced_initial_state(
        ...
        # Test error handler node - first retry
        ...
        # Test max retries exceeded (third call)
    ...
    async def test_router_decision_logic(self):
        ...
        state = create_enhanced_initial_state(
            ...
            company_id=uuid4(),
            initial_message="Tell me about compliance"
        )
        
        # Test routing decision
        updated_state = await graph._router_node(state)
        
        assert updated_state["next_node"] in ["profile", "compliance", "evidence", "response", "error", "end"]
    ...
    async def test_compliance_analyzer(self):
        ...
        state = create_enhanced_initial_state(
            ...
            company_id=uuid4(),
            initial_message="Analyze compliance"
        )

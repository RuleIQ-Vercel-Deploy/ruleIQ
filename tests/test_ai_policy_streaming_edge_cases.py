...
class TestStreamingEdgeCases:
    ...
    async def test_streaming_partial_failure_recovery(
        ...
        with patch.object(
            policy_generator, "_stream_with_google", side_effect=mock_partial_failure
        ), patch.object(
            policy_generator, "_stream_with_openai", side_effect=mock_partial_failure
        ):
    ...
    async def test_streaming_empty_response_handling(
        ...
        async def mock_empty_stream(*args, **kwargs):
            ...
            yield PolicyStreamingChunk(
                chunk_id="3", content="Actual content", chunk_type="content"
            )
            yield PolicyStreamingChunk(chunk_id="4", content="", chunk_type="complete")

        with patch.object(
            policy_generator, "_stream_with_google", side_effect=mock_empty_stream
        ):
            chunks = []
            async for chunk in policy_generator.generate_policy_stream(
                sample_request, sample_framework
    ...
    async def test_streaming_unicode_handling(
        ...
        with patch.object(
            policy_generator, "_stream_with_google", side_effect=mock_unicode_stream
        ):
            chunks = []
            async for chunk in policy_generator.generate_policy_stream(
                sample_request, sample_framework
    ...
    async def test_streaming_large_chunk_handling(
        ...
        async def mock_large_chunk_stream(*args, **kwargs):
            ...
            yield PolicyStreamingChunk(
                chunk_id="1", content=large_content, chunk_type="content"
            )
    ...
    @pytest.mark.asyncio
    async def test_streaming_rapid_chunks(
        self, policy_generator, sample_request, sample_framework
    ):
        ...
        with patch.object(
            policy_generator, "_stream_with_google", side_effect=mock_rapid_stream
        ):
            chunks = []
    ...
    async def test_streaming_duplicate_chunk_ids(
        ...
        with patch.object(
            policy_generator, "_stream_with_google", side_effect=mock_duplicate_ids
        ):
            chunks = []
            chunk_ids = set()
...
class TestStreamingConcurrency:
    ...
    async def test_stream_isolation(self):
        ...
        with patch.object(
            generator, "_stream_with_google", side_effect=mock_isolated_stream
        ):

...
def create_test_state(scenario: TestScenario = TestScenario.INITIAL, **overrides) -> EnhancedComplianceState:
    """
    ...
    if scenario == TestScenario.INITIAL:
        builder.with_status(WorkflowStatus.PENDING)\
               .with_node("start")\
               .add_message("system", "Workflow initialized")
    
    elif scenario == TestScenario.IN_PROGRESS:
        builder.with_status(WorkflowStatus.IN_PROGRESS)\
               .with_node("data_collection")\
               .add_message("system", "Processing compliance data")\
               .add_compliance_data("framework", "SOC2")\
               .add_tool_output("data_collector", {"records": 100})
    
    elif scenario == TestScenario.ERROR_STATE:
        builder.with_status(WorkflowStatus.FAILED)\
               .with_node("validation")\
               .add_error("ValidationError", "Invalid data format")\
               .with_retry(2)
    
    elif scenario == TestScenario.COMPLETED:
        builder.with_status(WorkflowStatus.COMPLETED)\
               .with_node("end")\
               .add_compliance_data("assessment_complete", True)\
               .add_compliance_data("compliance_score", 0.95)
    
    elif scenario == TestScenario.REVIEW_NEEDED:
        builder.with_status(WorkflowStatus.REVIEW_REQUIRED)\
               .with_node("human_review")\
               .add_compliance_data("requires_human_input", True)\
               .with_context(review_reason="Ambiguous compliance requirement")
    
    elif scenario == TestScenario.RETRY_REQUIRED:
        builder.with_status(WorkflowStatus.IN_PROGRESS)\
               .with_node("retry_handler")\
               .with_retry(1)\
...
def create_batch_states(count: int = 5, scenario: TestScenario = TestScenario.INITIAL) -> List[EnhancedComplianceState]:
    """

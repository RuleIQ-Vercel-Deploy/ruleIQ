    ...
    async def __call__(self, state: EnhancedComplianceState) -> EnhancedComplianceState:
        ...
        if self.behavior == "fail":
            ...
            state["messages"].append({
                "role": "system",
                "content": f"Node {self.name} completed with delay"
            })
        
        else:  # Default "pass" behavior
            state["messages"].append({
                "role": "system",
                "content": f"Node {self.name} executed successfully"
            })
...
class GraphTestHarness:
    ...
    def __init__(self, graph: StateGraph, nodes: Dict[str, TestNode], 
    ...
    async def run_with_state(self, initial_state: EnhancedComplianceState, 
                            config: Optional[Dict[str, Any]] = None) -> EnhancedComplianceState:
        """
...
def create_test_harness(graph_type: TestGraphType = TestGraphType.SIMPLE) -> GraphTestHarness:
    """

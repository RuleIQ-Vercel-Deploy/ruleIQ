...
class TestRetryStrategies:
    ...
    async def test_rate_limit_exponential_backoff(self):
        ...
        with patch("asyncio.sleep") as mock_sleep:
            ...
            # Test first retry (2^0 * 2 = 2 seconds base)
            result = await handler._handle_rate_limit(state)
            assert mock_sleep.called
            delay = mock_sleep.call_args[0][0]
            assert 2.0 <= delay <= 2.2  # Base delay + up to 10% jitter
            assert result["should_continue"] is True
            assert result["next_node"] == "router"

            # Test second retry (2^1 * 2 = 4 seconds base)
            state["retry_count"] = 1
            mock_sleep.reset_mock()
            result = await handler._handle_rate_limit(state)
            delay = mock_sleep.call_args[0][0]
            assert 4.0 <= delay <= 4.4  # Base delay + up to 10% jitter

            # Test third retry (2^2 * 2 = 8 seconds base)
            state["retry_count"] = 2
            mock_sleep.reset_mock()
            result = await handler._handle_rate_limit(state)
            delay = mock_sleep.call_args[0][0]
            assert 8.0 <= delay <= 8.8  # Base delay + up to 10% jitter

            # Test max delay cap
            state["retry_count"] = 10  # Should cap at 60 seconds
            mock_sleep.reset_mock()
            result = await handler._handle_rate_limit(state)
            delay = mock_sleep.call_args[0][0]
            assert delay <= 60.0
    ...
    async def test_database_linear_backoff(self):
        ...
        with patch("asyncio.sleep") as mock_sleep:
            ...
            # Test max delay cap
            state["retry_count"] = 50  # Should cap at 30 seconds
            mock_sleep.reset_mock()
            result = await handler._handle_database_error(state)
    ...
    async def test_api_error_fallback_strategy(self):
        ...
        with patch("asyncio.sleep") as mock_sleep:
            ...
            # First two retries should not use fallback
            ...
            # Third retry should switch to fallback
            state["retry_count"] = 2
            result = await handler._handle_api_error(state)
    ...
    async def test_validation_error_immediate_fallback(self):
        ...
        # Should immediately activate fallback
...
class TestErrorHandlerIntegration:
    ...
    async def test_error_handler_process_flow(self):
        ...
        with patch("asyncio.sleep") as mock_sleep:
            ...
            # Max retries exceeded - should activate fallback
    ...
    async def test_error_handler_self_failure(self):
        ...
        # Add error that will cause handler to fail
        state["errors"] = [{"error": "Test error"}]
        state["error_count"] = 1

        # Mock classify_error to raise exception
        with patch.object(
            handler, "_classify_error", side_effect=Exception("Handler failed")
        ):
            result = await handler.process(state)
            assert result["workflow_status"] == WorkflowStatus.FAILED
            assert result["should_continue"] is False
...
class TestRoutingFunctions:
    ...
    def test_should_route_to_error_handler(self):
        ...
        # No errors - should not route
        assert should_route_to_error_handler(state) is None

        # Has errors but no error count - should not route
        state["errors"] = [{"error": "Test"}]
        assert should_route_to_error_handler(state) is None

        # Has errors and error count - should route
        state["error_count"] = 1
        state["retry_count"] = 0
        assert should_route_to_error_handler(state) == "error_handler"

        # Max retries exceeded - should not route
        state["retry_count"] = 3
        assert should_route_to_error_handler(state) is None
...
class TestEnhancedGraphWithErrorHandling:
    ...
    async def test_graph_error_routing(self):
        ...
        # Simulate an error in profile builder
        state["errors"] = [
            {"error": "Profile builder failed", "node": "profile_builder"}
        ]
        state["error_count"] = 1
        ...
        # After max retries, should not route to error handler
        state["retry_count"] = 5
    ...
    async def test_graph_recovery_routing(self):
        ...
        # Failed workflow should route to end
        state["workflow_status"] = WorkflowStatus.FAILED
        assert graph._route_from_error_handler(state) == END

        # Otherwise should use recovery node logic
        state["workflow_status"] = WorkflowStatus.IN_PROGRESS
        state["last_successful_node"] = "compliance_analyzer"
        state["next_node"] = None  # Clear next_node to test recovery logic
    ...
    async def test_graph_node_error_tracking(self):
        ...
        # Mock error in router node
        with patch.object(
            graph.state_transition,
            "record_transition",

...
class TestSyncEvidenceStatusRefactored:
    ...
    async def test_sync_with_custom_cutoff(self, mock_get_db):
        ...
        with patch.object(
            node, "cleanup_stale_evidence", new=AsyncMock(return_value=7)
        ):
            state = {
                "user_id": str(uuid4()),
    ...
    async def test_sync_with_typed_dict_state(self, mock_get_db):
        ...
        # Mock cleanup
        with patch.object(
            node, "cleanup_stale_evidence", new=AsyncMock(return_value=3)
        ):
            ...
            assert (
                "Evidence sync completed: 3 items marked stale"

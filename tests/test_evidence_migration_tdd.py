...
class TestEvidenceAggregationAndDeduplication:
    ...
    async def test_evidence_merging_strategy(self):
        ...
        for group_name, evidences in evidence_groups.items():
            merged[group_name] = {
                "combined_score": round(sum(e["score"] for e in evidences) / len(evidences), 2),
...
class TestEvidenceCollectionWorkflows:
    ...
    async def test_single_evidence_processing_workflow(
        self, mock_db_session, mock_processor, mock_duplicate_detector, 
        sample_evidence_data, sample_state
    ):
        ...
        with patch('langgraph_agent.nodes.evidence_nodes.get_async_db', return_value=mock_get_db_gen()):
            # Process evidence
...
class TestErrorHandlingAndRecovery:
    ...
    async def test_database_error_handling(self, mock_db_session, sample_state, sample_evidence_data):
...
class TestIntegrationWithLangGraph:
    ...
    async def test_conditional_routing_based_on_evidence(self, sample_state):
        ...
        def route_evidence(state: EnhancedComplianceState) -> str:
            ...
            valid_count = sum(
                1 for e in state.evidence_items 
...
if __name__ == "__main__":
    pytest.main([__file__, "-v", "--cov=langgraph_agent.nodes.evidence_nodes", "--cov-report=term-missing"])
        ...
        return [
            MockChunk(candidates=[
                MockCandidate(content=MockContent(parts=[
                    MockPart(text="Complex ")
                ]))
            ]),
            MockChunk(candidates=[
                MockCandidate(content=MockContent(parts=[
                    MockPart(text="streaming ")
                ]))
            ]),
            MockChunk(text="response.")
        ]
...
class TestAIStreaming:
    ...
    async def test_stream_response_basic(self, compliance_assistant, mock_streaming_chunks):
        ...
        with patch.object(compliance_assistant, '_get_task_appropriate_model') as mock_get_model:
            mock_model = Mock()
            mock_model.generate_content_stream.return_value = MockStreamingResponse(mock_streaming_chunks)
    ...
    @pytest.mark.asyncio
    async def test_stream_response_with_complex_structure(self, compliance_assistant, mock_complex_chunks):
        """Test streaming response with complex candidate structure."""
        with patch.object(compliance_assistant, '_get_task_appropriate_model') as mock_get_model:
            mock_model = Mock()
            mock_model.generate_content_stream.return_value = MockStreamingResponse(mock_complex_chunks)
            mock_get_model.return_value = (mock_model, "test_instruction_id")
    ...
    async def test_stream_response_circuit_breaker_open(self, compliance_assistant):
        ...
        with patch.object(compliance_assistant, '_get_task_appropriate_model') as mock_get_model:
            ...
            with patch.object(compliance_assistant.circuit_breaker, 'is_model_available', return_value=False):
                chunks = []
                async for chunk in compliance_assistant._stream_response(
                    "System prompt", "User prompt", "test"
                ):
    ...
    async def test_analyze_assessment_results_stream(self, compliance_assistant):
        ...
        with patch.object(compliance_assistant.context_manager, 'get_conversation_context') as mock_context:
            mock_context.return_value = {'business_profile': {'industry': 'tech'}}
            
            with patch.object(compliance_assistant.prompt_templates, 'get_assessment_analysis_prompt') as mock_prompt:
    ...
    async def test_get_assessment_recommendations_stream(self, compliance_assistant):
        ...
        with patch.object(compliance_assistant.context_manager, 'get_conversation_context') as mock_context:
            ...
            with patch.object(compliance_assistant.prompt_templates, 'get_assessment_recommendations_prompt') as mock_prompt:
                mock_prompt.return_value = {'user': 'Recommendations prompt'}
                
                with patch.object(compliance_assistant, '_stream_response') as mock_stream:
                    mock_stream.return_value = async_generator_from_list(["Recommendation ", "content"])
    ...
    async def test_get_assessment_help_stream(self, compliance_assistant):
        ...
        with patch.object(compliance_assistant.context_manager, 'get_conversation_context') as mock_context:
            ...
            with patch.object(compliance_assistant.prompt_templates, 'get_assessment_help_prompt') as mock_prompt:
    ...
    async def test_streaming_success_recording(self, compliance_assistant, mock_streaming_chunks):
        ...
        with patch.object(compliance_assistant, '_get_task_appropriate_model') as mock_get_model:
            ...
            with patch.object(compliance_assistant.circuit_breaker, 'is_model_available', return_value=True):
                with patch.object(compliance_assistant.circuit_breaker, 'record_success') as mock_record:
                    chunks = []
                    async for chunk in compliance_assistant._stream_response(
                        "System prompt", "User prompt", "test"
                    ):
    ...
    async def test_streaming_failure_recording(self, compliance_assistant):
        ...
        with patch.object(compliance_assistant, '_get_task_appropriate_model') as mock_get_model:
            ...
            with patch.object(compliance_assistant.circuit_breaker, 'is_model_available', return_value=True):
                with patch.object(compliance_assistant.circuit_breaker, 'record_failure') as mock_record:
                    chunks = []
                    async for chunk in compliance_assistant._stream_response(
                        "System prompt", "User prompt", "test"
                    ):

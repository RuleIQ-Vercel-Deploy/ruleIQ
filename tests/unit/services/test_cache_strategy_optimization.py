...
class TestCacheStrategyOptimization:
    ...
    @pytest.fixture
    def optimized_cache_config(self):
    ...
    @pytest.fixture
    def cache_manager(self, optimized_cache_config):
    ...
    def test_intelligent_invalidation_triggers(self, cache_manager, sample_business_profile):
        ...
        assert any('business_profile_update' in key for key in cache_manager.invalidation_triggers.keys())
    ...
    def test_cache_strategy_metrics_collection(self, cache_manager):
        ...
        cache_manager.performance_history = {
            'cache_1': [
                {'timestamp': datetime.now(), 'response_time_ms': 150, 'hit': True, 'ttl_adjustment': 0.2},
                {'timestamp': datetime.now(), 'response_time_ms': 180, 'hit': True, 'ttl_adjustment': 0.2},
            ],
            'cache_2': [
                {'timestamp': datetime.now(), 'response_time_ms': 2500, 'hit': False, 'ttl_adjustment': -0.2},
            ]
        }
...
class TestCacheStrategyIntegration:
    ...
    @pytest.fixture
    async def assistant_with_optimized_cache(self, async_db_session):
        """AI assistant with optimized cache configuration."""
        from services.ai.assistant import ComplianceAssistant
        assistant = ComplianceAssistant(async_db_session)
        
        # Initialize with optimized cache
        cached_content_manager = await assistant._get_cached_content_manager()
        cached_content_manager.config.performance_based_ttl = True
        cached_content_manager.config.cache_warming_enabled = True
        cached_content_manager.config.intelligent_invalidation = True
        
        return assistant

    async def test_cache_strategy_metrics_endpoint_integration(self, assistant_with_optimized_cache):

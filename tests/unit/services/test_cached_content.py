...
class TestGoogleCachedContentManager:
    ...
    def cache_config(self):
        ...
        return CacheLifecycleConfig(default_ttl_hours=2, max_ttl_hours=8,
            min_ttl_minutes=15)
    ...
    def sample_business_profile(self):
        ...
        return {'id': str(uuid4()), 'company_name': 'Test Corp', 'industry':
            'Technology', 'employee_count': 150, 'country': 'UK',
            'handles_personal_data': True, 'processes_payments': False,
            'stores_health_data': False, 'provides_financial_services': 
            False, 'operates_critical_infrastructure': False,
            'has_international_operations': True, 'existing_frameworks': [
            'ISO27001'], 'cloud_providers': ['AWS'], 'saas_tools': [
            'Office365']}
    ...
    def test_cache_key_generation(self, cache_manager):
        ...
        key1 = cache_manager._generate_cache_key(CacheContentType.
            ASSESSMENT_CONTEXT, 'ISO27001', 'tech_company')
        key2 = cache_manager._generate_cache_key(CacheContentType.
            ASSESSMENT_CONTEXT, 'ISO27001', 'tech_company')
        assert key1 == key2
        assert key1.startswith('gai_cache:')
        key3 = cache_manager._generate_cache_key(CacheContentType.
            ASSESSMENT_CONTEXT, 'GDPR', 'tech_company')
        assert key1 != key3

    def test_business_profile_cache_key_generation(self, cache_manager,
        ...
        key2 = cache_manager._generate_business_profile_cache_key(
            similar_profile)
        assert key1 == key2
        ...
        key3 = cache_manager._generate_business_profile_cache_key(
            different_profile)
        assert key1 != key3
    ...
    def test_assessment_cache_content_building(self, cache_manager,
        sample_business_profile):
        """Test assessment cache content is built correctly."""
        framework_id = 'ISO27001'
        assessment_context = {'assessment_type': 'analysis'}
        content = cache_manager._build_assessment_cache_content(framework_id,
            sample_business_profile, assessment_context)
    ...
    def test_business_profile_cache_content_building(self, cache_manager,
        sample_business_profile):
    ...
    def test_framework_cache_content_building(self, cache_manager):
        ...
        content = cache_manager._build_framework_cache_content(framework_id,
            industry_context)
        assert len(content) > DEFAULT_RETRIES
        assert any('GDPR' in part for part in content)
        assert any('Technology' in part for part in content)

    def test_assessment_ttl_calculation(self, cache_manager,
        sample_business_profile):
        """Test TTL calculation for assessment cache."""
        ttl_stable = cache_manager._calculate_assessment_ttl('ISO27001',
            sample_business_profile)
        ttl_other = cache_manager._calculate_assessment_ttl('CUSTOM',
            sample_business_profile)
        assert ttl_stable >= ttl_other
        large_company_profile = sample_business_profile.copy()
        large_company_profile['employee_count'] = 2000
        ttl_large = cache_manager._calculate_assessment_ttl('ISO27001',
            large_company_profile)
        assert ttl_large >= ttl_stable

    def test_business_profile_ttl_calculation(self, cache_manager,
        sample_business_profile):
        """Test TTL calculation for business profile cache."""
        ttl = cache_manager._calculate_business_profile_ttl(
            sample_business_profile)
        assert ttl >= 1
        assert ttl <= cache_manager.config.max_ttl_hours
        large_company_profile = sample_business_profile.copy()
        large_company_profile['employee_count'] = 1000
        ttl_large = cache_manager._calculate_business_profile_ttl(
            large_company_profile)
    ...
    def test_framework_helper_methods(self, cache_manager):
        ...
        assert 'Information Security' in cache_manager._get_framework_type(
            'ISO27001')
    ...
    def test_data_processing_profile_generation(self, cache_manager,
        sample_business_profile):
        """Test data processing profile generation."""
        profile = cache_manager._get_data_processing_profile(
            sample_business_profile)
        assert 'Personal Data' in profile
        multi_data_profile = sample_business_profile.copy()
        multi_data_profile.update({'processes_payments': True,
            'stores_health_data': True})
        multi_profile = cache_manager._get_data_processing_profile(
            multi_data_profile)
        assert 'Personal Data' in multi_profile
        assert 'Payment Data' in multi_profile
        assert 'Health Data' in multi_profile

    def test_technology_summary_generation(self, cache_manager,
        sample_business_profile):
    ...
    def test_compliance_maturity_assessment(self, cache_manager,
        sample_business_profile):
        """Test compliance maturity assessment."""
        maturity = cache_manager._get_compliance_maturity(
            sample_business_profile)
        ...
        advanced_maturity = cache_manager._get_compliance_maturity(
            advanced_profile)
        assert 'Advanced' in advanced_maturity
        beginner_profile = sample_business_profile.copy()
        beginner_profile['existing_frameworks'] = []
        beginner_maturity = cache_manager._get_compliance_maturity(
            beginner_profile)
        assert 'Initial' in beginner_maturity

    @pytest.mark.asyncio
    @patch('google.generativeai.caching.CachedContent.create')
    async def test_assessment_cache_creation_success(self, mock_create,
        cache_manager, sample_business_profile):
        ...
        framework_id = 'ISO27001'
        result = await cache_manager.create_assessment_cache(framework_id,
            sample_business_profile)
    ...
    async def test_assessment_cache_creation_failure(self, mock_create,
        cache_manager, sample_business_profile):
    ...
    def test_get_cached_content_hit(self, cache_manager):
        ...
        cache_key = cache_manager._generate_cache_key(CacheContentType.
            ASSESSMENT_CONTEXT, 'ISO27001', 'test')
        cache_manager.active_caches[cache_key] = mock_cached_content
        result = cache_manager.get_cached_content(CacheContentType.
            ASSESSMENT_CONTEXT, 'ISO27001', 'test')
    ...
    async def test_cleanup_expired_caches(self, cache_manager):
        ...
        cache_manager.cache_metadata[cache_key] = {'type': CacheContentType
            .ASSESSMENT_CONTEXT.value, 'size_estimate_mb': 1.0}
...
class TestCachedContentEndToEnd:
    ...
    def assistant_with_cache(self, async_db_session):
        ...
        assistant._get_cached_content_manager = AsyncMock(return_value=
            assistant.cached_content_manager)
        assistant.context_manager = Mock()
        assistant.prompt_templates = Mock()
        assistant._generate_ai_response_with_cache = AsyncMock(return_value
            ='Mock response')
        assistant._parse_assessment_analysis_response = Mock(return_value={
    ...
    def mock_business_profile(self):
        ...
        return {'id': str(uuid4()), 'company_name': 'Integration Test Corp',
            'industry': 'Financial Services', 'employee_count': 500,
            'country': 'UK', 'handles_personal_data': True,
            'processes_payments': True, 'existing_frameworks': ['ISO27001',
            'SOC2']}

    @pytest.mark.asyncio
    @patch('google.generativeai.caching.CachedContent.create')
    async def test_assessment_analysis_with_cached_content(self,
        mock_create, assistant_with_cache, mock_business_profile):
        """Test assessment analysis uses cached content when available."""
        mock_cached_content = Mock()
        mock_cached_content.name = 'test_assessment_cache'
        mock_create.return_value = mock_cached_content
        mock_result = {'gaps': [], 'recommendations': [], 'request_id': str
            (uuid4()), 'generated_at': '2024-01-01T00:00:00Z'}
    ...
    @pytest.mark.asyncio
    async def test_cache_metrics_collection_integration(self,
        assistant_with_cache):
        """Test cache metrics collection in integration context."""
        cached_content_manager = (await assistant_with_cache.
            _get_cached_content_manager(),)

...
class TestContextAwareRecommendations:
    ...
    async def test_get_context_aware_recommendations_success(
        self, mock_db_session, mock_user, sample_business_context, 
        sample_maturity_analysis, sample_gaps_analysis
    ):
    ...
    async def test_analyze_compliance_maturity(self, mock_db_session, sample_business_context):
        ...
        existing_evidence = [
            {'evidence_type': 'policy'}, {'evidence_type': 'procedure'},
            {'evidence_type': 'log'}, {'evidence_type': 'assessment'}
        ]
    ...
    def test_prioritize_recommendations(self, mock_db_session, sample_business_context, sample_maturity_analysis):
    ...
    def test_add_automation_insights(self, mock_db_session, sample_business_context):
        ...
        enhanced = assistant._add_automation_insights(recommendations, sample_business_context)
    ...
    def test_get_fallback_recommendations(self, mock_db_session):
        ...
        fallback = assistant._get_fallback_recommendations('ISO27001', {'maturity_level': 'Basic'})
...
class TestWorkflowGeneration:
    ...
    async def test_generate_evidence_collection_workflow_success(self, mock_db_session, mock_user):
        ...
        with patch.object(assistant.context_manager, 'get_conversation_context') as mock_context:
            mock_context.return_value = {
                'business_profile': {'company_name': 'Test Corp', 'industry': 'Technology', 'employee_count': 100},
                'recent_evidence': []
            }
...
class TestPolicyGeneration:
    ...
    @pytest.mark.asyncio
    async def test_generate_customized_policy_success(self, mock_db_session, mock_user, sample_customization_options):
    ...
    def test_apply_healthcare_customizations(self, mock_db_session):
        ...
        healthcare_section = next(
            (s for s in customized['sections'] if 'Healthcare' in s['title']),
            None
        )
    ...
    def test_apply_financial_customizations(self, mock_db_session):
        ...
        financial_section = next(
            (s for s in customized['sections'] if 'Financial' in s['title']),
            None
        )
    ...
    def test_apply_size_customizations(self, mock_db_session):
        ...
        enterprise_policy = assistant._apply_size_customizations(policy.copy(), 'enterprise')
        assert 'implementation_notes' in enterprise_policy
        assert any('enterprise-grade' in note.lower() for note in enterprise_policy['implementation_notes'])
    ...
    def test_generate_compliance_mapping(self, mock_db_session):
        ...
        mapping = assistant._generate_compliance_mapping(policy, 'ISO27001', 'information_security')
    ...
    def test_get_fallback_policy(self, mock_db_session):
        ...
        fallback = assistant._get_fallback_policy('ISO27001', 'information_security', business_context)

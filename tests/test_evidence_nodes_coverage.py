...
class TestEvidenceCollectionNodeUncovered:
    ...
    async def test_process_pending_evidence(self, mock_processor_class, mock_get_db):
        ...
        pending_evidence.to_dict = MagicMock(return_value={
        ...
        mock_result.scalars = MagicMock(return_value=MagicMock(all=MagicMock(return_value=[pending_evidence])))
    ...
    async def test_aggregate_evidence_correct_field(self):
        ...
        evidence_list = [
            {"id": "1", "evidence_name": "Doc 1", "type": "Policy", "content": "Content 1"},
            {"id": "2", "evidence_name": "Doc 2", "type": "Policy", "content": "Content 2"},
            {"id": "3", "evidence_name": "Doc 3", "type": "Procedure", "content": "Content 3"}
        ]
    ...
    async def test_check_evidence_expiry_with_frequencies(self, mock_get_db):
        ...
        monthly_evidence.to_dict = MagicMock(return_value={"id": "monthly_id", "status": "expired"})
        ...
        quarterly_evidence.to_dict = MagicMock(return_value={"id": "quarterly_id", "status": "expired"})
        ...
        annually_evidence.to_dict = MagicMock(return_value={"id": "annually_id", "status": "expired"})
        
        mock_session = MagicMock(spec=AsyncSession)
        mock_result = MagicMock()
        mock_result.scalars = MagicMock(return_value=MagicMock(all=MagicMock(
            return_value=[monthly_evidence, quarterly_evidence, annually_evidence]
        )))
        mock_session.execute = AsyncMock(return_value=mock_result)
        mock_session.commit = AsyncMock()
        mock_get_db.return_value = async_generator([mock_session])

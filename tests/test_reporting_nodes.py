...
class TestGenerateReportNode:
    ...
    async def test_generate_compliance_report_success(self, mock_state):
        ...
        with patch(
            "langgraph_agent.nodes.reporting_nodes.generate_pdf_report"
        ) as mock_pdf:
    ...
    async def test_cleanup_permission_error(self):
        ...
        with patch("os.remove") as mock_remove:
            ...
            with patch(
                ...
                state["errors"].append(
                    {"type": "PermissionError", "message": "Access denied"}
                )
                state["error_count"] = 1
                mock_node.return_value = state
...
class TestReportHelperFunctions:
    ...
    def test_prepare_report_data(self):
        ...
        with patch(
            "langgraph_agent.nodes.reporting_nodes.prepare_report_data"
        ) as mock_prepare:

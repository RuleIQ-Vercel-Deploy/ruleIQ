...
async def run_complete_integration_test():
    ...
    logger.info(f"\n📊 Business Profile:")
    logger.info(
        f"  Industry: {business_profile['industry']}/{business_profile['sub_industry']}"
    )
    ...
    try:
        ...
        logger.info(
            f"  ✓ Total Applicable Regulations: {risk_assessment['total_regulations']}"
        )
        logger.info(
            f"  ✓ Average Risk Score: {risk_assessment['average_risk_score']:.2f}/10"
        )
        ...
        logger.info(
            f"  ✓ Automation Potential: {risk_assessment['automation_potential']:.1%}"
        )
        ...
        if risk_assessment["critical_regulations"]:
            ...
            for reg in risk_assessment["critical_regulations"][:3]:
                ...
                logger.info(
                    f"      Risk: {reg['risk']}, Penalty: {reg.get('max_penalty', 'N/A')}"
                )
        ...
        for item in roadmap:
            ...
            logger.info(
                f"      Timeline: {item['timeline']}, Risk: {item['risk_score']}"
            )
            items_shown += 1
        ...
        top_reg_ids = [
            reg["id"] for reg in risk_assessment["applicable_regulations"][:10]
        ]
        ...
        if top_reg_ids:
            ...
            logger.info(
                f"  ✓ Unique Controls Required: {overlaps['total_unique_controls']}"
            )
            logger.info(
                f"  ✓ Overlapping Controls: {overlaps['overlapping_control_count']}"
            )
            logger.info(
                f"  ✓ Efficiency Gain: {overlaps['efficiency_gain_percentage']:.1f}%"
            )
            ...
            if overlaps["overlapping_controls"]:
                ...
                for control in overlaps["overlapping_controls"][:3]:
                    ...
                    logger.info(
                        f"      Covers {control['regulation_count']} regulations"
                    )
        ...
        logger.info(
            f"  ✓ Industry Penalties (Last Year): ${enforcement['total_industry_penalties']:,.0f}"
        )
        ...
        if enforcement["high_risk_areas"]:
            ...
            for area in enforcement["high_risk_areas"][:2]:
                ...
                logger.info(
                    f"      Cases: {area['enforcement_count']}, Penalties: ${area['total_penalties']:,.0f}"
                )
    ...
    async with AutomationScorer(neo4j_uri, neo4j_user, neo4j_password) as scorer:
        ...
        logger.info(
            f"  ✓ Automation Opportunities: {automation_roadmap.total_opportunities}"
        )
        logger.info(f"  ✓ Quick Wins Available: {len(automation_roadmap.quick_wins)}")
        logger.info(
            f"  ✓ Strategic Initiatives: {len(automation_roadmap.strategic_initiatives)}"
        )
        logger.info(
            f"  ✓ Total Investment Hours: {automation_roadmap.total_investment_hours:,}"
        )
        logger.info(
            f"  ✓ Expected ROI Timeline: {automation_roadmap.expected_roi_months:.1f} months"
        )
        logger.info(
            f"  ✓ Automation Coverage: {automation_roadmap.automation_coverage:.1%}"
        )
        ...
        if automation_roadmap.quick_wins:
            ...
            for opp in automation_roadmap.quick_wins[:3]:
                ...
                logger.info(
                    f"      Annual Savings: ${opp.estimated_savings_annual:,.0f}"
                )
        ...
        if automation_roadmap.phases:
            ...
            for phase in automation_roadmap.phases[:3]:
                ...
                logger.info(
                    f"      Expected Savings: ${phase['expected_savings']:,.0f}/year"
                )
        ...
        if automation_roadmap.quick_wins or automation_roadmap.strategic_initiatives:
            ...
            logger.info("-" * 40)
            logger.info(
                f"  ✓ Total Investment Required: ${investment['total_investment']:,.0f}"
            )
            logger.info(
                f"  ✓ Annual Savings Potential: ${investment['annual_savings']:,.0f}"
            )
            logger.info(
                f"  ✓ Break-even Timeline: {investment['roi_months']:.1f} months"
            )
            logger.info(
                f"  ✓ 3-Year Net Value: ${investment['net_present_value_3y']:,.0f}"
            )
    ...
    async with TemporalTracker(neo4j_uri, neo4j_user, neo4j_password) as tracker:
        ...
        timeline = await tracker.generate_compliance_timeline(
            business_profile, horizon_days=365
        )
        ...
        if timeline.seasonal_patterns.get("peak_months"):
            ...
            peak_names = [
                month_names[m] for m in timeline.seasonal_patterns["peak_months"]
            ]
        ...
        if timeline.resource_forecast:
            ...
            logger.info("-" * 40)
            logger.info(
                f"  ✓ Team Utilization: {timeline.resource_forecast['average_utilization']:.1%}"
            )
            logger.info(
                f"  ✓ Monthly Capacity: {timeline.resource_forecast['monthly_capacity']:.0f} hours"
            )

            if timeline.resource_forecast["constrained_months"]:
                logger.info(
                    f"  ⚠️ Resource Constraints: {len(timeline.resource_forecast['constrained_months'])} months"
                )
        ...
        patterns = await tracker.track_amendment_patterns(
            test_regulation, lookback_years=3
        )

        logger.info(f"  ✓ Regulation: {test_regulation}")
        logger.info(
            f"  ✓ Historical Amendments: {patterns['total_amendments']} (3 years)"
        )
        logger.info(
            f"  ✓ Amendment Frequency: {patterns['amendments_per_year']:.1f}/year"
        )
        ...
        if patterns["next_predicted"]:
            logger.info(
                f"  ✓ Next Predicted Amendment: {patterns['next_predicted'].strftime('%B %Y')}"
            )
        ...
        for month_key in sorted(list(calendar.keys()))[:2]:
            ...
            if events:
                ...
                for event in events[:2]:
                    logger.info(
                        f"    • Day {event.event_date.strftime('%d')}: {event.regulation_title[:40]}..."
                    )
    ...
    summary = {
        ...
        "resource_utilization": (
            timeline.resource_forecast["average_utilization"]
            if timeline.resource_forecast
            else 0
        ),
    }
    ...
    logger.info(
        f"  1. Risk Profile: {summary['risk_level']} ({summary['high_risk_items']} high-risk regulations)"
    )
    logger.info(
        f"  2. Compliance Scope: {summary['applicable_regulations']} regulations apply"
    )
    logger.info(
        f"  3. Automation Potential: {summary['automation_opportunities']} opportunities ({summary['quick_wins']} quick wins)"
    )
    logger.info(
        f"  4. ROI Timeline: {summary['roi_months']:.1f} months to break-even on automation"
    )
    logger.info(
        f"  5. Urgent Actions: {summary['compliance_deadlines_30d']} deadlines in 30 days"
    )
    logger.info(
        f"  6. Overdue Items: {summary['overdue_items']} items need immediate attention"
    )
    ...
    logger.info("-" * 40)
    logger.info(
        "  1. Address {0} overdue compliance items immediately".format(
            summary["overdue_items"]
        )
    )
    logger.info("  2. Implement top 3 control overlaps to maximize efficiency")
    logger.info(
        "  3. Begin automation quick wins (ROI in {0:.0f} months)".format(
            summary["roi_months"]
        )
    )
    logger.info(
        "  4. Prepare for {0} upcoming deadlines".format(
            summary["compliance_deadlines_30d"]
        )
    )

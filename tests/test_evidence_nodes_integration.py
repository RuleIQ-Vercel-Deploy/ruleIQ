...
class TestEvidenceCollectionNodeIntegration:
    ...
    async def test_process_evidence_backward_compatibility(self, mock_get_db, mock_duplicate_detector, mock_evidence_processor):
    ...
    @patch('langgraph_agent.nodes.evidence_nodes.DuplicateDetector')
    @patch('langgraph_agent.nodes.evidence_nodes.get_async_db')
    async def test_process_evidence_with_state(self, mock_get_db, mock_duplicate_detector, mock_evidence_processor, minimal_state):
    ...
    async def test_check_evidence_expiry(self, mock_get_db, minimal_state):
        ...
        expired_evidence.to_dict = MagicMock(return_value={
            "id": "expired_id",
        ...
        mock_session = MagicMock(spec=AsyncSession)
        mock_result = MagicMock()
        mock_result.scalars = MagicMock(return_value=MagicMock(all=MagicMock(return_value=[expired_evidence])))
        mock_session.execute = AsyncMock(return_value=mock_result)
        mock_session.commit = AsyncMock()
        mock_get_db.return_value = async_generator([mock_session])
    ...
    async def test_aggregate_evidence_method(self):
        ...
        evidence_list = [
            {"id": "1", "evidence_name": "Doc 1", "type": "Policy", "status": "collected", "content": "Content 1"},
            {"id": "2", "evidence_name": "Doc 2", "type": "Policy", "status": "pending", "content": "Content 2"},
            {"id": "3", "evidence_name": "Doc 3", "type": "Procedure", "status": "collected", "content": "Content 3"}
        ]
    ...
    async def test_merge_evidence_method(self):
        ...
        evidence1 = {"id": "1", "type": "Policy", "content": {"data": "original"}, "score": 0.8}
    ...
    @patch('langgraph_agent.nodes.evidence_nodes.DuplicateDetector')
    @patch('langgraph_agent.nodes.evidence_nodes.get_async_db')
    async def test_concurrent_evidence_processing(self, mock_get_db, mock_duplicate_detector, mock_evidence_processor):
        ...
        evidence_items = [
            {"evidence_name": f"Doc {i}", "evidence_type": "Policy", "user_id": str(uuid4())}
            for i in range(5)
        ]

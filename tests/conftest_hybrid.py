...
@pytest.fixture
def sync_sample_business_profile(sync_db_session, sync_sample_user, sync_sample_business_profile_session):
    """Create a sample business profile for sync tests."""
    from database.business_profile import BusinessProfile
    
    # Check if profile already exists
    existing_profile = sync_db_session.query(BusinessProfile).filter(
        BusinessProfile.id == sync_sample_business_profile_session["id"]
    ).first()
    ...
    check_profile = sync_db_session.query(BusinessProfile).filter(BusinessProfile.id == profile.id).first()

...
async def test_iq_neo4j_integration():
    ...
    try:
        ...
        logger.info(
            f"  Automation Potential: {risk_assessment['automation_potential']}"
        )
        logger.info(
            f"  Estimated Timeline: {risk_assessment['estimated_implementation_months']} months"
        )
        ...
        for item in roadmap[:10]:  # Show first 10 items
            ...
            logger.info(
                f"      Risk: {item['risk_score']}, Timeline: {item['timeline']}"
            )
        ...
        if top_reg_ids:
            ...
            if overlaps["overlapping_controls"]:
                ...
                for control in overlaps["overlapping_controls"][:5]:
                    logger.info(
                        f"    • {control['control']} (covers {control['regulation_count']} regulations)"
                    )
        ...
        logger.info(
            f"  Total Industry Penalties: ${enforcement['total_industry_penalties']:,.0f}"
        )
        ...
        if enforcement["high_risk_areas"]:
            ...
            for area in enforcement["high_risk_areas"][:3]:
                ...
                logger.info(
                    f"      Cases: {area['enforcement_count']}, Total Penalties: ${area['total_penalties']:,.0f}"
                )
        ...
        if top_reg_ids:
            ...
            logger.info(f"\n🤖 Automation Analysis:")
            logger.info(
                f"  Average Automation Potential: {automation['average_automation_potential']}"
            )
            logger.info(
                f"  High Automation Candidates: {len(automation['high_automation_candidates'])}"
            )

            efficiency = automation["estimated_efficiency_gain"]
            logger.info(f"\n  Estimated Efficiency Gains:")
            logger.info(
                f"    • Hours Saved Annually: {efficiency['estimated_hours_saved_annually']}"
            )
            logger.info(f"    • Cost Savings: {efficiency['estimated_cost_savings']}")
            logger.info(
                f"    • Efficiency Improvement: {efficiency['efficiency_improvement']}"
            )
            ...
            if automation["implementation_roadmap"]:
                ...
                for phase in automation["implementation_roadmap"]:
                    ...
                    logger.info(
                        f"      Items: {len(phase['items'])}, Impact: {phase['expected_impact']}"
                    )

...
class TestUKComplianceFrameworksLoading:
    ...
    def uk_frameworks_data(self):
        ...
        return [{'name': 'ICO_GDPR_UK', 'display_name':
            ...
            'Data Protection Act 2018 & UK GDPR requirements as enforced by ICO'
            , 'category': 'Data Protection', 'applicable_indu': ['all'],
            'employee_thresh': 1, 'revenue_thresho': '£0+',
            'geographic_scop': ['UK', 'England', 'Scotland', 'Wales',
            'Northern Ireland'], 'key_requirement': [
            'Lawful basis for processing', 'Data subject rights',
            'Data Protection Impact Assessments',
            'Breach notification (72 hours to ICO)', 'Privacy by design'],
            'control_domains': ['Access Control', 'Data Minimization',
            'Consent Management'], 'evidence_types': ['Policies',
            'Procedures', 'Training Records', 'Audit Logs'],
            'complexity_scor': 8, 'implementation_': 16, 'estimated_cost_':
            '£15,000-£75,000'}, {'name': 'FCA_REGULATORY', 'display_name':
            'FCA Regulatory Requirements', 'description':
            'Financial Conduct Authority requirements for financial services',
            ...
            'Customer treatment (TCF)', 'Operational resilience',
            'Financial crime prevention', 'Data governance'],
            'complexity_scor': 9, 'implementation_': 24, 'estimated_cost_':
            '£25,000-£150,000'}]
...
class TestUKFrameworkAPIIntegration:
    ...
    def test_get_uk_frameworks_endpoint(self, client, db_session):
        ...
        uk_framework = ComplianceFramework(name='ICO_GDPR_UK', display_name
            ='UK GDPR', description='UK implementation', category=
            'Data Protection', geographic_scop=['UK'])
    ...
    def test_framework_assessment_compatibility(self, client, db_session):
        ...
        uk_framework = ComplianceFramework(name='ICO_GDPR_UK', display_name
            ='UK GDPR', description='UK implementation', category=
            'Data Protection', geographic_scop=['UK'], key_requirement=[
            'Data Protection', 'Consent Management'])
        db_session.add(uk_framework)
        db_session.commit()
        assessment_data = {'business_profile_id': 'test-profile-id',
            'framework_ids': [str(uk_framework.id)], 'assessment_type':
            'initial'}
...
class TestUKComplianceFrameworksDataIntegrity:
    ...
    def test_iso27001_template_mapping(self):
        ...
        uk_gdpr_controls = mapper.map_iso_to_uk_gdpr(['A.5.1.1', 'A.8.2.1',
            'A.12.6.1'])
        assert len(uk_gdpr_controls) == MAX_RETRIES
        assert 'Data Protection by Design' in uk_gdpr_controls[0][
            'uk_requirement']

    def test_framework_versioning(self):
        """Test framework versioning for UK regulatory updates"""
        framework = ComplianceFramework(name='ICO_GDPR_UK', version='1.0',
            description='Initial UK GDPR implementation')
        updated_framework = framework.create_new_version('1.1',
            'Updated for ICO guidance 2024')
...
class TestUKFrameworksPerformance:
    ...
    def test_framework_query_performance(self, db_session):
        ...
        for i in range(DEFAULT_LIMIT):
            framework = ComplianceFramework(name=f'UK_FRAMEWORK_{i}',
                display_name=f'UK Framework {i}', description=
                'Test framework', category='Test', geographic_scop=['UK'])
            db_session.add(framework)
        db_session.commit()
        start_time = time.time()
        frameworks = db_session.query(ComplianceFramework).filter(
            ComplianceFramework.geographic_scop.contains(['UK'])).all(),

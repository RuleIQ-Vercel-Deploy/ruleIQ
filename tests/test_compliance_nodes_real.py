...
class TestRealComplianceNodes:
    ...
    def mock_assessment(self):
        ...
        assessment.quick_wins = [
            {"action": "Update privacy policy", "effort": "low"}
        ]
    ...
    async def test_update_compliance_for_profile_success(
        ...
        with patch('langgraph_agent.nodes.compliance_nodes_real.get_user_for_profile') as mock_get_user:
            with patch('langgraph_agent.nodes.compliance_nodes_real.get_default_framework') as mock_get_framework:
                with patch('langgraph_agent.nodes.compliance_nodes_real.generate_readiness_assessment') as mock_generate:
                    mock_db = AsyncMock()
    ...
    async def test_batch_compliance_update_node(
        self, mock_state, mock_business_profile, mock_user, mock_framework, mock_assessment
    ):
        ...
        with patch('langgraph_agent.nodes.compliance_nodes_real.get_async_db') as mock_get_db:
            with patch('langgraph_agent.nodes.compliance_nodes_real.select') as mock_select:
                with patch('langgraph_agent.nodes.compliance_nodes_real.update_compliance_for_profile') as mock_update:
                    # Setup mock database
                    mock_db = AsyncMock()
                    ...
                    assert result["compliance_data"]["batch_update_results"]["total_profiles"] == 1
                    assert result["compliance_data"]["batch_update_results"]["updated_count"] == 1
    ...
    @pytest.mark.asyncio
    async def test_compliance_monitoring_node(
        self, mock_state, mock_business_profile
    ):
        ...
        with patch('langgraph_agent.nodes.compliance_nodes_real.get_async_db') as mock_get_db:
            with patch('langgraph_agent.nodes.compliance_nodes_real.select') as mock_select:
                with patch('langgraph_agent.nodes.compliance_nodes_real.update_compliance_for_profile') as mock_update:
                    ...
                    assert result["compliance_data"]["monitoring_results"]["total_profiles"] == 1
                    assert result["compliance_data"]["monitoring_results"]["alerts_generated"] == 1
                    assert len(result["compliance_data"]["monitoring_results"]["low_score_profiles"]) == 1

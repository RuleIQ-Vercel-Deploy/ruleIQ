...
async def master_graph(database_setup):
    ...
    rag_config = RAGConfig(embedding_provider='mistral', embedding_model=
        ...
        fallback_llm_model='gpt-4o-mini', mistral_api_key=MISTRAL_API_KEY,
        google_api_key=GOOGLE_API_KEY, openai_api_key=OPENAI_API_KEY)
    graph = await MasterIntegrationGraph.create(database_url=
...
class TestMasterIntegrationEndToEnd:
    ...
    async def test_complete_compliance_assessment_flow(self, master_graph,
        neo4j_service):
    ...
    async def test_checkpoint_persistence_and_recovery(self, master_graph):
        ...
        async for event in master_graph.run(session_id=session_id,
            ...
            'Start compliance assessment for our fintech startup',
            thread_id=thread_id):
        ...
        async for event in master_graph.run(session_id=session_id,
            company_id=company_id, user_input=
            'Continue with the assessment', thread_id=thread_id):
        ...
        if checkpoint_summary_1.get('conversation'
            ) and checkpoint_summary_2.get('conversation'):
...
class TestProductionReadiness:
    ...
    async def test_concurrent_sessions(self, master_graph):
        ...
        async def run_session(session_num):
            ...
            async for event in master_graph.run(session_id=session_id,
                company_id=company_id, user_input=
                f'Assessment for company {session_num}'):
                events.append(event)
    ...
    async def test_memory_efficiency(self, master_graph):
        ...
        for i in range(10):
            events = []
            async for event in master_graph.run(session_id=
                f'{session_id}_{i}', company_id=company_id, user_input=
                f'Query {i}: What are the compliance requirements?'):
                events.append(event)
    ...
    async def test_latency_requirements(self, master_graph):
        ...
        async for event in master_graph.run(session_id=session_id,
            company_id=company_id, user_input='Quick compliance check for GDPR'
            ):
            if event['type'
                ] == 'assistant_message' and first_response_time is None:
...
class TestServiceIntegration:
    ...
    async def test_neo4j_connection_pooling(self, neo4j_service):
        ...
        queries = ['MATCH (r:Regulation) RETURN count(r) as count',

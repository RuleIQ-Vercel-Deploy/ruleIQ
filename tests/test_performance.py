...
class TestAPIResponseTimes:
    ...
    def test_core_endpoint_response_times(self, client, authenticated_headers, performance_test_data):
    ...
    def test_database_query_performance(self, client, authenticated_headers, db_session, performance_test_data):
...
class TestAIGenerationPerformance:
    ...
    def test_ai_response_latency(self, client, authenticated_headers, mock_ai_client, performance_test_data):
    ...
    def test_ai_content_caching_performance(self, client, authenticated_headers, mock_ai_client):
        ...
        start_time = time.time()
        response_1 = client.post("/api/policies/generate",
        ...
        start_time = time.time()
        response_2 = client.post("/api/policies/generate",
                                headers=authenticated_headers,
                                json={"framework_id": "test-framework"})
    ...
    def test_batch_ai_processing_efficiency(self, client, authenticated_headers, mock_ai_client):
        ...
        for _ in range(5):
            start_time = time.time()
            client.post("/api/policies/generate",
                       headers=authenticated_headers,
    ...
    def test_concurrent_user_simulation(self, client, authenticated_headers, performance_test_data):
        ...
        for user_count in concurrent_users:
            if user_count <= 10:  # Limit for test environment
                success_rates = self._simulate_concurrent_users(client, authenticated_headers, user_count)

                # Verify success rate under load
                assert success_rates["success_rate"] >= 0.95, \
                    f"Success rate {success_rates['success_rate']:.2f} too low for {user_count} users"

                # Verify response times don't degrade too much
                assert success_rates["avg_response_time"] <= 5.0, \
...
class TestSoakTesting:
    ...
    def test_extended_operation_stability(self, client, authenticated_headers):
        ...
        if response_times:
            avg_response_time = statistics.mean(response_times)
            assert avg_response_time <= 3.0, f"Average response time degraded to {avg_response_time:.2f}s"
    ...
    def test_connection_pool_exhaustion(self, client, authenticated_headers):
        ...
        assert success_rate >= 0.8, f"System should handle connection exhaustion gracefully, got {success_rate:.2f}"
...
class TestResourceUtilization:
    ...
    def test_disk_io_efficiency(self, client, authenticated_headers):
        ...
        for _ in range(20):
            ...
            client.post("/api/readiness/reports",
                       headers=authenticated_headers,
    ...
    def test_response_compression_efficiency(self, client, authenticated_headers):
        ...
        compressed_response = client.get("/api/frameworks",
                                       headers={**authenticated_headers, "Accept-Encoding": "gzip"})

        # Request without compression
        uncompressed_response = client.get("/api/frameworks",
                                         headers={**authenticated_headers, "Accept-Encoding": "identity"})

        if compressed_response.status_code == 200 and uncompressed_response.status_code == 200:
...
class TestScalabilityLimits:
    ...
    def test_maximum_concurrent_assessments(self, client, authenticated_headers):
        ...
        success_rate = sum(results) / len(results)
        assert success_rate >= 0.8, f"Assessment creation success rate {success_rate:.2f} too low"
    ...
    def test_large_data_set_handling(self, client, authenticated_headers):
        ...
        assert processing_time <= 5.0, f"Large data processing too slow: {processing_time:.2f}s"

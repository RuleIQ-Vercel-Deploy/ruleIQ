...
class TestPolicyStreamingService:
    ...
    @pytest.mark.asyncio
    async def test_generate_policy_stream_metadata_chunk(self, policy_generator, sample_request, sample_framework):
    ...
    async def test_stream_with_google_success(self, policy_generator, sample_request):
        ...
        policy_generator.google_model.generate_content_async = AsyncMock(return_value=mock_response)
    ...
    async def test_stream_with_openai_success(self, policy_generator, sample_request):
        ...
        policy_generator.openai_client.chat.completions.create = AsyncMock(return_value=mock_stream)
    ...
    @pytest.mark.asyncio
    async def test_stream_with_circuit_breaker_open(self, policy_generator, sample_request, sample_framework):
        ...
        with patch('services.ai.policy_generator.google_breaker') as mock_breaker:
            mock_breaker.call = AsyncMock(side_effect=Exception("Circuit breaker is open"))
            ...
            with patch.object(policy_generator, '_stream_with_openai', side_effect=mock_openai_stream):
                chunks = []
                async for chunk in policy_generator.generate_policy_stream(sample_request, sample_framework):
                    chunks.append(chunk)
    ...
    @pytest.mark.asyncio
    async def test_stream_progress_updates(self, policy_generator, sample_request, sample_framework):
        ...
        with patch.object(policy_generator, '_stream_with_google', side_effect=mock_google_with_progress):
            chunks = []
            async for chunk in policy_generator.generate_policy_stream(sample_request, sample_framework):
                chunks.append(chunk)
...
class TestPolicyStreamingAPI:
    ...
    async def test_stream_endpoint_rate_limiting(self, client, auth_headers, mock_auth):
        ...
        with patch('api.routers.ai_policy.RateLimiter') as MockRateLimiter:
            ...
            mock_limiter.check_rate_limit = AsyncMock(side_effect=HTTPException(status_code=429, detail="Rate limit exceeded"))

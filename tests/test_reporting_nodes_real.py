...
class TestReportingNodesRealImplementation:
    ...
    async def test_generate_scheduled_reports_node_success(
        ...
        with patch('langgraph_agent.nodes.reporting_nodes_real.ReportScheduler') as MockScheduler, \
             ...
             patch('langgraph_agent.nodes.reporting_nodes_real.send_scheduled_report_email') as mock_send_email, \
             patch('langgraph_agent.nodes.reporting_nodes_real.save_report_file') as mock_save_file, \
             patch('langgraph_agent.nodes.reporting_nodes_real.should_run_schedule') as mock_should_run:
    ...
    async def test_generate_scheduled_reports_node_inactive_schedule(
        ...
        with patch('langgraph_agent.nodes.reporting_nodes_real.ReportScheduler') as MockScheduler:
            ...
            assert result_state.get("report_data", {}).get("scheduled_reports", {}).get("generated", 0) == 0
    ...
    async def test_generate_on_demand_report_node_success(
        ...
        sample_state["metadata"]["report_parameters"] = {"include_recommendations": True}
        
        # Add database to state
        sample_state["db_session"] = mock_db_session
        
        with patch('langgraph_agent.nodes.reporting_nodes_real.ReportGenerator') as MockGenerator, \
             patch('langgraph_agent.nodes.reporting_nodes_real.send_email_with_attachment') as mock_send_email:
            ...
            mock_generator.generate_report.assert_called_once_with(
                ...
                business_profile_id=UUID(sample_state["metadata"]["business_profile_id"]),
            ...
            # Verify email was sent
            mock_send_email.assert_called_once()
            assert mock_send_email.call_args.kwargs["recipients"] == ["manager@example.com"]
    ...
    async def test_generate_on_demand_report_node_missing_request(
        ...
        assert "user_id and business_profile_id are required" in result_state["errors"][0]["message"]
    ...
    async def test_send_summary_notifications_node_success(
        ...
        with patch('langgraph_agent.nodes.reporting_nodes_real.ReportScheduler') as MockScheduler, \
             patch('workers.reporting_tasks._send_email_sync') as mock_send_email:
...
class TestReportGeneratorIntegration:
    ...
    async def test_report_generator_executive_summary(
        ...
        mock_db_session.execute.side_effect = [
            ...
            MagicMock(scalars=MagicMock(return_value=MagicMock(first=MagicMock(return_value=mock_business_profile)))),
    ...
    async def test_report_generator_gap_analysis(
        ...
        mock_framework.controls = [
            {"id": f"control-{i}", "name": f"Control {i}"} 
            for i in range(10)
        ]
        ...
        mock_db_session.execute.side_effect = [
            ...
            MagicMock(scalars=MagicMock(return_value=MagicMock(first=MagicMock(return_value=mock_business_profile)))),
            # Get framework
            MagicMock(scalars=MagicMock(return_value=MagicMock(first=MagicMock(return_value=mock_framework)))),
            # Get evidence items
            MagicMock(scalars=MagicMock(return_value=MagicMock(all=MagicMock(return_value=mock_evidence_items))))
        ]
    ...
    def mock_business_profile(self):
        ...
        profile.to_dict = MagicMock(return_value={"id": str(profile.id), "company_name": profile.company_name})
        return profile
...
class TestErrorHandlingAndEdgeCases:
    ...
    async def test_multiple_schedules_batch_processing(
        ...
        with patch('langgraph_agent.nodes.reporting_nodes_real.ReportScheduler') as MockScheduler, \
             patch('langgraph_agent.nodes.reporting_nodes_real.ReportGenerator') as MockGenerator, \
             patch('langgraph_agent.nodes.reporting_nodes_real.PDFGenerator') as MockPDF, \
             patch('langgraph_agent.nodes.reporting_nodes_real.save_report_file') as mock_save_report, \
             patch('langgraph_agent.nodes.reporting_nodes_real.send_scheduled_report_email') as mock_send_email:
    ...
    async def test_partial_failure_handling(
        ...
        with patch('langgraph_agent.nodes.reporting_nodes_real.ReportScheduler') as MockScheduler, \
            ...
            mock_generator = AsyncMock()
            mock_generator.generate_report = AsyncMock(side_effect=[
                {"data": "report1"},
            ...
            failed_calls = [
                call for call in mock_scheduler.update_schedule_status.call_args_list
                if call[0][1] == "failed"
            ]

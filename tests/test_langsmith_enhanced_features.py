...
from config.langsmith_evaluators import (
    LangSmithEvaluator,
    EvaluationMetrics,
...
class TestEvaluationMetrics:
    ...
    def test_compliance_evaluation(self):
        ...
        assert metrics.compliance_score == 75.0  # 3 out of 4 passed
    ...
    def test_evidence_evaluation(self):
        ...
        assert metrics.latency_ms == 300
        assert metrics.evidence_quality == 62.5  # (100 + 25) / 2
        assert metrics.error_rate == 0.0
    ...
    def test_rag_evaluation(self):
        ...
        run_data = {
            ...
            "outputs": {"llm_output": "A" * 1000},  # Simulate 1000 character output
        }

        metrics = LangSmithEvaluator.evaluate_rag_run(run_data)

        assert metrics.latency_ms == 1200
        assert metrics.token_count == 250  # 1000/4
        assert metrics.cost_usd > 0  # Should calculate cost
        assert metrics.error_rate == 0.0
    ...
    def test_performance_benchmark(self):
        ...
        baseline = EvaluationMetrics(
            latency_ms=100, accuracy=0.9, error_rate=0.05, cost_usd=0.01
        )
        benchmark.set_baseline("test_op", baseline)

        # Compare with current
        current = EvaluationMetrics(
            latency_ms=90,  # 10% improvement
            accuracy=0.95,  # 5.5% improvement
            error_rate=0.03,  # 0.02 improvement
            cost_usd=0.008,  # 20% improvement
        )
...
class TestFeedbackCollection:
    ...
    def test_feedback_collection(self):
        ...
        assert len(collector.feedback_queue) == 3
        assert collector.feedback_stats["total_collected"] == 3
    ...
    def test_feedback_summary(self):
        ...
        assert summary["by_type"]["thumbs_up"] == 5
        assert summary["by_type"]["rating"] == 3
    ...
    def test_feedback_analysis(self):
        ...
        assert analysis["sentiment"]["positive"] == 2  # 1 thumbs up + 1 rating >= 4
        assert analysis["sentiment"]["negative"] == 2  # 1 thumbs down + 1 rating <= 2
        assert analysis["average_rating"] == 3.0  # (4 + 2) / 2
        assert analysis["correction_rate"] == 20.0  # 1 out of 5
    ...
    def test_improvement_recommendations(self):
        ...
        recommendations1 = FeedbackAnalyzer.generate_improvement_recommendations(
            analysis1
        )
        ...
        recommendations2 = FeedbackAnalyzer.generate_improvement_recommendations(
            analysis2
        )

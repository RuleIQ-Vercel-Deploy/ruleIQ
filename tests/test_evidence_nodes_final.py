...
class TestProcessEvidenceAdditionalPaths:
    ...
    async def test_process_evidence_with_existing_processor(self, mock_duplicate_detector, mock_processor_class, mock_get_db):
    ...
    @patch('langgraph_agent.nodes.evidence_nodes.get_async_db')
    @patch('langgraph_agent.nodes.evidence_nodes.DuplicateDetector')
    async def test_process_evidence_database_exception_reraise(self, mock_duplicate_detector, mock_get_db):
...
class TestProcessPendingEvidenceEdgeCases:
    ...
    async def test_process_pending_with_multiple_items(self, mock_processor_class, mock_get_db):
        ...
        evidence1.to_dict = MagicMock(return_value={
            "id": evidence1.id,
        ...
        evidence2.to_dict = MagicMock(return_value={
            "id": evidence2.id,
        ...
        mock_result.scalars = MagicMock(return_value=MagicMock(all=MagicMock(return_value=[evidence1, evidence2])))
...
class TestStaleEvidenceCleanup:
    ...
    @patch('langgraph_agent.nodes.evidence_nodes.EvidenceProcessor')
    @patch('langgraph_agent.nodes.evidence_nodes.DuplicateDetector')
    async def test_process_evidence_stale_evidence_cleanup(self, mock_duplicate_detector, mock_processor_class, mock_get_db):

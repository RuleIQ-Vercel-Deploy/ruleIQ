...
class TestDataEncryption:
    ...
    async def test_field_level_encryption(self, sample_sensitive_data):
                ...
                if 'personal_data' in encrypted:
                    ...
                    for field in ['phone', 'address']:
                        if field in encrypted['personal_data']:
                            ...
                            encrypted['personal_data'][field] = self.cipher.encrypt(value.encode()).decode()
                    ...
                    for field in ['phone', 'address']:
                        if field in decrypted['personal_data']:
                            try:
                                ...
                                decrypted['personal_data'][field] = self.cipher.decrypt(encrypted_value).decode()
                            except:
                                pass
        ...
        assert encrypted_data['personal_data']['phone'] != sample_sensitive_data['personal_data']['phone']
        ...
        assert decrypted_data['personal_data']['phone'] == sample_sensitive_data['personal_data']['phone']
    ...
    async def test_security_event_logging(self, sample_audit_event):
        ...
        class AuditLogger:
            ...
            def log_event(self, event_type: str, user_id: str, action: str, 
                ...
                """Log an audit event"""
                event = {
                    "event_id": hashlib.sha256(f"{datetime.utcnow()}{user_id}{action}".encode()).hexdigest()[:16],
                    "timestamp": datetime.utcnow().isoformat(),
            ...
            def log_permission_change(self, user_id: str, admin_id: str, 
...
class TestSecurityHeaders:
    ...
    async def test_security_headers_presence(self, security_headers_expected):
        ...
        class SecurityHeadersMiddleware:
            ...
            def add_conditional_headers(self, request_path: str, headers: Dict[str, str]) -> Dict[str, str]:
...
class TestVulnerabilityPrevention:
    ...
    async def test_sql_injection_prevention(self):
        ...
        class SQLSanitizer:
            ...
            def is_safe_input(self, user_input: str) -> bool:
                ...
                if "'" in user_input or '"' in user_input:
                    if any(keyword in input_upper for keyword in ['OR', 'AND', 'SELECT', 'DROP']):
                        return False
            ...
            def build_parameterized_query(self, query_template: str, params: List[Any]) -> Tuple[str, List[Any]]:
                """Build parameterized query (simulation)"""
                # Count placeholders
                placeholder_count = query_template.count('?')
                if placeholder_count != len(params):
                    raise ValueError(f"Parameter count mismatch: expected {placeholder_count}, got {len(params)}")
        ...
        for payload in malicious_inputs:
            assert not sanitizer.is_safe_input(payload), f"Malicious input '{payload}' should be detected"
        
        # Test safe inputs
        safe_inputs = ["john.doe@example.com", "user123", "Product Name", "12345"]
        for safe_input in safe_inputs:
            assert sanitizer.is_safe_input(safe_input), f"Safe input '{safe_input}' should be allowed"
        ...
        query_template, query_params = sanitizer.build_parameterized_query(query, params)

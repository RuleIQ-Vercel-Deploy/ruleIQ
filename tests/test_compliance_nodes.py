...
class TestComplianceCheckNode:
    ...
    async def test_compliance_check_successful(self, mock_state):
        ...
        with patch(
            ...
            assert (
                result["compliance_data"]["check_results"]["compliance_score"] == 50.0
            )
...
class TestExtractRequirements:
    ...
    def test_extract_requirements_with_keywords(self):
        ...
        assert len(requirements) == 2  # Only documents with "must" or "requirement"
    ...
    def test_extract_requirements_no_matching_content(self):
        ...
        documents = [
            {
                "content": "General compliance information.",
...
class TestCheckComplianceStatus:
    ...
    async def test_check_compliance_status_successful(self):
        ...
        with patch(
            "langgraph_agent.nodes.compliance_nodes.get_neo4j_service",
    ...
    async def test_check_compliance_status_all_satisfied(self):
        ...
        mock_records = [
            {
                "o": {"id": "ob1", "title": "Data Protection"},
                "evidence": [{"id": "ev1"}],
            },
            {
                "o": {"id": "ob2", "title": "Data Retention"},
                "evidence": [{"id": "ev2"}],
            },
        ]
    ...
    async def test_check_compliance_status_database_error(self):
        ...
        with patch(
            ...
            mock_service = MagicMock()
            mock_service.driver = None  # Simulate connection failure
            mock_get_service.return_value = mock_service
...
class TestAssessComplianceRisk:
    ...
    async def test_assess_risk_medium_level(self):
        ...
        state = {
            "compliance_data": {
                "check_results": {"violations": [{"id": str(i)} for i in range(2)]}
            }
        }
    ...
    async def test_assess_risk_high_level(self):
        ...
        state = {
            "compliance_data": {
                "check_results": {"violations": [{"id": str(i)} for i in range(3)]}
            }
        }

        result = await assess_compliance_risk(state)

        assert result["compliance_data"]["risk_assessment"]["score"] == 60
    ...
    async def test_assess_risk_critical_level(self):
        ...
        state = {
            "compliance_data": {
                "check_results": {"violations": [{"id": str(i)} for i in range(4)]}
            }
        }
    ...
    async def test_assess_risk_capped_at_100(self):
        ...
        assert result["compliance_data"]["risk_assessment"]["score"] == 100
        assert result["compliance_data"]["risk_assessment"]["level"] == "CRITICAL"
...
class TestIntegration:
    ...
    async def test_full_compliance_workflow(self):
        ...
        with patch(
            ...
            assert (
                final_state["compliance_data"]["risk_assessment"]["level"] == "MEDIUM"
            )

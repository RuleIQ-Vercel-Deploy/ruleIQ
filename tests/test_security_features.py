...
class TestRateLimiting:
    ...
    async def test_rate_limit_check_allowed(self):
        ...
        result = await RateLimitService.check_rate_limit(db=mock_db, user=
            mock_user, feature='ai_assessment', check_only=True)
    ...
    async def test_rate_limit_exception_when_exceeded(self):
        ...
        mock_oldest_result.scalar.return_value = datetime.now(timezone.utc
            ) - timedelta(hours=12)
        mock_db.execute.side_effect = [mock_count_result, mock_oldest_result]
        with pytest.raises(HTTPException) as exc_info:
            await RateLimitService.check_rate_limit(db=mock_db, user=
...
class TestSecurityAlerts:
    ...
    async def test_check_failed_logins_under_threshold(self):
        ...
        mock_db.execute.return_value = mock_result
        should_alert = await SecurityAlertService.check_failed_logins(db=
            mock_db, user=mock_user, ip_address='192.168.1.1')
    ...
    @patch.dict('os.environ', {'SMTP_HOST': 'smtp.test.com', 'SMTP_PORT':
        '587', 'SMTP_USER': 'test@test.com', 'SMTP_PASSWORD': 'testpass',
    ...
    async def test_send_failed_login_alert(self, mock_smtp_send):
        ...
        await SecurityAlertService.send_failed_login_alert(user=mock_user,
            failed_attempts=3, ip_address='192.168.1.1', user_agent=
            'Mozilla/5.0')
    ...
    async def test_log_and_check_login_attempt_success(self):
        ...
        await SecurityAlertService.log_and_check_login_attempt(db=mock_db,
            user=mock_user, success=True, ip_address='192.168.1.1',
    ...
    async def test_log_and_check_login_attempt_failure_triggers_alert(self,
        mock_create_task):
        ...
        with patch.object(SecurityAlertService, 'check_failed_logins',
            return_value=True):
            await SecurityAlertService.log_and_check_login_attempt(db=
                mock_db, user=mock_user, success=False, ip_address=
                '192.168.1.1', user_agent='Mozilla/5.0')
        mock_db.add.assert_called_once()
        audit_log = mock_db.add.call_args[0][0]

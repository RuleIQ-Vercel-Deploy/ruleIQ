...
class TestAPIContractValidation:
    ...
    @pytest.mark.asyncio
    async def test_authentication_endpoint_contract_validation(self,
                                                             async_client: httpx.AsyncClient):
        ...
        for field in required_fields:
            assert field in register_response, f"Required field {field} missing from register response"
    ...
    @pytest.mark.asyncio 
    async def test_iq_agent_endpoint_contract_validation(self,
                                                        async_client: httpx.AsyncClient,
                                                        auth_headers: Dict):
        ...
        with patch('services.iq_agent.IQComplianceAgent.process_query') as mock_query:
            ...
            response = await async_client.post("/iq-agent/query",
                                             json=query_data,
                                             headers=auth_headers)
    ...
    @pytest.mark.asyncio
    async def test_evidence_upload_contract_validation(self,
                                                      async_client: httpx.AsyncClient,
                                                      auth_headers: Dict):
        ...
        files = {
            "file": ("test_policy.pdf", b"Mock PDF content", "application/pdf")
        }
        metadata = {
        ...
        required_fields = ["id", "title", "description", "file_path", "evidence_type", "status", "created_at"]
        for field in required_fields:
    ...
    @pytest.mark.asyncio
    async def test_field_mapper_contract_validation(self,
                                                   async_client: httpx.AsyncClient,
                                                   auth_headers: Dict):
        ...
        assert retrieved_assessment["name"] == long_field_data["name"]  # Full name preserved

    @pytest.mark.asyncio
    async def test_pagination_contract_validation(self,
                                                 async_client: httpx.AsyncClient,
                                                 auth_headers: Dict):
    ...
    def test_cors_contract_validation(self, client: TestClient):
        ...
        response = client.options("/assessments",
                                headers={
...
class TestContractPerformance:
    ...
    async def test_schema_validation_performance(self,
        ...
        assert validation_time < 0.5, f"Schema validation took {validation_time:.3f}s, expected <0.5s"
...
class TestSecurityContractValidation:
    ...
    @pytest.mark.asyncio
    async def test_authentication_contract_security(self,
                                                   async_client: httpx.AsyncClient):
    ...
    @pytest.mark.asyncio
    async def test_input_sanitization_contract(self,
                                              async_client: httpx.AsyncClient,
                                              auth_headers: Dict):
    ...
    @pytest.mark.asyncio
    async def test_rate_limiting_contract_headers(self,
                                                 async_client: httpx.AsyncClient,
                                                 auth_headers: Dict):

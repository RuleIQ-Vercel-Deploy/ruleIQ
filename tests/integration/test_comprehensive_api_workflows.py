...
class TestComprehensiveAPIWorkflows:
    ...
    @pytest.mark.asyncio
    async def test_compliance_assessment_pipeline_integration(self, 
        ...
        response = await async_client.post("/assessments", json=assessment_data, headers=headers)
        assert response.status_code == 201
        ...
        response = await async_client.get(f"/assessments/{assessment_id}", headers=headers)
        assert response.status_code == 200
        ...
        response = await async_client.post(f"/assessments/{assessment_id}/generate-report",
                                         headers=headers)
        assert response.status_code == 200
        ...
        response = await async_client.get(f"/assessments/{assessment_id}/report",
                                        headers=headers)
        assert response.status_code == 200

    @pytest.mark.asyncio
    async def test_evidence_collection_workflow_integration(self,
                                                           async_client: httpx.AsyncClient,
                                                           test_user_with_token: Dict):
        ...
        files = {
            "file": ("privacy_policy.pdf", b"Mock PDF content", "application/pdf")
        }
        metadata = {
        ...
        response = await async_client.post(f"/evidence/{evidence_id}/map-compliance",
                                         json=mapping_data,
                                         headers=headers)
        assert response.status_code == 200
    ...
    @pytest.mark.asyncio
    async def test_ai_service_circuit_breaker_integration(self,
                                                         async_client: httpx.AsyncClient,
                                                         test_user_with_token: Dict):
        ...
        with patch('services.ai.assistant.ComplianceAssistant.analyze_assessment') as mock_ai:
            ...
            for _ in range(3):
                ...
                assert "fallback" in result.get("source", "").lower() or result.get("compliance_score") == 0.5
    ...
    @pytest.mark.asyncio
    async def test_concurrent_api_operations_integration(self,
                                                        async_client: httpx.AsyncClient,
                                                        test_user_with_token: Dict):
        ...
        async def update_assessment(update_data: Dict):
            return await async_client.patch(f"/assessments/{assessment_id}",
                                          json=update_data,
                                          headers=headers)
        ...
        assert final_state["status"] in ["draft", "in_progress"]  # One of the attempted values

    @pytest.mark.asyncio
    async def test_rate_limiting_integration_across_endpoints(self,
                                                            async_client: httpx.AsyncClient,
                                                            test_user_with_token: Dict):
    ...
    @pytest.mark.asyncio
    async def test_error_handling_integration_across_services(self,
                                                            async_client: httpx.AsyncClient,
                                                            test_user_with_token: Dict):
        ...
        response = await async_client.post("/assessments", json=invalid_assessment, headers=headers)
        ...
        with patch('services.external_integration.ExternalService.call') as mock_external:
            ...
            assert "warnings" in assessment or assessment.get("external_data_available") == False
...
class TestAPIWorkflowPerformance:
    ...
    @pytest.mark.asyncio
    async def test_assessment_workflow_performance(self,
                                                  async_client: httpx.AsyncClient,
                                                  test_user_with_token: Dict):

...
class TestEnhancedChatEndpoints:
    ...
    def test_context_aware_recommendations_success(self, client, authenticated_headers, sample_business_profile):
        """Test context-aware recommendations endpoint"""
        
        with patch('services.ai.assistant.ComplianceAssistant.get_context_aware_recommendations') as mock_ai:
    ...
    def test_evidence_collection_workflow_generation(self, client, authenticated_headers, sample_business_profile):
        """Test workflow generation endpoint"""
        
        with patch('services.ai.assistant.ComplianceAssistant.generate_evidence_collection_workflow') as mock_ai:
            mock_ai.return_value = {
    ...
    def test_policy_generation(self, client, authenticated_headers, sample_business_profile):
        ...
        with patch('services.ai.assistant.ComplianceAssistant.generate_customized_policy') as mock_ai:
            mock_ai.return_value = {
    ...
    def test_smart_compliance_guidance(self, client, authenticated_headers, sample_business_profile):
        """Test smart compliance guidance endpoint"""
        
        with patch('services.ai.assistant.ComplianceAssistant.get_context_aware_recommendations') as mock_rec:
            with patch('services.ai.assistant.ComplianceAssistant.analyze_evidence_gap') as mock_gap:
                ...
                if response.status_code == 200:
                    ...
                    # Check quick wins and automation opportunities
                    assert len(data['quick_wins']) <= 3
                    assert len(data['automation_opportunities']) <= 3
                else:
                    assert response.status_code == 400
    ...
    def test_ai_service_error_handling(self, client, authenticated_headers, sample_business_profile):
    ...
    def test_invalid_framework_parameter(self, client, authenticated_headers, sample_business_profile):
...
class TestEnhancedChatValidation:
    ...
    def test_workflow_generation_parameter_validation(self, client, authenticated_headers):
        ...
        response = client.post(
            "/api/chat/evidence-collection-workflow",
            headers=authenticated_headers
        )

...
class TestEvidenceClassificationAPI:
    ...
    def sample_evidence_data(self):
        ...
        return {'evidence_name': 'Security Policy Document', 'description':
            'Comprehensive information security policy covering access controls and data protection'
            , 'evidence_type': 'unknown', 'raw_data': json.dumps({
    ...
    def test_classify_single_evidence(self, client, authenticated_headers,
        sample_business_profile, sample_evidence_data):
        ...
        if create_response.status_code != HTTP_OK:
            pytest.skip(
                'Evidence creation failed - may need business profile setup')
        evidence_id = create_response.json()['id']
        with patch(
            'services.automation.evidence_processor.EvidenceProcessor._ai_classify_evidence'
            ) as mock_ai:
            mock_ai.return_value = {'suggested_type': 'policy_document',
                'confidence': 85, 'suggested_controls': ['A.5.1.1',
                'A.5.1.2', 'A.9.1.1'], 'reasoning':
                'Document contains policy language and security controls'}
            classify_request = {'evidence_id': evidence_id,
                'force_reclassify': False}
            response = client.post(f'/api/evidence/{evidence_id}/classify',
                json=classify_request, headers=authenticated_headers)
            ...
            assert response_data['suggested_controls'] == ['A.5.1.1',
                'A.5.1.2', 'A.9.1.1']
    ...
    def test_classify_evidence_force_reclassify(self, client,
        ...
        with patch(
            ...
            response1 = client.post(f'/api/evidence/{evidence_id}/classify',
                json={'evidence_id': evidence_id, 'force_reclassify': False
                }, headers=authenticated_headers)
            if response1.status_code == HTTP_OK:
                mock_ai.return_value = {'suggested_type': 'audit_report',
                    'confidence': 90, 'suggested_controls': ['A.9.2.1'],
                    'reasoning': 'Reclassified as audit report'}
                response2 = client.post(f'/api/evidence/{evidence_id}/classify'
                    , json={'evidence_id': evidence_id, 'force_reclassify':
                    False}, headers=authenticated_headers)
                if response2.status_code == HTTP_OK:
                    assert response2.json()['reasoning'
                        ] == 'Previously classified'
                response3 = client.post(f'/api/evidence/{evidence_id}/classify'
                    , json={'evidence_id': evidence_id, 'force_reclassify':
                    True}, headers=authenticated_headers)
                if response3.status_code == HTTP_OK:
                    assert (response3.json()['reasoning'] ==
                        'Reclassified as audit report',)
    ...
    def test_bulk_classify_evidence(self, client, authenticated_headers,
        ...
        for i in range(3):
            evidence_data = {'evidence_name': f'Test Evidence {i}',
                'description': f'Test description {i}', 'evidence_type':
                'unknown'}
            create_response = client.post('/api/evidence/', json=
        ...
        with patch(
            ...
            mock_ai.return_value = {'suggested_type': 'policy_document',
                'confidence': 75, 'suggested_controls': ['A.5.1.1'],
                'reasoning': 'Bulk classification test'}
            bulk_request = {'evidence_ids': evidence_ids,
                'force_reclassify': False, 'apply_high_confidence': True,
                'confidence_threshold': 70}
            response = client.post('/api/evidence/classify/bulk', json=
                bulk_request, headers=authenticated_headers)
            assert response.status_code == HTTP_OK
            assert_api_response_security(response)
            response_data = response.json()
    ...
    def test_get_control_mapping_suggestions(self, client,
        ...
        with patch(
            ...
            mock_ai.return_value = {'suggested_type': 'policy_document',
                'confidence': 85, 'suggested_controls': ['A.5.1.1', 'CC6.1',
                'Art. 32'], 'reasoning': 'Multi-framework control mapping'}
            mapping_request = {'evidence_id': evidence_id, 'frameworks': [
                'ISO27001', 'SOC2', 'GDPR']}
            response = client.post(
                f'/api/evidence/{evidence_id}/control-mapping', json=
                mapping_request, headers=authenticated_headers)
            assert response.status_code == HTTP_OK
            assert_api_response_security(response)
            response_data = response.json()
            assert response_data['evidence_id'] == evidence_id
    ...
    def test_get_classification_statistics(self, client,
        ...
        response = client.get('/api/evidence/classification/stats', headers
        ...
        assert (response_data['classified_evidence'] + response_data[
            'unclassified_evidence'] == response_data['total_evidence'],)
    ...
    def test_classify_nonexistent_evidence(self, client, authenticated_headers
        ...
        classify_request = {'evidence_id': fake_evidence_id,
            'force_reclassify': False}
        response = client.post(f'/api/evidence/{fake_evidence_id}/classify',
            json=classify_request, headers=authenticated_headers)
    ...
    def test_bulk_classify_with_invalid_evidence(self, client,
        ...
        fake_evidence_id = str(uuid4())
        bulk_request = {'evidence_ids': [fake_evidence_id],
            'force_reclassify': False, 'apply_high_confidence': True,
            'confidence_threshold': 70}
        response = client.post('/api/evidence/classify/bulk', json=
            bulk_request, headers=authenticated_headers)
...
class TestEvidenceClassificationValidation:
    ...
    def test_invalid_confidence_threshold(self, client, authenticated_headers):
        ...
        bulk_request = {'evidence_ids': [str(uuid4())],
    ...
    def test_classification_ai_service_error(self, client,
        ...
        evidence_data = {'evidence_name': 'Test Evidence', 'description':
            'Test description', 'evidence_type': 'unknown'}

...
class TestAIServiceIntegration:
    ...
    @pytest.mark.asyncio
    async def test_gemini_integration_with_circuit_breaker(self,
        ...
        with patch('services.ai.assistant.ComplianceAssistant._call_gemini_api') as mock_gemini:
            ...
            response = await async_client.post(f"/assessments/{assessment_id}/analyze",
                                             headers=auth_headers)
    ...
    @pytest.mark.asyncio
    async def test_ai_service_circuit_breaker_states(self,
                                                    async_client: httpx.AsyncClient,
                                                    auth_headers: Dict):
        """Test AI service circuit breaker state transitions"""
        
        assessment_data = {"name": "Circuit Breaker Test", "framework": "GDPR"}
        response = await async_client.post("/assessments", json=assessment_data, headers=auth_headers)
        assessment_id = response.json()["id"]
        ...
        with patch('services.ai.circuit_breaker.CircuitBreaker') as mock_cb_class:
            ...
            mock_cb.call = AsyncMock(return_value={"compliance_score": 0.8, "risk_level": "MEDIUM"})
        ...
        with patch('services.ai.assistant.ComplianceAssistant._call_gemini_api') as mock_gemini:
            ...
            for attempt in range(3):
                ...
                if attempt >= 2:
                    assert "fallback" in result.get("source", "").lower() or \
                           result.get("compliance_score") in [0.5, 0.0]  # Fallback scores
    ...
    @pytest.mark.asyncio
    async def test_multiple_ai_provider_fallback(self,
                                                async_client: httpx.AsyncClient,
                                                auth_headers: Dict):
...
class TestDatabaseIntegration:
    ...
    async def test_database_connection_pool_behavior(self,
        ...
        async def create_assessment(i: int):
            ...
            return await async_client.post("/assessments", 
                                         json=assessment_data,
                                         headers=auth_headers)
    ...
    async def test_database_transaction_consistency(self,
        ...
        response = await async_client.post("/evidence/upload",
                                         files=files,
                                         data=metadata,
                                         headers=auth_headers)
        assert response.status_code == 201
        ...
        response = await async_client.patch(f"/assessments/{assessment_id}",
                                          json=update_data,
                                          headers=auth_headers)
        assert response.status_code == 200
        ...
        response = await async_client.get(f"/evidence/{evidence_id}",
                                        headers=auth_headers)
        assert response.status_code == 200

    @pytest.mark.asyncio
    async def test_database_failover_behavior(self,
                                             async_client: httpx.AsyncClient,
                                             auth_headers: Dict):
...
class TestRedisIntegration:
    ...
    @pytest.mark.asyncio
    async def test_redis_caching_behavior(self,
                                         async_client: httpx.AsyncClient,
                                         auth_headers: Dict):
...
class TestEmailServiceIntegration:
    ...
    @pytest.mark.asyncio
    async def test_password_reset_email_integration(self,
                                                   async_client: httpx.AsyncClient):
        ...
        with patch('services.email_service.send_password_reset_email') as mock_email:
            ...
            if response.status_code == 200:
                mock_email.assert_called_once_with(registration_data["email"], 
                                                  reset_token=str)

    @pytest.mark.asyncio
    async def test_assessment_completion_notification(self,
                                                     async_client: httpx.AsyncClient,
                                                     auth_headers: Dict):
...
class TestFileStorageIntegration:
    ...
    @pytest.mark.asyncio
    async def test_file_upload_storage_integration(self,
                                                  async_client: httpx.AsyncClient,
                                                  auth_headers: Dict):
        """Test file upload and storage integration"""
        
        # Test file upload
        files = {"file": ("integration_test.pdf", b"Test PDF content", "application/pdf")}
        metadata = {
        ...
        response = await async_client.get(f"/evidence/{evidence_id}/download",
                                        headers=auth_headers)
    ...
    async def test_file_processing_integration(self,
        ...
        # Upload file
        files = {"file": ("process_test.pdf", b"Document content for processing", "application/pdf")}
        metadata = {
        ...
        with patch('services.document_processor.process_document') as mock_processor:
            ...
            response = await async_client.post(f"/evidence/{evidence_id}/process",
                                             headers=auth_headers)
...
class TestThirdPartyAPIIntegration:
    ...
    @pytest.mark.asyncio
    async def test_companies_house_api_integration(self,
                                                  async_client: httpx.AsyncClient,
                                                  auth_headers: Dict):
        ...
        with patch('services.companies_house.CompaniesHouseAPI.lookup_company') as mock_ch:
            ...
            response = await async_client.post("/integrations/companies-house/lookup",
                                             json=lookup_data,
                                             headers=auth_headers)
    ...
    async def test_external_api_timeout_handling(self,
        ...
        with patch('httpx.AsyncClient.get') as mock_get:
            ...
            response = await async_client.get("/integrations/health-check",
                                            headers=auth_headers)
    ...
    @pytest.mark.asyncio
    async def test_external_api_rate_limiting_respect(self,
                                                     async_client: httpx.AsyncClient,
                                                     auth_headers: Dict):
        ...
        with patch('services.external_integration.RateLimiter') as mock_limiter:
            ...
            responses = []
            for i in range(5):
                response = await async_client.post("/integrations/test-api",
            ...
            if any(code != 404 for code in status_codes):
                ...
                assert len(rate_limited_responses) > 0 or \
                       all(code in [200, 201, 503] for code in status_codes if code != 404)
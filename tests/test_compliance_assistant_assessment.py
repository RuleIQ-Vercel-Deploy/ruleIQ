...
def mock_context_manager():
    ...
    context_manager.get_conversation_context.return_value = {'business_profile'
...
def mock_prompt_templates():
    ...
    templates.get_assessment_help_prompt.return_value = {'system':
        'You are a compliance expert.', 'user': 'Help with this question.'}
    templates.get_assessment_followup_prompt.return_value = {'system':
        'Generate follow-up questions.', 'user': 'Based on these answers.'}
    templates.get_assessment_analysis_prompt.return_value = {'system':
        'Analyze assessment results.', 'user': 'Analyze these results.'}
    templates.get_assessment_recommendations_prompt.return_value = {'system':
...
class TestAssessmentHelp:
    ...
    async def test_get_assessment_help_success(self, compliance_assistant):
        ...
        with patch.object(compliance_assistant, '_generate_ai_response'
            ) as mock_ai:
            mock_ai.return_value = (
                '{"guidance": "Test guidance", "confidence_score": 0.9}',)
            result = await compliance_assistant.get_assessment_help(question_id
                ='q1', question_text='What is GDPR?', framework_id='gdpr',
                business_profile_id=uuid4(), section_id='data_protection',
...
class TestAssessmentFollowup:
    ...
    @pytest.mark.asyncio
    async def test_generate_assessment_followup_success(self,
        compliance_assistant):
        """Test successful followup generation."""
        current_answers = {'company_size': '50-100', 'industry':
            'technology', 'data_types': ['personal_data', 'financial_data']}
        with patch.object(compliance_assistant, '_generate_ai_response'
            ) as mock_ai:
            mock_ai.return_value = (
                '{"follow_up_questions": ["Question 1", "Question 2"]}',)
            result = await compliance_assistant.generate_assessment_followup(
                current_answers=current_answers, framework_id='gdpr',
                business_profile_id=uuid4(), assessment_context={'progress':
                50})
            assert result is not None
...
class TestAssessmentAnalysis:
    ...
    @pytest.mark.asyncio
    async def test_analyze_assessment_results_success(self,
        compliance_assistant):
        """Test successful assessment analysis."""
        assessment_results = {'gdpr_compliance': 'partial',
            'data_protection_measures': 'basic', 'privacy_policies': 'missing'}
        with patch.object(compliance_assistant, '_generate_ai_response'
            ) as mock_ai:
            mock_ai.return_value = '{"gaps": [], "recommendations": []}'
            result = await compliance_assistant.analyze_assessment_results(
                assessment_results=assessment_results, framework_id='gdpr',
                business_profile_id=uuid4())
            assert result is not None
...
class TestAssessmentRecommendations:
    ...
    @pytest.mark.asyncio
    async def test_get_assessment_recommendations_success(self,
        compliance_assistant):
        """Test successful recommendations generation."""
        gaps = [{'id': 'gap1', 'title': 'Missing privacy policy',
            'severity': 'high'}]
        business_profile = {'name': 'Test Company', 'industry':
            'technology', 'employee_count': 50}
        with patch.object(compliance_assistant, '_generate_ai_response'
            ) as mock_ai:
            mock_ai.return_value = (
                '{"recommendations": [], "implementation_plan": {}}')
            result = await compliance_assistant.get_assessment_recommendations(
                gaps=gaps, business_profile=business_profile, framework_id=
                'gdpr', existing_policies=['security_policy'],
                industry_context='technology', timeline_preferences='standard')
            assert result is not None
            assert 'recommendations' in result
...
class TestIntentClassification:
    ...
    def test_classify_assessment_intent(self, compliance_assistant):
        ...
        result = compliance_assistant._classify_intent(
            'Can you help me with this question?', assessment_context={
            'in_assessment': True, 'framework_id': 'gdpr'})
        ...
        result = compliance_assistant._classify_intent(
            'Can you identify gaps in my compliance?', assessment_context={
            'in_assessment': True})
...
class TestEntityExtraction:
    ...
    def test_extract_assessment_entities(self, compliance_assistant):
        ...
        result = compliance_assistant._extract_entities(
            ...
            assessment_context={'in_assessment': True, 'framework_id':
            'gdpr', 'section_id': 'data_protection'})

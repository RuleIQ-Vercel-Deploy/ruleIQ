...
async def api_detailed_health_check() -> Any:
    ...
    try:
        ...
        health_data = {'status': status, 'message': message, 'timestamp': datetime.now(timezone.utc).isoformat(), 'version': '2.0.0', 'api_version': 'v1', 'database': {'engine_initialized': engine_info.get('async_engine_initialized', False), 'recent_alerts': {'critical': critical_alerts, 'warning': warning_alerts, 'total': len(alerts)}}, 'secrets_vault': {'status': vault_health.get('status', 'unknown'), 'enabled': vault_health.get('enabled', False), 'vault_type': vault_health.get('vault_type', 'None'), 'region': vault_health.get('region', settings.secrets_vault_region), 'message': vault_health.get('message', 'No vault information available')}}
        return HealthCheckResponse(**health_data)
    except Exception as e:
        logger.error(f'API v1 detailed health check failed: {e}')
        return HealthCheckResponse(status='error', message=f'API v1 health check failed: {e!s}', timestamp=datetime.now(timezone.utc).isoformat(), version='2.0.0')

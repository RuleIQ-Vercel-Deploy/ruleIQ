...
function ShaderBackground() {
  ...
  const [webglOk, setWebglOk] = useState(true); // fallback if context fails

  // Cheap WebGL safety check to avoid crashing flaky GPUs
  useEffect(() => {
    try {
      const c = document.createElement('canvas');
      const gl =
        c.getContext('webgl2', { powerPreference: 'high-performance', antialias: false }) ||
  ...
  return (
    ...
    >
      {webglOk ? (
        <Canvas
          camera={camera}
          dpr={[1, 1.5]}  // cap pixel density for stability on Renoir
          frameloop="always"  // keep animation running as-is
          gl={{
            powerPreference: 'high-performance',  // optimized for APU
            antialias: false,  // big win for stability on Renoir
            alpha: false,
            stencil: false,
            depth: true,
          ...
          onCreated={({ gl }) => {
            ...
            gl.domElement.addEventListener('webglcontextlost', handleLoss, { passive: false });
          }}
        >
          <ShaderPlane />
        </Canvas>
      ) : (
        // Fallback if WebGL unavailable or lost
        <div className="w-full h-full bg-black" />
      )}

      {/* Overlays above canvas but below content */}
      <div className="pointer-events-none absolute inset-0 z-30 bg-gradient-to-t from-black/30 via-transparent to-black/20" />
    </div>
  );
}
...
export default function Hero({
  ...
  // Prevent SplitText double-initialization in React dev StrictMode
  const splitInst = useRef<SplitText | null>(null);

  useGSAP(
    () => {
      if (!headerRef.current || splitInst.current) return;

      let tl: gsap.core.Timeline | null = null;

      document.fonts.ready.then(() => {
        // Create once
        splitInst.current = new SplitText(headerRef.current!, { type: 'lines', wordsClass: 'lines' });

        gsap.set(splitInst.current!.lines, {
          filter: 'blur(16px)',
          yPercent: 30,
          autoAlpha: 0,
          scale: 1.06,
      ...
      // Cleanup for StrictMode remounts
      return () => {
        if (tl) tl.kill();
        if (splitInst.current) {
          splitInst.current.revert();
          splitInst.current = null;
        }
      };
    },
    { scope: sectionRef }
  ...
  return (
    <section ref={sectionRef} className="relative h-screen w-screen overflow-hidden bg-black">
      ...
      <div className="relative z-40 mx-auto flex max-w-7xl flex-col items-start gap-6 px-6 pb-24 pt-36 sm:gap-8 sm:pt-44 md:px-10 lg:px-16">
        <div
          ref={badgeRef}
      ...
      {/* Bottom fade above canvas, below content */}
      <div className="pointer-events-none absolute inset-x-0 bottom-0 z-30 h-24 bg-gradient-to-t from-black/40 to-transparent" />
    </section>
  );
}

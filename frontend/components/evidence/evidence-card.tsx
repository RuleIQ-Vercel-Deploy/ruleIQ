...
export function EvidenceCard({ evidence, isSelected, onSelectionChange }: EvidenceCardProps) {
  ...
  return (
    <Card className="bg-card border-border hover:border-gold/40 hover:shadow-lg transition-all duration-200">
      ...
      <CardContent className="space-y-4">
        <div className="flex items-center justify-between">
          <Badge variant={status?.value === "approved" ? "default" : status?.value === "pending" ? "secondary" : "destructive"}>
            {status?.label}
          </Badge>

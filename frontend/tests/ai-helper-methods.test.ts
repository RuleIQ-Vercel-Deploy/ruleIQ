...
describe('AI Helper Methods Tests', () => {
      ...
      const extractPolicies = (currentAnswers: any) => {
        ...
        policyQuestions.forEach(policy => {
          if (currentAnswers[policy] === 'Yes') {
            result.existing_policies.push(policy.replace(/_/g, ' ').replace(/\b\w/g, (l: string) => l.toUpperCase()));
          } else if (currentAnswers[policy] === 'No') {
            result.gaps_identified.push(policy.replace(/_/g, ' ').replace(/\b\w/g, (l: string) => l.toUpperCase()));
          }
        });
        ...
        complianceMeasures.forEach(measure => {
          if (currentAnswers[measure] === 'Yes') {
            result.compliance_measures.push(measure.replace(/_/g, ' ').replace(/\b\w/g, (l: string) => l.toUpperCase()));
          } else if (currentAnswers[measure] === 'No') {
            result.gaps_identified.push(measure.replace(/_/g, ' ').replace(/\b\w/g, (l: string) => l.toUpperCase()));
          }
        });
  ...
  describe('Industry Context Determination', () => {
    test('should determine correct regulations for technology industry', () => {
      ...
      const healthContext = extractIndustryContext(testBusinessProfiles[1], { ...testAnswers, industry: 'healthcare' });
      ...
      const financeContext = extractIndustryContext(testBusinessProfiles[2], { ...testAnswers, industry: 'financial services' });

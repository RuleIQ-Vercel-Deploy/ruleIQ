...
vi.mock('@/components/ui/select', () => ({
  ...
  SelectTrigger: React.forwardRef(({ children, className, disabled, ...props }: any, ref: any) => {
    return React.createElement('button', {
      ...
      className: `flex h-10 w-full items-center justify-between rounded-md border border-input bg-background px-3 py-2 text-sm ${className || ''}`.trim(),
      disabled,
      ...props
    }, children);
  }),
  ...
  SelectContent: React.forwardRef(({ children, className, ...props }: any, ref: any) => {
    return React.createElement('div', {
      ...
      className: `relative z-50 max-h-96 min-w-[8rem] overflow-hidden rounded-md border bg-popover text-popover-foreground shadow-md ${className || ''}`.trim(),
      ...props
    }, children);
  }),
  
  SelectItem: React.forwardRef(({ children, value, className, disabled, ...props }: any, ref: any) => {
    return React.createElement('div', {
      ref,
      role: 'option',
      'data-testid': 'select-item',
      'data-value': value,
      className: `relative flex w-full cursor-default select-none items-center rounded-sm py-1.5 pl-8 pr-2 text-sm ${className || ''}`.trim(),
      'data-disabled': disabled ? 'true' : 'false',
      ...props
    }, children);
  }),

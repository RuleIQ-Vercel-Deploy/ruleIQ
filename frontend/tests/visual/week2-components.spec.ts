...
test.describe('Week 2 Components Visual Regression', () => {
  ...
  test.describe('Form Controls', () => {
    ...
    test('Select component states', async ({ page }) => {
      // Closed state
      await expect(page.locator('[data-testid="select-default"]')).toHaveScreenshot('select-closed.png');
      ...
      // Error state
      await expect(page.locator('[data-testid="select-error"]')).toHaveScreenshot('select-error.png');
    });

    test('Checkbox component states', async ({ page }) => {
      // Unchecked
      await expect(page.locator('[data-testid="checkbox-default"]')).toHaveScreenshot('checkbox-unchecked.png');
      
      // Checked
      await page.locator('[data-testid="checkbox-default"]').click();
      await expect(page.locator('[data-testid="checkbox-default"]')).toHaveScreenshot('checkbox-checked.png');
      
      // Focus state
      await page.locator('[data-testid="checkbox-focus"]').focus();
      await expect(page.locator('[data-testid="checkbox-focus"]')).toHaveScreenshot('checkbox-focused.png');
      
      // Error state
      await expect(page.locator('[data-testid="checkbox-error"]')).toHaveScreenshot('checkbox-error.png');
    });
  ...
  test.describe('Card Components', () => {
    test('Basic card variations', async ({ page }) => {
      ...
      // With all sections
      await expect(page.locator('[data-testid="card-complete"]')).toHaveScreenshot('card-complete.png');
    });

    test('Assessment card states', async ({ page }) => {
      // Not started
      await expect(page.locator('[data-testid="assessment-not-started"]')).toHaveScreenshot('assessment-card-not-started.png');
      
      // In progress
      await expect(page.locator('[data-testid="assessment-in-progress"]')).toHaveScreenshot('assessment-card-in-progress.png');
      
      // Completed
      await expect(page.locator('[data-testid="assessment-completed"]')).toHaveScreenshot('assessment-card-completed.png');
      
      // Expired
      await expect(page.locator('[data-testid="assessment-expired"]')).toHaveScreenshot('assessment-card-expired.png');
    });
  });
  ...
  test.describe('Table Component', () => {
    ...
    test('Responsive table - mobile view', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="table-scroll"]')).toHaveScreenshot('table-mobile-scroll.png');
      
      // Stacked layout table
      await expect(page.locator('[data-testid="table-stacked"]')).toHaveScreenshot('table-mobile-stacked.png');
    });
  });
  ...
  test.describe('Dark Mode Compatibility', () => {
    test('Components in dark mode', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="dark-form-controls"]')).toHaveScreenshot('dark-form-controls.png');

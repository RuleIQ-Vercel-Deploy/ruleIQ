...
export const companyInfoSchema = z.object({
  ...
  annual_revenue: z.enum(ANNUAL_REVENUE_RANGES)
    .optional()
...
export const complianceProfileSchema = z.object({
  ...
  processes_payments: z.boolean()
    .describe('Whether the company processes payment information'),
    
  stores_health_data: z.boolean()
    .describe('Whether the company stores health or medical data'),
...
export const technologyStackSchema = z.object({
  cloud_providers: z.array(z.enum([...CLOUD_PROVIDERS, 'Other']))
    .default([])
    .describe('Cloud service providers used'),
    
  saas_tools: z.array(z.enum([...SAAS_TOOLS, 'Other']))
    .default([])
    .describe('Software-as-a-Service tools used'),
    
  development_tools: z.array(z.enum([...DEVELOPMENT_TOOLS, 'Other']))
    .default([])
...
export const complianceGoalsSchema = z.object({
  existing_frameworks: z.array(z.enum([...COMPLIANCE_FRAMEWORKS, 'Other']))
    .default([])
    .describe('Compliance frameworks currently implemented'),
    
  planned_frameworks: z.array(z.enum([...COMPLIANCE_FRAMEWORKS, 'Other']))
    .default([])
    .describe('Compliance frameworks planned for implementation'),
    
  compliance_budget: z.enum(COMPLIANCE_BUDGET_RANGES)
    .optional()
    .describe('Budget allocated for compliance activities'),
    
  compliance_timeline: z.enum(COMPLIANCE_TIMELINE_OPTIONS)
    .optional()
...
export const businessProfileSchema = businessProfileBaseSchema.refine(
  (data) => {
    // Custom validation: If handles personal data, should consider GDPR
    if (data.handles_personal_data && 
  ...
  (data) => {
    ...
    if (data.provides_financial_services && 
...
export function validateCompleteProfile(
  data: any
): { success: boolean; errors?: z.ZodError; data?: BusinessProfileFormData } {
...
export const customValidationRules = {
  ...
  validateCompanySize: (employeeCount: number, annualRevenue?: string): string[] => {
    ...
    if (employeeCount > 1000 && annualRevenue === 'Under £1M') {
      warnings.push('Large employee count with low revenue may indicate non-profit or government organization');
    }

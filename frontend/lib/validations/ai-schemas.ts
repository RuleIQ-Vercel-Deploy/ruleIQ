...
export const MaturityLevelSchema = z.enum(["initial", "developing", "defined", "managed", "optimized"]);
...
export const IntentTypeSchema = z.enum(["evidence_query", "compliance_check", "guidance_request", "general_query", "assessment_help"]);
...
export function validateAndTransformAIResponse<T extends AIResponseSchemaType>(
  ...
  if (!structuredValidation.success) {
    return {
      success: false,
      errors: structuredValidation.errors?.flatMap(getValidationErrors) || ["Unknown validation error"]
    };
  }
  ...
  if (!payloadValidation.success) {
    return {
      success: false,
      errors: payloadValidation.errors?.flatMap(getValidationErrors) || ["Payload validation failed"]
    };
  }

  // Check for validation warnings from metadata
  const warnings = structuredData.metadata.validation_errors?.length > 0 

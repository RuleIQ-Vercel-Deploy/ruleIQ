...
export class BusinessProfileFieldMapper {
  ...
  static fromAPI(data: any): BusinessProfile | null {
    ...
    Object.entries(data).forEach(([key, value]) => {
      ...
      const frontendKey = Object.entries(this.fieldMap)
        .find(([_, apiKey]) => apiKey === key)?.[0] || key;
  ...
  static getFrontendField(apiField: string): string {
    const entry = Object.entries(this.fieldMap)
      .find(([_, api]) => api === apiField);
  ...
  static validateMappings(): { isValid: boolean; missingMappings: string[] } {
    const requiredBackendFields = [
      'handles_persona', 'processes_payme', 'stores_health_d', 
      'provides_financ', 'operates_critic', 'has_internation',
      'existing_framew', 'planned_framewo', 'development_too',
      'compliance_budg', 'compliance_time'
    ];
  ...
  static transformFormDataForAPI(formData: any): any {
    ...
    const arrayFields = ['cloud_providers', 'saas_tools', 'development_too', 'existing_framew', 'planned_framewo'];
    ...
    const booleanFields = ['handles_persona', 'processes_payme', 'stores_health_d', 'provides_financ', 'operates_critic', 'has_internation'];
  ...
  static transformAPIResponseForFrontend(apiData: any): BusinessProfile | null {
    ...
    const arrayFields: (keyof BusinessProfile)[] = [
      'cloud_providers', 'saas_tools', 'development_tools', 
      'existing_frameworks', 'planned_frameworks'
    ];
    ...
    const booleanFields: (keyof BusinessProfile)[] = [
      'handles_personal_data', 'processes_payments', 'stores_health_data',
  ...
  static createUpdatePayload(
    ...
    updated: Partial<BusinessProfile>
  ): any {
    const changes: any = {};
...
export type APIField = typeof BusinessProfileFieldMapper.fieldMap[keyof typeof BusinessProfileFieldMapper.fieldMap] | string;

    ...
    (set, get) => ({
      ...
      submitAnswer: async (answerData: AssessmentAnswerRequest) => {
        try {
          ...
          const sessionProgress = await freemiumService.getSessionProgress(
            answerData.session_token,
          );
          ...
          const updatedProgress: AssessmentProgress = {
            ...
            time_elapsed_seconds:
              get().progress.time_elapsed_seconds + (answerData.time_spent_seconds || 0),
          };
      ...
      addResponse: (questionId: string, answer: string) => {
        ...
        if (Array.isArray(currentResponses)) {
          ...
          const updatedResponses = [
            ...currentResponses.filter((r) => r.question_id !== questionId),
            newResponse,
          ];
...
export const createFreemiumStore = () => {
  return create<FreemiumStore & FreemiumStoreComputed>()(
    persist(
      (set, get) => ({
        ...initialState,

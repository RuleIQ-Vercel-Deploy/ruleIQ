...
class AuthorizationException(RuleIQException):
    ...
    def __init__(self, message: str = "Access denied") -> None:
        super().__init__(message=message, error_code="AUTHORIZATION_ERROR", status_code=status.HTTP_403_FORBIDDEN)
...
class SecurityException(RuleIQException):
    ...
    def __init__(self, message: str, details: Optional[Dict[str, Any]] = None) -> None:
        super().__init__(
            message=message, error_code="SECURITY_ERROR", status_code=status.HTTP_403_FORBIDDEN, details=details
        )
...
class ErrorHandler:
    ...
    @staticmethod
    def create_error_response(exception: Exception, request: Optional[Request] = None) -> Dict[str, Any]:
    ...
    @staticmethod
    def log_error(exception: Exception, request: Optional[Request] = None, user_id: Optional[str] = None) -> None:
...
async def global_exception_handler(request: Request, exception: Exception) -> JSONResponse:
    ...
    if "details" in error_response["error"]:
        error_response["error"]["details"] = ErrorHandler.sanitize_error_details(error_response["error"]["details"])
...
class ValidationUtils:
    ...
    def validate_uuid(uuid_str: str, field_name: str = "id") -> str:
        ...
        try:
            ...
            raise ValidationException(f"Invalid {field_name} format", details={"field": field_name, "value": uuid_str})

    @staticmethod
    def validate_required_fields(data: Dict[str, Any], required_fields: List[str]) -> None:
        """Validate that required fields are present."""
        missing_fields = [
            field for field in required_fields if field not in data or data[field] is None or data[field] == ""
        ]
        if missing_fields:
            raise ValidationException("Missing required fields", details={"missing_fields": missing_fields})
...
class SecurityErrorHandler:
    ...
    def handle_authentication_error(details: Optional[Dict[str, Any]] = None) -> NoReturn:
        ...
        logger.warning(
            "Authentication attempt failed", extra={"details": ErrorHandler.sanitize_error_details(details or {})}
        )
    ...
    def handle_authorization_error(resource: str, action: str, user_id: Optional[str] = None) -> NoReturn:
        ...
        logger.warning(
            "Authorization attempt failed", extra={"resource": resource, "action": action, "user_id": user_id}
        )
...
def create_error_template(
    ...
    return RuleIQException(message=message, error_code=code, status_code=status_code, details=details)

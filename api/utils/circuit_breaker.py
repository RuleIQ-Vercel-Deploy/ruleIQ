...
class CircuitBreakerOpenException(APIError):
    ...
    def __init__(self, service_name: str, failure_count: int, recovery_time: Optional[float] = None) -> None:
        message = (f"Circuit breaker open for {service_name} (failures: {failure_count})",)
        if recovery_time:
            message += f", recovery in {recovery_time:.1f}s"
        super().__init__(message=message, status_code=503)
        self.details = {"service_name": service_name, "failure_count": failure_count, "recovery_time": recovery_time}
        ...
        async with self._lock:
            ...
            if self._failure_count >= self.config.failure_threshold:
                if self._state != CircuitBreakerState.OPEN:
                    ...
                    logger.error("Circuit breaker %s opened after %s failures" % (self.name, self._failure_count))

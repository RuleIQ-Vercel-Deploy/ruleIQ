...
async def execute_foundation_evidence_collection(
    ...
    async for db in get_async_db():
        try:
                ...
                try:
                    ...
                    for evidence_type in provider_evidence_types:
                        try:
                            ...
                            progress = min(100, int(len(completed_types) / total_evidence_types * 100))
                            await evidence_service.update_collection_status(
                                collection_id=collection_id, status="running", progress_percentage=progress
                            )
                            logger.info("Completed %s collection from %s" % (evidence_type, provider))
                        except Exception as e:
                            logger.error("Failed to collect %s from %s: %s" % (evidence_type, provider, e))
                            continue
                    await client.close()
                except Exception as e:
            ...
            try:
                await evidence_service.update_collection_status(
                    collection_id=collection_id, status="failed", current_activity="Collection failed", errors=[str(e)]
                )

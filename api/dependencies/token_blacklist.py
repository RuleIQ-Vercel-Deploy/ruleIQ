...
class EnhancedTokenBlacklist:
    ...
    async def blacklist_token(
        ...
        try:
            ...
            logger.info("Token blacklisted: hash=%s..., reason=%s, user_id=%s" % (token_hash[:8], reason, user_id))
            return True
    ...
    async def blacklist_user_tokens(
        self, user_id: str, reason: str = "security_action", exclude_current_token: Optional[str] = None
    ) -> int:
        """
        ...
        try:
            logger.warning(
                "Mass token blacklist requested for user %s - requires token registry implementation" % user_id
            )

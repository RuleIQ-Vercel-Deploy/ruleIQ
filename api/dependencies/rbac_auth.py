...
from api.dependencies.auth import get_current_user as base_get_current_user
from api.dependencies.auth import (
    oauth2_scheme,
)
...
class UserWithRoles:
    ...
    def __init__(
        self, user: User, roles: List[Dict], permissions: List[str], accessible_frameworks: List[Dict]
    ) -> None:
        self.user = user
        self.roles = roles
...
def create_access_token_with_roles(user_id: UUID, roles: List[Dict], permissions: List[str]) -> str:
    ...
    """
    role_claims = {"roles": [role["name"] for role in roles], "permissions": permissions, "role_details": roles}
    token_data = {
        "sub": str(user_id),
    ...
    try:
        ...
        try:
            ...
            if token_roles != current_roles:
                logger.warning(
                    "Token roles mismatch for user %s: token=%s, db=%s" % (user.id, token_roles, current_roles)
                )
...
def require_role(role: str) -> Any:
    ...
    async def check_role(current_user: UserWithRoles = Depends(get_current_active_user_with_roles)) -> UserWithRoles:
...
def require_any_role(roles: List[str]) -> Any:
    ...
    async def check_roles(current_user: UserWithRoles = Depends(get_current_active_user_with_roles)) -> UserWithRoles:
        if not current_user.has_any_role(roles):
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN, detail=f"One of these roles required: {', '.join(roles)}"
            )
        return current_user
...
def require_framework_access_level(framework_id: str, access_level: str = "read") -> Any:
    ...
    async def check_access(current_user: UserWithRoles = Depends(get_current_active_user_with_roles)) -> UserWithRoles:

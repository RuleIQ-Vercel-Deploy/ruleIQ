...
def validate_file_content(file_content: bytes, content_type: str, filename: str = "") -> bool:
    ...
    try:
        ...
        if content_type != detected_type:
            logger.warning("Content type mismatch: declared=%s, detected=%s" % (content_type, detected_type))
            return False
    except OSError as e:
...
def check_for_malicious_content(file_content: bytes, filename: str) -> bool:
    ...
    if filename.endswith((".txt", ".csv", ".json", ".xml")):
        try:
            ...
            suspicious_patterns = ["<script[^>]*>", "javascript:", "on\\w+\\s*=", "eval\\s*\\(", "exec\\s*\\("]
...
def analyze_file_comprehensively(file_content: bytes, filename: str, content_type: str) -> FileAnalysisReport:
    ...
    if file_size > max_size:
        ...
        security_score += 0.3
    if content_type != detected_type:
        threats_detected.append(f"MIME type mismatch: declared={content_type}, detected={detected_type}")
        security_score += 0.4
    _, ext = os.path.splitext(filename.lower())
    if ext in DANGEROUS_EXTENSIONS:
    ...
    async def validator(file: UploadFile = File(...)) -> Tuple[UploadFile, FileAnalysisReport]:
        ...
        for pattern in DANGEROUS_PATTERNS:
            if re.search(pattern, file.filename):
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid filename contains dangerous patterns"
                )
        content = await file.read()
        await file.seek(0)
        analysis_report = analyze_file_comprehensively(content, file.filename, file.content_type)
        if security_level == "strict":
            if analysis_report.validation_result in [ValidationResult.SUSPICIOUS, ValidationResult.MALICIOUS]:
                raise HTTPException(
        ...
        if enable_content_validation:
            if not validate_file_content(content, file.content_type, file.filename):
                raise HTTPException(
                    status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail="File content does not match declared type"
                )
        ...
        if analysis_report.threats_detected:
            logger.warning("Threats detected in %s: %s" % (analysis_report.filename, analysis_report.threats_detected))
...
def quarantine_file(file_content: bytes, analysis_report: FileAnalysisReport) -> str:
    ...
    logger.warning("File quarantined: %s - %s" % (quarantine_filename, analysis_report.threats_detected))
...
class EnhancedFileValidator:
    ...
    async def validate_and_analyze(self, file: UploadFile) -> Tuple[UploadFile, FileAnalysisReport, Optional[str]]:
        ...
        if self.security_level == "strict":
            if analysis_report.validation_result in [ValidationResult.SUSPICIOUS, ValidationResult.MALICIOUS]:
                ...
                raise HTTPException(
                    status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail="File rejected - Security risk detected"
                )

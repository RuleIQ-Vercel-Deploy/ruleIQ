...
DANGEROUS_EXTENSIONS = [
    ...
    ".js", ".vbs", ".jar", ".app", ".dmg", ".deb", ".rpm",
    ".sh", ".bash", ".ps1", ".psm1", ".msi", ".cab"
]
...
def get_file_validator(
    ...
    async def validator(file: UploadFile = File(...)) -> Tuple[UploadFile, FileAnalysisReport]:
        ...
        if analysis_report.threats_detected:
            logger.warning(f"Threats detected in {analysis_report.filename}: {analysis_report.threats_detected}")
    ...
    async def validate_and_analyze(self, file: UploadFile) -> Tuple[UploadFile, FileAnalysisReport, Optional[str]]:
        ...
        if self.security_level == "strict":
            if analysis_report.validation_result in [ValidationResult.SUSPICIOUS, ValidationResult.MALICIOUS]:
                ...
                raise HTTPException(
                    status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
                    detail=f"File rejected - Security risk detected",
                )
            ...
            if analysis_report.validation_result == ValidationResult.MALICIOUS:
                ...
                raise HTTPException(
                    status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
                    detail=f"File rejected - Malicious content detected",
                )

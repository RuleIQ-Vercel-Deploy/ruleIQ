...
async def get_policy(
    policy_id: UUID, current_user: User = Depends(get_current_active_user), db: AsyncSession = Depends(get_async_db)
) -> Any:
...
async def update_policy_status(
    ...
    return {"id": policy.id, "status": policy.status, "approved": status_update.get("approved", False)}


@router.put("/{id}/approve")
async def approve_policy(policy_id: UUID, current_user: User = Depends(get_current_active_user)) -> Dict[str, Any]:

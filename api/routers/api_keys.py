...
from api.dependencies.auth import get_api_key_auth, get_current_active_user
from database.db_setup import get_db
from database.redis_client import get_redis_client
from database.user import User
from services.api_key_management import APIKeyManager, APIKeyStatus, APIKeyType
...
async def create_api_key(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to create API key: {str(e)}"
        )
...
async def rotate_api_key(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to rotate API key: {str(e)}"
        )

...
async def get_usage_dashboard(
    current_user: User = Depends(get_current_active_user), db: AsyncSession = Depends(get_async_db), days: int = 7
) -> Any:
    """
...
async def get_feature_usage(
    feature: str, current_user: User = Depends(get_current_active_user), db: AsyncSession = Depends(get_async_db)
) -> Any:
    """
...
async def reset_demo_usage(
    ...
    stmt = select(AuditLog).where(
        and_(AuditLog.user_id == current_user.id, AuditLog.action.like("rate_limit:%"), AuditLog.timestamp >= cutoff)
    )
    result = await db.execute(stmt)

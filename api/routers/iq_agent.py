...
from services.iq_agent import IQComplianceAgent, create_iq_agent
...
async def get_iq_agent() -> IQComplianceAgent:
    ...
    if _iq_agent is None:
        try:
            ...
            raise HTTPException(
                status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail=f"IQ Agent initialization failed: {str(e)}"
            )
    return _iq_agent
...
async def get_neo4j_service() -> Neo4jGraphRAGService:
    ...
    if _neo4j_service is None:
        try:
            ...
            raise HTTPException(
                status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail=f"Neo4j service initialization failed: {str(e)}"
            )
...
async def query_compliance_analysis(
    ...
    """
    try:
        logger.info("Processing compliance query from user %s: %s..." % (current_user.id, request.query[:100]))
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Internal server error during compliance analysis"
        )
...
async def store_compliance_memory(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to store memory: {str(e)}"
        )
...
async def retrieve_compliance_memories(
    ...
    try:
        ...
        return MemoryRetrievalResponseWrapper(
            success=True, data=result, message=f"Retrieved {len(result.retrieved_memories)} relevant memories"
        )
    except Exception as e:
        logger.error("Error retrieving compliance memories: %s" % str(e))
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to retrieve memories: {str(e)}"
        )
...
async def initialize_compliance_graph_endpoint(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to initialize graph: {str(e)}"
        )
...
async def iq_agent_health_check(
    include_stats: bool = Query(default=True, description="Include detailed statistics in response")
) -> IQHealthCheckResponse:
    """
    ...
    try:
        ...
        try:
            ...
            if include_stats:
                memory_stats = {
                    ...
                    "memory_types": list(
                        set(memory.memory_type.value for memory in iq_agent.memory_manager.memory_store.values())
                    ),
                }
        except Exception as e:
        ...
        return IQHealthCheckResponse(success=True, data=health_response, message=f"IQ Agent status: {overall_status}")
    except Exception as e:
        logger.error("Health check error: %s" % str(e))
        return IQHealthCheckResponse(
            success=False,
            data=HealthCheckResponse(status="error", neo4j_connected=False, graph_statistics={}, memory_statistics={}),
...
async def _log_query_metrics(user_id: UUID, query: str, response_status: str, nodes_traversed: int) -> None:
...
async def _initialize_graph_background(clear_existing: bool = False, load_sample_data: bool = True) -> None:

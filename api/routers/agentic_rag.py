...
from services.agentic_rag import AgenticRAGResponse, AgenticRAGSystem
...
async def find_code_examples(
    ...
    try:
        if not rag_system.use_agentic_rag:
            raise HTTPException(
                status_code=HTTP_SERVICE_UNAVAILABLE, detail="Code example search requires agentic RAG to be enabled"
            )
        example_query = f"Show me code examples for: {request.topic}"
        result = await rag_system.query_documentation(
            query=example_query, source_filter=request.framework, query_type="code_examples", max_results=5
        )
        ...
        raise HTTPException(status_code=HTTP_INTERNAL_SERVER_ERROR, detail=f"Failed to find examples: {str(e)}")
...
async def fact_check_response(
    ...
    try:
        ...
        raise HTTPException(status_code=HTTP_INTERNAL_SERVER_ERROR, detail=f"Fact-checking failed: {str(e)}")
...
async def query_with_validation(
    ...
    try:
        ...
        raise HTTPException(status_code=HTTP_INTERNAL_SERVER_ERROR, detail=f"Validated query failed: {str(e)}")

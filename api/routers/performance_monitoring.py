...
async def get_performance_overview(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to get performance overview"
        )
...
async def get_database_performance(
    ...
    try:
        ...
        if (
            db_metrics.avg_query_time < 0.05
            and db_metrics.connection_pool_utilization < 0.7
        ):
            rating = "excellent"
        elif (
            db_metrics.avg_query_time < 0.1
            and db_metrics.connection_pool_utilization < 0.8
        ):
            rating = "good"
        elif (
            db_metrics.avg_query_time < 0.2
            and db_metrics.connection_pool_utilization < 0.9
        ):
            rating = "warning"
        else:
            rating = "critical"
...
async def get_cache_performance(
    ...
    try:
        ...
        if cache_metrics.hit_rate >= 0.9:
            rating = "excellent"
        elif cache_metrics.hit_rate >= 0.8:
            rating = "good"
...
async def get_api_performance(
    ...
    try:
        ...
        if api_metrics.avg_response_time < 0.1 and api_metrics.p95_response_time < 0.5:
            ...
            api_metrics.avg_response_time < 0.2 and api_metrics.p95_response_time < 1.0
        ):
            rating = "good"
        elif (
            api_metrics.avg_response_time < 0.5 and api_metrics.p95_response_time < 2.0
        ):
            rating = "warning"
        else:
            rating = "critical"
...
async def get_system_metrics(
    ...
    try:
        ...
        if cpu_ok and memory_ok and disk_ok:
            ...
            system_metrics["cpu_percent"] > 90 or system_metrics["memory_percent"] > 90
        ):
            status = "critical"
        else:
            status = "warning"
...
async def get_performance_trends(
    hours: int = Query(
        24, ge=1, le=168, description="Number of hours to analyze (1-168)"
    ),
    current_user: User = Depends(get_current_active_user),
...
async def configure_performance_alerts(
    ...
    try:
        ...
        await cache.set(
            "performance_alerts_config", alerts_config, ttl=86400
        )  # 24 hours
...
async def start_performance_monitoring(
    interval: int = Query(
        60, ge=10, le=300, description="Monitoring interval in seconds"
    ),
    current_user: User = Depends(get_current_active_user),
):
    """

...
async def get_business_profile_by_id(
    ...
    await audit_service.log_data_access(user_id=str(current_user.id),
        resource='business_profile', resource_id=str(id), action='read', 
        db=sync_db)
    return profile
...
async def update_business_profile_by_id(
    ...
    audit_service = AuditLogger(sync_db)
    await audit_service.log_data_access(user_id=str(current_user.id),
        resource='business_profile', resource_id=str(profile_id), action='update', 
...
async def delete_business_profile_by_id(
    ...
    audit_service = AuditLogger(sync_db)
    await audit_service.log_data_access(user_id=str(current_user.id),
        resource='business_profile', resource_id=str(profile_id), action='delete', 
        db=sync_db)
...
async def get_profile_compliance_status(
    ...
    audit_service = AuditLogger(sync_db)
    await audit_service.log_data_access(user_id=str(current_user.id),
        resource='compliance_status', resource_id=str(profile_id), action='read', 
        db=sync_db)
    return {
...
async def invite_team_member(
    ...
    audit_service = AuditLogger(sync_db)
    await audit_service.log_data_access(user_id=str(current_user.id),
        resource='team_invite', resource_id=str(profile_id), action='create', 
...
async def get_profile_activity(
    ...
    return {
        'profile_id': str(profile_id), 
        'activities': [
            {'activity_id': 'act_001', 'type': 'profile_updated', 'description': 'Business profile information updated', 'user': current_user.email, 'timestamp': '2024-01-15T14:30:00Z'}, 
            {'activity_id': 'act_002', 'type': 'assessment_completed', 'description': 'GDPR compliance assessment completed', 'user': current_user.email, 'timestamp': '2024-01-14T10:00:00Z'}, 
            {'activity_id': 'act_003', 'type': 'evidence_uploaded', 'description': 'New evidence document uploaded', 'user': current_user.email, 'timestamp': '2024-01-13T16:45:00Z'}
        ], 
        'total': 3, 
        'limit': limit
    }

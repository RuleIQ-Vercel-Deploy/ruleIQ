...
async def configure_aws_integration(
    ...
    try:
        if config.auth_type == "access_key":
            ...
            credentials_dict = {"access_key_id": config.access_key_id, "secret_access_key": config.secret_access_key}
            auth_type = AuthType.API_KEY
        elif config.auth_type == "role_assumption":
            if not config.role_arn:
                raise HTTPException(
                    status_code=HTTP_BAD_REQUEST, detail="Role ARN is required for role_assumption auth type"
                )
            credentials_dict = {"role_arn": config.role_arn, "external_id": config.external_id}
            auth_type = AuthType.ROLE_ASSUMPTION
        else:
            raise HTTPException(
                status_code=HTTP_BAD_REQUEST, detail="Invalid auth_type. Must be 'access_key' or 'role_assumption'"
            )
        ...
        logger.error("Error configuring AWS integration for user %s: %s" % (str(current_user.id), e))
        raise HTTPException(status_code=HTTP_INTERNAL_SERVER_ERROR, detail=f"Configuration failed: {str(e)}")
...
async def configure_okta_integration(
    ...
    try:
        ...
        integration_config = await store_integration_config(
            user_id=str(current_user.id), provider="okta", credentials=credentials, health_info=health_check, db=db
        )
        ...
        logger.error("Error configuring Okta integration for user %s: %s" % (str(current_user.id), e))
        raise HTTPException(status_code=HTTP_INTERNAL_SERVER_ERROR, detail=f"Configuration failed: {str(e)}")
...
async def configure_google_workspace_integration(
    ...
    try:
        ...
        if not connection_test[0]:
            raise HTTPException(
                status_code=HTTP_BAD_REQUEST, detail=f"Google Workspace connection failed: {connection_test[1]}"
            )
        integration_config = await store_integration_config(
            user_id=str(current_user.id),
            provider="google_workspace",
        ...
        logger.error("Error configuring Google Workspace integration for user %s: %s" % (str(current_user.id), e))
        raise HTTPException(status_code=HTTP_INTERNAL_SERVER_ERROR, detail=f"Configuration failed: {str(e)}")
...
async def configure_microsoft_integration(
    ...
    try:
        ...
        if not connection_test[0]:
            raise HTTPException(
                status_code=HTTP_BAD_REQUEST, detail=f"Microsoft Graph connection failed: {connection_test[1]}"
            )
        integration_config = await store_integration_config(
            user_id=str(current_user.id),
            provider="microsoft_365",
        ...
        logger.error("Error configuring Microsoft 365 integration for user %s: %s" % (str(current_user.id), e))
        raise HTTPException(status_code=HTTP_INTERNAL_SERVER_ERROR, detail=f"Configuration failed: {str(e)}")
...
async def start_foundation_evidence_collection(
    ...
    try:
            ...
            for provider, integration in integration_map.items():
                if provider == "aws":
                    evidence_types.extend(
                        ["iam_policies", "iam_users", "iam_roles", "security_groups", "cloudtrail_logs"]
                    )
                elif provider == "okta":
                    evidence_types.extend(["users", "groups", "applications", "system_logs"])
                elif provider == "google_workspace":
                    evidence_types.extend(["user_directory", "access_groups", "admin_activity_logs"])
        ...
        logger.error("Error starting evidence collection for user %s: %s" % (str(current_user.id), e))
        raise HTTPException(status_code=HTTP_INTERNAL_SERVER_ERROR, detail=f"Failed to start collection: {str(e)}")


@router.get("/collect/{collection_id}/status", response_model=EvidenceCollectionStatus)
async def get_collection_status(
    collection_id: str, current_user: User = Depends(get_current_active_user), db: AsyncSession = Depends(get_async_db)
) -> Any:
    ...
    try:
        ...
        total_expected = len(collection.evidence_types_requested) if collection.evidence_types_requested else 1
        evidence_collected = len(collection.evidence_types_completed) if collection.evidence_types_completed else 0
        progress_percentage = min(100, int(evidence_collected / total_expected * 100)) if total_expected > 0 else 0
        ...
        raise HTTPException(status_code=HTTP_INTERNAL_SERVER_ERROR, detail=f"Failed to get status: {str(e)}")
...
async def get_collection_results(
    ...
    try:
        ...
        return {
            ...
            "evidence": [
                CollectedEvidence(
                    ...
                    quality_score=(
                        sum(item.quality_score.values()) / len(item.quality_score) if item.quality_score else 0.0
                    ),
        ...
        raise HTTPException(status_code=HTTP_INTERNAL_SERVER_ERROR, detail=f"Failed to get results: {str(e)}")
...
async def check_foundation_integrations_health(
    ...
    try:
        ...
        for integration in user_integrations:
            try:
                ...
                health_results.append(
                    {"integration_id": str(integration.id), "provider": integration.provider, **health_result}
                )
                await client.close()
            except Exception as e:
                health_results.append(
        ...
        raise HTTPException(status_code=HTTP_INTERNAL_SERVER_ERROR, detail=f"Health check failed: {str(e)}")

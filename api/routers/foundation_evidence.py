...
from database.services.integration_service import (
    IntegrationService, EvidenceCollectionService,
    store_integration_config, get_user_integrations, 
    get_integration_by_id, decrypt_integration_credentials,
...
async def configure_google_workspace_integration(
    ...
    try:
        ...
        if not connection_test[0]:
            raise HTTPException(status_code=400, detail=f"Google Workspace connection failed: {connection_test[1]}")
        ...
        logger.error(f"Error configuring Google Workspace integration for user {current_user.id}: {e}")
        raise HTTPException(status_code=500, detail=f"Configuration failed: {str(e)}")
...
async def configure_microsoft_integration(
    ...
    try:
        ...
        if not connection_test[0]:
            raise HTTPException(status_code=400, detail=f"Microsoft Graph connection failed: {connection_test[1]}")
    ...
    try:
        ...
        if not evidence_types:
            ...
            for provider, integration in integration_map.items():
                if provider == "aws":
                    evidence_types.extend(["iam_policies", "iam_users", "iam_roles", "security_groups", "cloudtrail_logs"])
                elif provider == "okta":
...
async def get_collection_status(
    ...
    try:
        ...
        return EvidenceCollectionStatus(
            ...
            quality_score=sum(collection.quality_score.values()) / len(collection.quality_score) if collection.quality_score else 0.0,
        ...
        return {
            ...
            "evidence": [
                CollectedEvidence(
                    ...
                    quality_score=sum(item.quality_score.values()) / len(item.quality_score) if item.quality_score else 0.0,

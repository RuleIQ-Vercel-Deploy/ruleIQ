...
async def role_based_access_control(
    resource: str = "default", action: str = "read", current_user: User = Depends(get_current_active_user)
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        raise HTTPException(status_code=HTTP_INTERNAL_SERVER_ERROR, detail=f"RBAC check failed: {str(e)}")
...
async def business_logic_vulnerabilities(
    ...
    try:
        ...
        if "admin" not in user_roles and "security_tester" not in user_roles:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN, detail="Insufficient privileges for security testing"
            )
        ...
        raise HTTPException(status_code=HTTP_INTERNAL_SERVER_ERROR, detail=f"Security test failed: {str(e)}")
...
async def security_status(current_user: User = Depends(get_current_active_user)) -> Dict[str, Any]:
    ...
    try:
        return {
            ...
            "recommendations": ["Regular security audits", "Keep dependencies updated", "Monitor access logs"],
        }
    except Exception as e:
        raise HTTPException(status_code=HTTP_INTERNAL_SERVER_ERROR, detail=f"Security status check failed: {str(e)}")
...
async def get_csp_violations(current_user: User = Depends(get_current_active_user)) -> Any:
    ...
    if current_user.role != "admin":
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Only administrators can view CSP violations")
    return csp_handler.get_violations_summary()


@router.delete("/csp-violations", response_model=Dict[str, str])
async def clear_csp_violations(current_user: User = Depends(get_current_active_user)) -> Dict[str, Any]:
    """
    ...
    if current_user.role != "admin":
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="Only administrators can clear CSP violations"
        )

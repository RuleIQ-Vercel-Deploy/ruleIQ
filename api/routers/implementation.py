...
async def create_plan(
    ...
    plan = await generate_implementation_plan(
        db, current_user, plan_data.framework_id, 
    ...
    plan_dict = {
        ...
        'business_profile_id': plan.business_profile_id, 'framework_id': plan.framework_id, 
        'title': plan.title, 'status': plan.status,
        'phases': plan.phases, 'planned_start_date': plan.planned_start_date, 
        'planned_end_date': plan.planned_end_date,
        'actual_start_date': plan.actual_start_date, 'actual_end_date': plan.actual_end_date, 
        'created_at': plan.created_at, 'updated_at': plan.updated_at, 
        'total_phases': total_phases, 'total_tasks': total_tasks, 
...
async def get_implementation_recommendations(
    ...
    return {
        'recommendations': [
            ...
            {
                'id': 'rec_002', 'priority': 'medium', 
                'category': 'security', 'title': 'Implement MFA',
                'description': 'Deploy multi-factor authentication for all user accounts',
                'estimated_effort_hours': 8, 'impact_score': 9, 
                'dependencies': ['user_management_system'], 
                'automation_available': False
            }, 
            {
                'id': 'rec_003', 'priority': 'low', 
                'category': 'documentation', 'title': 'Update Security Policies', 
...
async def get_implementation_resources(
    ...
    return {
        ...
        'resources': {
            'templates': [
                {
                    'id': 'tpl_001', 'name': 'Risk Assessment Template', 'type': 'document',
                    'format': 'docx', 'size_kb': 245, 
                    'download_url': f'/api/v1/resources/{frameworkId}/templates/risk-assessment.docx'
                },
                {
                    'id': 'tpl_002', 'name': 'Policy Templates Bundle', 'type': 'bundle', 
                    'format': 'zip', 'size_kb': 1024, 
                    'download_url': f'/api/v1/resources/{frameworkId}/templates/policies-bundle.zip'
                }
            ],
            'guides': [
                {
                    'id': 'guide_001', 'title': f'{frameworkId} Implementation Guide', 
                    'description': f'Step-by-step guide for implementing {frameworkId}', 
                    'pages': 45, 'format': 'pdf', 
                    'url': f'/api/v1/resources/{frameworkId}/guides/implementation-guide.pdf'
                },
                {
                    'id': 'guide_002', 'title': 'Quick Start Checklist', 
            ...
            'tools': [
                {
                    'id': 'tool_001', 'name': 'Gap Analysis Spreadsheet',
                    'description': 'Excel tool for conducting gap analysis', 'type': 'spreadsheet', 
                    'download_url': f'/api/v1/resources/{frameworkId}/tools/gap-analysis.xlsx'
                }
            ],
            'videos': [
                {
                    'id': 'vid_001', 'title': f'Understanding {frameworkId}', 
                    'duration_minutes': 15, 
...
async def get_implementation_plan_analytics(
    ...
    return {
        ...
        'analytics': {
            ...
            'effort': {
                'planned_hours': 240, 'actual_hours': 165, 'remaining_hours': 75, 'efficiency_ratio': 0.92
            }, 
            'risks': [
                {
                    'id': 'risk_001', 'category': 'resource',

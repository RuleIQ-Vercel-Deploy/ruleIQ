...
async def create_plan(
    ...
    total_tasks = (
        sum(len(phase.get("tasks", [])) for phase in plan.phases) if plan.phases else 0
    )
...
async def get_plan(
    ...
    estimated_duration_weeks = (
        sum(phase.get("duration_weeks", 0) for phase in plan.phases)
        if plan.phases
        else 12
    )
...
async def update_task(
    ...
    plan = await update_task_status(
        db, current_user, plan_id, task_id, task_update.status
    )
    if not plan:
        raise HTTPException(status_code=404, detail="Implementation plan not found")
    ...
    return {
        ...
        "analytics": {
            ...
            "timeline": {
                ...
                "planned_end": (datetime.utcnow() + timedelta(days=60)).isoformat(),
                "actual_start": (datetime.utcnow() - timedelta(days=30)).isoformat(),
                "projected_end": (datetime.utcnow() + timedelta(days=65)).isoformat(),

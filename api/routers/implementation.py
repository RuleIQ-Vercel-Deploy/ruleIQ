...
async def get_implementation_resources(
    frameworkId: str, current_user: User = Depends(get_current_active_user), db: AsyncSession = Depends(get_async_db)
) -> Dict[str, Any]:
...
async def get_implementation_plan_analytics(
    planId: UUID, current_user: User = Depends(get_current_active_user), db: AsyncSession = Depends(get_async_db)
) -> Dict[str, Any]:
    ...
    return {
        ...
        "analytics": {
            ...
            "effort": {"planned_hours": 240, "actual_hours": 165, "remaining_hours": 75, "efficiency_ratio": 0.92},
            "risks": [
                {
                    "id": "risk_001",
                    "category": "resource",
            ...
            "quality_metrics": {"tasks_passed_review": 22, "tasks_requiring_rework": 2, "quality_score": 91.7},
        },
        "generated_at": datetime.now(timezone.utc).isoformat(),
    }

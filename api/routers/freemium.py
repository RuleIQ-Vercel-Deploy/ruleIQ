...
async def capture_lead(
    ...
    try:
        ...
        await db.rollback()
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to capture lead information"
        )
...
async def start_assessment_session(
    ...
    try:
        ...
        await scoring_service.track_event(
            ...
            metadata={"business_type": session_data.business_type, "assessment_type": session_data.assessment_type},
        )
        ...
        await db.rollback()
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to start assessment session"
        )
...
async def get_session_progress(session_token: str, db: AsyncSession = Depends(get_async_db)) -> SessionResponse:
    """
    ...
    try:
        result = await db.execute(
            select(FreemiumAssessmentSession).where(FreemiumAssessmentSession.session_token == session_token)
        )
        session = result.scalar_one_or_none()
        if not session:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Assessment session not found")
        if session.is_expired():
            raise HTTPException(status_code=status.HTTP_410_GONE, detail="Assessment session has expired")
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to retrieve session progress"
        )
...
async def submit_assessment_answer(
    session_token: str, answer_data: AnswerSubmissionRequest, db: AsyncSession = Depends(get_async_db)
) -> dict:
    """
    ...
    try:
        ...
        score_impact = await assessment_service.calculate_answer_score_impact(
            question_id=answer_data.question_id, answer=answer_data.answer, confidence=answer_data.answer_confidence
        )
        scoring_service = LeadScoringService(db)
        await scoring_service.track_event(
        ...
        await db.rollback()
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to process assessment answer"
        )
...
async def get_assessment_results(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to generate assessment results"
        )


@router.get(
    "/health", summary="Freemium API health check", description="Check freemium API health and database connectivity"
)

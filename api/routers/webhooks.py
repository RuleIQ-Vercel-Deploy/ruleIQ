...
router = APIRouter(prefix="/api/v1/webhooks", tags=["Webhooks"], responses={(404): {"description": "Not found"}})
...
async def stripe_webhook(
    ...
    try:
        ...
        is_valid, error = await service.verify_webhook(request=request, provider="stripe", payload=payload)
        if not is_valid:
        ...
        await service.log_webhook_attempt(
            provider="stripe", endpoint="/webhooks/stripe", is_valid=True, metadata={"event_type": event_type}
        )
...
async def github_webhook(
    ...
    try:
        payload = await request.body()
        service = WebhookVerificationService(redis_client)
        is_valid, error = await service.verify_webhook(request=request, provider="github", payload=payload)
        if not is_valid:
        ...
        await service.log_webhook_attempt(
            provider="github", endpoint="/webhooks/github", is_valid=True, metadata={"event_type": event_type}
        )
        return {"received": True}
    except HTTPException:
        raise
...
async def sendgrid_webhook(
    ...
    try:
        payload = await request.body()
        service = WebhookVerificationService(redis_client)
        is_valid, error = await service.verify_webhook(request=request, provider="sendgrid", payload=payload)
        if not is_valid:
        ...
        await service.log_webhook_attempt(
            provider="sendgrid", endpoint="/webhooks/sendgrid", is_valid=True, metadata={"event_count": len(events)}
        )
        return {"received": True}
    except HTTPException:
        raise
...
async def test_generate_signature(
    ...
    if os.getenv("ENVIRONMENT", "production").lower() not in ["development", "test", "local"]:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="This endpoint is only available in development"
        )
    service = WebhookVerificationService()
    signature = await service.generate_webhook_signature(payload=payload, secret=secret, provider=provider)
    return {
        "provider": provider,
        "signature": signature,

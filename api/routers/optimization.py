...
async def model_selection(request: ModelSelectionRequest, current_user=Depends(get_current_user)) -> Dict[str, Any]:
    """Select the optimal AI model based on task requirements."""
    try:
        selected_model = ComplianceAssistant.select_optimal_model(
            task_type=request.task_type, complexity=request.complexity, prefer_speed=request.prefer_speed
        )
        return {

...
async def model_selection(
    ...
    try:
        ...
        raise HTTPException(status_code=HTTP_INTERNAL_SERVER_ERROR, detail=f"Model selection failed: {str(e)}")


@router.get("/model-health")
async def model_health_check(current_user: User = Depends(get_current_active_user)) -> Dict[str, Any]:
...
async def performance_metrics(current_user: User = Depends(get_current_active_user)) -> Dict[str, Any]:
...
async def model_fallback_chain(
    ...
    try:
        ...
        raise HTTPException(status_code=HTTP_INTERNAL_SERVER_ERROR, detail=f"Fallback chain failed: {str(e)}")
...
async def reset_circuit_breaker(
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to reset circuit breaker")
    ...
    try:
        ...
        if hasattr(cache_manager, "redis") and cache_manager.redis:
            try:
                ...
                pass
        return {"status": "success", "metrics": stats, "timestamp": datetime.now(timezone.utc).isoformat()}

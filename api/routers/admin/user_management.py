...
from database.rbac import AuditLog, Permission, Role, RolePermission, UserRole
...
async def get_user(
    user_id: UUID, current_user: UserWithRoles = Depends(require_permission("user_list")), db: Session = Depends(get_db)
) -> Any:
...
async def create_user(
    ...
    for role_name in request.roles:
        ...
        if role:
            try:
                rbac.assign_role_to_user(user_id=user.id, role_id=role.id, granted_by=current_user.id)
...
async def update_user(
    ...
    if request.email is not None:
        existing = db.query(User).filter(and_(User.email == request.email, User.id != user_id)).first()
        if existing:
...
async def delete_user(
    ...
    user.updated_at = datetime.now(timezone.utc)
    rbac = RBACService(db)
    user_roles = db.query(UserRole).filter(and_(UserRole.user_id == user_id, UserRole.is_active)).all()
    for user_role in user_roles:
        rbac.revoke_role_from_user(user_id=user_id, role_id=user_role.role_id, revoked_by=current_user.id)
    db.commit()
...
async def list_roles(
    current_user: UserWithRoles = Depends(require_permission("admin_roles")), db: Session = Depends(get_db)
) -> Dict[str, Any]:
    ...
    for role, user_count in roles_with_counts:
        ...
        permission_data = [
            {"id": str(perm.id), "name": perm.name, "display_name": perm.display_name, "category": perm.category}
...
async def get_admin_statistics(
    current_user: UserWithRoles = Depends(require_permission("admin_audit")), db: Session = Depends(get_db)
) -> Any:
...
async def initialize_rbac_system_endpoint(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to initialize RBAC system: {str(e)}"
        )
...
async def cleanup_expired_roles(
    ...
    return {"message": f"Cleaned up {expired_count} expired role assignments", "expired_count": expired_count}

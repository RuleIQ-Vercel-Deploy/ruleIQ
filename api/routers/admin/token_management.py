...
async def remove_token_from_blacklist(
    ...
    if not success:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Token not found in blacklist")
...
async def blacklist_user_tokens(
    ...
    count = await blacklist.blacklist_user_tokens(
        user_id=request.user_id, reason=request.reason, exclude_current_token=request.exclude_current_token
    )
    return {

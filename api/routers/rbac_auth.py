...
from api.dependencies.auth import ACCESS_TOKEN_EXPIRE_MINUTES, verify_password
...
from api.middleware.rate_limiter import RateLimited, auth_rate_limit
...
async def login_with_roles(form_data: OAuth2PasswordRequestForm = Depends(), db: Session = Depends(get_db)) -> Any:
    ...
    rbac._log_audit(
        ...
        details={"email": user.email, "roles": [role["name"] for role in roles], "permissions_count": len(permissions)},
    )
...
async def refresh_access_token(refresh_token: str, db: Session = Depends(get_db)) -> Any:
    ...
    return TokenResponse(
        access_token=access_token, refresh_token=new_refresh_token, expires_in=ACCESS_TOKEN_EXPIRE_MINUTES * 60
    )
...
async def assign_role_to_user(
    ...
    try:
        ...
        user_role = rbac.assign_role_to_user(
            user_id=assignment.user_id, role_id=assignment.role_id, granted_by=current_user.id, expires_at=expires_at
        )

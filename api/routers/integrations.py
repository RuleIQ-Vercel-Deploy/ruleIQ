...
AVAILABLE_PROVIDERS = {"google_workspace": {"name": "Google Workspace"}, "microsoft_365": {"name": "Microsoft 365"}}
...
async def connect_integration(
    ...
    try:
        ...
        if not encrypted_creds:
            ...
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Credential encryption failed."
            )
        stmt = select(Integration).where(Integration.user_id == str(current_user.id), Integration.provider == provider)
        ...
        if config:
            ...
            logger.info("Integration updated for user %s, provider %s" % (str(current_user.id), provider))
        else:
            config = Integration(
            ...
            logger.info("New integration created for user %s, provider %s" % (str(current_user.id), provider))
        await db.commit()
        ...
        await db.rollback()
        logger.error(
            "Database error connecting integration for user %s, provider %s: %s" % (str(current_user.id), provider, e),
            exc_info=True,
        )
        raise HTTPException(
        ...
        await db.rollback()
        logger.error(
            "Error connecting integration for user %s, provider %s: %s" % (str(current_user.id), provider, e),
            exc_info=True,
        )
...
async def disconnect_integration(
    provider: str, current_user: User = Depends(get_current_active_user), db: AsyncSession = Depends(get_async_db)
) -> None:
    ...
    try:
        ...
        if not config:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail=f"Integration '{provider}' not configured."
            )
        await db.delete(config)
        await db.commit()
        logger.info("Integration %s disconnected for user %s" % (provider_key, str(current_user.id)))
    except SQLAlchemyError as e:
        await db.rollback()
        logger.error(
            "Database error disconnecting %s for user %s: %s" % (provider_key, str(current_user.id), e), exc_info=True
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Database operation failed.",
...
async def get_integration_status(
    ...
    try:
        ...
        logger.error(
            "Database error fetching status for %s, user %s: %s" % (provider_key, str(current_user.id), e),
            exc_info=True,
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def test_integration(
    integrationId: str, current_user: User = Depends(get_current_active_user), db: AsyncSession = Depends(get_async_db)
) -> Dict[str, Any]:
...
async def get_sync_history(
    ...
    return {
        "integration_id": integrationId,
        "sync_history": [
            {"timestamp": "2024-01-15T10:00:00Z", "status": "success", "records_synced": 150, "duration_seconds": 45},
            {"timestamp": "2024-01-14T10:00:00Z", "status": "success", "records_synced": 148, "duration_seconds": 42},
        ],
        "total_syncs": 2,
    }
...
async def get_integration_logs(
    ...
    return {
        ...
        "logs": [
            ...
            {"timestamp": "2024-01-15T15:29:00Z", "level": "info", "message": "Sync started", "details": {}},
        ],
        "total_logs": 2,
    }
...
async def oauth_callback(
    callback_data: dict, current_user: User = Depends(get_current_active_user), db: AsyncSession = Depends(get_async_db)
) -> Dict[str, Any]:

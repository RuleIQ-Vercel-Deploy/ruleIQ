...
from services.ai.cost_management import AICostManager, BudgetAlertService, CostOptimizationService, CostTrackingService
...
class CostTrackingRequest(BaseModel):
    ...
    response_quality_score: Optional[float] = Field(None, ge=0, le=1, description="Response quality score (0-1)")
...
class CostAnalyticsResponse(BaseModel):
    ...
    service_breakdown: Dict[str, Dict[str, Any]] = Field(..., description="Cost breakdown by service")
...
async def track_ai_usage(request: CostTrackingRequest, current_user: User = Depends(get_current_active_user)) -> Any:
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to track usage: {str(e)}"
        )
...
async def get_cost_trends(
    ...
    try:
        ...
        return CostTrendsResponse(
            trends=trends, growth_rate=growth_rate, seasonal_patterns=seasonal_patterns, anomalies=anomalies
        )
    except Exception as e:
        logger.error("Failed to get cost trends: %s" % str(e))
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to retrieve trends: {str(e)}"
        )
...
async def configure_budget(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to configure budget: {str(e)}"
        )
...
async def get_budget_status() -> Any:
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to retrieve budget status: {str(e)}"
        )
...
async def get_budget_alerts() -> Any:
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to retrieve alerts: {str(e)}"
        )
...
async def get_optimization_recommendations() -> Any:
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to retrieve recommendations: {str(e)}"
        )
...
async def select_optimal_model(request: ModelRoutingRequest) -> Any:
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to select model: {str(e)}"
        )


@router.get(
    "/reports/monthly", dependencies=[Depends(get_current_active_user), Depends(RateLimited(requests=5, window=60))]
)
async def generate_monthly_report(
    ...
    try:
        ...
        if format == "pdf":
            raise HTTPException(
                status_code=status.HTTP_501_NOT_IMPLEMENTED, detail="PDF generation not implemented yet"
            )
        return report
    except Exception as e:
...
@router.get(
    "/usage/by-service", dependencies=[Depends(get_current_active_user), Depends(RateLimited(requests=30, window=60))]
)
async def get_usage_by_service(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to retrieve service usage: {str(e)}"
        )
...
@router.delete(
    "/cache/clear", dependencies=[Depends(get_current_active_user), Depends(RateLimited(requests=5, window=3600))]
)

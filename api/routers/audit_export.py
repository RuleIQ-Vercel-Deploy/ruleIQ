...
def _get_compliance_recommendations(logs: List[AuditLog]) -> List[str]:
    ...
    review_logs = [
        log
        for log in logs
        if "review" in log.action.lower() or "audit" in log.action.lower()
    ]
    if not review_logs:
...
def _get_framework_specific_report(framework: str, logs: List[AuditLog]) -> Dict:
    ...
    if framework.upper() == "GDPR":
        return {
            ...
            "data_deletion_requests": len(
                [log for log in logs if "data_deletion" in log.action]
            ),
            "consent_updates": len([log for log in logs if "consent" in log.action]),
            "breach_notifications": len(
                [log for log in logs if "breach" in log.action]
            ),
        }
    elif framework.upper() == "ISO27001":
        return {
            "access_control_events": len(
                [log for log in logs if _is_security_event(log.action)]
            ),
            "change_management": len(
                [log for log in logs if _is_modification(log.action)]
            ),
            "incident_responses": len(
                [log for log in logs if "incident" in log.action]
            ),

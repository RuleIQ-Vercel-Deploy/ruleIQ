...
async def export_audit_logs(
    ...
    if request.format == "csv":
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail=f"Unsupported export format: {request.format}"
        )
...
async def get_security_events(
    current_user: User = Depends(get_current_active_user), db: AsyncSession = Depends(get_async_db), days: int = 30
) -> Dict[str, Any]:
    """
...
async def generate_compliance_report(
    ...
    query = select(AuditLog).where(and_(AuditLog.user_id == current_user.id, AuditLog.timestamp >= cutoff_date))
    result = await db.execute(query)
    logs = result.scalars().all()
    ...
    report = {
        ...
        "period": {"start": cutoff_date.isoformat(), "end": datetime.now(timezone.utc).isoformat(), "days": 90},
        "summary": {
            "total_events": len(logs),
...
def _export_csv(logs: List[AuditLog], user: User) -> StreamingResponse:
    ...
    writer.writerow(
        ["Timestamp", "User", "Action", "Resource Type", "Resource ID", "IP Address", "User Agent", "Status", "Details"]
    )
    for log in logs:
        writer.writerow(
            [
...
def _get_compliance_recommendations(logs: List[AuditLog]) -> List[str]:
    ...
    review_logs = [log for log in logs if "review" in log.action.lower() or "audit" in log.action.lower()]
    if not review_logs:
...
def _get_framework_specific_report(framework: str, logs: List[AuditLog]) -> Dict:
    ...
    if framework.upper() == "GDPR":
        return {
            "data_access_requests": len([log for log in logs if "data_access_request" in log.action]),

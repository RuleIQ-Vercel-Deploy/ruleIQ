...
class AuditExportRequest(BaseModel):
    ...
    start_date: datetime
    end_date: datetime
    format: str = "csv"  # csv, json, or txt
...
async def export_audit_logs(
    ...
    # For SMBs, only export logs for the current user
    ...
    # Exclude system actions unless requested
...
async def get_security_events(
    ...
    failed_logins = len([l for l in logs if "login_failure" in l.action])
    successful_logins = len([l for l in logs if "login_success" in l.action])
...
async def generate_compliance_report(
    ...
    # Categorize actions for compliance
    data_access_logs = [l for l in logs if _is_data_access(l.action)]
    modification_logs = [l for l in logs if _is_modification(l.action)]
    security_logs = [l for l in logs if _is_security_event(l.action)]
...
def _export_txt(logs: List[AuditLog], user: User) -> StreamingResponse:
    ...
    output.write(f"Audit Log Export - {datetime.utcnow().isoformat()}\n")
    ...
    output.seek(0)
    filename = f"audit_log_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}.txt"
...
def _get_compliance_recommendations(logs: List[AuditLog]) -> List[str]:
    ...
    if len(logs) < 100:
        recommendations.append("Low audit activity - ensure all significant actions are being logged")
    
    # Check for regular backups
    backup_logs = [l for l in logs if "backup" in l.action.lower()]
    if not backup_logs:
        recommendations.append("No backup activity detected - implement regular data backups")
    
    # Check for security reviews
    review_logs = [l for l in logs if "review" in l.action.lower() or "audit" in l.action.lower()]
    if not review_logs:
...
def _get_framework_specific_report(framework: str, logs: List[AuditLog]) -> Dict:
    ...
    if framework.upper() == "GDPR":
        return {
            "data_access_requests": len([l for l in logs if "data_access_request" in l.action]),
            "data_deletion_requests": len([l for l in logs if "data_deletion" in l.action]),
            "consent_updates": len([l for l in logs if "consent" in l.action]),
            "breach_notifications": len([l for l in logs if "breach" in l.action])
        }
    elif framework.upper() == "ISO27001":
        return {
            "access_control_events": len([l for l in logs if _is_security_event(l.action)]),
            "change_management": len([l for l in logs if _is_modification(l.action)]),
            "incident_responses": len([l for l in logs if "incident" in l.action]),
            "risk_assessments": len([l for l in logs if "risk" in l.action])
        }
    else:
        return {
            "message": f"No specific report format for {framework}"
        }
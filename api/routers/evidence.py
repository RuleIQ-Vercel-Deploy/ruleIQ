...
async def create_new_evidence(
    ...
    evidence = await EvidenceService.create_evidence_item(db=db, user=current_user, evidence_data=evidence_dict)
...
async def search_evidence_items(
    ...
    return {"results": search_results, "total_count": len(evidence_items), "page": page, "page_size": page_size}


@router.post("/validate", response_model=EvidenceValidationResult)
async def validate_evidence_quality(
    evidence_data: dict, db: AsyncSession = Depends(get_async_db), current_user: User = Depends(get_current_active_user)
) -> Dict[str, Any]:
    ...
    return {
        ...
        "issues": [],
        "recommendations": ["Consider adding more detailed metadata", "Include version control information"],
    }


@router.get("/requirements/{framework_id}", response_model=EvidenceRequirementsResponse)
async def get_evidence_requirements(
    framework_id: UUID, db: AsyncSession = Depends(get_async_db), current_user: User = Depends(get_current_active_user)
) -> Dict[str, Any]:
...
async def identify_evidence_requirements(
    request_data: dict, db: AsyncSession = Depends(get_async_db), current_user: User = Depends(get_current_active_user)
) -> Dict[str, Any]:
    ...
    requirements = [
        ...
        {
            "control_id": (
                request_data.get("control_ids", ["", ""])[1] if len(request_data.get("control_ids", [])) > 1 else ""
            ),
            "evidence_type": "log",
            "title": "Access Logs",
...
async def get_evidence_details(
    evidence_id: UUID, db: AsyncSession = Depends(get_async_db), current_user: User = Depends(get_current_active_user)
) -> Any:
...
async def update_evidence_item(
    ...
    evidence, status = await EvidenceService.update_evidence_item(
        db=db, user=current_user, evidence_id=evidence_id, update_data=evidence_update.model_dump(exclude_unset=True)
    )
    if status == "not_found":
        raise HTTPException(status_code=HTTP_NOT_FOUND, detail="Evidence not found")
    elif status == "unauthorized":
        ...
        raise HTTPException(status_code=HTTP_BAD_REQUEST, detail=status.replace("validation_error: ", ""))
    return EvidenceService._convert_evidence_item_to_response(evidence)
...
async def delete_evidence_item(
    ...
    success, status = await EvidenceService.delete_evidence_item(db=db, user=current_user, evidence_id=evidence_id)
    if status == "not_found":
        raise HTTPException(status_code=HTTP_NOT_FOUND, detail="Evidence not found")
    elif status == "unauthorized":
        raise HTTPException(status_code=HTTP_FORBIDDEN, detail="Access denied")
...
async def bulk_update_evidence_status(
    ...
    return EvidenceBulkUpdateResponse(
        updated_count=updated_count, failed_count=failed_count, failed_ids=failed_ids if failed_ids else None
    )
...
async def upload_evidence_file_route(
    ...
    logger.info(
        ...
        % (analysis_report.filename, analysis_report.validation_result.value, analysis_report.security_score)
    )
    ...
    if not evidence:
        if secure_path.exists():
            secure_path.unlink()
        raise HTTPException(status_code=HTTP_NOT_FOUND, detail="Failed to upload or link file to evidence")
...
async def get_evidence_dashboard(
    ...
    dashboard_data = await EvidenceService.get_evidence_dashboard(db=db, user=current_user, framework_id=framework_id)
...
async def classify_evidence_with_ai(
    ...
    try:
        ...
        if status == "not_found":
            ...
            raise HTTPException(status_code=HTTP_FORBIDDEN, detail="Access denied")
        if not request.force_reclassify and evidence.metadata and evidence.metadata.get("ai_classification"):
    ...
    try:
        ...
        for evidence_id in request.evidence_ids:
            try:
                ...
                classification = await processor._ai_classify_evidence(evidence)
                applied = False
                if request.apply_high_confidence and classification["confidence"] >= request.confidence_threshold:
                    evidence.evidence_type = classification["suggested_type"]
                    applied = True
        ...
        raise HTTPException(status_code=HTTP_INTERNAL_SERVER_ERROR, detail="Bulk classification failed")
    ...
    try:
        ...
        for framework in request.frameworks:
            if framework.upper() == "ISO27001":
                mappings = [control for control in classification["suggested_controls"] if control.startswith("A.")]
                ...
                mappings = [control for control in classification["suggested_controls"] if control.startswith("Art.")]
...
async def get_classification_statistics(
    ...
    try:
        ...
        raise HTTPException(status_code=HTTP_INTERNAL_SERVER_ERROR, detail="Failed to get classification statistics")
...
async def get_evidence_quality_analysis(
    ...
    try:
        ...
        raise HTTPException(status_code=HTTP_INTERNAL_SERVER_ERROR, detail="Quality analysis failed")
...
async def detect_evidence_duplicates(
    ...
    try:
        ...
        scorer = QualityScorer()
        duplicates = await scorer.detect_semantic_duplicates(evidence, candidates, request.similarity_threshold / 100)
        ...
        raise HTTPException(status_code=HTTP_INTERNAL_SERVER_ERROR, detail="Duplicate detection failed")
...
async def batch_duplicate_detection(
    ...
    try:
        ...
        if len(evidence_items) < 2:
            raise HTTPException(status_code=HTTP_BAD_REQUEST, detail="At least 2 valid evidence items required")
        scorer = QualityScorer()
        duplicate_analysis = await scorer.batch_duplicate_detection(evidence_items, request.similarity_threshold / 100)
        ...
        raise HTTPException(status_code=HTTP_INTERNAL_SERVER_ERROR, detail="Batch duplicate detection failed")
...
async def get_quality_benchmark(
    ...
    try:
        ...
        if not user_evidence:
            raise HTTPException(status_code=HTTP_BAD_REQUEST, detail="No evidence found for benchmarking")
        scorer = QualityScorer()
        user_scores = []
        ...
        if not user_scores:
            raise HTTPException(status_code=HTTP_BAD_REQUEST, detail="No evidence matches the specified criteria")
        ...
        raise HTTPException(status_code=HTTP_INTERNAL_SERVER_ERROR, detail="Quality benchmarking failed")
...
async def get_quality_trends(
    ...
    try:
        ...
        if not filtered_evidence:
            raise HTTPException(status_code=HTTP_BAD_REQUEST, detail="No evidence found in the specified time period")
        scorer = QualityScorer()
        daily_scores = {}
        ...
        for date_str, scores in sorted(daily_scores.items()):
            daily_data.append(
                {"date": date_str, "average_score": round(sum(scores) / len(scores), 2), "evidence_count": len(scores)}
            )
        if len(daily_data) >= 2:
        ...
        return QualityTrendResponse(
            ...
            insights=insights,
            recommendations=(
                ["Focus on evidence completeness", "Improve documentation quality"] if avg_score < 75 else []
            ),
        )
    except Exception as e:
        logger.error("Error getting quality trends: %s" % e, exc_info=True)
        raise HTTPException(status_code=HTTP_INTERNAL_SERVER_ERROR, detail="Quality trend analysis failed")

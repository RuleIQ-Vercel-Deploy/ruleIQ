...
async def error_handler_middleware(request: Request, call_next) -> Any:
    ...
    try:
        ...
        return JSONResponse(
            ...
            content={
                "detail": exc.message,
                "error": {"type": exc.__class__.__name__, "message": exc.message, "request_id": request_id},
            },
        )
    except ValidationError as exc:
        logger.warning("ValidationError caught", extra={"request_id": request_id, "errors": exc.errors()})
        return JSONResponse(
            status_code=422,
            content={
                "detail": exc.errors(),
        ...
        logger.error(
            "SQLAlchemyError caught: %s" % exc, exc_info=settings.is_development, extra={"request_id": request_id}
        )
        return JSONResponse(
            status_code=500,
            content={

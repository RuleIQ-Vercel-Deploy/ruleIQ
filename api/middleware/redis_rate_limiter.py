    ...
    async def _get_redis_client(self) -> redis.Redis:
        ...
        if self.redis_client is None:
            self.redis_client = redis.from_url(
                self.redis_url, decode_responses=True, socket_connect_timeout=5, socket_timeout=5, retry_on_timeout=True
            )
...
general_limiter = RedisRateLimiter(
    requests_per_window=settings.rate_limit_requests, window_seconds=settings.rate_limit_window, key_prefix="general"
)
...
async def redis_rate_limit_middleware(request: Request, call_next) -> Any:
    ...
    if not allowed:
        return JSONResponse(
            ...
            content={
                "error": {"message": "Rate limit exceeded", "code": "RATE_LIMIT_EXCEEDED", "retry_after": retry_after}
            },
            headers={
...
def redis_rate_limit(requests_per_window: int = 60, window_seconds: int = 60, key_prefix: str = "custom") -> Any:
    """Create a custom Redis rate limit dependency."""
    limiter = RedisRateLimiter(
        requests_per_window=requests_per_window, window_seconds=window_seconds, key_prefix=key_prefix
    )

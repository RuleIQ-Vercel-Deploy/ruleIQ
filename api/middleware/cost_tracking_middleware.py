...
class CostTrackingMiddleware(BaseHTTPMiddleware):
    ...
    async def _check_user_budget_limits(self, user_id: str) -> None:
        ...
        try:
            ...
            logger.debug(
                f"Checking budget limits for user: {user_id}, key: {user_limit_key}"
            )
            ...
            if today_usage:
                ...
                if total_cost >= daily_limit:
                    ...
                    logger.warning(
                        f"User {user_id} approaching daily cost limit: ${total_cost:.2f}"
                    )
    ...
    async def _track_successful_request(
        ...
        try:
            ...
            if token_usage:
                ...
                logger.debug(
                    f"Tracked successful request {tracking_info['request_id']}: ${result['cost_usd']:.6f}"
                )
    ...
    def _estimate_failed_request_tokens(
    ...
    def _add_cost_headers(self, response: Response, request: Request) -> None:
        ...
        try:
            if hasattr(request.state, "cost_result"):
                ...
                response.headers["X-AI-Efficiency-Score"] = str(
                    cost_result["efficiency_score"]
                )
    ...
    async def broadcast_cost_update(self, cost_data: Dict[str, Any]) -> None:
        ...
        for client_id, connection in self.active_connections.items():
            try:
                ...
                logger.warning(
                    f"Failed to send cost update to client {client_id}: {str(e)}"
                )
    ...
    async def send_budget_alert(self, alert_data: Dict[str, Any]) -> None:
        ...
        for client_id, connection in self.active_connections.items():
            try:
                ...
                logger.warning(
                    f"Failed to send budget alert to client {client_id}: {str(e)}"
                )

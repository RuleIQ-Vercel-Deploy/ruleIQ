...
class CostTrackingMiddleware(BaseHTTPMiddleware):
    ...
    async def _check_user_budget_limits(self, user_id: str) -> None:
        ...
        try:
            ...
            today_usage = await self.cost_tracker.get_usage_by_time_range(
                start_time=datetime.combine(date.today(), datetime.min.time()), end_time=datetime.now()
            )
            if today_usage:
                ...
                if total_cost >= daily_limit:
                    ...
                    logger.warning("User %s approaching daily cost limit: $%s" % (user_id, total_cost))
        except HTTPException:
            raise
        except requests.RequestException as e:
            logger.error("Failed to check user budget limits: %s" % str(e))

    async def _track_successful_request(self, request: Request, response: Response, start_time: float) -> None:
        ...
        try:
            ...
            if token_usage:
                ...
                logger.debug("Tracked successful request %s: $%s" % (tracking_info["request_id"], result["cost_usd"]))
        except (requests.RequestException, KeyError, IndexError) as e:
            logger.error("Failed to track successful request: %s" % str(e))

    async def _track_failed_request(self, request: Request, error: HTTPException, start_time: float) -> None:
    ...
    async def _track_error_request(self, request: Request, error: Exception, start_time: float) -> None:
    ...
    async def _extract_token_usage(self, request: Request, response: Response) -> Optional[Dict[str, Any]]:
...
class RealTimeCostMonitor:
    ...
    async def connect_client(self, client_id: str, websocket) -> None:
        ...
        self.active_connections[client_id] = {"websocket": websocket, "connected_at": datetime.now()}
    ...
    async def send_budget_alert(self, alert_data: Dict[str, Any]) -> None:
        ...
        message = {"type": "budget_alert", "timestamp": datetime.now().isoformat(), "alert": alert_data}
        for client_id, connection in self.active_connections.items():
            try:
                await connection["websocket"].send_json(message)
            except (json.JSONDecodeError, KeyError, IndexError) as e:

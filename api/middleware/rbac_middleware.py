...
class RBACMiddleware(BaseHTTPMiddleware):
    ...
    def _configure_route_permissions(self) -> Dict[Pattern, Dict[str, List[str]]]:
        ...
        route_config = {
            ...
            "/api/reports.*": {"GET": ["report_view"], "POST": ["report_export", "report_schedule"]},
            "/api/business-profiles/?$": {"GET": ["user_list"], "POST": []},
            "/api/business-profiles/[^/]+/?$": {"GET": [], "PUT": [], "DELETE": ["user_delete"], "PATCH": []},
        }
    ...
    async def dispatch(self, request: Request, call_next) -> Response:
        ...
        try:
            ...
            if self.enable_audit_logging and current_user:
                await self._log_access(current_user, request, response.status_code, time.time() - start_time)
            return response
        except HTTPException as e:
            if self.enable_audit_logging:
                await self._log_access_failure(request, e.status_code, str(e.detail), time.time() - start_time)
            raise
        except Exception as e:
            logger.error("RBAC middleware error: %s" % e, exc_info=True)
            if self.enable_audit_logging:
                await self._log_access_failure(request, 500, f"Internal error: {str(e)}", time.time() - start_time)
    ...
    async def _log_access(self, user: UserWithRoles, request: Request, status_code: int, duration: float) -> None:
        """
    ...
    async def _log_access_failure(self, request: Request, status_code: int, reason: str, duration: float) -> None:
        """

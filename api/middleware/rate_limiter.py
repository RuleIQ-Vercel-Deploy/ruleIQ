...
class RateLimiter:
    ...
    async def _cleanup_old_entries(self) -> None:
        ...
        minute_ago = current_time - 60
        self.requests = {
            k: [t for t in v if t > minute_ago] for k, v in self.requests.items() if any((t > minute_ago for t in v))
        }
...
if settings.is_testing:
    ...
    general_limiter = RateLimiter(requests_per_minute=settings.rate_limit_per_minute)
    auth_limiter = RateLimiter(requests_per_minute=settings.auth_rate_limit_per_minute)

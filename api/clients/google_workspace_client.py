...
class GoogleWorkspaceAPIClient(BaseAPIClient):
    ...
    def _get_service(self, service_name: str, version: str):
        ...
        if key not in self.service_cache:
            if not GOOGLE_AVAILABLE:
                ...
                self.service_cache[key] = build(service_name, version, credentials=self.credentials_obj)
        ...
        try:
            ...
            evidence_data = {
                ...
                "summary": {
                    ...
                    "mfa_compliance_rate": mfa_enabled / total_users * 100 if total_users > 0 else 0,
                },
            }
        ...
        try:
            ...
            evidence_data = {
                ...
                "summary": {
                    ...
                    "security_groups": len([g for g in groups if "security" in g.get("name", "").lower()]),
                    "distribution_groups": len([g for g in groups if "distribution" in g.get("name", "").lower()]),
                },
            }
            return CollectionResult(
    ...
    async def collect_admin_logs_evidence(self) -> CollectionResult:
        ...
        try:
            ...
            start_time = (datetime.now(timezone.utc) - timedelta(days=7)).strftime("%Y-%m-%dT%H:%M:%S.%fZ")
            activities_result = (
                service.activities()
            ...
            evidence_data = {
                ...
                "summary": {
                    ...
                    "unique_users": len(set(act.get("actor", {}).get("email", "") for act in activities)),
                    "event_types": list(
                        set(event.get("name", "") for act in activities for event in act.get("events", []))
                    ),
                },
            }
            return CollectionResult(
        ...
        try:
            ...
            evidence_data = {
                ...
                "summary": {
                    ...
                    "primary_domain": next((d["domainName"] for d in domains if d.get("isPrimary")), None),
    ...
    def _calculate_users_quality(self, total_users: int, mfa_enabled: int, suspended: int) -> EvidenceQuality:
    ...
    def _calculate_groups_quality(self, groups: List[Dict], memberships: Dict) -> EvidenceQuality:
        ...
        has_proper_naming = (
            sum(1 for g in groups if len(g.get("name", "")) > DEFAULT_RETRIES) / len(groups) > CONFIDENCE_THRESHOLD
        )
    ...
    def get_supported_evidence_types(self) -> List[str]:
        ...
        return ["user_directory", "access_groups", "admin_activity_logs", "user_access_logs", "domain_configuration"]

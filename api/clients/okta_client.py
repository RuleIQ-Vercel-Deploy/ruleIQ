        ...
        try:
            ...
            async with self.session.get(
                ...
                if response.status == 200:
                    ...
                    logger.info(f"Okta authentication successful for user: {user_info.get('profile', {}).get('login')}")
                    return True
                        ...
                        evidence_item = self.create_evidence_item(
                            ...
                            data={
                                ...
                                "credentials": {
                                    ...
                                    "recovery_question": user.get('credentials', {}).get('recovery_question', {}),
                            ...
                            quality_score=self._calculate_user_quality_score(user, factors_response.data)
                        )
                        
                        evidence.append(evidence_item)
                        
                    except Exception as e:
                        self.logger.warning(f"Failed to get full details for user {user['profile']['login']}: {e}")
                ...
                for group in response.data:
                    try:
                        ...
                        evidence_item = self.create_evidence_item(
                            ...
                            quality_score=self._calculate_group_quality_score(group, members_response.data)
                        )
                        
                        evidence.append(evidence_item)
                        
                    except Exception as e:
                        self.logger.warning(f"Failed to get full details for group {group['profile']['name']}: {e}")
                        continue
                ...
                for app in response.data:
                    try:
                        ...
                        evidence_item = self.create_evidence_item(
                            ...
                            compliance_controls=["CC6.1", "CC6.2", "CC6.8"],  # SOC 2 application access controls
                            quality_score=self._calculate_app_quality_score(app, users_response.data)
                        )
                        
                        evidence.append(evidence_item)
                        
                    except Exception as e:
                        self.logger.warning(f"Failed to get full details for app {app['label']}: {e}")
                        continue
...
class OktaLogsCollector(BaseEvidenceCollector):
    ...
    async def collect(self, since: datetime = None, event_types: List[str] = None, **kwargs) -> List[EvidenceItem]:
        """Collect system logs for audit evidence"""
        if not since:
                ...
                for log_entry in response.data:
                    evidence_item = self.create_evidence_item(
                        ...
                        data={
                            ...
                            } if log_entry.get('actor') else None,
                            "client": {
                                ...
                                "geographical_context": log_entry.get('client', {}).get('geographicalContext', {})
                            } if log_entry.get('client') else None,
                            ...
                            } if log_entry.get('outcome') else None,
    ...
    def _calculate_log_quality_score(self, log_entry: Dict) -> float:
        ...
        security_events = [
            'user.authentication.auth_via_mfa', 'user.authentication.sso',
            'user.lifecycle.create', 'user.lifecycle.delete',
            'policy.lifecycle.create', 'policy.lifecycle.update',

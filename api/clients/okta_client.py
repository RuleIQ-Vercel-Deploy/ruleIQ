...
class OktaAPIClient(BaseAPIClient):
        ...
        try:
            ...
            async with self.session.get(f"{self.base_url}/users/me", headers=headers) as response:
                if response.status == HTTP_OK:
                    ...
                    logger.info(
                        "Okta authentication successful for user: %s" % user_info.get("profile", {}).get("login")
                    )
                    return True
    ...
    def get_supported_evidence_types(self) -> List[str]:
        ...
        return ["users", "groups", "applications", "policies", "system_logs", "mfa_factors", "zones", "auth_servers"]
                    ...
                    try:
                        ...
                        evidence_item = self.create_evidence_item(
                            ...
                            data={
                                ...
                                "applications": [
                                    {"id": app["id"], "label": app["label"], "link_url": app.get("linkUrl")}
                        ...
                        self.logger.warning(
                            "Failed to get full details for user %s: %s" % (user["profile"]["login"], e)
                        )
                        evidence_item = self.create_evidence_item(
                            evidence_type="identity_user",
                            resource_id=user["id"],
                    ...
                    try:
                        ...
                        evidence_item = self.create_evidence_item(
                            ...
                            data={
                                ...
                                "roles": [
                                    {"id": role["id"], "type": role["type"], "status": role["status"]}
                                    for role in roles_response.data
                                ],
                            },
                            compliance_controls=["CC6.1", "CC6.2"],
                        ...
                        self.logger.warning(
                            "Failed to get full details for group %s: %s" % (group["profile"]["name"], e)
                        )
                        continue
                url = None
                if hasattr(response, "headers") and "Link" in response.headers:
            ...
            while url:
                ...
                for app in response.data:
                    try:
                        ...
                        self.logger.warning("Failed to get full details for app %s: %s" % (app["label"], e))
                        continue
                url = None
                if hasattr(response, "headers") and "Link" in response.headers:
                    link_header = response.headers["Link"]
...
class OktaLogsCollector(BaseEvidenceCollector):
    ...
    async def collect(self, since: datetime = None, event_types: List[str] = None, **kwargs) -> List[EvidenceItem]:
        ...
        evidence = []
        try:
            params = {"since": since.strftime("%Y-%m-%dT%H:%M:%S.%fZ"), "limit": 1000, "sortOrder": "DESCENDING"}
            if event_types:

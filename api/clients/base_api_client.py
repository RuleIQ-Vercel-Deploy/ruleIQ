...
class BaseAPIClient(ABC):
    ...
    async def ensure_authenticated(self) -> bool:
        ...
        if (not self.authenticated or 
            not self.last_auth_check or 
            (now - self.last_auth_check).total_seconds() > 3600):  # Re-auth every hour
        ...
        for attempt in range(request.retry_attempts):
            try:
                ...
                if attempt < request.retry_attempts - 1:
                    ...
                    logger.warning(f"Rate limited by {self.provider_name}, retrying in {wait_time}s")
                ...
                if attempt < request.retry_attempts - 1:
                    wait_time = (2 ** attempt)
                    logger.warning(f"Request failed for {self.provider_name}, retrying in {wait_time}s: {e}")
                    await asyncio.sleep(wait_time)
                    last_exception = e
                    continue
                else:
    ...
    async def _execute_request(self, request: APIRequest) -> APIResponse:
        ...
        try:
            ...
            logger.debug(f"Making {request.method} request to {self.provider_name}: {request.endpoint}")
            ...
            async with self.session.request(
                ...
                if response.status == 429:
                    ...
                    raise APIConnectionException(f"Server error from {self.provider_name}: {response.status}")
                elif response.status >= 400:
                    error_text = await response.text()
                    raise APIException(f"Client error from {self.provider_name}: {response.status} - {error_text}")
                
                # Parse response data
                response_data = await self._parse_response(response)
                
                # Log successful request
                logger.debug(f"Successful {request.method} request to {self.provider_name}: {response.status}")
            ...
            raise APITimeoutException(f"Request to {self.provider_name} timed out after {request.timeout}s")
        except aiohttp.ClientError as e:
            raise APIConnectionException(f"Connection error to {self.provider_name}: {str(e)}")
        except Exception as e:
            if isinstance(e, (APIException, APITimeoutException, APIConnectionException, APIAuthenticationException, APIRateLimitException)):
                raise e
            else:

...
class BaseAPIClient(ABC):
        ...
        for attempt in range(request.retry_attempts):
            try:
                ...
                if attempt < request.retry_attempts - 1:
                    ...
                    logger.warning("Rate limited by %s, retrying in %ss" % (self.provider_name, wait_time))
                ...
                if attempt < request.retry_attempts - 1:
                    wait_time = 2**attempt
                    logger.warning("Request failed for %s, retrying in %ss: %s" % (self.provider_name, wait_time, e))
                    await asyncio.sleep(wait_time)
                    last_exception = e
                    continue
                else:
    ...
    async def _execute_request(self, request: APIRequest) -> APIResponse:
        ...
        try:
            ...
            logger.debug("Making %s request to %s: %s" % (request.method, self.provider_name, request.endpoint))
            ...
            async with self.session.request(
                ...
                logger.debug("Successful %s request to %s: %s" % (request.method, self.provider_name, response.status))
                return APIResponse(

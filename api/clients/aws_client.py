...
from .base_api_client import (
    BaseAPIClient, APICredentials, APIRequest, APIResponse, 
    EvidenceItem, BaseEvidenceCollector,
    APIException, APIAuthenticationException, APIConnectionException,
    AuthType
)
...
class AWSAPIClient(BaseAPIClient):
    ...
    async def authenticate(self) -> bool:
        ...
        try:
            ...
            logger.info(f"AWS authentication successful for account: {caller_identity.get('Account')}")
            return True
    ...
    def get_supported_evidence_types(self) -> List[str]:
        ...
        return [
            "iam_policies", "iam_users", "iam_roles", "security_groups",
            "vpc_configuration", "cloudtrail_logs", "config_rules",
            "guardduty_findings", "inspector_findings", "compliance_reports",
            "s3_buckets", "ec2_instances"
        ]
                ...
                for policy in page['Policies']:
                    try:
                        ...
                        evidence_item = self.create_evidence_item(
                            ...
                            compliance_controls=["CC6.1", "CC6.2", "CC6.3"],  # SOC 2 Logical Access controls
                            quality_score=self._calculate_policy_quality_score(policy, policy_version['PolicyVersion']['Document'])
                        )
                        
                        evidence.append(evidence_item)
                        
                    except ClientError as e:
                        self.logger.warning(f"Failed to get details for policy {policy['PolicyName']}: {e}")
                        continue
                ...
                for user in page['Users']:
                    try:
                        ...
                        try:
                            ...
                            password_last_used = login_profile.get('LoginProfile', {}).get('PasswordResetRequired')
                        except ClientError:
                        ...
                        evidence_item = self.create_evidence_item(
                            ...
                            data={
                                ...
                                "attached_policies": [policy['PolicyName'] for policy in user_policies['AttachedPolicies']],
                            ...
                            compliance_controls=["CC6.1", "CC6.2", "CC6.7"],  # SOC 2 user access controls
                            quality_score=self._calculate_user_quality_score(user, mfa_devices['MFADevices'], access_keys['AccessKeyMetadata'])
                        )
                        
                        evidence.append(evidence_item)
                        
                    except ClientError as e:
                        self.logger.warning(f"Failed to get details for user {user['UserName']}: {e}")
                        continue
                    ...
                    try:
                        ...
                        evidence_item = self.create_evidence_item(
                            ...
                            data={
                                ...
                                "last_used": role.get('RoleLastUsed', {}).get('LastUsedDate', '').isoformat() if role.get('RoleLastUsed', {}).get('LastUsedDate') else None,
                                "last_used_region": role.get('RoleLastUsed', {}).get('Region'),
                                "attached_policies": [policy['PolicyName'] for policy in role_policies['AttachedPolicies']],
                                "inline_policies": inline_policies['PolicyNames'],
                                "tags": role.get('Tags', [])
                            },
                            compliance_controls=["CC6.1", "CC6.3"],  # SOC 2 role-based access controls
                        ...
                        self.logger.warning(f"Failed to get details for role {role['RoleName']}: {e}")
                        continue
        ...
        try:
            ...
            if event_types:
                ...
                security_events = [
                    ...
                    'ConsoleLogin', 'AssumeRole', 'CreateLoginProfile', 'DeleteLoginProfile'
                ]
                ...
                try:
                    ...
                    for event in response['Events']:
                        ...
                        evidence_item = self.create_evidence_item(
                            ...
                            compliance_controls=["CC6.1", "CC7.2", "CC7.3"],  # SOC 2 monitoring and logging
                    ...
                    self.logger.warning(f"Failed to collect events for {lookup_attr['AttributeValue']}: {e}")
                    continue

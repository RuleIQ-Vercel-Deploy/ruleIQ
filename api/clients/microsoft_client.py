...
class MicrosoftGraphAPIClient(BaseAPIClient):
    ...
    async def collect_users_evidence(self) -> CollectionResult:
        ...
        try:
            ...
            users_with_recent_signin = sum(1 for user in users 
                                         if user.get('lastSignInDateTime') and 
        ...
        try:
            ...
            evidence_data = {
                ...
                'summary': {
                    ...
                    'enabled_service_principals': len([sp for sp in service_principals if sp.get('accountEnabled', False)]),
                    'multi_tenant_apps': len([app for app in applications if app.get('signInAudience') == 'AzureADMultipleOrgs'])
                }
            }
    ...
    def _calculate_signin_logs_quality(self, all_logs: List[Dict], successful: List[Dict], failed: List[Dict]) -> EvidenceQuality:

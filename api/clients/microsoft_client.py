...
from .base_api_client import APICredentials, AuthType, BaseAPIClient, CollectionResult, EvidenceQuality
...
class MicrosoftGraphAPIClient(BaseAPIClient):
    ...
    async def test_connection(self) -> Tuple[bool, str]:
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(f"{self.get_base_url()}/organization", headers=headers) as response:
                    if response.status == HTTP_OK:
    ...
    async def collect_users_evidence(self) -> CollectionResult:
        ...
        try:
            ...
            quality_score = self._calculate_users_quality(total_users, enabled_users, users_with_recent_signin)
            evidence_data = {
                "users": users,
                "summary": {
                    ...
                    "activity_rate": users_with_recent_signin / total_users * 100 if total_users > 0 else 0,
                },
            }
        ...
        try:
            ...
            evidence_data = {
                ...
                "summary": {
                    ...
                    "security_group_ratio": len(security_groups) / len(groups) * 100 if groups else 0,
                },
            }
            return CollectionResult(
    ...
    async def collect_applications_evidence(self) -> CollectionResult:
        ...
        try:
            apps_data = await self._make_graph_request(
                "applications",
                params={"$select": "id,displayName,createdDateTime,signInAudience,requiredResourceAccess", "$top": 999},
            )
            applications = apps_data.get("value", [])
            sp_data = await self._make_graph_request(
                "servicePrincipals",
                params={"$select": "id,displayName,servicePrincipalType,accountEnabled,createdDateTime", "$top": 999},
            )
            ...
            evidence_data = {
                ...
                "summary": {
                    ...
                    "multi_tenant_apps": len(
                        [app for app in applications if app.get("signInAudience") == "AzureADMultipleOrgs"]
                    ),
                },
            }
            return CollectionResult(
    ...
    async def collect_signin_logs_evidence(self) -> CollectionResult:
        ...
        try:
            start_date = (datetime.now(timezone.utc) - timedelta(days=7)).strftime("%Y-%m-%dT%H:%M:%S.%fZ")
            ...
            quality_score = self._calculate_signin_logs_quality(sign_ins, successful_signins, failed_signins)
            evidence_data = {
                "sign_ins": sign_ins,
                "summary": {
                    ...
                    "success_rate": len(successful_signins) / len(sign_ins) * 100 if sign_ins else 0,
        ...
        try:
            ...
            evidence_data = {
                ...
                "summary": {
                    ...
                    "activity_types": list(set(log.get("activityDisplayName", "") for log in audit_logs)),
                },
            }
            return CollectionResult(
    ...
    def _calculate_groups_quality(self, groups: List[Dict], security_groups: List[Dict]) -> EvidenceQuality:
    ...
    def _calculate_applications_quality(self, apps: List[Dict], service_principals: List[Dict]) -> EvidenceQuality:

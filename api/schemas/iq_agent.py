...
from api.schemas.base import BaseSchema, StandardResponse, TimestampMixin
...
class ComplianceSummary(BaseSchema):
    ...
    top_gaps: List[str] = Field(
        ..., 
        description="Top compliance gaps identified",
        max_items=5
    )
...
class ComplianceEvidence(BaseSchema):
    ...
    controls_executed: int = Field(
        ..., 
        description="Number of controls executed"
    )
    evidence_stored: int = Field(
        ..., 
        description="Number of evidence items stored"
    )
    metrics_updated: int = Field(
        ..., 
...
class IQAgentResponse(BaseSchema):
    ...
    graph_context: GraphContext = Field(
        ..., 
        description="Graph analysis context"
    )
...
class GraphInitializationRequest(BaseSchema):
    ...
    clear_existing: bool = Field(
        default=False,
        description="Whether to clear existing graph data"
    )

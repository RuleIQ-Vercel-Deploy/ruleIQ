...
class BusinessProfileBase(BaseModel):
    ...
    development_too: List[str] = Field(default_factory=list, max_items=10)
    existing_framew: List[str] = Field(default_factory=list, max_items=10)
    planned_framewo: List[str] = Field(default_factory=list, max_items=10)
    compliance_budg: Optional[str] = Field(None, pattern=r'^(Under £\d+K|£\d+K-£\d+K|Over £\d+K)$')
    compliance_time: Optional[str] = Field(None, pattern=r'^\d+ months?$')

    @validator('company_name')
    def validate_company_name(self, v):
        return InputValidators.validate_company_name(v)

    @validator('cloud_providers', 'saas_tools', 'development_too', 'existing_framew', 'planned_framewo', each_item=True)
...
class BusinessProfileUpdate(BaseModel):
    ...
    handles_persona: Optional[bool] = None
    processes_payme: Optional[bool] = None
    stores_health_d: Optional[bool] = None
    provides_financ: Optional[bool] = None
    operates_critic: Optional[bool] = None
    has_internation: Optional[bool] = None
    cloud_providers: Optional[List[str]] = None
    saas_tools: Optional[List[str]] = None
    development_too: Optional[List[str]] = None
    existing_framew: Optional[List[str]] = None
    planned_framewo: Optional[List[str]] = None
    compliance_budg: Optional[str] = None
    compliance_time: Optional[str] = None

class BusinessProfileResponse(BusinessProfileBase):
    id: UUID
    user_id: UUID
    assessment_comp: bool

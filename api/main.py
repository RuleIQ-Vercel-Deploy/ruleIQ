...
from api.routers.admin import data_access, token_management, user_management
...
from database import _AsyncSessionLocal as AsyncSessionLocal
...
logging.basicConfig(
    level=getattr(logging, settings.log_level.value), format="%(asctime)s - %(name)s - %(levelname)s - %(message)s"
)
...
async def http_exception_handler(request: Request, exc: HTTPException) -> JSONResponse:
    ...
    return JSONResponse(
        ...
        content={"error": {"message": exc.detail, "type": "http_exception", "status_code": exc.status_code}},
    )
...
async def sqlalchemy_exception_handler(request: Request, exc: SQLAlchemyError) -> JSONResponse:
    ...
    return JSONResponse(
        status_code=500,
        content={"error": {"message": "Database operation failed", "type": "database_error", "status_code": 500}},
    )
...
async def general_exception_handler(request: Request, exc: Exception) -> JSONResponse:
    ...
    return JSONResponse(
        status_code=500,
        content={"error": {"message": "Internal server error", "type": "internal_error", "status_code": 500}},
    )
...
async def api_health_detailed() -> Dict[str, Any]:
    ...
    return {
        ...
        "version": "1.0.0",
        "api_version": "v1",
        "components": {"database": db_status, "ai_services": ai_status, "redis": "not_configured"},
    }
...
async def readiness_check() -> Dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(
            status_code=HTTP_SERVICE_UNAVAILABLE, detail="Service not ready - database connection failed"
        )
    finally:
        if db is not None:
            db.close()

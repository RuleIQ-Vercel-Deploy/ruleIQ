...
class FieldValidator:
    ...
    def validate_string(value: Any, min_length: int = 0, max_length: int = 1000, 
                       allow_empty: bool = True, pattern: Optional[str] = None) -> str:
    ...
    @staticmethod
    def validate_list(value: Any, max_items: int = 100, 
...
class WhitelistValidator:
    ...
    def _get_field_definitions(self) -> Dict[str, Dict[str, Any]]:
        ...
        definitions = {
            "EvidenceItem": {
                ...
                "evidence_type": {
                    "type": FieldType.ENUM,
                    "allowed_values": ["document", "log", "screenshot", "certificate", "policy", "other"],
                    "required": False
                },
                "status": {
                    "type": FieldType.ENUM,
    ...
    def validate_field(self, field_name: str, value: Any) -> Any:
        ...
        if field_name not in self.field_definitions:
            raise ValidationError(f"Field '{field_name}' is not allowed for updates on {self.model_name}")

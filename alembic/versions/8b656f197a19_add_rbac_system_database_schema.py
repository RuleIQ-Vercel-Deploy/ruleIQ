...
def upgrade():
    ...
    op.create_table('framework_access',
    sa.Column('id', sa.UUID(), nullable=False),
    sa.Column('role_id', sa.UUID(), nullable=False),
    sa.Column('framework_id', sa.UUID(), nullable=False),
    sa.Column('access_level', sa.Enum('read', 'write', 'admin', name='access_level_enum'), nullable=False),
    sa.Column('granted_at', sa.DateTime(), nullable=True),
    sa.Column('granted_by', sa.UUID(), nullable=True),
    sa.Column('is_active', sa.Boolean(), nullable=True),
    sa.ForeignKeyConstraint(['framework_id'], ['compliance_frameworks.id'], name=op.f('fk_framework_access_framework_id_compliance_frameworks')),
    sa.ForeignKeyConstraint(['granted_by'], ['users.id'], name=op.f('fk_framework_access_granted_by_users')),
    sa.ForeignKeyConstraint(['role_id'], ['roles.id'], name=op.f('fk_framework_access_role_id_roles')),
    ...
    sa.Column('granted_at', sa.DateTime(), nullable=True),
    sa.Column('granted_by', sa.UUID(), nullable=True),
    sa.ForeignKeyConstraint(['granted_by'], ['users.id'], name=op.f('fk_role_permissions_granted_by_users')),
    ...
    sa.Column('ip_address', sa.String(length=45), nullable=True),
    sa.Column('user_agent', sa.String(length=500), nullable=True),
    sa.Column('severity', sa.Enum('info', 'warning', 'error', 'critical', name='severity_enum'), nullable=False),
    sa.Column('timestamp', sa.DateTime(), nullable=True),
    sa.ForeignKeyConstraint(['session_id'], ['user_sessions.id'], name=op.f('fk_audit_logs_session_id_user_sessions')),
    ...
    sa.Column('access_type', sa.Enum('own_data', 'organization_data', 'all_data', name='data_access_enum'), nullable=False),
    sa.Column('granted_at', sa.DateTime(), nullable=True),
    sa.Column('granted_by', sa.UUID(), nullable=True),
    sa.Column('is_active', sa.Boolean(), nullable=True),
    sa.ForeignKeyConstraint(['business_profile_id'], ['business_profiles.id'], name=op.f('fk_data_access_business_profile_id_business_profiles')),
    sa.ForeignKeyConstraint(['granted_by'], ['users.id'], name=op.f('fk_data_access_granted_by_users')),
    sa.ForeignKeyConstraint(['user_id'], ['users.id'], name=op.f('fk_data_access_user_id_users')),
    sa.PrimaryKeyConstraint('id', name=op.f('pk_data_access')),
    sa.UniqueConstraint('user_id', 'business_profile_id', name='uq_user_business_data_access')
    )

...
def upgrade() -> None:
    ...
    op.create_table(
        'api_key_scopes',
        sa.Column('id', postgresql.UUID(as_uuid=True), primary_key=True, default=uuid.uuid4),
        sa.Column('key_id', sa.String(50), sa.ForeignKey('api_keys.key_id', ondelete='CASCADE'), nullable=False),
    ...
    op.create_index('idx_api_key_scopes_key_scope', 'api_key_scopes', ['key_id', 'scope'], unique=True)

...
class EvidenceCollectionNode:
    ...
    async def process_evidence(self, state_or_evidence: Union[EnhancedComplianceState, Dict[str, Any]], evidence_data: Optional[Dict[str, Any]]=None) -> Union[EnhancedComplianceState, Dict[str, Any]]:
        """
        ...
        try:
            ...
            async for db in get_async_db():
                try:
                    ...
                    if not return_evidence_only:
                        state['evidence_items'].append({'id': str(new_evidence.id), 'type': new_evidence.evidence_type, 'status': 'processed'})
                    ...
                    if return_evidence_only:
                        return {'id': str(new_evidence.id), 'type': new_evidence.evidence_type, 'status': 'processed', 'evidence_name': new_evidence.evidence_name}
            ...
            if not return_evidence_only:
                ...
                state['messages'].append(SystemMessage(content=f'Evidence processing failed: {str(e)}'))
            else:
    ...
    async def cleanup_stale_evidence(self, db: AsyncSession, cutoff_days: int=90) -> int:
        ...
        stmt = update(EvidenceItem).where(EvidenceItem.status == 'collected', EvidenceItem.collected_at < cutoff_date).values(status='rejected').execution_options(synchronize_session=False)
        result = await db.execute(stmt)
        await db.commit()
        return result.rowcount

    @with_langsmith_tracing('evidence.sync_status')
    async def sync_evidence_status(self, state: Union[EnhancedComplianceState, Dict[str, Any]], cutoff_days: int=90) -> Union[EnhancedComplianceState, Dict[str, Any]]:
        """
            ...
            async for db in get_async_db():
                try:
                    ...
                    if isinstance(state, dict):
                        state['sync_results'] = {'updated_count': updated_count, 'timestamp': datetime.now(timezone.utc).isoformat()}
                        if 'messages' not in state:
                            state['messages'] = []
                        state['messages'].append(SystemMessage(content=f'Evidence sync completed: {updated_count} items marked stale'))
    ...
    async def check_evidence_expiry(self, state: EnhancedComplianceState) -> EnhancedComplianceState:
        """
        ...
        try:
            async for db in get_async_db():
                try:
                    ...
                    warning_stmt = select(EvidenceItem).where(and_(EvidenceItem.status == 'collected', EvidenceItem.collected_at <= warning_date, EvidenceItem.collected_at > expiry_date))
                    warning_results = await db.execute(warning_stmt)
                    warning_items = warning_results.scalars().all()
                    expired_stmt = select(EvidenceItem).where(and_(EvidenceItem.status == 'collected', EvidenceItem.collected_at <= expiry_date))
                    ...
                    if expired_items:
                        await db.commit()
                    state['expiry_check'] = {'warning_count': len(warning_items), 'expired_count': len(expired_items), 'timestamp': datetime.now(timezone.utc).isoformat()}
                    state['messages'].append(SystemMessage(content=f'Expiry check: {len(warning_items)} warnings, {len(expired_items)} expired'))
                    logger.info(f'Evidence expiry check: {len(warning_items)} warnings, {len(expired_items)} expired')
                except Exception as e:
                    await db.rollback()
    ...
    async def collect_all_integrations(self, state: EnhancedComplianceState) -> EnhancedComplianceState:
        """
        ...
        try:
            ...
            for integration in integrations:
                try:
                    ...
                    result = {'integration': integration, 'status': 'collected', 'count': 0, 'timestamp': datetime.now(timezone.utc).isoformat()}
                    collection_results.append(result)
                except Exception as e:
                    logger.error(f'Error collecting from {integration}: {e}')
                    collection_results.append({'integration': integration, 'status': 'failed', 'error': str(e)})
            state['collection_results'] = collection_results
            state['messages'].append(SystemMessage(content=f'Evidence collection completed for {len(integrations)} integrations'))
        except Exception as e:
    ...
    async def process_pending_evidence(self, state: EnhancedComplianceState) -> EnhancedComplianceState:
        """
        ...
        try:
            async for db in get_async_db():
                try:
                    ...
                    stmt = select(EvidenceItem).where(EvidenceItem.status == 'not_started').limit(100)
                    result = await db.execute(stmt)
                    ...
                    if pending_items:
                        await db.commit()
                    state['pending_processing'] = {'processed_count': processed_count, 'total_count': len(pending_items), 'timestamp': datetime.now(timezone.utc).isoformat()}
                    state['messages'].append(SystemMessage(content=f'Processed {processed_count}/{len(pending_items)} pending items'))
    ...
    async def validate_evidence(self, evidence: Dict[str, Any]) -> Dict[str, Any]:
        ...
        score_factors = {'has_source': 0.2, 'has_timestamp': 0.2, 'has_metadata': 0.2, 'content_length': 0.4}
        score = 0.0
    ...
    async def sync_evidence_timestamps(self, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            async for db in get_async_db():
                result = await db.execute("UPDATE evidence_items SET synced_at = NOW() WHERE status = 'pending'")
                await db.commit()

...
export function DocumentOutline({ editor }: DocumentOutlineProps) {
    ...
    <Accordion type="multiple" className="w-full" defaultValue={nodes.map((n) => n.id)}>
      {nodes.map((node) => (
        <AccordionItem key={node.id} value={node.id} className="border-b-0">
          <AccordionTrigger
            className={`pl-${(node.level - 1) * 2} rounded-md py-1 text-sm hover:bg-gray-700/50 hover:no-underline`}
          >
            {node.text}
          </AccordionTrigger>
  ...
  return (
    <div className="text-eggshell-white h-full w-64 overflow-y-auto border-r border-border bg-background p-4">

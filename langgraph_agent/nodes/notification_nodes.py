...
class EnhancedNotificationNode:
    ...
    async def _send_to_dlq(self, state: Dict[str, Any]) -> None:
        ...
        dlq_message = {
            ...
            "failure_reason": (
                state.get("errors", [])[-1] if state.get("errors") else "Unknown"
            ),
        }
    ...
    async def send_throttled_notifications(
        self, state: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    async def send_with_pii_redaction(self, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Redact credit cards
        message = re.sub(
            r"\b\d{4}[\s-]?\d{4}[\s-]?\d{4}[\s-]?\d{4}\b", "[REDACTED]", message
        )
    ...
    async def send_compliance_alert(self, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            delivery_method = state.get("task_params", {}).get(
            ...
            if (
                self._circuit_breaker_status(delivery_method)
                == CircuitBreakerStatus.OPEN
            ):
                state["task_status"] = "circuit_breaker_open"
                state["errors"] = [
                    {"circuit_breaker": "open", "service": delivery_method}
                ]
                return state
    ...
    async def _send_to_channel(
        ...
        if channel == "email":
            ...
            return await self._send_sms_notification(
                recipient=params.get("recipient"), message=params.get("message", "")
            )
        elif channel == "slack":
    ...
    async def broadcast_notification(self, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            delivery_metrics.failed = (
                delivery_metrics.total_recipients - delivery_metrics.delivered
            )

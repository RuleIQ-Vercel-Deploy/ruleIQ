...
async def generate_scheduled_reports_node(state: ComplianceAgentState) -> ComplianceAgentState:
    """
    ...
    try:
        ...
        logger.info(f"Scheduled report generation complete: {reports_generated} generated, {reports_failed} failed")
...
async def generate_on_demand_report_node(state: ComplianceAgentState) -> ComplianceAgentState:
    """
...
async def send_summary_notifications_node(state: ComplianceAgentState) -> ComplianceAgentState:
    """
    ...
    try:
        ...
        logger.info(f"Summary notifications complete: {notifications_sent} sent, {notifications_failed} failed")
...
def prepare_summary_content(summary: Dict[str, Any]) -> str:
    ...
    for schedule in schedules[:5]:  # Limit to first 5
        schedule_details.append(
            f"  - {schedule.report_type} ({schedule.frequency})"
        )
...
async def send_email_with_attachment(
    ...
    return _send_email_sync(recipients, subject, body, attachments if attachments else None)
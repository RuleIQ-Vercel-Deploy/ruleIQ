...
async def distribute_report_node(state: UnifiedComplianceState) -> UnifiedComplianceState:
    """
...
async def cleanup_old_reports_node(state: UnifiedComplianceState) -> UnifiedComplianceState:
    """
    ...
    report_directory = state.get("metadata", {}).get(
        "report_directory",
        getattr(settings, "REPORT_DIRECTORY", "/tmp/reports")
    )

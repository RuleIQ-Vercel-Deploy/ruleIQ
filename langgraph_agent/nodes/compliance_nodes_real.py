...
async def get_default_framework(db: AsyncSession) -> Optional[ComplianceFrameworkModel]:
    ...
    result = await db.execute(
        select(ComplianceFrameworkModel).limit(1)
    )
    return result.scalars().first()


async def get_user_for_profile(db: AsyncSession, profile: BusinessProfileModel) -> Optional[UserModel]:
...
async def batch_compliance_update_node(state: UnifiedComplianceState) -> UnifiedComplianceState:
    """
    ...
    try:
        async for db in get_async_db():
            ...
            logger.info(f"Batch compliance update completed: {len(updated_profiles) - len(errors)} successful, {len(errors)} errors")
...
async def single_compliance_check_node(state: UnifiedComplianceState) -> UnifiedComplianceState:
    """
...
async def compliance_monitoring_node(state: UnifiedComplianceState) -> UnifiedComplianceState:
    """
    ...
    try:
        async for db in get_async_db():
            ...
            logger.info(f"Compliance monitoring completed: {len(profiles)} profiles monitored, {len(alerts)} alerts generated")

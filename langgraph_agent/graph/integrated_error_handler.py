...
class IntegratedErrorHandler:
    ...
    async def handle_node_error(
        ...
        if breaker_state == "OPEN":
            ...
            logger.info(
                f"Circuit breaker HALF_OPEN for {node_name}, attempting test request"
            )
            ...
            if self._should_open_circuit(node_name):
                ...
                if self._is_retryable_error(error):
                    ...
                    state["should_continue"] = False
                    logger.error(
                        f"Non-retryable error in {node_name}: {type(error).__name__}"
                    )
    ...
    def _check_circuit_breaker(self, node_name: str) -> str:
        ...
        if breaker["state"] == "OPEN":
            ...
            if breaker["success_count"] >= self.half_open_requests:
                ...
                breaker["success_count"] = 0
                logger.info(
                    f"Circuit breaker for {node_name} CLOSED after successful recovery"
                )
                return "CLOSED"
            return "HALF_OPEN"
    ...
    def wrap_node(self, node_func: Callable) -> Callable:
        ...
        async def wrapped(state: UnifiedComplianceState) -> UnifiedComplianceState:
            ...
            if breaker_state == "OPEN":
                logger.warning(
                    f"Circuit breaker OPEN for {node_name}, skipping execution"
                )
            ...
            try:
                ...
                if (
                    "steps_remaining" in result
                    and node_name in result["steps_remaining"]
                ):

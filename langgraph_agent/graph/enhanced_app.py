...
class EnhancedComplianceGraph:
    ...
    def _create_checkpointer(self):
        ...
        if settings.database_url and AsyncPostgresSaver:
            try:
                ...
                # Fix the database URL for psycopg
                db_url = settings.database_url
                if "asyncpg" in db_url:
                    db_url = db_url.replace("postgresql+asyncpg://", "postgresql://")

                # For AsyncPostgresSaver, we need an async connection
                # But we can pass the connection string directly
                checkpointer = AsyncPostgresSaver.from_conn_string(db_url)
                # Note: setup() is async and needs to be called separately
    ...
    async def async_setup(self):
        ...
        if hasattr(self.checkpointer, "setup"):
            # PostgreSQL checkpointer has async setup
            await self.checkpointer.setup()
            logger.info("PostgreSQL checkpointer setup completed")

    @classmethod
    async def create(cls, checkpointer: Optional[Any] = None):
        """
    ...
    def _build_graph(self) -> StateGraph:
        ...
        # Add conditional edges from all nodes to error handler
        for node_name in [
            "profile_builder",
            "compliance_analyzer",
            "evidence_collector",
        ...
        # Add error handler recovery routing
        graph.add_conditional_edges(
            "error_handler",
    ...
    async def _router_node(
        ...
        try:
            ...
            if state["messages"]:
                ...
                # Simple routing logic (to be enhanced)
            ...
            # Mark router as last successful node
    ...
    async def _profile_builder_node(
        ...
        try:
            ...
            # Simple extraction (to be enhanced with NLP)
    ...
    async def _compliance_analyzer_node(
        ...
        try:
            ...
            if location and (
                "california" in location.lower() or "ca" in location.lower()
            ):
...
"""
            ...
            state["estimated_cost"] = tokens * 0.00001  # Mock pricing
    ...
    async def _error_handler_node(
        ...
        if state["retry_count"] <= max_retries:
            ...
            wait_time = min(2 ** (state["retry_count"] - 1), 30)  # Cap at 30 seconds
            ...
            # Store retry attempt in history
            ...
            # Clear the last error to allow retry
            if state.get("errors"):
                state["last_error_time"] = datetime.now()
                # Store error for analysis but allow retry
                state["metadata"]["last_error"] = (
                    state["errors"][-1] if state["errors"] else None
                )

            # Call the centralized error handler for additional processing
            try:
                state = await self.error_handler.process(state)
            except Exception as e:
                logger.warning(
                    f"Error handler processing failed: {e}, continuing with retry"
                )
            ...
            # Try to call centralized error handler for final processing
            try:
                state = await self.error_handler.process(state)
            except Exception as e:
    ...
    def _route_from_error_handler(self, state: EnhancedComplianceState) -> str:
        ...
        """
        # If workflow failed, go to END
        if state.get("workflow_status") == WorkflowStatus.FAILED:
            return END

        # Otherwise use the recovery node logic
    ...
    async def run(
        ...
        **kwargs,
        ...
        try:
            ...
            yield {
                "type": "summary",
                "conversation": self.state_aggregator.get_conversation_summary(
                    final_state.values
                ),
                "compliance": self.state_aggregator.get_compliance_summary(
                    final_state.values
                ),
                "performance": self.state_aggregator.get_performance_metrics(
                    final_state.values
                ),
            }

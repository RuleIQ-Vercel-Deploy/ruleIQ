        ...
        if use_mock:
            ...
            try:
                response = await assistant.generate_response(
                    prompt=test_prompt, context={"framework": "general"}
                )
...
async def test_evidence_analysis() -> Optional[bool]:
    ...
    try:
        ...
        analysis = await assistant.analyze_evidence(
            evidence=test_evidence, framework_id="SOC2", control_id="CC6.1"
        )
...
async def run_comprehensive_ai_test():
    ...
    results["tests"].append(
        {"test": "ai_model_initialization", "status": "pass" if init_ok else "fail"}
    )

    # Test 3: Basic AI Functionality
    basic_ok = await test_basic_ai_functionality()
    results["tests"].append(
        {"test": "basic_ai_functionality", "status": "pass" if basic_ok else "fail"}
    )

    # Test 4: Assessment AI
    assessment_ok = await test_assessment_ai()
    results["tests"].append(
        {"test": "assessment_ai", "status": "pass" if assessment_ok else "fail"}
    )

    # Test 5: Policy Generation
    policy_ok = await test_policy_generation()
    results["tests"].append(
        {"test": "policy_generation", "status": "pass" if policy_ok else "fail"}
    )

    # Test 6: Evidence Analysis
    evidence_ok = await test_evidence_analysis()
    results["tests"].append(
        {"test": "evidence_analysis", "status": "pass" if evidence_ok else "fail"}
    )

...
async def assign_user_role() -> Optional[bool]:
    ...
    try:
        ...
        # Get user ID
        user_result = await conn.fetchrow("SELECT id FROM users WHERE email = $1", user_email)
        ...
        role_result = await conn.fetchrow("SELECT id FROM rbac_roles WHERE name = 'business_user'")
        ...
        existing_assignment = await conn.fetchrow(
            "SELECT id FROM rbac_user_roles WHERE user_id = $1 AND role_id = $2", user_id, role_id
        )
        ...
        if existing_assignment:
            ...
            # Assign the role
            await conn.execute(
                "INSERT INTO rbac_user_roles (user_id, role_id) VALUES ($1, $2)", user_id, role_id
            )

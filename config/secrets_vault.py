...
class SecretsVault:
    ...
    def __init__(self, backend: str = "auto", region_name: str = "us-east-1", secret_name: str = "ruleiq-production-secrets"):
    ...
    def _init_aws_backend(self):
        ...
        try:
            ...
            logger.warning(f"⚠️ SecretsVault: AWS initialization failed, falling back to environment: {e}")
            self.backend = "env"
            self.aws_client = None
    ...
    def _get_from_aws(self, secret_key: str) -> Optional[str]:
        ...
        try:
            ...
            if secret_key in secrets:
                logger.debug(f"🔐 SecretsVault: Retrieved '{secret_key}' from AWS Secrets Manager")
                return secrets[secret_key]
            else:
                logger.warning(f"⚠️ SecretsVault: '{secret_key}' not found in AWS vault, checking environment")
                return self._get_from_env(secret_key)
    ...
    def _get_from_doppler(self, secret_key: str) -> Optional[str]:
        ...
        if value:
            logger.debug(f"🔐 SecretsVault: Retrieved '{secret_key}' from Doppler environment ({direct_key})")
            return value
        ...
        if value:
            logger.debug(f"🔐 SecretsVault: Retrieved '{secret_key}' from Doppler vault format ({vault_env_key})")
            return value
        ...
        if not doppler_token and not doppler_project:
            logger.warning(f"⚠️ SecretsVault: Doppler not configured, but backend set to 'doppler'")
            logger.info("💡 Configure Doppler: https://docs.doppler.com/docs/vercel")
        
        logger.warning(f"⚠️ SecretsVault: '{secret_key}' not found in Doppler environment")
        return None
    ...
    def _get_from_vercel(self, secret_key: str) -> Optional[str]:
        ...
        if value:
            logger.debug(f"🔐 SecretsVault: Retrieved '{secret_key}' from Vercel environment ({direct_key})")
            return value
        ...
        if value:
            logger.debug(f"🔐 SecretsVault: Retrieved '{secret_key}' from Vercel vault format ({vault_env_key})")
            return value
        
        logger.warning(f"⚠️ SecretsVault: '{secret_key}' not found in Vercel environment")
        return None
    ...
    def _get_from_env(self, secret_key: str) -> Optional[str]:
        ...
        if value:
            logger.debug(f"🔐 SecretsVault: Retrieved '{secret_key}' from environment ({vault_env_key})")
            return value
        ...
        if value:
            logger.debug(f"🔐 SecretsVault: Retrieved '{secret_key}' from direct environment ({direct_key})")
            return value
        
        logger.warning(f"⚠️ SecretsVault: '{secret_key}' not found in vault or environment")
        return None
    ...
    def _get_all_secrets_aws(self) -> Dict[str, str]:
        ...
        try:
            response = self.aws_client.get_secret_value(SecretId=self.secret_name)
            secrets = json.loads(response['SecretString'])
            logger.info(f"🔐 SecretsVault: Retrieved {len(secrets)} secrets from AWS Secrets Manager")
            return secrets
    ...
    def _get_all_secrets_vercel(self) -> Dict[str, str]:
        ...
        logger.info(f"🔐 SecretsVault: Retrieved {len(secrets)} secrets from Vercel environment")
        return secrets
    ...
    def _get_all_secrets_env(self) -> Dict[str, str]:
        ...
        logger.info(f"🔐 SecretsVault: Retrieved {len(secrets)} secrets from local environment")
        return secrets
    ...
    def _health_check_env(self) -> Dict[str, Any]:
        ...
        return {
            "status": "healthy",
            "message": "Using local environment variables"
        }
    ...
    def _rotate_secret_aws(self, secret_key: str, new_value: str) -> bool:
        ...
        try:
            ...
            logger.info(f"✅ SecretsVault: Successfully rotated secret '{secret_key}' in AWS")
            return True
            
        except Exception as e:
            logger.error(f"❌ SecretsVault: Failed to rotate secret '{secret_key}' in AWS: {e}")
            return False
    ...
    def _rotate_secret_doppler(self, secret_key: str, new_value: str) -> bool:
        ...
        """
        logger.warning(f"⚠️ SecretsVault: Doppler secret rotation requested for '{secret_key}'")
    ...
    def _rotate_secret_vercel(self, secret_key: str, new_value: str) -> bool:
        ...
        This method logs the rotation request for manual action.
        """
        logger.warning(f"⚠️ SecretsVault: Vercel secret rotation requested for '{secret_key}'")
    ...
    def _rotate_secret_env(self, secret_key: str, new_value: str) -> bool:
        ...
        This method logs the rotation request for manual action.
        """
        logger.warning(f"⚠️ SecretsVault: Local environment secret rotation requested for '{secret_key}'")
        logger.info("📝 Local environment secrets must be rotated manually:")
        logger.info(f"   1. Update .env file or environment variable: {secret_key.upper()}")

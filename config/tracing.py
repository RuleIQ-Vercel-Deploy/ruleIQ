...
class CustomTracer:
    ...
    def trace_database_operation(
        ...
        return self.start_span(
            f"db.{operation}", attributes=attributes, kind=trace.SpanKind.CLIENT
        )
    ...
    def trace_ai_operation(
        self, operation: str, model: str, tokens: Optional[int] = None
    ):
    ...
    def decorator(func):
        def wrapper(*args, **kwargs):
            ...
            with tracer.trace_external_call(
                service, str(args[0]) if args else "unknown"
            ):
                return func(*args, **kwargs)

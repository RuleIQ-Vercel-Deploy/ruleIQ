...
MODEL_METADATA = {ModelType.GEMINI_25_PRO: ModelMetadata(name=ModelType.
    GEMINI_25_PRO.value, cost_score=8.0, speed_score=6.0, capability_score=
    10.0, max_tokens=8192, timeout_seconds=45.0), ModelType.GEMINI_25_FLASH:
    ModelMetadata(name=ModelType.GEMINI_25_FLASH.value, cost_score=4.0,
    speed_score=9.0, capability_score=8.0, max_tokens=4096, timeout_seconds
    =30.0), ModelType.GEMMA_3: ModelMetadata(name=ModelType.GEMMA_3.value,
    cost_score=1.0, speed_score=8.0, capability_score=5.0, max_tokens=2048,
...
class AIConfig:
    ...
    def __init__(self) ->None:
        ...
        self.generation_config = {'temperature': 0.7, 'top_p': 0.8, 'top_k':
            40, 'max_output_tokens': 4096}
        self.safety_settings = [{'category': 'HARM_CATEGORY_HARASSMENT',
            'threshold': 'BLOCK_ONLY_HIGH'}, {'category':
        ...
        self.retry_config = {'default': {'max_attempts': 3, 'base_delay':
            ...
            'linear_backoff', 'jitter': True}, 'long_operations': {
            'max_attempts': 5, 'base_delay': 2.0, 'max_delay': 120.0,
            'strategy': 'exponential_backoff', 'jitter': True},
            'critical_operations': {'max_attempts': 7, 'base_delay': 1.0,
        ...
        self.offline_config = {'mode': os.getenv('AI_OFFLINE_MODE',
            'enhanced'), 'database_path': os.getenv('AI_OFFLINE_DB_PATH',
            'data/offline_ai.db'), 'cache_ttl_hours': int(os.getenv(
            'AI_OFFLINE_CACHE_TTL', '24')), 'max_history_size': int(os.
            getenv('AI_OFFLINE_HISTORY_SIZE', '1000')),
            'enable_request_queuing': os.getenv('AI_OFFLINE_QUEUE_REQUESTS',
            'true').lower() == 'true'}
    ...
    def get_model(self, model_type: Optional[ModelType]=None,
        ...
        model_params = {'model_name': model_name, 'generation_config': self
    ...
    def get_compliance_optimized_config(self) ->Dict[str, Any]:
        ...
        return {'temperature': 0.3, 'top_p': 0.8, 'top_k': 40,
            'max_output_tokens': 4096}

    def get_creative_config(self) ->Dict[str, Any]:
        """Get AI configuration for more creative content generation"""
        return {'temperature': 0.8, 'top_p': 0.9, 'top_k': 50,
            'max_output_tokens': 2048}

    def get_structured_output_config(self, response_schema: Optional[Dict[
        str, Any]]=None) ->Dict[str, Any]:
        """Get AI configuration optimized for structured JSON output"""
        config = {'temperature': 0.2, 'top_p': 0.8, 'top_k': 40,
    ...
    def get_model_with_schema(self, model_type: Optional[ModelType]=None,
        system_instruction: Optional[str]=None, response_schema_type:
        Optional[str]=None, tools: Optional[List[Dict[str, Any]]]=None
        ...
        if response_schema_type:
            generation_config = self.get_schema_aware_config(
                response_schema_type)
        else:
            generation_config = self.generation_config
        model_params = {'model_name': model_name, 'generation_config':
            generation_config, 'safety_settings': self.safety_settings}
        if system_instruction:
            model_params['system_instruction'] = system_instruction
        if tools:
    ...
    def get_retry_config(self, operation_type: str='default') ->Dict[str, Any]:
        ...
        """
        return self.retry_config.get(operation_type, self.retry_config[
            'default'])
    ...
    def get_model_timeout(self, model_type: ModelType) ->float:
        ...
        return self.circuit_breaker_config['model_timeouts'].get(model_type
            .value, 30.0)
    ...
    def update_retry_config(self, operation_type: str, **kwargs) ->None:
        ...
        if operation_type not in self.retry_config:
            self.retry_config[operation_type] = self.retry_config['default'
                ].copy()
    ...
    def get_model_metadata(self, model_type: ModelType) ->ModelMetadata:
        ...
        return MODEL_METADATA.get(model_type, MODEL_METADATA[ModelType.
            GEMINI_25_FLASH])

    def get_fallback_models(self, preferred_model: Optional[ModelType]=None
        ) ->List[ModelType]:
    ...
    def calculate_task_complexity_score(self, task_context: Dict[str, Any]
        ) ->float:
        """
    ...
    def get_optimal_model(self, task_complexity: str='medium', prefer_speed:
        bool=False, task_context: Optional[Dict[str, Any]]=None) ->ModelType:
        """
        ...
        if task_complexity == 'auto' and task_context:
            complexity_score = self.calculate_task_complexity_score(
                task_context)
            if complexity_score < 0.3:
            ...
            logger.debug('Auto-calculated task complexity: %s (score: %s)' %
                (task_complexity, complexity_score))
        if task_complexity == 'simple':
            selected_model = (ModelType.GEMINI_25_FLASH_LIGHT if
                prefer_speed else ModelType.GEMINI_25_FLASH)
        ...
        logger.info('Model selected: %s for %s task (efficiency: %s)' % (
            selected_model.value, task_complexity, metadata.efficiency_score))
        return selected_model

    def select_model_with_fallback(self, circuit_breaker, preferred_model:
        Optional[ModelType]=None, task_complexity: str='medium') ->ModelType:
        """
        ...
        raise AIServiceException(message=
            'No AI models available - all circuits are open', service_name=
...
def get_ai_model(model_type: Optional[ModelType]=None, task_complexity: str
    ...
    Any]]=None, system_instruction: Optional[str]=None, tools: Optional[
    List[Dict[str, Any]]]=None) ->genai.GenerativeModel:
    """
    ...
    if model_type is None:
        try:
            model_type = ai_config.get_optimal_model(task_complexity,
                prefer_speed, task_context)
        except (ValueError, TypeError):
...
def get_structured_ai_model(response_schema_type: str, model_type: Optional
    ...
    if model_type is None:
        model_type = ai_config.get_optimal_model(task_complexity,
            prefer_speed, task_context)
    return ai_config.get_model_with_schema(model_type=model_type,
...
async def generate_compliance_content(prompt: str, model_type: Optional[
    ...
    ai_config.update_generation_config(**ai_config.
        get_compliance_optimized_config())
    try:

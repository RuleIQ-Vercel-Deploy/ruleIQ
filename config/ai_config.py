...
class AIConfig:
    ...
    def __init__(self):
        ...
        self.offline_config = {
            ...
            "enable_request_queuing": os.getenv("AI_OFFLINE_QUEUE_REQUESTS", "true").lower() == "true"
        }
    ...
    def get_structured_output_config(self, response_schema: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    def get_model_timeout(self, model_type: ModelType) -> float:
        ...
        return self.circuit_breaker_config["model_timeouts"].get(
            model_type.value, 30.0  # Default timeout
        )
    ...
    def get_optimal_model(self, task_complexity: str = "medium", prefer_speed: bool = False, task_context: Optional[Dict[str, Any]] = None) -> ModelType:
        ...
        if task_complexity == "auto" and task_context:
            ...
            logger.debug(f"Auto-calculated task complexity: {task_complexity} (score: {complexity_score:.2f})")
        
        # Model selection decision tree
        if task_complexity == "simple":
            selected_model = ModelType.GEMINI_25_FLASH_LIGHT if prefer_speed else ModelType.GEMINI_25_FLASH
        ...
        logger.info(f"Model selected: {selected_model.value} for {task_complexity} task (efficiency: {metadata.efficiency_score:.2f})")

...
class StateTransitionTracker:
    ...
    def __init__(self, max_history: int = 5000):
        ...
        self._current_states: Dict[str, Tuple[str, float]] = {}  # workflow_id -> (state, start_time)
    ...
    def define_state_machine(self, state_machine: Dict[str, Any]):
        ...
        is_simple_format = all(
            isinstance(v, list) for v in state_machine.values()
        )
...
class MemoryUsageTracker:
    ...
    def track_component(self, component_name: str, size_bytes: int):
        ...
        if len(self._component_memory[component_name]) > 100:
            self._component_memory[component_name] = \
                self._component_memory[component_name][-100:]
        ...
        for (error_type, component), errors in error_groups.items():
            if len(errors) >= 3:  # Minimum for a pattern
                ...
                if intervals:
                    ...
                    if std_interval < avg_interval * 0.3:  # Low variance indicates pattern
                        patterns.append({
                            "error_type": error_type,
                            "component": component,
...
class LangGraphMetricsCollector:
    ...
    async def start_workflow(self, workflow_id: str, workflow_name: str, metadata: Optional[Dict[str, Any]] = None) -> None:
        """
    ...
    async def track_memory_usage(
        ...
        Args:
            ...
            metadata: Optional metadata
        """
        await self.memory_tracker.track_usage(
            node_name=node_name,
            memory_mb=memory_mb
        )

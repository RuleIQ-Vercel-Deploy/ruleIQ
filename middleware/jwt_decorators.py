            ...
            async def wrapper(*args, **kwargs):
                ...
                if current_user:
                    ...
                    if user_role not in allowed_roles:
                        raise HTTPException(
                            status_code=status.HTTP_403_FORBIDDEN, detail=f"Role required: {', '.join(allowed_roles)}"
                        )
                return await func(*args, **kwargs)
...
async def get_current_user_optional(
    credentials: Optional[HTTPAuthorizationCredentials] = Depends(security), db=Depends(get_async_db)
) -> Optional[User]:
    """

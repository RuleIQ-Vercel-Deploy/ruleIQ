...
class JWTAuthMiddleware:
    ...
    def __init__(
        ...
        self.protected_patterns = [re.compile(pattern) for pattern in self.CRITICAL_PROTECTED_ROUTES]
        if custom_protected_paths:
            self.protected_patterns.extend([re.compile(pattern) for pattern in custom_protected_paths])
    ...
    async def __call__(self, request: Request, call_next):
        ...
        if not payload:
            await self.log_auth_event(request, "INVALID_TOKEN", False, details={"reason": "Token validation failed"})
            return JSONResponse(
                status_code=status.HTTP_401_UNAUTHORIZED,
        ...
        await self.log_auth_event(request, "AUTHENTICATION_SUCCESS", True, user_id=request.state.user_id)

...
async def _update_all_compliance_scores_async() -> Dict[str, Any]:
    ...
    async for db in get_async_db():
        try:
            ...
            return {'status': 'completed', 'total_profiles': total_profiles, 'updated_count': updated_count, 'failed_count': failed_count}
...
@celery_app.task(bind=True, autoretry_for=(DatabaseException, Exception), retry_kwargs={'max_retries': 5, 'countdown': 90}, retry_backoff=True, retry_backoff_max=600, retry_jitter=True, rate_limit='3/m')
def update_all_compliance_scores(self) -> Any:
...
def check_compliance_alerts(self) -> Any:

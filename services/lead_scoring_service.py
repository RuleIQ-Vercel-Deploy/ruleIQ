...
class LeadScoringService:
    ...
    def __init__(self, db_session: Session):
        ...
        self.SCORING_RULES = {
            ...
            "repeat_visit": {"base_score": 8, "category": "engagement"},
            "referral_shared": {"base_score": 20, "category": "advocacy"},
        }
        ...
        self.LEAD_QUALIFICATION_THRESHOLDS = {
            ...
            "hot": 100,
            "qualified": 150,
            "mql": 200  # Marketing Qualified Lead
        }
    ...
    async def calculate_lead_score(self, lead_id: uuid.UUID) -> Dict[str, Any]:
        ...
        try:
            ...
            events = self.db.query(LeadScoringEvent).filter_by(lead_id=lead_id).order_by(
                desc(LeadScoringEvent.event_timestamp)
            ).all()
            ...
            result = {
                ...
                "last_activity": events[0].event_timestamp if events else None,
    ...
    async def get_lead_analytics(
        ...
        try:
            ...
            lead = self.db.query(AssessmentLead).filter_by(id=lead_id).first()
            if not lead:
                raise ValueError(f"Lead not found: {lead_id}")
            
            # Get events within time window
            events = self.db.query(LeadScoringEvent).filter(
                and_(
                    LeadScoringEvent.lead_id == lead_id,
                    LeadScoringEvent.event_timestamp >= cutoff_date
                )
            ).order_by(desc(LeadScoringEvent.event_timestamp)).all()
            
            # Get assessment sessions
            sessions = self.db.query(FreemiumAssessmentSession).filter_by(
                lead_id=lead_id
            ).order_by(desc(FreumiumAssessmentSession.created_at)).all()
            ...
            analytics = {
                ...
                "activity_summary": {
                    ...
                    "days_active": len(set(e.event_timestamp.date() for e in events))
                },
    ...
    async def get_cohort_analytics(
        ...
        try:
            ...
            leads = self.db.query(AssessmentLead).filter(
                and_(
            ...
            events = self.db.query(LeadScoringEvent).filter(
                LeadScoringEvent.lead_id.in_(lead_ids)
            ).order_by(LeadScoringEvent.event_timestamp).all()
            
            # Get conversion events
            conversions = self.db.query(ConversionEvent).filter(
                ConversionEvent.lead_id.in_(lead_ids)
            ).all()
    ...
    def _apply_time_decay(self, events: List[LeadScoringEvent], total_score: int) -> int:
        ...
        for event in events:
            days_ago = (now - event.event_timestamp).days
    ...
    def _calculate_conversion_probability(
        ...
        unique_days = len(set(e.event_timestamp.date() for e in events))
        if unique_days > 3:
            probability *= 1.2
    ...
    def _calculate_engagement_metrics(self, events: List[LeadScoringEvent]) -> Dict[str, Any]:
        ...
        now = datetime.utcnow()
        event_dates = [e.event_timestamp for e in events]
        ...
        return {
            ...
            "engagement_consistency": len(set(e.event_timestamp.date() for e in events)),
            "peak_activity_hour": max(set(e.event_timestamp.hour for e in events), key=lambda h: sum(1 for e in events if e.event_timestamp.hour == h)),
    ...
    def _analyze_engagement_patterns(self, events: List[LeadScoringEvent]) -> Dict[str, Any]:
        ...
        return {
            ...
            "engagement_trend": "increasing" if len(events) > 5 and events[0].event_timestamp > events[-1].event_timestamp else "stable"
        }
    ...
    def _generate_behavioral_insights(
        ...
        # Time-based insights
        if events:
            hours = [e.event_timestamp.hour for e in events]
    ...
    def _suggest_next_actions(
        self,
        events: List[LeadScoringEvent],
        sessions: List[FreumiumAssessmentSession],
        lead: AssessmentLead
    ) -> List[Dict[str, str]]:
        ...
        if events and (datetime.utcnow() - events[0].event_timestamp).days > 7:
            actions.append({
    ...
    def _calculate_retention_metrics(
        ...
        for event in events:
            ...
            week = event.event_timestamp.isocalendar()[1]

...
class LeadScoringService:
    ...
    async def track_event(
        ...
        try:
            ...
            logger.info(
                f"Event tracked successfully: {event.id}, score impact: {score_impact}"
            )
            return event
    ...
    async def calculate_lead_score(self, lead_id: uuid.UUID) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Score calculated: {total_score} ({lead_status}) for lead {lead_id}"
            )
            return result
    ...
    async def get_lead_analytics(
        self, lead_id: uuid.UUID, days_back: int = 30
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            analytics = {
                ...
                "conversion_journey": self._analyze_conversion_journey(
                    events, sessions
                ),
    ...
    def _apply_time_decay(
        self, events: List[LeadScoringEvent], total_score: int
    ) -> int:
    ...
    def _calculate_engagement_metrics(
        self, events: List[LeadScoringEvent]
    ) -> Dict[str, Any]:
    ...
    def _analyze_engagement_patterns(
        ...
        return {
            "most_active_day": (
                max(daily_events.keys(), key=lambda d: len(daily_events[d]))
                if daily_events
                else None
            ),
            ...
            "engagement_trend": (
                "increasing"
                if len(events) > 5 and events[0].created_at > events[-1].created_at
                else "stable"
            ),
        }
    ...
    def _analyze_conversion_journey(
        ...
        journey_stages = {
            "awareness": len(
                [e for e in events if e.event_type in ["lead_capture", "email_open"]]
            ),
            "interest": len(
                [
                    e
                    for e in events
                    if e.event_type in ["assessment_start", "pricing_viewed"]
                ]
            ),
            "consideration": len(
                [
                    e
                    for e in events
                    if e.event_type in ["assessment_complete", "results_viewed"]
                ]
            ),
            "intent": len(
                [
                    e
                    for e in events
                    if e.event_type in ["demo_requested", "consultation_booked"]
                ]
            ),
            "conversion": len(
        ...
        return {
            ...
            "current_stage": (
                max(journey_stages.keys(), key=lambda k: journey_stages[k])
            ...
            "assessment_funnel": {
                ...
                "completion_rate": (
                    len([s for s in sessions if s.completion_status == "completed"])
                    / len(sessions)
                    if sessions
                    else 0
                ),
                "avg_questions_answered": (
                    sum(s.questions_answered for s in sessions) / len(sessions)
                    if sessions
                    else 0
                ),
            },
    ...
    def _generate_behavioral_insights(
        ...
        if events:
            ...
            if (
                max(set(hours), key=hours.count) < 9
                or max(set(hours), key=hours.count) > 17
            ):
    ...
    def _suggest_next_actions(
        ...
        if completed_sessions and not any(
            e.event_type == "results_viewed" for e in events
        ):
            actions.append(
                {
    ...
    def _group_conversions_by_type(
    ...
    def _calculate_avg_conversion_time(
        ...
        for conversion in conversions:
            if conversion.lead_id in lead_dict:
                ...
                time_to_convert = (
                    conversion.created_at - lead_created
                ).total_seconds() / 3600  # Hours
                conversion_times.append(time_to_convert)

        return (
            sum(conversion_times) / len(conversion_times) if conversion_times else None
        )

    def _calculate_lead_status_distribution(
        self, leads: List[AssessmentLead]
    ) -> Dict[str, int]:
    ...
    def _calculate_retention_metrics(
        ...
        for week_num in range(1, 53):  # 52 weeks
            active_leads = sum(
                1 for weeks in lead_weekly_activity.values() if week_num in weeks
            )

...
class SessionManager:
    ...
    async def revoke_all_user_sessions(self, user_id: str, except_current: Optional[str] = None) -> int:
        """
    ...
    async def get_user_sessions(self, user_id: str, include_expired: bool = False) -> List[Dict[str, Any]]:
        """
    ...
    async def _store_session_redis(self, session_id: str, user_id: str, session_data: Dict[str, Any]) -> None:
        """Store session in Redis"""
        if not self.redis_client:
            return
        session_key = f"{self.session_prefix}{session_id}"
        await self.redis_client.setex(session_key, int(self.session_timeout.total_seconds()), json.dumps(session_data))
        user_sessions_key = f"{self.user_sessions_prefix}{user_id}"
    ...
    async def _track_device(self, user_id: str, device_fingerprint: str, device_info: Dict[str, Any]) -> None:
        """Track device usage"""
        if not self.redis_client:
            return
    ...
    async def detect_suspicious_activity(self, user_id: str, request: Request) -> List[str]:
        ...
        recent_sessions = [
            s
            for s in sessions
            if (datetime.now(timezone.utc) - datetime.fromisoformat(s["created_at"])).total_seconds()
...
async def validate_session_dependency(request: Request, session_id: Optional[str] = None) -> Dict[str, Any]:
    """

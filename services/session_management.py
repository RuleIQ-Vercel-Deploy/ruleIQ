...
class SessionManager:
    ...
    async def validate_session(
        ...
        # Additional validation if request provided
        if request:
            ...
            if settings.strict_session_validation:
                if request.client and request.client.host != session_data["ip_address"]:
                    logger.warning(f"Session {session_id} IP mismatch")
            ...
            if self.enable_device_tracking:
                ...
                if current_fingerprint != session_data["device_fingerprint"]:
                    logger.warning(f"Session {session_id} device fingerprint mismatch")
                    session_data["status"] = SessionStatus.SUSPICIOUS.value
                    await self._update_session(session_id, session_data)
    ...
    async def revoke_session(self, session_id: str, reason: str = "manual") -> bool:
        ...
        await self.redis_client.setex(
            session_key,
            60,  # Keep revoked session for 1 minute for logging
    ...
    def _generate_device_fingerprint(self, device_info: Dict[str, Any]) -> str:
        ...
        # Create a stable fingerprint from device information
    ...
    def _extract_location_info(self, request: Request) -> Dict[str, Any]:
        ...
        # Basic implementation - can be enhanced with GeoIP services
        location_info = {
    ...
    async def _check_concurrent_sessions(self, user_id: str) -> None:
        ...
        if active_count >= self.max_concurrent_sessions:
            # Get oldest session and revoke it
    ...
    async def _store_session_redis(
        ...
        await self.redis_client.expire(
            user_sessions_key, int(self.session_timeout.total_seconds())
        )
    ...
    async def _track_device(
        ...
        await self.redis_client.setex(
            device_key,
            86400 * 30,  # Keep device data for 30 days
    ...
    async def detect_suspicious_activity(
        ...
        recent_sessions = [
            ...
            for s in sessions
            if (
                datetime.utcnow() - datetime.fromisoformat(s["created_at"])
            ).total_seconds()
            < 300
        ]
    ...
    def _is_tor_exit_node(self, ip: str) -> bool:
        ...
        # This would need a real TOR exit node list in production
        # For now, just a placeholder
        tor_ranges = [
            "198.96.155.3",  # Example TOR exit node
        ]
    ...
    if _session_manager is None:
        ...
        if settings.redis_url:
            try:
                ...
                logger.warning(
                    f"Failed to connect to Redis for session management: {e}"
                )
                redis_client = None
...
async def validate_session_dependency(
    ...
    if not session_id:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED, detail="No session provided"
        )

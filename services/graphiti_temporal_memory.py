...
class GraphitiTemporalMemory:
    ...
    async def query_at_time(self, timestamp: datetime, entity_type: Optional[str] = None) -> List[TemporalNode]:
    ...
    async def detect_temporal_patterns(
        ...
        if len(timeline) > 10:
            recent_rate = len([c for c in timeline if datetime.fromisoformat(c["valid_from"]) > datetime.now() - timedelta(days=30)])
            older_rate = len([c for c in timeline if datetime.fromisoformat(c["valid_from"]) <= datetime.now() - timedelta(days=30)])
    ...
    def _dict_to_temporal_node(self, data: Dict) -> TemporalNode:
        ...
        return TemporalNode(
            ...
            valid_from=datetime.fromisoformat(data["valid_from"]) if isinstance(data["valid_from"], str) else data["valid_from"],
            valid_to=datetime.fromisoformat(data["valid_to"]) if data.get("valid_to") and isinstance(data["valid_to"], str) else data.get("valid_to"),

...
class CommunicationProtocol:
    ...
    async def send_message(self, message: Message, wait_for_ack: bool = False) -> Optional[UUID]:
        ...
        try:
            ...
            if wait_for_ack:
                try:
                    await asyncio.wait_for(self.acknowledgments[message.message_id].wait(), timeout=self.timeout)
    ...
    async def receive_message(self, agent_id: UUID, timeout: Optional[float] = None) -> Optional[Message]:
    ...
    async def request_response(
        self, from_agent: UUID, to_agent: UUID, request_payload: Dict[str, Any], timeout: float = 30.0
    ...
    async def broadcast_notification(
        self, from_agent: UUID, notification: Dict[str, Any], priority: MessagePriority = MessagePriority.NORMAL
    ) -> bool:
    ...
    async def handoff_session(self, from_agent: UUID, to_agent: UUID, session_data: Dict[str, Any]) -> bool:
    ...
    async def sync_agents(self, agent_ids: List[UUID], sync_data: Dict[str, Any]) -> Dict[UUID, bool]:

...
class TrustProgressionAlgorithm:
    ...
    # Scoring weights
    SCORE_WEIGHTS = {"approval_rate": 0.40, "success_rate": 0.30, "consistency": 0.20, "complexity": 0.10}
    ...
    def _calculate_approval_rate(self) -> float:
        ...
        recent_approvals = [m.value for m in self.metrics_history if m.metric_type == MetricType.APPROVAL_RATE][
            -100:
        ]  # Last 100 actions
    ...
    def _calculate_success_rate(self) -> float:
        ...
        recent_successes = [m.value for m in self.metrics_history if m.metric_type == MetricType.SUCCESS_RATE][-100:]
    ...
    def _calculate_complexity_score(self) -> float:
        ...
        complexity_metrics = [m.value for m in self.metrics_history if m.metric_type == MetricType.COMPLEXITY_SCORE][
            -50:
        ]
        ...
        if complex_actions:
            complex_success_rate = sum(1 for a in complex_actions if a.get("successful", False)) / len(complex_actions)
        else:
    ...
    async def check_promotion_eligibility(self) -> Tuple[bool, Optional[TrustLevel], List[str]]:
        ...
        if self.last_promotion_date:
            ...
            if days_since_promotion < thresholds.cooldown_days:
                reasons.append(f"Cooldown period: {thresholds.cooldown_days - days_since_promotion} days remaining")

        # Check trust score
        current_score = self.calculate_trust_score()
        if current_score.overall_score < thresholds.min_score:
            reasons.append(f"Trust score too low: {current_score.overall_score:.1f} < {thresholds.min_score}")
        ...
        recent_violations = [
            v for v in self.violation_history if (datetime.now(timezone.utc) - v["timestamp"]).days < 30
        ]
    ...
    async def promote_trust_level(self, authorized_by: str, reason: str, is_override: bool = False) -> Dict[str, Any]:
        """
        ...
        if not is_override:
            ...
            if not eligible:
                return {"success": False, "current_level": self.current_trust_level.name, "reasons": reasons}
        else:
    ...
    async def demote_trust_level(
        ...
        demotion_levels = {"low": 0, "medium": 1, "high": 2, "critical": 3}  # Warning only  # Back to L0
        ...
        if levels_to_demote == 0:
            ...
            return {"success": True, "action": "warning", "current_level": self.current_trust_level.name}
        ...
        logger.warning(f"User {self.user_id} demoted from {old_level.name} to {new_level.name} - {reason}")
    ...
    async def _handle_anomaly(self, action_record: Dict[str, Any]) -> None:
        ...
        if len(recent_anomalies) >= 3:
            await self.demote_trust_level(reason="Multiple anomalies detected in behavior", severity="medium")
    ...
    def get_audit_trail(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "recent_actions": len(
                [a for a in self.action_history if (datetime.now(timezone.utc) - a["timestamp"]).days < 7]
            ),
        }

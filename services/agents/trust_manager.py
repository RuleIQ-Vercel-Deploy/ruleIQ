...
class TrustManager:
    ...
    def __init__(self, db_session: Session, rules: Optional[TrustProgressionRules] = None):
    ...
    def calculate_trust_metrics(
        self, agent_id: UUID, session_id: Optional[UUID] = None, time_window: timedelta = timedelta(days=7)
    ) -> TrustMetric:
        ...
        try:
            ...
            query = self.db.query(Decision).filter(Decision.agent_id == agent_id, Decision.created_at >= cutoff_time)
            ...
            if agent and agent.config:
                current_trust_level = TrustLevel(agent.config.get("trust_level", TrustLevel.L0_OBSERVED.value))
    ...
    def evaluate_trust_progression(
        ...
        try:
            ...
            # Check if enough decisions for evaluation
                ...
                reason = (
                    f"Insufficient decisions ({trust_metric.total_decisions}/{self.rules.min_decisions_for_evaluation})"
                )
            ...
            # Check for upgrade conditions
                ...
                if trust_metric.confidence_avg >= self.rules.min_confidence_for_decisions:
                    ...
                    reason = f"High accuracy ({
                        trust_metric.accuracy_rate:.2%}) and confidence ({
                        trust_metric.confidence_avg:.2f})"
                else:
    ...
    def apply_trust_transition(self, agent_id: UUID, force_evaluation: bool = False) -> Tuple[bool, str]:
        ...
        try:
            ...
            current_level = TrustLevel(agent.config.get("trust_level", TrustLevel.L0_OBSERVED.value))
    ...
    def apply_degradation(self, agent_id: UUID, levels_to_drop: int = 1, reason: str = "Manual degradation") -> bool:
    ...
    def get_required_approvals(self, trust_level: TrustLevel, action_type: str) -> Dict[str, Any]:
        ...
        approval_matrix = {
            TrustLevel.L0_OBSERVED: {"all_actions": True, "requires_human": True, "auto_approve": []},
            TrustLevel.L1_ASSISTED: {"all_actions": False, "requires_human": True, "auto_approve": ["read", "suggest"]},
            TrustLevel.L2_SUPERVISED: {
                "all_actions": False,
                "requires_human": True,
            ...
            TrustLevel.L4_AUTONOMOUS: {"all_actions": False, "requires_human": False, "auto_approve": ["all"]},
        }
        ...
        return {
            ...
            "requires_approval": (
                rules["all_actions"] or action_type not in rules["auto_approve"] and "all" not in rules["auto_approve"]
            ),

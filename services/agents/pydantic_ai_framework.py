...
class BaseComplianceAgent:
    ...
    def __init__(self, trust_level: int, rag_system: Optional[AgenticRAGSystem] = None) -> None:
        ...
        self.agent = Agent(
            "gemini-1.5-pro", result_type=ComplianceAgentResponse, system_prompt=self._build_system_prompt()
        )
...
{trust_level_specifics.get(self.trust_level, '')}"""
    ...
    async def _log_interaction(self, request: str, response: ComplianceAgentResponse, context: AgentContext) -> None:
...
class AgentOrchestrator:
    ...
    async def route_request(self, request: str, context: AgentContext) -> ComplianceAgentResponse:
        ...
        try:
            ...
            logger.info("Routing request to %s agent (trust level %s)" % (agent_type, self.trust_level))
    ...
    def _classify_request(self, request: str) -> str:
        ...
        scores = {"gdpr": gdpr_score, "companies_house": companies_score, "employment": employment_score}
...
def create_agent_orchestrator(trust_level: int, rag_system: Optional[AgenticRAGSystem] = None) -> AgentOrchestrator:

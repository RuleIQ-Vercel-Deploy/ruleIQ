    ...
    async def get_cache_metrics(self) -> CacheMetrics:
        ...
        try:
            if (
                ...
                miss_rate = (
                    keyspace_misses / total_requests if total_requests > 0 else 0
                )
                ...
                return CacheMetrics(
                    ...
                    miss_rate=miss_rate,
                    eviction_rate=(
                        info.get("evicted_keys", 0) / total_requests
                        if total_requests > 0
                        else 0
                    ),
                ...
                return CacheMetrics(
                    hit_rate=0.8,  # Estimated
                    miss_rate=0.2,
                    eviction_rate=0.0,
                    memory_usage=len(self.cache_manager.memory_cache)
                    * 1024,  # Estimate
    ...
    async def get_api_metrics(self) -> APIMetrics:
        ...
        return APIMetrics(
            ...
            p99_response_time=sorted(all_response_times)[
                int(len(all_response_times) * 0.99)
            ],
            ...
            recommendations.append(
                {
                    ...
                    "suggested_value": int(
                        metrics["database"]["connection_pool_size"] * 1.5
                    ),
    ...
    async def start_monitoring(self, interval: int = 60) -> None:
        ...
        while self.monitoring_active:
            try:
                ...
                await self.cache_manager.set(
                    "performance_metrics:latest", metrics, ttl=300
                )

                # Log performance alerts
                if metrics["performance_score"] < 70:
                    logger.warning(
                        f"Performance score low: {metrics['performance_score']:.1f}/100"
                    )
                ...
                if (
                    len(self.performance_history) > 1440
    ...
    async def get_performance_trends(self, hours: int = 24) -> Dict[str, Any]:
        ...
        return {
            ...
            "trends": {
                ...
                "database_query_time": {
                    "avg": statistics.mean(
                        [m.database_query_time for m in recent_metrics]
                    ),

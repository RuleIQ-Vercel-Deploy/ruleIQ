...
class ComplianceRetrievalQueries:
    ...
    async def get_regulatory_coverage_analysis(
        ...
        """
        result = await self.neo4j.execute_query(query, {"domain": domain_name, "jurisdiction": jurisdiction})
        coverage_data = []
        domain_coverage = {}
            ...
            coverage_data.append(
                {
                    ...
                    "regulation": {"code": record["regulation_code"], "name": record["regulation_name"]},
        ...
        overall_coverage = (round(sum(domain_coverage.values()) / len(domain_coverage), 3) if domain_coverage else 0.0,)
        return QueryResult(
    ...
    async def analyze_compliance_gaps(self, business_functions: Optional[List[str]] = None) -> QueryResult:
        ...
        for record in result:
            ...
            severity_multiplier = {"critical": 4, "high": 3, "medium": 2, "low": 1}.get(risk_level, 1)
            ...
            gap_data.append(
                {
                    ...
                    "domain": {"name": record["domain_name"], "business_impact": record["domain_impact"]},
                    "risk_assessment": {
                    ...
                    "priority_level": (
                        "critical" if gap_severity >= 8 else "high" if gap_severity >= DEFAULT_RETRIES else "medium",
                    ),
                }
            )
        return QueryResult(
    ...
    async def analyze_temporal_regulatory_changes(
        ...
        result = await self.neo4j.execute_query(
            query,
            {"lookback_date": lookback_date.strftime("%Y-%m-%d"), "forecast_date": forecast_date.strftime("%Y-%m-%d")},
        )
        temporal_data = []
        recent_changes = 0
        upcoming_reviews = 0
        ...
        return QueryResult(
            ...
            data=temporal_data,
            metadata={
                "analysis_period": {"lookback_months": lookback_months, "forecast_months": forecast_months},
                "change_summary": {
    ...
    async def analyze_enforcement_learning_patterns(self, violation_types: Optional[List[str]] = None) -> QueryResult:

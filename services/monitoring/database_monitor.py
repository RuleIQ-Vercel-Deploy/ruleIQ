...
class DatabaseMonitor:
        ...
        if metrics.utilization_percent >= self.thresholds["pool_utilization_critical"]:
            self._create_alert(
                ...
                self.thresholds["pool_utilization_critical"],
                 f"Critical pool utilization: {metrics.utilization_percent:.1f}% ({metrics.total_connections}/{metrics.pool_size} connections)",)elif metrics.utilization_percent >= self.thresholds["pool_utilization_warning"]:self._create_alert(AlertLevel.WARNING,"pool_utilization",metrics.utilization_percent,self.thresholds["pool_utilization_warning"],f"High pool utilization: {metrics.utilization_percent:.1f}% ({metrics.total_connections}/{metrics.pool_size} connections)",)if metrics.overflow >= self.thresholds["overflow_critical"]:self._create_alert(AlertLevel.CRITICAL,"pool_overflow",metrics.overflow,self.thresholds["overflow_critical"],f"Critical pool overflow: {metrics.overflow} overflow connections",)elif metrics.overflow >= self.thresholds["overflow_warning"]:self._create_alert(AlertLevel.WARNING,"pool_overflow",metrics.overflow,self.thresholds["overflow_warning"],f"Pool overflow detected: {metrics.overflow} overflow connections",)def _check_session_alerts(self, metrics: SessionMetrics) -> None:"""Check session metrics against thresholds and create alerts"""if not metrics or not self.thresholds:returnif metrics.long_running_sessions >= self.thresholds["long_running_session_critical"]:self._create_alert(AlertLevel.CRITICAL,"long_running_sessions",metrics.long_running_sessions,self.thresholds["long_running_session_critical"],f"Long-running sessions detected: {metrics.long_running_sessions} sessions running longer than {self.thresholds['long_running_session_warning']}s",)elif metrics.long_running_sessions >= self.thresholds["long_running_session_warning"]:self._create_alert(AlertLevel.WARNING,"long_running_sessions",metrics.long_running_sessions,self.thresholds["long_running_session_warning"],f"Potential session leak: {metrics.active_sessions} active sessions",)def _create_alert(self, level: AlertLevel, metric: str, value: float, threshold: float, message: str) -> None:"""Create and store an alert."""alert = DatabaseAlert(timestamp=datetime.now(timezone.utc),level=level,metric=metric,value=value,threshold=threshold,message=message,)self.alerts.append(alert)log_method = {AlertLevel.INFO: logger.info,AlertLevel.WARNING: logger.warning,AlertLevel.CRITICAL: logger.error,}[level]log_method(f"Database Alert [{level.value.upper()}]: {message}")
    ...
    def _cleanup_old_metrics(self) -> None:
        ...
        self.session_metrics_history = [m for m in self.session_metrics_history if m.timestamp > cutoff]

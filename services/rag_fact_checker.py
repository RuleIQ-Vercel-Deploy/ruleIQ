...
class RAGFactChecker:
    ...
    async def comprehensive_fact_check(
        ...
        try:
            ...
            self_critiques = await self._perform_self_criticism(response_text, sources, original_query)
            source_quality_score = await self._assess_source_quality(sources)
            overall_score = self._calculate_overall_score(fact_check_results, self_critiques, source_quality_score)
    ...
    async def _fact_check_claim(self, claim: str, sources: List[Dict[str, Any]], full_response: str) -> FactCheckResult:
...
{source.get('content', '')[:1000]}"""
    ...
    def _calculate_overall_score(
        self, fact_check_results: List[FactCheckResult], self_critiques: List[SelfCritique], source_quality_score: float
    ) -> float:
        """Calculate overall quality score"""
        weights = {"fact_accuracy": 0.4, "self_critique": 0.4, "source_quality": 0.2}
        if fact_check_results:
            fact_score = sum(1.0 if result.is_factual else 0.3 for result in fact_check_results) / len(
                fact_check_results
            )
        else:
            fact_score = 0.8

...
class RAGFactChecker:
            ...
            if self.openai_client:
                ...
                try:
                    ...
                    return FactCheckResult(
                        ...
                        confidence=FactCheckConfidence(
                            result_data.get("confidence", "uncertain")
                        ),
    ...
    def _create_fallback_fact_check(self, claim: str) -> FactCheckResult:
        ...
        return FactCheckResult(
            claim=claim,
            is_factual=True,  # Conservative: assume true if can't verify
        ...
        try:
            ...
            if self.openai_client:
                ...
                try:
                    ...
                    logger.warning(
                        f"Failed to parse criticism result for {aspect}: {e}"
                    )
    ...
    async def _generate_recommendations(
        ...
        uncertain_claims = [
            ...
            if r.confidence == FactCheckConfidence.UNCERTAIN
        ]
        if uncertain_claims:
            recommendations.append(
    ...
    def _identify_flagged_issues(
        ...
        critical_critiques = [
            c for c in self_critiques if c.severity == CriticSeverity.CRITICAL
        ]
        ...
        high_severity = [
            ...
            if c.severity == CriticSeverity.HIGH and c.score < 0.3
        ]
        if high_severity:
            flagged_issues.append(
    ...
    async def quick_fact_check(
        ...
        try:
            ...
            for claim in claims[:3]:
                result = await self._fact_check_claim(claim, sources, response_text)
                if (
                    not result.is_factual
                    or result.confidence == FactCheckConfidence.UNCERTAIN
                ):
                    suspicious_count += 1

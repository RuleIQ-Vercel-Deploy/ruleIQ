...
class GraphRAGResearchEngine:
    ...
    async def conduct_research(self, query: ResearchQuery, use_cache: bool = True) -> ResearchResult:
        """
    ...
    def _build_cypher_query(self, query: ResearchQuery) -> str:
        ...
        return query_templates.get(query.research_type, query_templates[ResearchType.OBLIGATION_EXTRACTION])

    def _generate_research_prompts(self, query: ResearchQuery, graph_context: Dict[str, Any]) -> List[str]:
    ...
    async def _synthesize_findings(
        ...
        synthesis_result = await self.llm_client.agenerate([synthesis_prompt.format(**synthesis_context)])
        try:
            return json.loads(synthesis_result.generations[0][0].text)
        except json.JSONDecodeError:
            return {"synthesis": synthesis_result.generations[0][0].text}

    def _structure_results(self, synthesized: Dict[str, Any], query: ResearchQuery) -> ResearchResult:

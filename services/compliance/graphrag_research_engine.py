...
class GraphRAGResearchEngine:
    ...
    async def conduct_research(
        ...
        try:
            ...
            research_result = self._structure_results(
                synthesized_results, query
            )
    ...
    def _generate_research_prompts(
        ...
        prompt_template = self.RESEARCH_PROMPTS.get(
            ...
            self.RESEARCH_PROMPTS["obligation_extraction"]
        )
    ...
    async def _execute_llm_research(
        self,
        prompts: List[str]
    ) -> List[Dict[str, Any]]:
        ...
        for i in range(0, len(prompts), batch_size):
            ...
            tasks = [
                self.llm_client.agenerate([prompt])
                for prompt in batch
            ]

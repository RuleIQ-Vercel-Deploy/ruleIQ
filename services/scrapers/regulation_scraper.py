...
class RegulationScraper:
            ...
            try:
                ...
                scraped_data = {
                    ...
                    "raw_markdown": result.markdown[
                        :5000
                    ],  # Store first 5000 chars for analysis
                }
                ...
                logger.info(
                    f"Successfully scraped {regulation_id}: {scraped_data['content_length']} chars"
                )
                return scraped_data
    ...
    def _extract_penalties(self, content: str) -> List[Dict[str, Any]]:
        ...
        if re.search(r"imprison(?:ment|ed)", content, re.IGNORECASE):
            penalties.append(
                {"type": "criminal", "description": "Potential imprisonment"}
            )
    ...
    async def scrape_all_regulations(self, batch_size: int = 5):
        ...
        for i in range(0, len(all_regulations), batch_size):
            ...
            logger.info(
                f"Processing batch {i//batch_size + 1}/{(len(all_regulations)-1)//batch_size + 1}"
            )
        ...
        logger.info(
            f"Scraping complete: {successful}/{len(all_regulations)} successful"
        )
        return report

    async def _update_neo4j_with_scraped_data(
    ...
    try:
        ...
        for reg_id, url in test_urls[:1]:  # Test first one
            ...
            if result["success"]:
                ...
                logger.info(
                    f"  Requirements found: {len(result['extracted_requirements'])}"
                )
                logger.info(f"  Penalties found: {len(result['extracted_penalties'])}")
                logger.info(
                    f"  Controls identified: {len(result['extracted_controls'])}"
                )

...
class RegulationScraper:
        ...
        for attempt in range(retry_count):
            try:
                ...
                scraped_data = {
                    ...
                    "content_hash": hashlib.md5(result.markdown.encode(, usedforsecurity=False)).hexdigest(),
                ...
                logger.info("Successfully scraped %s: %s chars" % (regulation_id, scraped_data["content_length"]))
                return scraped_data
    ...
    async def scrape_all_regulations(self, batch_size: int = 5) -> Any:
                    ...
                    for item in data["items"]:
                        if "url" in item and item["url"]:
                            all_regulations.append(
                                {"id": item["id"], "url": item["url"], "title": item.get("title", "Unknown")}
                            )
        ...
        for i in range(0, len(all_regulations), batch_size):
            ...
            logger.info("Processing batch %s/%s" % (i // batch_size + 1, (len(all_regulations) - 1) // batch_size + 1))
            tasks = []
            for reg in batch:

...
async def get_all_frameworks(db: AsyncSession) ->List[ComplianceFramework]:
    ...
    try:
        result = await db.execute(select(ComplianceFramework).where(
            ComplianceFramework.is_active).order_by(ComplianceFramework.name))
        return result.scalars().all()
    except SQLAlchemyError as e:
        logger.error('Database error while getting all frameworks: %s' % e,
            exc_info=True)
...
async def get_framework_by_id(db: AsyncSession, user: User, framework_id: UUID
    ...
    try:
        ...
        logger.error('Database error while getting framework %s: %s' % (
...
def calculate_framework_relevance(profile: BusinessProfile, framework:
    ...
    if profile.industry in framework.applicable_indu:
        score += 50
    if (framework.employee_thresh and profile.employee_count >= framework.
        employee_thresh):
        score += 25
    if profile.data_sensitivity in ['High', 'Medium'
...
async def get_relevant_frameworks(db: AsyncSession, user: User) ->List[Dict]:
    ...
    try:
        profile_res = await db.execute(select(BusinessProfile).where(
            BusinessProfile.user_id == user.id))
        profile = profile_res.scalars().first()
        if not profile:
            logger.warning(
                'No business profile found for user %s to determine relevant frameworks.'
                 % user.id)
            return []
        ...
        for framework in frameworks:
            ...
            if relevance_score > 0:
                relevant_frameworks.append({'framework': framework.to_dict(
                    ), 'relevance_score': relevance_score})
        return sorted(relevant_frameworks, key=lambda x: x[
            'relevance_score'], reverse=True)
    except SQLAlchemyError as e:
        logger.error(
            'Database error while getting relevant frameworks for user %s: %s'
...
async def initialize_default_frameworks(db: AsyncSession) ->None:
    ...
    frameworks_data = [{'name': 'GDPR', 'display_name':
        ...
        'employee_thresh': 1, 'geographic_scop': ['EU', 'UK'],
        'key_requirement': ['Lawful basis for processing',
        'Data subject rights', 'Privacy by design and default',
        'Data protection impact assessments', 'Breach notification'],
        'control_domains': ['Data Processing', 'Consent Management',
        'Data Subject Rights', 'Privacy Impact Assessment',
        'Breach Management'], 'complexity_scor': 3, 'implementation_': 16,
        'estimated_cost_': '£10,000-£50,000'}, {'name': 'Cyber Essentials',
        'display_name': 'Cyber Essentials', 'description':
        'A UK government-backed scheme to help organizations protect against common cyber attacks.'
        , 'category': 'Cybersecurity', 'applicable_indu': ['All'],
        'employee_thresh': 1, 'geographic_scop': ['UK'], 'key_requirement':
        ...
        'Access Management', 'Malware Protection', 'Update Management'],
        'complexity_scor': 2, 'implementation_': 8, 'estimated_cost_':
        '£5,000-£15,000'}, {'name': 'ISO 27001', 'display_name':
        'ISO 27001 Information Security Management', 'description':
        'International standard for information security management systems',
        'category': 'Information Security', 'applicable_indu': ['All'],
        'employee_thresh': 50, 'geographic_scop': ['Global'],
        'key_requirement': ['Information Security Management System',
        'Risk Assessment and Treatment', 'Security Controls Implementation',
        'Continuous Monitoring', 'Management Review'], 'control_domains': [
        'ISMS Framework', 'Risk Management', 'Asset Management',
        'Access Control', 'Cryptography', 'Physical Security',
        'Incident Management'], 'complexity_scor': 4, 'implementation_': 20,
    ...
    try:
        for framework_data in frameworks_data:
            ...
            if not existing:
                logger.info('Creating new framework: %s' % framework_data[
                    'name'])
        ...
        await db.rollback()
        logger.error(
            'Database error while initializing default frameworks: %s' % e,
            exc_info=True)
        raise DatabaseException('Failed to initialize default frameworks.'
            ) from e

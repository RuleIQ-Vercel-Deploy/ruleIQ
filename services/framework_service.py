...
async def get_all_frameworks(db: AsyncSession) -> List[ComplianceFramework]:
    ...
    try:
        result = await db.execute(
            select(ComplianceFramework).where(ComplianceFramework.is_active).order_by(ComplianceFramework.name)
        )
...
async def get_framework_by_id(db: AsyncSession, user: User, framework_id: UUID) -> Optional[ComplianceFramework]:
    ...
    try:
        ...
        logger.error("Database error while getting framework %s: %s" % (framework_id, e), exc_info=True)
...
async def get_framework_by_name(db: AsyncSession, name: str) -> Optional[ComplianceFramework]:
    ...
    try:
        result = await db.execute(select(ComplianceFramework).where(ComplianceFramework.name == name))
        return result.scalars().first()
    except SQLAlchemyError as e:
        logger.error("Database error while getting framework by name '%s': %s" % (name, e), exc_info=True)
...
async def get_relevant_frameworks(db: AsyncSession, user: User) -> List[Dict]:
    ...
    try:
        profile_res = await db.execute(select(BusinessProfile).where(BusinessProfile.user_id == user.id))
        profile = profile_res.scalars().first()
        if not profile:
            logger.warning("No business profile found for user %s to determine relevant frameworks." % user.id)
            return []
        ...
        for framework in frameworks:
            ...
            if relevance_score > 0:
                relevant_frameworks.append({"framework": framework.to_dict(), "relevance_score": relevance_score})
        return sorted(relevant_frameworks, key=lambda x: x["relevance_score"], reverse=True)
    except SQLAlchemyError as e:
        logger.error("Database error while getting relevant frameworks for user %s: %s" % (user.id, e), exc_info=True)

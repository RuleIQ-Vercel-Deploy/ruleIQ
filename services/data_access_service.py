...
class DataAccessService:
    ...
    def can_access_business_profile(self, user: UserWithRoles, profile_id: UUID, profile_owner_id: UUID = None) -> bool:
        """
        ...
        if profile_owner_id is None:
            profile = self.db.query(BusinessProfile).filter(BusinessProfile.id == profile_id).first()
            if not profile:
                return False
    ...
    def grant_data_access(
        self, user_id: UUID, access_type: str, business_profile_id: UUID = None, granted_by: UUID = None
    ) -> DataAccess:
        """
        ...
        if existing:
            ...
            data_access = DataAccess(
                user_id=user_id, business_profile_id=business_profile_id, access_type=access_type, granted_by=granted_by
            )
    ...
    def get_accessible_business_profiles(self, user: UserWithRoles) -> List[UUID]:
        ...
        if access_level == "own_data":
            profiles = self.db.query(BusinessProfile.id).filter(BusinessProfile.user_id == user.id).all()
            return [profile.id for profile in profiles]
        if access_level == "organization_data":
            profiles = self.db.query(BusinessProfile.id).all()
            return [profile.id for profile in profiles]

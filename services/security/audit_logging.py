...
class AuditLoggingService:
    ...
    async def get_user_events(self, user_id: str, limit: int = 100, db: Session = None) -> List[Dict[str, Any]]:
    ...
    async def get_security_events(self, hours: int = 24, db: Session = None) -> List[Dict[str, Any]]:
        ...
        if db:
            events = db.query(AuditLog).filter(AuditLog.action.in_(security_actions), AuditLog.timestamp >= since).all()
            return [self._log_to_dict(event) for event in events]
        return []
    ...
    async def _cache_event(self, log_entry: Dict[str, Any]) -> None:
        ...
        if log_entry["action"] in [AuditEventAction.LOGIN_FAILED.value, AuditEventAction.PERMISSION_DENIED.value]:
        ...
        if log_entry["action"] == AuditEventAction.LOGIN_FAILED.value:
            ...
            if user_id:
                ...
                if len(recent_failures) >= 5:
                    alerts.append({"type": "MULTIPLE_FAILED_LOGINS", "user_id": user_id, "count": len(recent_failures)})

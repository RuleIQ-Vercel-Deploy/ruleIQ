...
class AuthorizationService:
    ...
    async def check_permission(self, user: User, resource_type: str, permission_type: str, resource_id: Optional[str]=None, context: Optional[Dict[str, Any]]=None) -> bool:
        """
        ...
        if cached is not None:
            return cached
        has_permission = await self._evaluate_permission(user, resource_type, permission_type, resource_id, context)
        await self.cache.set(cache_key, has_permission, expire_seconds=300)
        return has_permission

    async def _evaluate_permission(self, user: User, resource_type: str, permission_type: str, resource_id: Optional[str]=None, context: Optional[Dict[str, Any]]=None) -> bool:
        ...
        if await self._check_role_permission(user, resource_type, permission_type):
            ...
            return True
        if resource_id:
            if await self._check_resource_permission(user, resource_type, permission_type, resource_id):
                return True
        if await self._check_delegated_permission(user, resource_type, permission_type, resource_id):
            return True
        if context:
    ...
    async def _check_role_permission(self, user: User, resource_type: str, permission_type: str) -> bool:
        """Check role-based permission"""
        user_permissions = self._get_user_permissions(user)
        permission_patterns = [f'{resource_type}:{permission_type}', f'{resource_type}:*', f'*:{permission_type}', '*:*']
    ...
    async def _check_resource_permission(self, user: User, resource_type: str, permission_type: str, resource_id: str) -> bool:
    ...
    async def _check_delegated_permission(self, user: User, resource_type: str, permission_type: str, resource_id: Optional[str]=None) -> bool:
    ...
    async def _check_dynamic_permission(self, user: User, resource_type: str, permission_type: str, context: Dict[str, Any]) -> bool:
    ...
    async def grant_permission(self, user_id: str, resource_type: str, permission_type: str, resource_id: Optional[str]=None, expires_in: Optional[int]=None) -> None:
    ...
    async def revoke_permission(self, user_id: str, resource_type: str, permission_type: str, resource_id: Optional[str]=None) -> None:
    ...
    async def delegate_permission(self, from_user_id: str, to_user_id: str, resource_type: str, permission_type: str, resource_id: Optional[str]=None, expires_in: Optional[int]=None) -> None:
    ...
    def check_endpoint_permission(self, required_permission: str, resource_type: Optional[str]=None) -> Any:
        ...
        def decorator(func) -> Any:
            ...
            async def wrapper(request: Request, *args, **kwargs) -> Any:
                ...
                context = {'client_ip': request.client.host, 'method': request.method, 'path': request.url.path}
                has_permission = await self.check_permission(user, resource_type or 'general', required_permission, resource_id, context)
    ...
    async def check_cross_tenant_access(self, user: User, tenant_id: str, requested_tenant_id: str) -> bool:

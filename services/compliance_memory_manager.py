...
class ComplianceMemoryManager:
    ...
    async def store_conversation_memory(
        self, user_query: str, agent_response: str, compliance_context: Dict[str, Any], importance_score: float = 0.5
    ) -> str:
    ...
    async def store_knowledge_graph_memory(
        self, graph_query_result: Dict[str, Any], query_category: QueryCategory, importance_score: float = 0.8
    ) -> str:
    ...
    async def retrieve_contextual_memories(
        self, query: str, context: Dict[str, Any], max_memories: int = 10, relevance_threshold: float = 0.5
        ...
        for memory_id in all_candidate_memories:
            ...
            relevance_score = self._calculate_memory_relevance(memory, query, query_entities, query_tags, context)
            if relevance_score >= relevance_threshold:
                relevant_memories.append(memory)
                relevance_scores.append(relevance_score)
                memory.access_count += 1
                memory.last_accessed = datetime.now(timezone.utc)
        sorted_pairs = sorted(zip(relevant_memories, relevance_scores), key=lambda x: x[1], reverse=True)
        if sorted_pairs:
    ...
    async def consolidate_compliance_knowledge(self, time_window_days: int = 30) -> Dict[str, Any]:
        ...
        recent_memories = [memory for memory in self.memory_store.values() if memory.timestamp >= cutoff_date]
        domain_knowledge = {}
        regulation_insights = {}
        risk_patterns = {}
        for memory in recent_memories:
            ...
            if "compliance_context" in content:
                ...
                for domain in domains:
                    if domain not in domain_knowledge:
                        domain_knowledge[domain] = {"memories": [], "risk_levels": [], "recent_queries": []}
            ...
            for regulation in regulations:
                if regulation not in regulation_insights:
                    regulation_insights[regulation] = {"mention_count": 0, "contexts": [], "risk_assessments": []}
    ...
    def _generate_tags(self, user_query: str, agent_response: str, context: Dict[str, Any]) -> List[str]:
    ...
    async def _update_memory_clusters(self, memory_node: MemoryNode) -> None:
        ...
        cluster.importance_score = np.mean(
            [self.memory_store[mid].importance_score for mid in cluster.memory_ids if mid in self.memory_store]
        )
    ...
    async def _retrieve_by_tags(self, tags: List[str]) -> List[str]:
        ...
        return [memory_id for memory_id, memory in self.memory_store.items() if any(tag in memory.tags for tag in tags)]
    ...
    def _calculate_memory_relevance(
        self, memory: MemoryNode, query: str, query_entities: List[str], query_tags: List[str], context: Dict[str, Any]
    ) -> float:
    ...
    def _calculate_consolidation_score(self, memories: List[MemoryNode]) -> float:
        ...
        recent_memories = sum(
            1 for memory in memories if memory.timestamp >= datetime.now(timezone.utc) - timedelta(days=7)
        )

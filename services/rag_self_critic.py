...
from services.rag_fact_checker import QualityAssessment, RAGFactChecker
...
class RAGSelfCriticCommands:
    ...
    async def fact_check_command(self, query: str, max_results: int = 5) -> Dict[str, Any]:
        ...
        try:
            ...
            rag_response = await self.rag_system.query_documentation(query=query, max_results=max_results)
            ...
            assessment = await self.fact_checker.comprehensive_fact_check(
                response_text=rag_response.answer, sources=rag_response.sources, original_query=query
            )
    ...
    async def assess_command(self, query: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"query": query, "rag_response": rag_response.dict(), "assessment": assessment.dict()}
        except Exception as e:
    ...
    def _display_fact_check_results(self, assessment: QualityAssessment, rag_response) -> None:
        ...
        for i, fact_check in enumerate(assessment.fact_check_results):
            ...
            logger.info("   %s. %s %s %s..." % (i + 1, status, confidence_emoji, fact_check.claim[:80]))
    ...
    def _display_critique_results(self, critiques) -> None:
        ...
        for critique in critiques:
            score_emoji = "🟢" if critique.score >= CONFIDENCE_THRESHOLD else "🟡" if critique.score >= 0.6 else "🔴"
...
async def main() -> None:
    ...
    parser.add_argument(
        "command", choices=["fact-check", "quick-check", "critique", "assess", "benchmark"], help="Command to run"
    )

...
async def generate_readiness_assessment(
    ...
    new_assessment = ReadinessAssessmentModel(
        ...
        score_breakdown={"policy": policy_score, "implementation": implementation_score, "evidence": evidence_score},
...
async def get_readiness_dashboard(db: AsyncSession, user: UserModel) -> Dict[str, Any]:
    ...
    for fw in all_frameworks:
        assessment_stmt = (
            ...
            .where(ReadinessAssessmentModel.user_id == user.id, ReadinessAssessmentModel.framework_id == fw.id)
    ...
    avg_score = (
        sum(f["assessment"].overall_score for f in framework_data) / total_frameworks if total_frameworks > 0 else 0
    )

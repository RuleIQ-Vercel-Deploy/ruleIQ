...
from scipy.stats import ttest_ind, chi2_contingency, mannwhitneyu, kstest
import warnings
...
class ABTestingFramework:
    ...
    def record_metric(self, experiment_id: str, variant: str, user_id: str,
                     primary_metric_value: Union[float, int, str, bool],
    ...
    def analyze_experiment(self, experiment_id: str, 
        ...
        if len(data) < config.min_sample_size:
            logger.warning(f"Insufficient data for analysis: {len(data)} < {config.min_sample_size}")
    ...
    def _group_data_by_variant(self, data: List[ExperimentData]) -> Dict[str, List[Union[float, int, str, bool]]]:
    ...
    def _select_statistical_test(self, metric_type: MetricType, 
                                variant_data: Dict[str, List]) -> StatisticalTest:
        """
    ...
    def _perform_statistical_test(self, test_type: StatisticalTest,
        ...
        if test_type in [StatisticalTest.T_TEST, StatisticalTest.WELCH_T_TEST]:
            ...
            if test_type == StatisticalTest.T_TEST:
                ...
                df = (control_std**2/len(control_data) + treatment_std**2/len(treatment_data))**2 / \
        ...
        power = self._calculate_power(observed_effect_size, len(control_data), len(treatment_data), alpha)
    ...
    def _generate_recommendation(self, is_significant: bool, practical_significance: bool,
                               effect_size: float, p_value: float, power: float) -> str:
...
def create_ai_model_experiment(model_a: str, model_b: str, 
...
def create_prompt_optimization_experiment(original_prompt: str, optimized_prompt: str,
                                        metric: str = "task_completion_rate") -> str:
    """

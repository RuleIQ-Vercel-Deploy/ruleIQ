...
class IndustryRegulationLookupTool(BaseTool):
    ...
    async def execute(self, parameters: Dict[str, Any], context: Optional[Dict[str, Any]] = None) -> ToolResult:
        ...
        try:
            ...
            filtered_regulations.sort(key=lambda x: priority_order.get(x["priority"], 2), reverse=True)
            result_data = {
...
class ComplianceRequirementsTool(BaseTool):
    ...
    async def execute(self, parameters: Dict[str, Any], context: Optional[Dict[str, Any]] = None) -> ToolResult:
        ...
        try:
            ...
            logger.info(
                "Compliance requirements check completed: %s requirements found for %s" % (len(requirements), framework)
            )
            return ToolResult(
                success=True,
                data=result_data,
            ...
            return ToolResult(success=False, error=f"Compliance requirements check execution failed: {e!s}")
    ...
    def _get_framework_requirements(
        ...
        if specific_areas:
            requirements = [
                ...
                if any(
                    area.lower() in req["title"].lower() or area.lower() in req["description"].lower()
    ...
    def _prioritize_requirements(self, requirements: List[Dict[str, Any]]) -> List[Dict[str, str]]:
        ...
        for req in mandatory_reqs:
            priorities.append(
                {"requirement": req["title"], "priority": "critical", "reason": "Mandatory compliance requirement"}
            )
        optional_reqs = [req for req in requirements if not req["mandatory"]]
        for req in optional_reqs:
            priorities.append(
                {"requirement": req["title"], "priority": "medium", "reason": "Best practice recommendation"}
            )
        return priorities

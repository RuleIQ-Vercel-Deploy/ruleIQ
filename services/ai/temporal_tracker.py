...
class TemporalTracker:
    ...
    def _parse_timeline_to_days(self, timeline: str) -> Optional[int]:
        ...
        month_match = re.search(r'(\d+)[-\s]*(?:to[-\s]*)?(\d+)?\s*month', timeline_lower)
        if month_match:
    ...
    def _estimate_effort_from_complexity(self, complexity: int) -> int:
        ...
        base_hours = {
            ...
            6: 160, 7: 200, 8: 280, 9: 360, 10: 480
        }
    ...
    def _generate_resource_forecast(
        ...
        monthly_capacity = (
            ...
            self.resource_params["days_per_week"] * 4 *
        ...
        constrained_months = [
            month for month, hours in monthly_hours.items()
    ...
    async def track_amendment_patterns(
        ...
        async with self.driver.session() as session:
            ...
            patterns = {
                ...
                "average_days_between": self._calculate_avg_days_between(amendment_history),
    ...
    async with TemporalTracker(neo4j_uri, neo4j_user, neo4j_password) as tracker:
        ...
        if timeline.overdue_events:
            ...
            for event in timeline.overdue_events[:3]:
                ...
                logger.info(f"    {days_overdue} days overdue - {event.action_required}")
        ...
        if timeline.seasonal_patterns.get("peak_months"):
            ...
            logger.info(f"  Recommendation: {timeline.seasonal_patterns['recommendation']}")
            
        # Resource forecast
        if timeline.resource_forecast:
            logger.info(f"\n👥 Resource Forecast:")
            logger.info(f"  Average Utilization: {timeline.resource_forecast['average_utilization']:.1%}")
            logger.info(f"  Monthly Capacity: {timeline.resource_forecast['monthly_capacity']} hours")
            if timeline.resource_forecast['constrained_months']:
                logger.info(f"  Constrained Months: {len(timeline.resource_forecast['constrained_months'])}")
            logger.info(f"  Recommendation: {timeline.resource_forecast['recommendation']}")
        ...
        if patterns['next_predicted']:
            logger.info(f"  Next Predicted: {patterns['next_predicted'].strftime('%Y-%m-%d')}")
        ...
        for month_key in sorted(list(calendar.keys()))[:3]:
            ...
            for event in events[:3]:
                logger.info(f"    • {event.event_date.strftime('%d')}: {event.regulation_title[:50]}")
                logger.info(f"      {event.urgency.value.upper()} - {event.action_required[:50]}")

...
class TemplateProcessor:
    ...
    def parse_iso27001_template(self, template_path: str) -> ParsedTemplate:
        ...
        try:
            ...
            parsed = ParsedTemplate(success=True, template_type=template_type, sections=sections, variables=variables)
            self.template_cache[template_path] = parsed
            return parsed
        except Exception as e:
            logger.error("Failed to parse template %s: %s" % (template_path, str(e)))
            return ParsedTemplate(
                success=False, template_type="unknown", sections={}, variables=[], error_message=str(e)
            )
...
class PolicyGenerator:
    ...
    async def _stream_with_google(self, prompt: str, request: "PolicyGenerationRequest"):
        ...
        try:
            ...
            response = model.generate_content(
                prompt,
                stream=True,
                generation_config=genai.GenerationConfig(temperature=0.7, top_p=0.9, max_output_tokens=4000),
            )
            accumulated_text = ""
            chunk_count = 0
    ...
    def _build_policy_prompt(self, request: PolicyGenerationRequest, framework: ComplianceFramework) -> str:
    ...
    def _generate_mock_policy(self, request: PolicyGenerationRequest, framework: ComplianceFramework) -> str:
...
Policy content to be customized.""",
    ...
    def _generate_cache_key(self, request: PolicyGenerationRequest, framework: ComplianceFramework) -> str:
...
Improvements: {', '.join(feedback)}""",
    ...
    def validate_uk_policy(self, policy_content: str, framework: ComplianceFramework) -> PolicyValidationResult:
        """

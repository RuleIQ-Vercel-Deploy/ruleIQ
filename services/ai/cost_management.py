...
class AlertType(Enum):
    ...
    SERVICE_BUDGET_WARNING = "service_budget_warning"
    SERVICE_BUDGET_EXCEEDED = "service_budget_exceeded"
...
class ModelCostConfig:
    ...
    def calculate_input_cost(self, tokens: int) -> Decimal:
        ...
        return (Decimal(tokens) / Decimal(1_000_000)) * self.input_cost_per_million

    def calculate_output_cost(self, tokens: int) -> Decimal:
        """Calculate cost for output tokens."""
        return (Decimal(tokens) / Decimal(1_000_000)) * self.output_cost_per_million
...
class AIUsageMetrics:
    ...
    def aggregate(self, other: "AIUsageMetrics") -> "AIUsageMetrics":
        ...
        if (
            ...
            or self.model_name != other.model_name
        ):
            raise ValueError(
...
class CostTrackingService:
    ...
    async def track_usage(
        ...
        if not model_config:
            logger.warning(
                f"Unknown model {model_name}, using default cost calculation"
            )
            # Use Gemini Pro as default
        ...
        logger.debug(f"Tracked usage: {service_name}/{model_name} - ${cost_usd:.6f}")
        return usage
    ...
    async def _store_usage_in_redis(self, usage: AIUsageMetrics) -> None:
        ...
        # Store in multiple time-based keys for efficient querying
        ...
        user_key = (
            f"ai_usage:user:{usage.user_id}:{timestamp.date()}"
            if usage.user_id
            else None
        )
        ...
        await self.redis.expire(f"ai_usage:record:{usage_id}", 86400 * 30)  # 30 days
        ...
        pipe.expire(date_key, 86400 * 90)  # 90 days
        ...
        pipe.expire(hour_key, 86400 * 7)  # 7 days
        ...
        pipe.expire(service_key, 86400 * 90)  # 90 days
        ...
        pipe.expire(model_key, 86400 * 90)  # 90 days
        ...
        if user_key:
            ...
            pipe.expire(user_key, 86400 * 90)  # 90 days
    ...
    async def _flush_usage_buffer(self) -> None:
        ...
        logger.info(
            f"Flushing {len(self.usage_buffer)} usage records to persistent storage"
        )
    ...
    async def get_usage_by_time_range(
        ...
        # For now, query daily aggregations
    ...
    async def identify_cost_anomalies(
        ...
        costs = [float(trend["cost"]) for trend in trends[:-1]]  # Exclude today
...
class BudgetAlertService:
    ...
    async def set_service_budget(self, service_name: str, amount: Decimal) -> None:
        ...
        logger.info(f"Budget for {service_name} set to ${amount}")
    ...
    async def set_user_daily_limit(self, user_id: str, amount: Decimal) -> None:
        ...
        logger.info(f"Daily limit for user {user_id} set to ${amount}")
...
class CostOptimizationService:
    ...
    async def analyze_model_efficiency(
        self, usage_data: List[AIUsageMetrics]
    ) -> CostOptimization:
        ...
        return CostOptimization(
            ...
            priority=(
                "high"
                if estimated_savings > current_cost * Decimal("0.2")
                else "medium"
            ),
        )
    ...
    async def analyze_caching_opportunities(
        ...
        for request_group in similar_requests:
            if request_group["count"] > 1:
                # Calculate savings from caching (avoid cost for repeated requests)
    ...
    async def analyze_batch_opportunities(
        ...
        for group in grouped_requests:
            if len(group) > 1:
                # Assume 20% savings from batching
    ...
    async def analyze_prompt_efficiency(
        ...
        if avg_input_tokens > 2000:
            ...
            potential_savings += cost_per_success * Decimal(
                "0.3"
            )  # 30% savings from compression

        if success_rate < 0.9:
            recommendations.append("Improve prompt clarity to reduce retries")
            retry_cost = (
                cost_per_success
                * Decimal(str(1 - success_rate))
        ...
        return CostOptimization(
            ...
            priority=(
                "high"
                if potential_savings > cost_per_success * Decimal("0.2")
                else "low"
            ),
        )
...
class AICostManager:
    ...
    async def get_daily_summary(self, target_date: date) -> Dict[str, Any]:
        ...
        efficiency_metrics = {
            "cost_per_request": (
                daily_costs["total_cost"] / daily_costs["total_requests"]
                if daily_costs["total_requests"] > 0
                else Decimal("0")
            ),
            "cost_per_token": (
                daily_costs["total_cost"] / daily_costs["total_tokens"]
    ...
    async def get_optimization_recommendations(self) -> List[Dict[str, Any]]:
        ...
        model_optimization = await self.optimization_service.analyze_model_efficiency(
            usage_data
        )
    ...
    async def generate_monthly_report(self, year: int, month: int) -> Dict[str, Any]:
        ...
        optimization_report = (
            await self.optimization_service.generate_optimization_report(analysis_data)
        )
...
# Intelligent routing and optimization classes
class IntelligentModelRouter:
    ...
    async def select_optimal_model(
        ...
        **USES GEMINI 2.5 MODELS: gemini-2.5-flash-lite, gemini-2.5-flash, gemini-2.5-pro**
        ...
        complexity_score = (
            len(task_description.split()) / 100
        ...
        return {
            "model": (
                recommended_models[0] if recommended_models else "gemini-2.5-flash"
            ),
...
class DynamicCacheManager:
    ...
    async def should_cache_request(self, request_data: Dict[str, Any]) -> bool:
        ...
        # Cache if cost * frequency > threshold
        cache_threshold = Decimal("0.05")  # $0.05
...
class BatchRequestOptimizer:
    ...
    async def optimize_batch(self, requests: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        batch_cost = individual_cost * Decimal("0.85")  # 15% savings
    ...
    async def predict_future_costs(
        ...
        if len(costs) < 2:
            ...
            for day in range(prediction_horizon_days):
                ...
                # Add seasonality (mock weekly pattern)

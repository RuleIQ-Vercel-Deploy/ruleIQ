...
class IngestionIntegrationTest:
    ...
    async def setup(self):
        ...
        async for session in get_db():
            self.iq_agent = IQComplianceAgent(
                neo4j_service=neo4j_service, postgres_session=session
            )
            break
    ...
    async def test_phase1_ingestion(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if uk_path.exists():
                ...
                logger.info(
                    f"✓ UK regulations ingested: {uk_metrics.items_processed} items"
                )
            else:
            ...
            results["business_trigger_test"] = {
                ...
                "high_risk_count": sum(
                    1 for r in applicable if r.get("risk_score", 0) >= 8
                ),
            ...
            logger.info(
                f"  High risk (≥8): {results['business_trigger_test']['high_risk_count']}"
            )
            ...
            logger.info(
                f"✓ Average automation potential: {automation_stats.get('average_potential', 0):.2f}"
            )
    ...
    async def test_phase2_relationships(self) -> Dict[str, Any]:
        ...
        try:
            ...
            results["conflict_test"] = {
                ...
                "conflict_types": list(
                    set(c.get("conflict_type", "unknown") for c in conflicts)
                ),
            }
    ...
    async def test_iq_integration(self) -> Dict[str, Any]:
        ...
        try:
            ...
            results["perception"] = {
                "identified_regulations": len(
                    perception.get("applicable_regulations", [])
                ),
                "risk_level": perception.get("risk_level"),
                "top_risks": perception.get("top_risks", [])[:3],
            }
            logger.info(
                f"✓ IQ identified {results['perception']['identified_regulations']} regulations"
            )
            ...
            results["planning"] = {
                ...
                "priority_actions": [
                    a.get("title") for a in plan.get("actions", [])[:3]
                ],
                "considers_dependencies": any(
                    "depend" in str(a).lower() for a in plan.get("actions", [])
                ),
                "automation_enabled": any(
                    a.get("automation_potential", 0) > 0.5
                    for a in plan.get("actions", [])
                ),
            }
            logger.info(
                f"✓ IQ generated {results['planning']['action_count']} compliance actions"
            )
            ...
            logger.info(
                f"✓ Analyzed {results['enforcement_analysis']['patterns_identified']} enforcement patterns"
            )
    ...
    async def generate_report(self):
        ...
        report = {
            ...
            "summary": {
                ...
                "iq_integration_success": "error"
                not in self.metrics.get("iq_integration", {}),
        ...
        logger.info(
            f"Phase 1 (Enhanced Metadata): {'✓ PASSED' if summary['phase1_success'] else '✗ FAILED'}"
        )
        logger.info(
            f"Phase 2 (Relationships): {'✓ PASSED' if summary['phase2_success'] else '✗ FAILED'}"
        )
        logger.info(
            f"IQ Integration: {'✓ PASSED' if summary['iq_integration_success'] else '✗ FAILED'}"
        )

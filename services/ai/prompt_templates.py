...
class AdvancedPromptSanitizer:
    ...
    def _analyze_context_specific(self, text: str, context: str) -> List[str]:
        ...
        threats = []
        if context == "code":
            code_threats = ["exec\\s*\\(", "eval\\s*\\(", "__import__", "subprocess", "os\\.system", "shell=True"]
...
def sanitize_input(input_string: str, context: str = "general") -> str:
    ...
    if analysis.threat_level == ThreatLevel.MALICIOUS and analysis.confidence > CONFIDENCE_THRESHOLD:
...
def validate_prompt_safety(prompt_dict: Dict[str, str], context: Dict[str, Any] = None) -> Dict[str, Any]:
    ...
    if not validation_report["safe"]:
        logger.warning("Prompt safety validation failed: %s" % validation_report["threats_detected"])
...
class PromptTemplates:
    ...
    def _validate_and_secure_prompt(
        ...
        if not validation_result["safe"]:
            logger.error("%s prompt failed safety validation: %s" % (prompt_type, validation_result))
            return {
    ...
    def get_intent_classification_prompt(self, message: str, context: Dict[str, Any]) -> Dict[str, str]:
        ...
        """
        prompt_dict = {"system": system_instruction, "system_instruction": system_instruction, "user": user_prompt}
    ...
    def get_assessment_analysis_prompt(
        self, assessment_results: Dict[str, Any], framework_id: str, business_context: Optional[Dict[str, Any]] = None
    ) -> Dict[str, str]:
        ...
        system_instruction = get_system_instruction(
            ...
            additional_context={"assessment_analysis": True, "structured_output": True, "output_format": "JSON"},
        )
        business_info = business_context or {}
        user_prompt = f"""
        ...
        """
        return {"system": system_instruction, "system_instruction": system_instruction, "user": user_prompt}
...
If you need clarification on any aspect of their request, feel free to ask follow-up questions."""
    ...
    def get_policy_generation_prompt(
        self, framework: str, policy_type: str, business_context: Dict[str, Any], customization_options: Dict[str, Any]
    ) -> Dict[str, str]:

...
class AIResponseCache:
    ...
    def __init__(self) ->None:
        ...
        self.max_ttl = 86400
        self.min_ttl = 300
        self.ttl_config = {ContentType.RECOMMENDATION: 7200, ContentType.
            POLICY: 86400, ContentType.WORKFLOW: 14400, ContentType.
            ANALYSIS: 3600, ContentType.GUIDANCE: 7200, ContentType.GENERAL:
            1800}
        self.metrics = {'hits': 0, 'misses': 0, 'total_requests': 0,
            'cache_size_bytes': 0, 'cost_savings': 0.0}
    ...
    async def get_cached_response(self, prompt: str, context: Optional[Dict
        [str, Any]]=None, similarity_threshold: float=0.85) ->Optional[Dict
        [str, Any]]:
        """
    ...
    async def cache_response(self, prompt: str, response: str, context:
        Optional[Dict[str, Any]]=None, metadata: Optional[Dict[str, Any]]=None
        ) ->bool:
        """
        ...
        try:
            ...
            ttl = self._calculate_intelligent_ttl(content_type, response,
                context)
            cache_key = self._generate_cache_key(prompt, context)
            cache_data = {'response': response, 'content_type':
                content_type.value, 'cached_at': datetime.now(timezone.utc)
                .isoformat(), 'ttl': ttl, 'prompt_hash': cache_key,
                'metadata': metadata or {}, 'context_summary': self.
            ...
            if success:
                ...
                logger.debug(
                    'Cached response with TTL %ss for content type %s' % (
                    ttl, content_type.value))
            return success
        except Exception as e:
    ...
    def _generate_cache_key(self, prompt: str, context: Optional[Dict[str,
        ...
        context_key = ''
        if context:
            stable_context = {'framework': context.get('framework'),
                'industry': context.get('business_context', {}).get(
                'industry'), 'org_size': context.get('business_context', {}
                ).get('employee_count', 0) // 100 * 100, 'content_type':
        ...
        return (
            f'ai_response:{hashlib.sha256(combined_input.encode()).hexdigest()[:16]}'
            ,)

    def _classify_content_type(self, response: str, context: Optional[Dict[
        str, Any]]=None) ->ContentType:
        ...
        if context and context.get('content_type'):
            try:
                ...
                pass
        if any(keyword in response_lower for keyword in ['recommend',
            'suggest', 'priority', 'should implement']):
            return ContentType.RECOMMENDATION
        elif any(keyword in response_lower for keyword in ['policy',
            'procedure', 'governance', 'compliance requirement']):
            return ContentType.POLICY
        elif any(keyword in response_lower for keyword in ['workflow',
            'step', 'phase', 'implementation']):
            return ContentType.WORKFLOW
        elif any(keyword in response_lower for keyword in ['analysis',
            'assessment', 'evaluation', 'gap']):
            return ContentType.ANALYSIS
        elif any(keyword in response_lower for keyword in ['guidance',
            'help', 'how to', 'best practice']):
            return ContentType.GUIDANCE
        else:
            return ContentType.GENERAL

    def _calculate_intelligent_ttl(self, content_type: ContentType,
        response: str, context: Optional[Dict[str, Any]]=None) ->int:
        ...
        if context:
            if context.get('business_context', {}).get('maturity_level'
                ) == 'Initial':
    ...
    async def _find_similar_cached_response(self, prompt: str, context:
        Dict[str, Any], threshold: float) ->Optional[Dict[str, Any]]:
    ...
    def _summarize_context(self, context: Optional[Dict[str, Any]]=None
        ) ->Dict[str, Any]:
        """Create a summary of context for cache metadata."""
        if not context:
            return {}
        return {'framework': context.get('framework'), 'content_type':
            context.get('content_type'), 'business_industry': context.get(
            'business_context', {}).get('industry'), 'has_business_context':
            bool(context.get('business_context'))}
    ...
    async def get_cache_metrics(self) ->Dict[str, Any]:
        ...
        hit_rate = self.metrics['hits'] / self.metrics['total_requests'
            ] * 100 if self.metrics['total_requests'] > 0 else 0
        return {'hit_rate_percentage': round(hit_rate, 2), 'total_hits':

...
class GapValidationModel(BaseModel):
    ...
    framework_reference: str = Field(..., min_length=1, description="Framework reference")
    current_state: str = Field(..., min_length=1, description="Current compliance state")
    ...
    technical_complexity: float = Field(..., ge=0.0, le=1.0, description="Technical complexity score")
    regulatory_requirement: bool = Field(..., description="Whether this is a regulatory requirement")
    estimated_effort: ImplementationEffort = Field(..., description="Estimated implementation effort")
...
class RecommendationValidationModel(BaseModel):
    ...
    description: str = Field(..., min_length=10, description="Detailed recommendation description")
    ...
    framework_references: List[str] = Field(..., min_items=1, description="Framework references")
    addresses_gaps: List[str] = Field(default_factory=list, description="Gap IDs this addresses")
    effort_estimate: ImplementationEffort = Field(..., description="Implementation effort estimate")
    implementation_timeline: str = Field(..., min_length=1, description="Implementation timeline")
    impact_score: float = Field(..., ge=0.0, le=1.0, description="Implementation impact score")
    ...
    automation_potential: float = Field(default=0.0, ge=0.0, le=1.0, description="Automation potential")
...
class ImplementationPlanValidationModel(BaseModel):
    ...
    phases: List[ImplementationPhaseValidationModel] = Field(..., min_items=1, description="Implementation phases")
    resource_allocation: Dict[str, str] = Field(default_factory=dict, description="Resource allocation")
...
class RiskAssessmentValidationModel(BaseModel):
    ...
    top_risk_factors: List[str] = Field(..., min_items=1, description="Top risk factors")
    risk_mitigation_priorities: List[str] = Field(..., min_items=1, description="Risk mitigation priorities")
    regulatory_compliance_risk: float = Field(..., ge=0.0, le=100.0, description="Regulatory compliance risk")
    operational_risk: float = Field(..., ge=0.0, le=100.0, description="Operational risk")
    reputational_risk: float = Field(..., ge=0.0, le=100.0, description="Reputational risk")
...
class ComplianceInsightValidationModel(BaseModel):
    ...
    actionable_steps: List[str] = Field(..., min_items=1, description="Actionable steps")
...
class EvidenceRequirementValidationModel(BaseModel):
    ...
    automation_potential: float = Field(..., ge=0.0, le=1.0, description="Automation potential")
    estimated_effort: ImplementationEffort = Field(..., description="Estimated effort")
    validation_criteria: List[str] = Field(..., min_items=1, description="Validation criteria")
...
class ComplianceMetricsValidationModel(BaseModel):
    ...
    overall_compliance_score: float = Field(..., ge=0.0, le=100.0, description="Overall compliance score")
    framework_scores: Dict[str, float] = Field(default_factory=dict, description="Framework-specific scores")
    ...
    gap_count_by_severity: Dict[SeverityLevel, int] = Field(default_factory=dict, description="Gap count by severity")
...
class GapAnalysisValidationModel(BaseModel):
    ...
    estimated_total_effort: str = Field(..., min_length=1, description="Total effort estimate")
    critical_gap_count: int = Field(..., ge=0, description="Count of critical gaps")
    medium_high_gap_count: int = Field(..., ge=0, description="Count of medium/high gaps")
    compliance_percentage: float = Field(..., ge=0.0, le=100.0, description="Compliance percentage")
    framework_coverage: Dict[str, float] = Field(default_factory=dict, description="Framework coverage")
...
class RecommendationResponseValidationModel(BaseModel):
    ...
    recommendations: List[RecommendationValidationModel] = Field(..., min_items=1, description="List of recommendations")
    implementation_plan: ImplementationPlanValidationModel = Field(..., description="Implementation plan")
    prioritization_rationale: str = Field(..., min_length=10, description="Prioritization rationale")
    quick_wins: List[str] = Field(default_factory=list, description="Quick win recommendation IDs")
    long_term_initiatives: List[str] = Field(default_factory=list, description="Long-term initiative IDs")
    resource_summary: Dict[str, Any] = Field(default_factory=dict, description="Resource summary")
    ...
    def validate_recommendation_ids(self, v, values, field):
        if 'recommendations' in values:
            ...
            if invalid_ids:
                raise ValueError(f"Invalid recommendation IDs in {field.name}: {invalid_ids}")
        return v
...
class AssessmentAnalysisValidationModel(BaseModel):
    ...
    recommendations: List[RecommendationValidationModel] = Field(..., description="Recommendations")
    risk_assessment: RiskAssessmentValidationModel = Field(..., description="Risk assessment")
    compliance_insights: List[ComplianceInsightValidationModel] = Field(..., description="Compliance insights")
    evidence_requirements: List[EvidenceRequirementValidationModel] = Field(..., description="Evidence requirements")
    compliance_metrics: ComplianceMetricsValidationModel = Field(..., description="Compliance metrics")
...
class GuidanceValidationModel(BaseModel):
    ...
    confidence_score: float = Field(..., ge=0.0, le=1.0, description="Guidance confidence score")
    ...
    common_pitfalls: List[str] = Field(default_factory=list, description="Common pitfalls")
    implementation_tips: List[str] = Field(default_factory=list, description="Implementation tips")
...
class FollowUpQuestionValidationModel(BaseModel):
    ...
    expected_answer_type: Literal["text", "boolean", "multiple_choice", "numeric"] = Field(..., description="Expected answer type")
...
class FollowUpValidationModel(BaseModel):
    ...
    follow_up_questions: List[FollowUpQuestionValidationModel] = Field(..., description="Follow-up questions")
    ...
    assessment_completeness: float = Field(..., ge=0.0, le=1.0, description="Assessment completeness")
    priority_areas: List[str] = Field(..., description="Priority areas")
    suggested_next_steps: List[str] = Field(..., min_items=1, description="Suggested next steps")


class IntentClassificationValidationModel(BaseModel):
    """Validation model for intent classification."""
    intent_type: Literal["evidence_query", "compliance_check", "guidance_request", "general_query", "assessment_help"] = Field(..., description="Classified intent type")
...
class ResponseMetadataValidationModel(BaseModel):
    ...
    validation_status: Literal["valid", "invalid", "partially_valid"] = Field(..., description="Validation status")
...
class StructuredAIResponseValidationModel(BaseModel):
    ...
    metadata: ResponseMetadataValidationModel = Field(..., description="Response metadata")
...
def validate_ai_response(response_data: Dict[str, Any], response_type: str) -> tuple[bool, List[str], Optional[BaseModel]]:
    """
...
def create_validation_report(response_data: Dict[str, Any], response_type: str) -> Dict[str, Any]:
    ...
    is_valid, errors, validated_model = validate_ai_response(response_data, response_type)

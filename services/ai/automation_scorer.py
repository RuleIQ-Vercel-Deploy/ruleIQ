...
class AutomationScorer:
    ...
    def _load_automation_patterns(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "workflow": {
                "technologies": ["Airflow", "Temporal", "Step Functions", "GitHub Actions"],
                "complexity_factors": {
                    "steps": 0.2,
                    "conditions": 0.3,
                    "approvals": 0.3,
    ...
    def _identify_quick_wins(
        ...
        if regulation.get("automation_potential", 0) > 0.7:
            quick_wins.append("High automation potential - prioritize for implementation")
    ...
    async def generate_automation_roadmap(
        ...
        async with self.driver.session() as session:
            ...
            avg_roi = np.mean([opp.roi_months for opp in opportunities if opp.roi_months < float('inf')])
    ...
    async with AutomationScorer(neo4j_uri, neo4j_user, neo4j_password) as scorer:
        ...
        if roadmap.quick_wins:
            ...
            for opp in roadmap.quick_wins[:5]:
                ...
                logger.info(f"    Score: {opp.automation_score:.2f}, Hours: {opp.effort_hours}, ROI: {opp.roi_months:.1f} months")
                
        if roadmap.phases:
            logger.info(f"\n📅 Implementation Phases:")
            for phase in roadmap.phases:
                logger.info(f"  Phase {phase['phase']}: {phase['name']} ({phase['duration']})")
                logger.info(f"    Items: {len(phase['items'])}, Hours: {phase['effort_hours']}, Savings: ${phase['expected_savings']:,.0f}")
        ...
        if roadmap.quick_wins or roadmap.strategic_initiatives:
            ...
            logger.info(f"  Break-even: {investment['break_even_date'][:10] if investment['break_even_date'] else 'N/A'}")

...
class ToolRegistry:
    ...
    def register_tool(self, tool: BaseTool, tool_type: ToolType) ->None:
        ...
        logger.info("Registered tool '%s' of type '%s'" % (tool.name,
            tool_type.value))
    ...
    def get_function_schemas(self, tool_names: Optional[List[str]]=None
        ) ->List[Dict[str, Any]]:
        """
        ...
        if tool_names is None:
            ...
            tools = [self.get_tool(name) for name in tool_names if self.
                get_tool(name)]
    ...
    def get_tool_statistics(self) ->Dict[str, Any]:
        ...
        return {'total_tools': len(self._tools), 'tools_by_type': {
            tool_type.value: len(tools) for tool_type, tools in self.
    ...
    def validate_function_schema(schema: Dict[str, Any]) ->bool:
        ...
        for field_name in required_fields:
            if field_name not in schema:
                logger.error(
                    "Missing required field '%s' in function schema" %
                    field_name)
                return False
...
class ToolExecutor:
    ...
    async def execute_tool(self, tool_name: str, parameters: Dict[str, Any],
        ...
        try:
            ...
            execution_time = (datetime.now() - start_time).total_seconds()
            error_result = ToolResult(success=False, error=
                f'Tool execution failed: {e!s}', execution_time=execution_time)
    ...
    def _record_execution(self, tool_name: str, parameters: Dict[str, Any],
        result: ToolResult, context: Optional[Dict[str, Any]]=None) ->None:
        """Record tool execution for monitoring"""
        execution_record = {'timestamp': datetime.now().isoformat(),
            'tool_name': tool_name, 'parameters': parameters, 'success':
            result.success, 'execution_time': result.execution_time,
            'error': result.error, 'context': context or {}}
    ...
    def get_execution_statistics(self) ->Dict[str, Any]:
        ...
        successful_executions = sum(1 for exec in self.execution_history if
            exec['success'])
        avg_execution_time = sum(exec['execution_time'] for exec in self.
            execution_history) / total_executions
        tool_usage = {}

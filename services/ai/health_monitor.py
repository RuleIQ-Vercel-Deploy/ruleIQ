...
class ServiceHealthMonitor:
    ...
    async def _perform_health_check(self, service_name: str, health_check: HealthCheck):
        ...
        try:
            ...
            if health_check.test_function:
                ...
                result = await self._check_endpoint_health(health_check.endpoint, health_check.timeout)
            else:
    ...
    async def _update_health_metrics(self, service_name: str, result: Dict[str, Any], response_time: float):
        ...
        if is_healthy:
            ...
            if metrics.consecutive_failures >= health_check.failure_threshold:
                if metrics.status != ServiceStatus.UNHEALTHY:
                    self.logger.warning(f"Service {service_name} is unhealthy - {metrics.consecutive_failures} consecutive failures")
    ...
    async def _update_performance_metrics(self, service_name: str, response_time: float, success: bool):
        """Update performance metrics"""
        metrics = self.services[service_name]
        ...
        if recent_checks:
            ...
            if response_times:
                ...
                metrics.p99_response_time = sorted_times[p99_index] if p99_index < len(sorted_times) else sorted_times[-1]
    
    def _store_health_history(self, service_name: str, metrics: HealthMetrics, result: Dict[str, Any]):
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(endpoint, timeout=aiohttp.ClientTimeout(total=timeout)) as response:
                    if response.status == 200:
                        ...
                        return {"healthy": False, "error": f"HTTP {response.status}", "status_code": response.status}
        except Exception as e:
    ...
    async def _check_model_health(self, model_type: ModelType) -> Dict[str, Any]:
        ...
        try:
            ...
            if response and response.text:
                return {"healthy": True, "model": model_type.value, "response_length": len(response.text)}
            else:
                return {"healthy": False, "error": "No response generated", "model": model_type.value}
    ...
    def get_health_trends(self) -> Dict[str, Any]:
        ...
        for service_name, metrics in self.services.items():
            ...
            if len(recent_history) >= 10:
                ...
                first_half_success = sum(1 for h in first_half if h.get("success", False)) / len(first_half)
                second_half_success = sum(1 for h in second_half if h.get("success", False)) / len(second_half)
                
                trend_direction = "improving" if second_half_success > first_half_success else "declining" if second_half_success < first_half_success else "stable"

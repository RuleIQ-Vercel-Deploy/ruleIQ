...
class SmartEvidenceCollector:
    ...
    async def create_collection_plan(
        ...
        """
        try:
            evidence_requirements = await self._generate_evidence_requirements(framework, business_context)
            evidence_gaps = self._analyze_evidence_gaps(evidence_requirements, existing_evidence or [])
            ...
            plan = CollectionPlan(
                ...
                completion_target_date=datetime.now(timezone.utc) + timedelta(weeks=target_completion_weeks),
                tasks=optimized_tasks,
                automation_opportunities=automation_opportunities,
            )
            self.active_plans[plan.plan_id] = plan
            logger.info("Created collection plan %s with %s tasks" % (plan.plan_id, len(optimized_tasks)))
            return plan
    ...
    async def _generate_evidence_requirements(
        ...
        framework_requirements = {
            "ISO27001": [
                {"control_id": "A.5.1.1", "evidence_type": "policy_document", "title": "Information Security Policy"},
                {"control_id": "A.9.1.1", "evidence_type": "policy_document", "title": "Access Control Policy"},
                {"control_id": "A.12.4.1", "evidence_type": "log_analysis", "title": "Event Logging"},
                {
                    "control_id": "A.12.6.1",
            ...
            "GDPR": [
                {"control_id": "Art.5", "evidence_type": "policy_document", "title": "Data Processing Principles"},
                {"control_id": "Art.13", "evidence_type": "policy_document", "title": "Privacy Notices"},
                {"control_id": "Art.25", "evidence_type": "system_configuration", "title": "Privacy by Design"},
                {"control_id": "Art.32", "evidence_type": "system_configuration", "title": "Security of Processing"},
                {"control_id": "Art.33", "evidence_type": "policy_document", "title": "Breach Notification Procedures"},
            ],
            "SOC2": [
                {"control_id": "CC6.1", "evidence_type": "system_configuration", "title": "Logical Access Controls"},
                {"control_id": "CC6.2", "evidence_type": "log_analysis", "title": "System Monitoring"},
                {"control_id": "CC7.1", "evidence_type": "system_configuration", "title": "System Operations"},
                {"control_id": "CC8.1", "evidence_type": "policy_document", "title": "Change Management"},
            ],
        }
        ...
        if industry in ["healthcare", "medical"]:
            base_requirements.extend(
                [
                    {"control_id": "HIPAA-1", "evidence_type": "policy_document", "title": "HIPAA Privacy Policy"},
                    {"control_id": "HIPAA-2", "evidence_type": "training_record", "title": "HIPAA Training Records"},
                ]
            )
        elif industry in ["finance", "banking"]:
            base_requirements.extend(
                [
                    {"control_id": "SOX-1", "evidence_type": "policy_document", "title": "Financial Controls Policy"},
                    {"control_id": "PCI-1", "evidence_type": "system_configuration", "title": "Payment Card Security"},
                ]
            )
        if employee_count >= DEFAULT_LIMIT:
            base_requirements.extend(
                [
                    {"control_id": "HR-1", "evidence_type": "policy_document", "title": "HR Security Procedures"},
                    {"control_id": "VENDOR-1", "evidence_type": "policy_document", "title": "Vendor Management Policy"},
                ]
            )
    ...
    def _optimize_task_scheduling(self, tasks: List[EvidenceTask], target_weeks: int) -> List[EvidenceTask]:
    ...
    def _calculate_automation_opportunities(self, tasks: List[EvidenceTask]) -> Dict[str, Any]:
        ...
        automated_tasks = len(
            [
                t
                for t in tasks
                if t.automation_level in [AutomationLevel.FULLY_AUTOMATED, AutomationLevel.SEMI_AUTOMATED]
            ]
        )
        total_manual_effort = sum(t.metadata.get("base_effort", t.estimated_effort_hours) for t in tasks)
        ...
        savings_percentage = effort_savings / total_manual_effort * 100 if total_manual_effort > 0 else 0
        return {
            "total_tasks": total_tasks,
            "automatable_tasks": automated_tasks,
            "automation_percentage": round(automated_tasks / total_tasks * 100, 1) if total_tasks > 0 else 0,
    ...
    async def update_task_status(
        self, plan_id: str, task_id: str, status: CollectionStatus, completion_notes: Optional[str] = None
    ) -> bool:
    ...
    async def get_next_priority_tasks(self, plan_id: str, limit: int = 5) -> List[EvidenceTask]:
        ...
        pending_tasks.sort(key=lambda t: (t.priority.value, t.due_date or datetime.max, t.estimated_effort_hours))

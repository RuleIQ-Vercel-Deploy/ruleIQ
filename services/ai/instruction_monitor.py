...
class InstructionPerformanceMonitor:
    ...
    def __init__(self, max_metrics_history: int = 10000, persistence: Optional[InstructionPersistence] = None):
    ...
    def _complete_ab_test(self, test_id: str):
        ...
        if significance["significant"]:
            ...
            if primary_metric == InstructionMetricType.RESPONSE_QUALITY:
                ...
                improvement = (perf_b.avg_user_satisfaction - perf_a.avg_user_satisfaction) / perf_a.avg_user_satisfaction
    ...
    def _calculate_statistical_significance(
        self,
        perf_a: InstructionPerformanceData,
        perf_b: InstructionPerformanceData
    ) -> Dict[str, Any]:
    ...
    def _generate_summary_stats(self, time_window: timedelta) -> Dict[str, Any]:
        ...
        return {
            ...
            ]) if any(m.metric_type == InstructionMetricType.RESPONSE_QUALITY for m in recent_metrics) else 0,
            "avg_user_satisfaction": statistics.mean([
                m.value for m in recent_metrics
                if m.metric_type == InstructionMetricType.USER_SATISFACTION
            ]) if any(m.metric_type == InstructionMetricType.USER_SATISFACTION for m in recent_metrics) else 0,
    ...
    def _analyze_performance_trends(self, time_window: timedelta) -> Dict[str, Any]:
        ...
        for i in range(time_buckets):
            ...
            if bucket_metrics:
                quality_scores = [
                    m.value for m in bucket_metrics
                    if m.metric_type == InstructionMetricType.RESPONSE_QUALITY
                ]
                buckets.append({
                    "period": f"bucket_{i}",
        ...
        return {
            ...
            "overall_trend": "improving" if len(buckets) >= 2 and buckets[-1]["avg_quality"] > buckets[0]["avg_quality"] else "stable"
        }
    ...
    def _load_data(self):
        ...
        try:
            ...
            logger.info(f"Loaded {len(metrics)} metrics and {len(self.performance_cache)} performance records")
        except Exception as e:

...
class InstructionPerformanceMonitor:
    ...
    def __init__(self, max_metrics_history: int = 10000, persistence: Optional[InstructionPersistence] = None) -> None:
    ...
    def get_instruction_recommendations(
        self, instruction_type: str, framework: Optional[str] = None, task_complexity: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        """
        ...
        """
        recommendations = []
        matching_instructions = self._find_matching_instructions(instruction_type, framework, task_complexity)
    ...
    def _update_performance_cache(self, instruction_id: str) -> None:
        ...
        if not metrics:
            return
        quality_scores = [m.value for m in metrics if m.metric_type == InstructionMetricType.RESPONSE_QUALITY]
        satisfaction_scores = [m.value for m in metrics if m.metric_type == InstructionMetricType.USER_SATISFACTION]
        response_times = [m.value for m in metrics if m.metric_type == InstructionMetricType.RESPONSE_TIME]
        token_efficiency = [m.value for m in metrics if m.metric_type == InstructionMetricType.TOKEN_EFFICIENCY]
        error_metrics = [m.value for m in metrics if m.metric_type == InstructionMetricType.ERROR_RATE]
        effectiveness = [m.value for m in metrics if m.metric_type == InstructionMetricType.INSTRUCTION_EFFECTIVENESS]
        ...
        performance = InstructionPerformanceData(
            ...
            avg_user_satisfaction=statistics.mean(satisfaction_scores) if satisfaction_scores else 0.0,
    ...
    def _update_ab_test_metrics(self, metric: InstructionMetric) -> None:
        ...
        for test_id, test_config in self.active_ab_tests.items():
            if metric.instruction_id in [test_config.instruction_a_id, test_config.instruction_b_id]:
    ...
    def _complete_ab_test(self, test_id: str) -> None:
        ...
        if significance["significant"]:
            ...
            if primary_metric == InstructionMetricType.RESPONSE_QUALITY:
                improvement = (perf_b.avg_response_quality - perf_a.avg_response_quality) / perf_a.avg_response_quality
        ...
        result = ABTestResult(
            ...
            winner=winner,
            recommendation=self._generate_ab_test_recommendation(winner, improvement, test_config.target_improvement),
    ...
    def _calculate_performance_for_window(
        ...
        metrics = [m for m in self.metrics_history if m.instruction_id == instruction_id and m.timestamp >= cutoff_time]
        if not metrics:
            return None
        quality_scores = [m.value for m in metrics if m.metric_type == InstructionMetricType.RESPONSE_QUALITY]
        satisfaction_scores = [m.value for m in metrics if m.metric_type == InstructionMetricType.USER_SATISFACTION]
    ...
    def _generate_summary_stats(self, time_window: timedelta) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_response_quality": (
                statistics.mean(
                    [m.value for m in recent_metrics if m.metric_type == InstructionMetricType.RESPONSE_QUALITY]
                )
            ...
            "avg_user_satisfaction": (
                statistics.mean(
                    [m.value for m in recent_metrics if m.metric_type == InstructionMetricType.USER_SATISFACTION]
                )
                if any(m.metric_type == InstructionMetricType.USER_SATISFACTION for m in recent_metrics)
                else 0
            ),
            "overall_error_rate": (
                len([m for m in recent_metrics if m.metric_type == InstructionMetricType.ERROR_RATE and m.value > 0])
                / len(recent_metrics)
                if recent_metrics
                else 0
            ),
    ...
    def _analyze_performance_trends(self, time_window: timedelta) -> Dict[str, Any]:
        ...
        for i in range(time_buckets):
            ...
            if bucket_metrics:
                quality_scores = [
                    m.value for m in bucket_metrics if m.metric_type == InstructionMetricType.RESPONSE_QUALITY
                ]
                buckets.append(
                    {
                        "period": f"bucket_{i}",
    ...
    def _load_data(self) -> None:
        ...
        try:
            ...
            logger.info("Loaded %s metrics and %s performance records" % (len(metrics), len(self.performance_cache)))
        except Exception as e:
...
instruction_monitor = InstructionPerformanceMonitor(
    persistence=FilePersistence() if os.getenv("ENVIRONMENT") == "production" else MemoryPersistence()
)

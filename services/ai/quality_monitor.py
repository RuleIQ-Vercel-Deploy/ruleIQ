...
class AIQualityMonitor:
    ...
    async def assess_response_quality(
        ...
        try:
            ...
            improvement_suggestions = self._generate_improvement_suggestions(dimension_scores, response_text, context)
            ...
            logger.debug("Quality assessment completed for response %s: %s" % (response_id, overall_score))
            return assessment
    ...
    def _score_relevance(self, response_text: str, prompt: str, context: Optional[Dict[str, Any]] = None) -> float:
        ...
        if overlap_ratio >= 0.7:
            ...
            score += 1.0
        if "?" in prompt and any(word in response_text.lower() for word in ["yes", "no", "should", "recommend"]):
            score += 0.5
        return min(10.0, score)
    ...
    def _score_completeness(self, response_text: str, prompt: str, context: Optional[Dict[str, Any]] = None) -> float:
        ...
        if any(indicator in response_text for indicator in ["1.", "2.", "•", "-"]):
            score += 1.0
        coverage_indicators = ["steps", "requirements", "considerations", "examples", "implementation"]
        coverage_count = sum(1 for indicator in coverage_indicators if indicator in response_text.lower())
    ...
    def _score_actionability(self, response_text: str, context: Optional[Dict[str, Any]] = None) -> float:
        ...
        score = 6.0
        action_words = ["implement", "create", "establish", "develop", "conduct", "review", "update"]
        action_count = sum(1 for word in action_words if word in response_text.lower())
        score += action_count * 0.5
        if any(indicator in response_text.lower() for indicator in ["step", "procedure", "process"]):
            score += 1.5
        if any(indicator in response_text.lower() for indicator in ["daily", "weekly", "monthly", "annually"]):
            score += 1.0
        if any(role in response_text.lower() for role in ["manager", "officer", "team", "responsible"]):
            score += 1.0
        return min(10.0, score)
    ...
    def _calculate_overall_score(self, dimension_scores: Dict[QualityDimension, QualityScore]) -> float:
        ...
        weighted_sum = sum(score.score * weights.get(dimension, 0.1) for dimension, score in dimension_scores.items())
        ...
        for dimension, score in dimension_scores.items():
            if score.score < 6.0:
                if dimension == QualityDimension.ACCURACY:
                    ...
                    suggestions.append("Better address the specific question or request in the prompt")
                elif dimension == QualityDimension.COMPLETENESS:
                    suggestions.append("Provide more comprehensive coverage with additional details and examples")
                elif dimension == QualityDimension.CLARITY:
                    suggestions.append("Use clearer language and better structure with bullet points or numbered lists")
                elif dimension == QualityDimension.ACTIONABILITY:
                    suggestions.append("Include more specific implementation steps and practical guidance")
                elif dimension == QualityDimension.COMPLIANCE_ALIGNMENT:
                    suggestions.append("Better align with specific compliance framework requirements and controls")
        return suggestions
    ...
    def _identify_improvement_areas(self, assessments: List[QualityAssessment]) -> List[str]:
        ...
        for dimension, avg_score in dimension_averages.items():
            if avg_score < 7.0:
                improvement_areas.append(f"{dimension.value.replace('_', ' ').title()}: {avg_score:.1f}/10")

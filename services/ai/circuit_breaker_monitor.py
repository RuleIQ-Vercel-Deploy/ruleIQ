...
class CircuitBreakerMonitor:
    ...
    def __init__(self, circuit_breaker: AICircuitBreaker, config: Optional[MonitoringConfig] = None) -> None:
    ...
    async def _send_alert(self, alert: Alert) -> None:
        ...
        try:
            if self.config.enable_logging:
                ...
                self.logger.log(
                    log_level,
                    "CIRCUIT BREAKER ALERT [%s]: %s - %s" % (alert.severity.value.upper(), alert.title, alert.message),
                    extra={"alert_id": alert.id, "model_name": alert.model_name, "context": alert.context},
                )
        ...
        return {
            ...
            "metrics_summary": {
                ...
                "average_failure_rate": (
                    sum(m["failure_rate"] for m in recent_metrics) / len(recent_metrics) if recent_metrics else 0
                ),
            },
            "model_availability": {model: info["available"] for model, info in health_status["models"].items()},

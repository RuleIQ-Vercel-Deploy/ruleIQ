...
class AIModelTester:
    ...
    def setup_model_comparison(
        ...
        experiment_id = create_ai_model_experiment(
            model_a=control_model, model_b=treatment_model, metric=metric.value, min_effect_size=min_effect_size
        )
        model_pair = f"{control_model}_vs_{treatment_model}"
        self.active_experiments[model_pair] = experiment_id
        logger.info("Set up model comparison: %s vs %s (exp: %s)" % (control_model, treatment_model, experiment_id))
    ...
    def setup_prompt_test(
        ...
        config = ExperimentConfig(
            ...
            metric_type=(
                ...
                if metric in [ComplianceMetric.TASK_COMPLETION_TIME, ComplianceMetric.ASSESSMENT_ACCURACY]
...
class ComplianceEffectivenessTester:
    ...
    def setup_methodology_test(
        ...
        config = ExperimentConfig(
            ...
            primary_metric=metric.value,
            secondary_metrics=["evidence_collection_rate", "user_satisfaction", "task_completion_time"],
        ...
        experiment_id = self.framework.create_experiment(config)
        logger.info("Set up compliance methodology test: %s (exp: %s)" % (methodology_name, experiment_id))
        return experiment_id
...
class ABTestingManager:
    ...
    def start_ai_model_experiment(
        self, control_model: str, treatment_model: str, metric: ComplianceMetric = ComplianceMetric.ASSESSMENT_ACCURACY
    ) -> str:
        """Start an AI model comparison experiment."""
        experiment_id = self.model_tester.setup_model_comparison(control_model, treatment_model, metric)
        self.framework.start_experiment(experiment_id)
        self.active_experiments["model_comparison"].append(experiment_id)
        return experiment_id

    def start_prompt_experiment(self, original_prompt: str, optimized_prompt: str, task_type: str) -> str:
        """Start a prompt optimization experiment."""
        experiment_id = self.prompt_tester.setup_prompt_test(original_prompt, optimized_prompt, task_type)
        self.framework.start_experiment(experiment_id)
        self.active_experiments["prompt_optimization"].append(experiment_id)
        return experiment_id

    def start_compliance_experiment(self, methodology_name: str, control_approach: str, treatment_approach: str) -> str:
...
def record_ai_performance(experiment_id: str, model_name: str, user_id: str, accuracy_score: float) -> bool:
    """Quick record for AI model performance."""
    manager = get_ab_testing_manager()
    return manager.model_tester.record_model_performance(experiment_id, model_name, user_id, accuracy_score)

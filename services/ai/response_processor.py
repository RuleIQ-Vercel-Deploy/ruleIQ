...
class AIResponseProcessor:
    ...
    def process_structured_response(
        ...
        try:
            ...
            processing_time_ms = int((datetime.utcnow() - processing_start_time).total_seconds() * 1000)
            ...
            if is_valid and validated_model:
                ...
                logger.info(f"Successfully validated {response_type} response", extra={
                ...
                return self._handle_validation_failure(
                    response_data, response_type, model_used,
                    processing_time_ms, fallback_data, errors
                )
    ...
    def _handle_parsing_failure(
        ...
        if extracted_json:
            try:
                ...
                if is_valid and validated_model:
                    ...
                    logger.warning(f"Recovered from JSON parsing failure for {response_type}", extra={
                        "response_type": response_type,
                        "model_used": model_used,
        ...
        # Complete failure
        logger.error(f"Failed to parse {response_type} response", extra={
            "response_type": response_type,
            "model_used": model_used,
            "errors": errors,
    ...
    def _handle_validation_failure(
        ...
        if partial_response:
            ...
            logger.warning(f"Created partial response for {response_type} after validation failure", extra={
                "response_type": response_type,
                "model_used": model_used,
        ...
        logger.error(f"Complete validation failure for {response_type}", extra={
            "response_type": response_type,
            "model_used": model_used,
    ...
    def _use_fallback_response(
        ...
        structured_response = self._create_structured_response(
            fallback_data, response_type, model_used,
            processing_time_ms, False, errors, fallback_used=True
        )
        
        logger.info(f"Using fallback response for {response_type}", extra={
            "response_type": response_type,
            "model_used": model_used,
            "fallback_used": True
        })
    ...
    def _create_structured_response(
        ...
        metadata = {
            ...
            "validation_status": "valid" if validation_passed and not errors else "invalid" if errors else "partially_valid",
            "validation_errors": errors
        }
        ...
        try:
            if response_type == "gap_analysis":
                ...
                return {
                    ...
                    "prioritization_rationale": response_data.get("prioritization_rationale", "Partial analysis available"),

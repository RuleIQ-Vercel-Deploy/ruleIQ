...
class AIResponseProcessor:
    ...
    def process_structured_response(
        ...
        errors = []
        try:
            processing_time_ms = int((datetime.now(timezone.utc) - processing_start_time).total_seconds() * 1000)
            try:
                response_data = json.loads(raw_response)
            except json.JSONDecodeError as e:
                errors.append(f"JSON parsing failed: {e!s}")
                return self._handle_parsing_failure(
                    raw_response, response_type, model_used, processing_time_ms, fallback_data, errors
                )
            is_valid, validation_errors, validated_model = validate_ai_response(response_data, response_type)
            ...
            if is_valid and validated_model:
                ...
                structured_response = self._create_structured_response(
                    validated_model.dict(), response_type, model_used, processing_time_ms, is_valid, errors
                )
                logger.info(
                ...
                return self._handle_validation_failure(
                    response_data, response_type, model_used, processing_time_ms, fallback_data, errors
                )
            ...
            return self._handle_processing_exception(raw_response, response_type, model_used, fallback_data, errors)
        ...
        if extracted_json:
            try:
                ...
                if is_valid and validated_model:
                    ...
                    structured_response = self._create_structured_response(
                        validated_model.dict(), response_type, model_used, processing_time_ms, True, errors
                    )
                    logger.warning(
                        "Recovered from JSON parsing failure for %s" % response_type,
                        extra={"response_type": response_type, "model_used": model_used, "recovery_successful": True},
                    )
                    return True, structured_response, errors
            except Exception as e:
                errors.append(f"JSON extraction recovery failed: {e!s}")
        if fallback_data:
            return self._use_fallback_response(fallback_data, response_type, model_used, processing_time_ms, errors)
        logger.error(
    ...
    def _handle_validation_failure(
        ...
        logger.error(
            ...
            extra={"response_type": response_type, "model_used": model_used, "validation_errors": errors},
        )
        return False, None, errors
    ...
    def _use_fallback_response(
        ...
        structured_response = self._create_structured_response(
            fallback_data, response_type, model_used, processing_time_ms, False, errors, fallback_used=True
        )
        logger.info(
    ...
    def _create_structured_response(
        ...
        metadata = {
            ...
            "validation_status": (
                "valid" if validation_passed and not errors else "invalid" if errors else "partially_valid"
            ),
        ...
        try:
            if response_type == "gap_analysis":
                return {
                    ...
                    "estimated_total_effort": response_data.get("estimated_total_effort", "Unknown"),
                ...
                return {
                    ...
                    "implementation_plan": response_data.get(
                        "implementation_plan", {"total_duration_weeks": 12, "phases": [], "success_metrics": []}
                    ),
                    "prioritization_rationale": response_data.get(
                        "prioritization_rationale", "Partial analysis available"
                    ),
                    "timeline_overview": response_data.get("timeline_overview", "Timeline needs refinement"),
    ...
    def get_validation_stats(self) -> Dict[str, Any]:
        ...
        return {
            **self.validation_stats,
            "success_rate": self.validation_stats["validation_successes"] / total if total > 0 else 0.0,
            "failure_rate": self.validation_stats["validation_failures"] / total if total > 0 else 0.0,
            "recovery_rate": self.validation_stats["error_recoveries"] / total if total > 0 else 0.0,

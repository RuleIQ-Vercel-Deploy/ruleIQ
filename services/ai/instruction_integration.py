...
class InstructionManager:
    ...
    def get_instruction_with_monitoring(
        ...
        **kwargs,
        ...
        instruction_id = self._create_instruction_id(instruction_type, framework, user_persona, task_complexity)
    ...
    def get_model_with_instruction(
        ...
        **kwargs,
        ...
        model = get_ai_model(
            ...
            task_context={"framework": framework, "task_type": instruction_type, "business_context": business_profile},
    ...
    def start_instruction_ab_test(
        ...
        variant_id, variant_content = self.get_instruction_with_monitoring(
            instruction_type=instruction_type, framework=framework, task_complexity="medium", **variant_context
        )
    ...
    def get_instruction_analytics(
        self, instruction_type: Optional[str] = None, framework: Optional[str] = None, time_window_days: int = 7
    ) -> Dict[str, Any]:
        """
        ...
        report = self.monitor.generate_performance_report(time_window=time_window, include_ab_tests=True)
    ...
    def optimize_instruction(self, instruction_id: str, optimization_type: str = "quality") -> Dict[str, Any]:
        """
        ...
        suggestions = []
        if optimization_type == "quality" and performance.avg_response_quality < CONFIDENCE_THRESHOLD:
            suggestions.append(
                {
                    "type": "specificity",
    ...
    def _create_instruction_id(
        self, instruction_type: str, framework: Optional[str], user_persona: Optional[str], task_complexity: str
    ) -> str:
        """Create a unique instruction ID based on parameters"""
        components = [instruction_type, framework or "generic", user_persona or "default", task_complexity]
        return f"instr_{'-'.join(components)}"

    def _calculate_effectiveness_score(
        self, quality: Optional[float], satisfaction: Optional[float], response_time: Optional[float], had_error: bool
    ) -> float:

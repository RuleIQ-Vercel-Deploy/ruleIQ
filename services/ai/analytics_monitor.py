...
class AIAnalyticsMonitor:
    ...
    async def _create_alert(
        self, level: AlertLevel, title: str, description: str, metadata: Optional[Dict[str, Any]] = None
    ) -> None:
        """Create a new alert."""
        alert = Alert(
    ...
    async def get_cost_analytics(self, days: int = 30) -> Dict[str, Any]:
        ...
        start_time = end_time - timedelta(days=days)
        cost_metrics = [
            m for m in self.metrics if m.timestamp >= start_time and m.name in ["cost_estimate", "token_usage"]
        ]
    ...
    async def get_quality_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "quality_trends": {"recommendations": 8.7, "policies": 8.9, "workflows": 8.3, "analysis": 8.1},
            "feedback_summary": {

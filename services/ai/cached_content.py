...
class GoogleCachedContentManager:
        ...
        return {
            ...
            'cache_types': {
                cache_type.value: len([
                    k for k, m in self.cache_metadata.items() 
    ...
    def _calculate_assessment_ttl(self, framework_id: str, business_profile: Dict[str, Any]) -> int:
        ...
        return max(
            self.config.min_ttl_minutes // 60,
            min(self.config.max_ttl_hours, base_ttl)
        )
    ...
    def record_cache_performance(self, cache_key: str, response_time_ms: int, hit: bool = True):
        ...
        logger.debug(f"Cache performance recorded for {cache_key}: {response_time_ms}ms, adjustment: {ttl_adjustment}")
    ...
    def add_to_warming_queue(self, content_type: CacheContentType, context: Dict[str, Any], priority: int = 5):
    ...
    def _should_warm_cache(self, entry: Dict[str, Any]) -> bool:
        ...
        cache_key = self._generate_cache_key(content_type, context.get('framework_id', ''), 
    ...
    def _invalidate_assessment_caches(self, framework_id: str, business_profile_id: str):
        ...
        for cache_key, metadata in self.cache_metadata.items():
            if (metadata.get('framework_id') == framework_id and 
                metadata.get('business_profile_id') == business_profile_id and
                metadata.get('type') == CacheContentType.ASSESSMENT_CONTEXT.value):
                keys_to_invalidate.append(cache_key)
    ...
    def get_cache_strategy_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'intelligent_invalidation': {
                ...
                'recent_triggers': len([t for t in self.invalidation_triggers.values() 

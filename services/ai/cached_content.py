...
class GoogleCachedContentManager:
    ...
    async def create_assessment_cache(
        ...
        try:
            cache_key = self._generate_cache_key(
                CacheContentType.ASSESSMENT_CONTEXT, framework_id, business_profile.get("id", "unknown")
            )
            ...
            cache_content = self._build_assessment_cache_content(framework_id, business_profile, assessment_context)
            ...
            self.metrics["cache_creates"] += 1
            self.metrics["total_size_cached_mb"] += self.cache_metadata[cache_key]["size_estimate_mb"]
    ...
    async def create_business_profile_cache(
        ...
        try:
            ...
            self.metrics["cache_creates"] += 1
            self.metrics["total_size_cached_mb"] += self.cache_metadata[cache_key]["size_estimate_mb"]
            logger.info("Created business profile cache: %s with %sh TTL" % (display_name, ttl_hours))
            return cached_content
        except (OSError, json.JSONDecodeError, requests.RequestException) as e:
    ...
    def get_cache_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "cache_types": {
                cache_type.value: len([k for k, m in self.cache_metadata.items() if m.get("type") == cache_type.value])
    ...
    def _generate_business_profile_cache_key(self, business_profile: Dict[str, Any]) -> str:
        ...
        similarity_factors = {
            "industry": business_profile.get("industry", ""),
            "employee_count_range": self._get_employee_count_range(business_profile.get("employee_count", 0)),
            "existing_frameworks": sorted(business_profile.get("existing_frameworks", [])),
            "has_international_operations": business_profile.get("has_international_operations", False),
        ...
        return self._generate_cache_key(
            CacheContentType.BUSINESS_PROFILE, hashlib.sha256(similarity_key.encode()).hexdigest()[:12]
        )
    ...
    def _build_assessment_cache_content(
        self, framework_id: str, business_profile: Dict[str, Any], assessment_context: Optional[Dict[str, Any]] = None
    ) -> List[str]:
        ...
        if business_profile.get("cloud_providers"):
            content_parts.append(f"Cloud Providers: {', '.join(business_profile['cloud_providers'])}")
        ...
        if business_profile.get("existing_frameworks"):
            content_parts.append(f"Existing Frameworks: {', '.join(business_profile['existing_frameworks'])}")
    ...
    def _build_framework_cache_content(self, framework_id: str, industry_context: Optional[str] = None) -> List[str]:
    ...
    def record_cache_performance(self, cache_key: str, response_time_ms: int, hit: bool = True) -> None:
        ...
        logger.debug(
            "Cache performance recorded for %s: %sms, adjustment: %s" % (cache_key, response_time_ms, ttl_adjustment)
        )
    ...
    def add_to_warming_queue(self, content_type: CacheContentType, context: Dict[str, Any], priority: int = 5) -> None:
        ...
        logger.debug("Added %s to cache warming queue with priority %s" % (content_type.value, priority))
        ...
        try:
            if content_type == CacheContentType.ASSESSMENT_CONTEXT:
                await self.create_assessment_cache(
                    context["framework_id"], context["business_profile"], context.get("assessment_context")
                )
            elif content_type == CacheContentType.BUSINESS_PROFILE:
                await self.create_business_profile_cache(context["business_profile"])
            elif content_type == CacheContentType.FRAMEWORK_CONTEXT:
                await self.create_framework_cache(context["framework_id"], context.get("industry_context", "General"))
    ...
    def trigger_intelligent_invalidation(self, trigger_type: str, context: Dict[str, Any]) -> None:
        ...
        if trigger_type == "business_profile_update":
            ...
            self._invalidate_assessment_caches(context["framework_id"], context["business_profile_id"])
    ...
    def get_cache_strategy_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "cache_warming": {
                ...
                "high_priority_items": len([e for e in self.cache_warming_queue if e["priority"] <= 2]),
            },

...
class AITimeoutException(AIServiceException):
    ...
    def __init__(self, timeout_seconds: float, service_name: str=
        'AI Service', context: Optional[Dict[str, Any]]=None) ->None:
        message = (
            f'{service_name} request timed out after {timeout_seconds} seconds'
            ,)
...
class AIQuotaExceededException(AIServiceException):
    ...
    def __init__(self, quota_type: str='requests', service_name: str=
        'AI Service', context: Optional[Dict[str, Any]]=None) ->None:
...
class AIModelException(AIServiceException):
    ...
    def __init__(self, model_name: str, model_error: str, service_name: str
        ='AI Service', context: Optional[Dict[str, Any]]=None) ->None:
        message = f"{service_name} model '{model_name}' error: {model_error}"
        super().__init__(message=message, service_name=service_name,
            error_code='AI_MODEL_ERROR', context=context)
...
class AIContentFilterException(AIServiceException):
    ...
    def __init__(self, filter_reason: str, service_name: str='AI Service',
...
class AIParsingException(BusinessLogicException):
    ...
    def __init__(self, response_text: str, expected_format: str,
        parsing_error: str, context: Optional[Dict[str, Any]]=None) ->None:
        message = (
            f'Failed to parse AI response as {expected_format}: {parsing_error}'
            ,)
        super().__init__(message)
...
class AIValidationException(BusinessLogicException):
    ...
    def __init__(self, validation_errors: list, response_data: Dict[str,
        Any], context: Optional[Dict[str, Any]]=None) ->None:
        message = (
            f"AI response validation failed: {', '.join(validation_errors)}")
        super().__init__(message)
...
class ModelUnavailableException(AIServiceException):
    ...
    def __init__(self, model_name: str, reason: str='Circuit breaker open',
        service_name: str='AI Service', context: Optional[Dict[str, Any]]=None
        ) ->None:
...
class ModelTimeoutException(AIServiceException):
    ...
    def __init__(self, model_name: str, timeout_seconds: float, operation:
        str='generate_content', service_name: str='AI Service', context:
        Optional[Dict[str, Any]]=None) ->None:
        message = (
            f"Model '{model_name}' timed out after {timeout_seconds}s during {operation}"
            ,)
        super().__init__(message=message, service_name=service_name,
            error_code='MODEL_TIMEOUT', context=context)
        self.model_name = model_name
        self.timeout_seconds = timeout_seconds
...
class ModelOverloadedException(AIServiceException):
    ...
    def __init__(self, model_name: str, retry_after: Optional[int]=None,
        ...
        super().__init__(message=message, service_name=service_name,
            error_code='MODEL_OVERLOADED', context=context)
        self.model_name = model_name
...
class ModelConfigurationException(AIServiceException):
    ...
    def __init__(self, model_name: str, config_error: str, service_name:
        str='AI Service', context: Optional[Dict[str, Any]]=None) ->None:
...
class CircuitBreakerException(AIServiceException):
    ...
    def __init__(self, circuit_state: str, model_name: Optional[str]=None,
        failure_count: Optional[int]=None, service_name: str=
        'AI Circuit Breaker', context: Optional[Dict[str, Any]]=None) ->None:
        if model_name:
            message = (
                f"Circuit breaker {circuit_state} for model '{model_name}'")
        else:
            message = f'Circuit breaker {circuit_state}'
        if failure_count:
            message += f' (failures: {failure_count})'
        super().__init__(message=message, service_name=service_name,
            error_code='CIRCUIT_BREAKER_OPEN', context=context)
...
class SchemaValidationException(AIServiceException):
    ...
    def __init__(self, response_type: str, validation_errors: list,
        response_data: Optional[Dict[str, Any]]=None, model_name: str=
        'unknown', service_name: str='AI Schema Validator', context:
        Optional[Dict[str, Any]]=None) ->None:
        ...
        if validation_errors:
            ...
            if len(validation_errors) > MAX_RETRIES:
                error_summary += (
                    f' (and {len(validation_errors) - 3} more errors)')
        super().__init__(message=error_summary, service_name=service_name,
            error_code='SCHEMA_VALIDATION_FAILED', context=context)
...
class ResponseProcessingException(AIServiceException):
    ...
    def __init__(self, response_type: str, processing_stage: str,
        original_error: str, model_name: str='unknown', service_name: str=
        'AI Response Processor', context: Optional[Dict[str, Any]]=None
        ) ->None:
        message = (
            f'Response processing failed at {processing_stage} for {response_type}: {original_error}'
            ,)
        super().__init__(message=message, service_name=service_name,
            error_code='RESPONSE_PROCESSING_FAILED', context=context)
        self.response_type = response_type
...
class ModelRetryExhaustedException(AIServiceException):
    ...
    def __init__(self, model_name: str, attempts: int, last_error: str,
        service_name: str='AI Service', context: Optional[Dict[str, Any]]=None
        ) ->None:
        message = (
            ...
            ,)
        super().__init__(message=message, service_name=service_name,
            error_code='RETRY_EXHAUSTED', context=context)
        self.model_name = model_name
...
GEMINI_ERROR_MAPPING = {'DEADLINE_EXCEEDED': ModelTimeoutException,
    ...
    'UNAUTHENTICATED': AIServiceException, 'UNAVAILABLE':
    ModelUnavailableException, 'INTERNAL': AIServiceException}
ERROR_PATTERN_MAPPING = {'timeout': ModelTimeoutException, 'quota':
    ModelOverloadedException, 'rate limit': ModelOverloadedException,
    'overloaded': ModelOverloadedException, 'unavailable':
    ModelUnavailableException, 'not found': ModelUnavailableException,
    'safety': AIContentFilterException, 'filter': AIContentFilterException,
    'configuration': ModelConfigurationException, 'invalid model':
...
def map_gemini_error(error: Exception, model_name: str='unknown', context:
    ...
    if error_type and error_type.name in GEMINI_ERROR_MAPPING:
        ...
        if exception_class == ModelTimeoutException:
            return ModelTimeoutException(model_name=model_name,
                timeout_seconds=30.0, service_name='Google Gemini', context
                =context)
        elif exception_class == ModelOverloadedException:
            return ModelOverloadedException(model_name=model_name,
                service_name='Google Gemini', context=context)
        elif exception_class == ModelUnavailableException:
            return ModelUnavailableException(model_name=model_name, reason=
                error_message, service_name='Google Gemini', context=context)
        elif exception_class == ModelConfigurationException:
            return ModelConfigurationException(model_name=model_name,
                config_error=error_message, service_name='Google Gemini',
                context=context)
        else:
            return exception_class(message=error_message, service_name=
                'Google Gemini', error_code=error_type.name, context=context)
    error_lower = error_message.lower()
    for pattern, exception_class in ERROR_PATTERN_MAPPING.items():
        if pattern in error_lower:
            if exception_class == ModelTimeoutException:
                return ModelTimeoutException(model_name=model_name,
                    timeout_seconds=30.0, service_name='Google Gemini',
                    context=context)
            elif exception_class == ModelOverloadedException:
                return ModelOverloadedException(model_name=model_name,
                    service_name='Google Gemini', context=context)
            elif exception_class == ModelUnavailableException:
                return ModelUnavailableException(model_name=model_name,
                    reason=error_message, service_name='Google Gemini',
                    context=context)
            elif exception_class == ModelConfigurationException:
                return ModelConfigurationException(model_name=model_name,
                    config_error=error_message, service_name=
                    'Google Gemini', context=context)
            elif exception_class == AIContentFilterException:
                return AIContentFilterException(filter_reason=error_message,
                    service_name='Google Gemini', context=context)
...
def handle_ai_error(error: Exception, operation: str, model_name: str=
    ...
    Optional[Any]=None) ->tuple[Optional[Any], AIServiceException]:
    """
    ...
    logger.error('AI error in %s: %s' % (operation, ai_exception.message),
        extra={'operation': operation, 'error_code': ai_exception.
        error_code, 'service_name': ai_exception.service_name, 'context':
        ai_exception.context})

...
class SafetyProfile:
    ...
    harassment_threshold: HarmBlockThreshold = (HarmBlockThreshold.
        BLOCK_MEDIUM_AND_ABOVE)
    hate_speech_threshold: HarmBlockThreshold = (HarmBlockThreshold.
        BLOCK_MEDIUM_AND_ABOVE)
    sexually_explicit_threshold: HarmBlockThreshold = (HarmBlockThreshold.
        BLOCK_MEDIUM_AND_ABOVE)
    dangerous_content_threshold: HarmBlockThreshold = (HarmBlockThreshold.
        BLOCK_MEDIUM_AND_ABOVE)
            ...
            if not user_permissions.get(permission, False):
                logger.warning(
                    "Access denied: User %s with role %s lacks permission '%s' for %s"
                     % (self.user_id, self.user_role, permission, func.
                    __name__))
...
class AdvancedSafetyManager:
    ...
    def __init__(self, user_context: Optional[Dict[str, Any]]=None) ->None:
        ...
        self.role_permissions = {'viewer': {'can_view_metrics': True,
            ...
            'can_modify_settings': True, 'can_override_safety': False,
            'max_content_risk_level': 'high'}, 'admin': {'can_view_metrics':
            True, 'can_view_audit_trail': True, 'can_create_profiles': True,
            'can_modify_settings': True, 'can_override_safety': True,
        ...
        self.compliance_keywords = ['gdpr', 'iso27001', 'sox', 'hipaa',
            'pci dss', 'regulation', 'compliance', 'audit', 'control',
            'policy', 'procedure']
        logger.info(
    ...
    def _initialize_default_profiles(self) ->None:
        ...
        self.safety_profiles['permissive'] = SafetyProfile(name=
            'Permissive Testing', level=SafetyLevel.PERMISSIVE,
            ...
            BLOCK_MEDIUM_AND_ABOVE, dangerous_content_threshold=
            HarmBlockThreshold.BLOCK_ONLY_HIGH, min_confidence_threshold=
            0.5, require_citations=False, enable_audit_logging=True)
        self.safety_profiles['standard'] = SafetyProfile(name=
            'Standard Compliance', level=SafetyLevel.STANDARD,
            content_types=[ContentType.ASSESSMENT_GUIDANCE, ContentType.
            COMPLIANCE_ADVICE, ContentType.GENERAL_INQUIRY],
            ...
            min_confidence_threshold=0.7, require_citations=True,
            enable_audit_logging=True)
        self.safety_profiles['strict'] = SafetyProfile(name=
            'Strict Regulatory', level=SafetyLevel.STRICT, content_types=[
            ContentType.REGULATORY_INTERPRETATION, ContentType.
            POLICY_GENERATION], harassment_threshold=HarmBlockThreshold.
            BLOCK_LOW_AND_ABOVE, hate_speech_threshold=HarmBlockThreshold.
            BLOCK_LOW_AND_ABOVE, sexually_explicit_threshold=
            HarmBlockThreshold.BLOCK_LOW_AND_ABOVE,
            dangerous_content_threshold=HarmBlockThreshold.
            BLOCK_LOW_AND_ABOVE, min_confidence_threshold=0.85,
            escalation_confidence_threshold=0.7, require_regulatory_context
            =True, require_citations=True, enable_audit_logging=True,
            max_response_length=3000)
        self.safety_profiles['enterprise'] = SafetyProfile(name=
            'Enterprise Maximum Safety', level=SafetyLevel.ENTERPRISE,
            content_types=list(ContentType), harassment_threshold=
            HarmBlockThreshold.BLOCK_LOW_AND_ABOVE, hate_speech_threshold=
            HarmBlockThreshold.BLOCK_LOW_AND_ABOVE,
            sexually_explicit_threshold=HarmBlockThreshold.
            BLOCK_LOW_AND_ABOVE, dangerous_content_threshold=
            HarmBlockThreshold.BLOCK_LOW_AND_ABOVE,
            min_confidence_threshold=0.9, escalation_confidence_threshold=
            0.8, require_regulatory_context=True, require_citations=True,
            enable_audit_logging=True, max_response_length=2500,
            blocked_keywords=['hack', 'exploit', 'bypass', 'circumvent',
            'illegal', 'unlawful', 'fraudulent', 'ignore', 'skip', 'avoid'])
    ...
    def can_access_content_type(self, content_type: ContentType) ->bool:
        ...
        content_risk_levels = {ContentType.GENERAL_QUESTION: 'low',
            ContentType.REGULATORY_INTERPRETATION: 'high', ContentType.
            POLICY_GENERATION: 'high', ContentType.EVIDENCE_CLASSIFICATION:
            'medium', ContentType.ASSESSMENT_GUIDANCE: 'medium',
    ...
    def get_role_specific_safety_profile(self, content_type: ContentType
        ) ->str:
        """Get safety profile appropriate for user role and content type."""
        if self.user_role == 'admin' and self.check_permission(
            'can_override_safety'):
            return 'permissive'
        elif self.user_role == 'compliance_officer':
            return 'balanced'
        elif content_type in [ContentType.REGULATORY_INTERPRETATION,
            ContentType.POLICY_GENERATION]:
            return 'strict'
        else:
            return 'standard'

    def log_authorization_decision(self, action: str, permission: str,
        granted: bool, context: Optional[Dict[str, Any]]=None) ->None:
        """Log authorization decisions for audit purposes."""
        decision_record = SafetyDecisionRecord(decision_id=str(uuid4()),
            user_id=self.user_id, content_type=ContentType.GENERAL_QUESTION if
            context and context.get('content_type') == 'unknown' else
            ContentType.GENERAL_INQUIRY, safety_profile='standard',
            input_content=f'AUTHORIZATION: {action}', decision=
            SafetyDecision.ALLOW if granted else SafetyDecision.BLOCK,
            confidence_score=1.0, reasoning=
            f"Permission '{permission}' {'granted' if granted else 'denied'} for role '{self.user_role}'"
            , applied_filters=[], timestamp=datetime.now(), metadata={
            'permission_requested': permission, 'user_context': self.
            user_context, 'additional_context': context or {}})
    ...
    def get_content_filters_for_role(self) ->Dict[str, Any]:
        ...
        base_filters = {'blocked_patterns': self.global_blocked_patterns.
            copy(), 'compliance_keywords': self.compliance_keywords.copy()}
    ...
    def get_safety_settings(self, content_type: ContentType, profile_name:
        str='standard', context: Optional[Dict[str, Any]]=None) ->Dict[
        ...
        """
        profile = self.safety_profiles.get(profile_name, self.
            safety_profiles['standard'])
        settings = {HarmCategory.HARM_CATEGORY_HARASSMENT: profile.
            harassment_threshold, HarmCategory.HARM_CATEGORY_HATE_SPEECH:
            profile.hate_speech_threshold, HarmCategory.
            HARM_CATEGORY_SEXUALLY_EXPLICIT: profile.
            sexually_explicit_threshold, HarmCategory.
            HARM_CATEGORY_DANGEROUS_CONTENT: profile.
            dangerous_content_threshold}
        if content_type in [ContentType.REGULATORY_INTERPRETATION,
            ContentType.POLICY_GENERATION]:
    ...
    def evaluate_content_safety(self, input_content: str, response_content:
        str, content_type: ContentType, confidence_score: float,
        profile_name: str='standard', user_id: Optional[str]=None,
        session_id: Optional[str]=None) ->SafetyDecisionRecord:
        """
        ...
        if not self.can_access_content_type(content_type):
            self.log_authorization_decision(action=
                'evaluate_content_safety', permission='access_content_type',
                granted=False, context={'content_type': content_type.value})
            raise PermissionError(
                f"User role '{self.user_role}' cannot access content type '{content_type.value}'"
                )
        self.log_authorization_decision(action='evaluate_content_safety',
            permission='access_content_type', granted=True, context={
            'content_type': content_type.value})
        role_appropriate_profile = self.get_role_specific_safety_profile(
            content_type)
        ...
        risk_level_map = {'minimal': 'low', 'standard': 'medium', 'strict':
            'high', 'regulatory': 'critical'}
        profile_risk = risk_level_map.get(profile.level.value, 'medium')
        allowed_levels = {'low': ['low'], 'medium': ['low', 'medium'],
            'high': ['low', 'medium', 'high'], 'critical': ['low', 'medium',
            'high', 'critical']}
        ...
        blocked_found = self._check_blocked_content(input_content + ' ' +
            response_content, profile)
        if blocked_found:
            if decision == SafetyDecision.ALLOW:
                decision = SafetyDecision.MODIFY
                reasoning = (
                    f"Blocked keywords found: {', '.join(blocked_found)}")
        ...
        if profile.require_regulatory_context and content_type in [ContentType
            .REGULATORY_INTERPRETATION, ContentType.COMPLIANCE_ADVICE]:
        ...
        if profile.require_citations and not self._has_citations(
            response_content):
            if decision == SafetyDecision.ALLOW:
                decision = SafetyDecision.MODIFY
                reasoning = 'Missing required citations'
            applied_filters.append('citation_requirement')
        record = SafetyDecisionRecord(decision_id=decision_id, content_type
            =content_type, safety_profile=profile_name, input_content=
            input_content[:500], decision=decision, confidence_score=
            confidence_score, reasoning=reasoning, applied_filters=
            applied_filters, timestamp=datetime.now(), user_id=user_id,
            session_id=session_id, metadata={'response_length': len(
            response_content), 'profile_level': profile.level.value})
    ...
    def _has_regulatory_context(self, content: str) ->bool:
        ...
        content_lower = content.lower()
        compliance_mentions = sum(1 for keyword in self.compliance_keywords if
            keyword in content_lower)
        regulatory_indicators = ['regulation', 'requirement', 'compliance',
            'standard', 'framework', 'guideline', 'mandate', 'obligation']
        regulatory_mentions = sum(1 for indicator in regulatory_indicators if
    ...
    def _update_metrics(self, record: SafetyDecisionRecord) ->None:
        ...
        total_confidence = self.metrics.avg_confidence_score * (self.
            metrics.total_decisions - 1) + record.confidence_score
        self.metrics.avg_confidence_score = (total_confidence / self.
        ...
        low_confidence_count = sum(1 for r in self.decision_history[-1000:] if
            r.confidence_score < 0.7)
        self.metrics.low_confidence_rate = low_confidence_count / min(1000,
            len(self.decision_history))

    def modify_response_content(self, content: str, applied_filters: List[str]
        ) ->str:
...
[Disclaimer: This information is provided for general guidance only. Specific regulatory requirements may vary by jurisdiction and industry. Please consult relevant regulations and qualified compliance professionals.]"""
    ...
    def get_safety_metrics(self) ->Dict[str, Any]:
        ...
        self.log_authorization_decision(action='get_safety_metrics',
            permission='can_view_metrics', granted=True)
        if self.metrics.total_decisions == 0:
            return {'total_decisions': 0, 'decision_rates': {'allowed_rate':
                0.0, 'blocked_rate': 0.0, 'modified_rate': 0.0,
                'escalated_rate': 0.0}, 'confidence_metrics': {
                'average_confidence': 0.0, 'low_confidence_rate': 0.0},
                'profile_usage': {}, 'content_type_distribution': {}}
        total = self.metrics.total_decisions
        return {'total_decisions': total, 'decision_rates': {'allowed_rate':
            round(self.metrics.allowed_count / total, 3), 'blocked_rate':
            round(self.metrics.blocked_count / total, 3), 'modified_rate':
            round(self.metrics.modified_count / total, 3), 'escalated_rate':
            round(self.metrics.escalated_count / total, 3)},
            'confidence_metrics': {'average_confidence': round(self.metrics
            .avg_confidence_score, 3), 'low_confidence_rate': round(self.
            metrics.low_confidence_rate, 3)}, 'profile_usage': self.metrics
            .profile_usage.copy(), 'content_type_distribution': self.
            metrics.content_type_distribution.copy(), 'recent_decisions':
            len([r for r in self.decision_history if (datetime.now() - r.
            timestamp).total_seconds() < HOUR_SECONDS])}

    async def get_audit_trail(self, hours: int=24, user_id: Optional[str]=
        None, content_type: Optional[ContentType]=None) ->List[Dict[str, Any]]:
        ...
        return [{'decision_id': record.decision_id, 'timestamp': record.
            timestamp.isoformat(), 'content_type': record.content_type.
            value, 'decision': record.decision.value, 'confidence_score':
            record.confidence_score, 'reasoning': record.reasoning,
            'applied_filters': record.applied_filters, 'safety_profile':
            record.safety_profile, 'user_id': record.user_id, 'session_id':
            record.session_id} for record in filtered_records]

    async def create_custom_profile(self, name: str, base_profile: str=
        'standard', overrides: Optional[Dict[str, Any]]=None) ->str:
        """Create a custom safety profile."""
        self.require_permission('can_create_profiles',
            'create custom safety profiles')
        ...
        profile_data = {'name': name, 'level': base.level, 'content_types':
            base.content_types.copy(), 'harassment_threshold': base.
            harassment_threshold, 'hate_speech_threshold': base.
            hate_speech_threshold, 'sexually_explicit_threshold': base.
            sexually_explicit_threshold, 'dangerous_content_threshold':
            base.dangerous_content_threshold, 'min_confidence_threshold':
            base.min_confidence_threshold,
            'escalation_confidence_threshold': base.
            escalation_confidence_threshold, 'blocked_keywords': base.
            blocked_keywords.copy(), 'allowed_keywords': base.
            allowed_keywords.copy(), 'require_citations': base.
            require_citations, 'require_regulatory_context': base.
            require_regulatory_context, 'enable_audit_logging': base.
            enable_audit_logging, 'max_response_length': base.
            max_response_length}
        if overrides:
        ...
        logger.info('User %s created custom safety profile: %s' % (self.
            user_id, name))
        return profile_key

    def validate_prompt_security(self, prompt_text: str, context_type: str=
        ...
        if not self.check_permission('process_sensitive_content') and analysis[
            'threat_level'] in ['suspicious', 'malicious']:
            analysis['recommended_action'] = 'ESCALATE_FOR_REVIEW'
            analysis['reason'
                ] = 'Insufficient permissions for handling suspicious content'
        self.log_authorization_decision(action='validate_prompt_security',
            permission='process_sensitive_content', granted=analysis[
            'recommended_action'] != 'ESCALATE_FOR_REVIEW', metadata={
            'threat_level': analysis['threat_level'], 'confidence':
...
class RoleBasedSafetyMiddleware:
    ...
    def get_safety_manager(self, user_context: Dict[str, Any]
        ) ->AdvancedSafetyManager:
        """Get safety manager with user context."""
        return get_safety_manager(user_context)

    def check_content_access(self, user_role: str, content_type: ContentType
        ) ->bool:

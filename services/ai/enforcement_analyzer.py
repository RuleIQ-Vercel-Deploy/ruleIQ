...
class EnforcementAnalyzer:
    ...
    def __init__(self, enforcement_db_path: str = "data/enforcement/uk_enforcement_database.json"):
    ...
    def _analyze_patterns(self) -> None:
        ...
        for pattern_tag, actions in pattern_groups.items():
            ...
            for action in actions:
                violations.extend(action.get("violation_details", {}).get("secondary_violations", []))
            ...
            self.patterns_cache[pattern_tag] = EnforcementPattern(
                ...
                evidence_references=[a["id"] for a in actions[:3]]  # Top 3 for evidence
            )
    ...
    def get_enforcement_evidence(self, 
                                regulation: str,
    ...
    def calculate_risk_adjustment(self,
        ...
        if business_context.get("serves_vulnerable_customers"):
            vulnerable_cases = [e for e in evidence if "vulnerable" in str(e.get("violation", "")).lower()]
            if vulnerable_cases:
    ...
    def identify_compliance_gaps(self,
        ...
        sector_actions = [
            a for a in self.enforcement_data.get("enforcement_actions", [])
        ...
        for gap, frequency in gap_frequency.items():
            ...
            examples = [
                a["id"] for a in sector_actions
                if gap in a.get("compliance_gaps_identified", [])
            ][:3]
            
            gaps.append({
                "gap_id": gap,
                "priority_score": frequency * 2,  # Higher frequency = higher priority
    ...
    def get_regulatory_trends(self, 
        ...
        for reg, actions in regulator_actions.items():
            ...
            trends.append(RegulatoryTrend(
                regulator=reg,
                trend_direction=direction,
                focus_areas=focus_areas,
                recent_penalty_avg=avg_recent,
                year_over_year_change=(avg_recent - avg_older) / avg_older if avg_older > 0 else 0,
    ...
    def generate_enforcement_summary(self, 
                                    regulation: str,
...
class IQEnforcementIntegration:
    ...
    def learn_from_enforcement(self, 
                              regulation_id: str,
    ...
    def get_evidence_for_claim(self,
                              claim_type: str,
                              regulation: str,
...
if __name__ == "__main__":
    ...
    evidence = analyzer.get_enforcement_evidence(
        regulation="gdpr",
        sector="finance"
    )
    ...
    for trend in trends[:3]:
        print(f"  - {trend.regulator}: {trend.trend_direction} "

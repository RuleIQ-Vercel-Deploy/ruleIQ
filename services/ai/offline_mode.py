...
class OfflineDatabase:
    ...
    def store_cached_response(self, operation: str, context_hash: str, response: str, confidence: float) -> None:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            """,
                (operation, context_hash, response, confidence, datetime.now().isoformat(), operation, context_hash),
            )
            conn.commit()
...
class OfflineModeManager:
    ...
    def __init__(self, mode: OfflineMode = OfflineMode.ENHANCED) -> None:
        ...
        self.logger = logging.getLogger(__name__)
        self.offline_stats = {"requests_served": 0, "cache_hits": 0, "template_uses": 0, "assessment_completions": 0}
        self.service_status = {"ai_service": False, "last_check": datetime.now(), "consecutive_failures": 0}
    ...
    def _get_cached_response(self, operation: str, context: Dict[str, Any]) -> Optional[FallbackResponse]:
...
• Contact support for urgent assistance"""
            ...
            return FallbackResponse(
                content=f"The {
                    operation.replace(
                        '_',
                        ' ')} service is currently unavailable offline. Please try again when AI services are restored, or use the basic assessment tools available.",
                confidence=0.5,
    ...
    def _handle_unknown_operation(self, operation: str, context: Dict[str, Any]) -> FallbackResponse:
    ...
    def _generate_context_hash(self, context: Dict[str, Any]) -> str:
        ...
        return hashlib.md5(context_string.encode(), usedforsecurity=False).hexdigest()[:16]
    ...
    def sync_offline_requests(self) -> Dict[str, Any]:
        ...
        if self.service_status["ai_service"]:
            ...
            return {
                ...
                "sync_status": "ready",
                "high_priority_requests": len([r for r in pending_requests if r.priority >= MAX_RETRIES]),
        ...
        if ai_service_available:
            if not self.service_status["ai_service"]:
                ...
                if sync_status["pending_requests"] > 0:
                    self.logger.info("Syncing %s offline requests" % sync_status["pending_requests"])

...
class IQNeo4jIntegration:
    ...
    async def assess_compliance_risk(
        ...
        avg_risk = (
            total_risk_score / len(applicable_regulations)
    ...
    async def get_compliance_roadmap(
        ...
        """
        # First get the risk assessment
        assessment = await self.assess_compliance_risk(business_profile)
        regulations = assessment["applicable_regulations"]

        # Query for dependencies to order properly
        roadmap = []
        processed = set()

        for reg in sorted(
            regulations, key=lambda x: x["adjusted_risk_score"], reverse=True
        ):
            ...
            roadmap.append(
                {
                    ...
                    "controls": reg["suggested_controls"][:5],  # Top 5 controls
                }
            )
    ...
    async def find_control_overlaps(self, regulation_ids: List[str]) -> Dict[str, Any]:
        ...
        efficiency_gain = (
            (1 - (overlapping_controls / total_controls)) * 100
            if total_controls > 0
            else 0
        )
                ...
                patterns.append(
                    {
                        ...
                        "recent_cases": record["enforcements"][:5],  # Last 5 cases
                    }
                )
    ...
    async def get_automation_opportunities(
        ...
        async with self.driver.session() as session:
            ...
            async for record in result:
                ...
                opportunity = {
                    ...
                    "complexity": record["complexity"] or 5,
                    "automatable_controls": self._identify_automatable_controls(
                        record["controls"]
                    ),
        ...
        avg_automation = (
            total_automation_score / len(opportunities) if opportunities else 0
        )
    ...
    def _categorize_risk_level(self, avg_risk: float, high_risk_count: int) -> str:
        ...
        if avg_risk >= 8 or high_risk_count >= 5:
            return "CRITICAL"
        elif avg_risk >= 6 or high_risk_count >= 3:
            return "HIGH"
        elif avg_risk >= 4:
            return "MEDIUM"
        else:
    ...
    def _calculate_automation_potential(self, regulations: List[Dict]) -> float:
        ...
        if not regulations:
            return 0
        scores = [
            r["automation_potential"] for r in regulations if r["automation_potential"]
        ]
    ...
    def _estimate_timeline(self, regulations: List[Dict]) -> int:
        ...
        for reg in regulations:
            ...
            if "month" in str(timeline).lower():
                # Extract months from strings like "6-12 months"
                ...
                total_months += 6  # Default estimate
    ...
    def _identify_top_controls(self, regulations: List[Dict]) -> List[str]:
        ...
        # Return top 10 controls
        sorted_controls = sorted(
            control_counts.items(), key=lambda x: x[1], reverse=True
        )
    ...
    def _get_prioritization_reason(self, regulation: Dict) -> str:
        ...
        elif regulation["enforcement_count"] > 5:
    ...
    def _optimize_control_order(self, overlaps: List[Dict]) -> List[Dict]:
        ...
        # Sort by coverage ratio and regulation count
        return sorted(
            overlaps,
            key=lambda x: (x["coverage_ratio"], x["regulation_count"]),
            reverse=True,
        )[:10]
    ...
    def _generate_enforcement_recommendations(self, patterns: List[Dict]) -> List[str]:
        ...
        high_frequency = [p for p in patterns if p["enforcement_count"] >= 5]
        if high_frequency:
            recommendations.append(
    ...
    def _estimate_efficiency_gain(self, opportunities: List[Dict]) -> Dict[str, Any]:
        ...
        medium_auto = len(
            [o for o in opportunities if 0.4 <= o["automation_score"] < 0.7]
        )

        # Rough estimates of time savings
        time_saved_hours = (high_auto * 40) + (medium_auto * 20)  # Hours per year
        cost_saved = time_saved_hours * 150  # Assuming $150/hour
    ...
    def _create_automation_roadmap(self, opportunities: List[Dict]) -> List[Dict]:
        ...
        # Phase 1: High automation, low complexity
        phase1 = [
            o
            for o in opportunities
            if o["automation_score"] >= 0.7 and o["complexity"] <= 5
        ]
        if phase1:
            roadmap.append(
                {
        ...
        # Phase 2: High automation, higher complexity
        phase2 = [
            o
            for o in opportunities
            if o["automation_score"] >= 0.7 and o["complexity"] > 5

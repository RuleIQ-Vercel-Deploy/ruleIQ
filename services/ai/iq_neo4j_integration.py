...
class IQNeo4jIntegration:
    ...
    def _generate_enforcement_recommendations(self, patterns: List[Dict]) -> List[str]:
        ...
        if high_frequency:
            recommendations.append(f"Implement automated monitoring for {len(high_frequency)} high-frequency violation areas")
            
        return recommendations
        
    def _identify_automatable_controls(self, controls: List[str]) -> List[str]:
        """Identify which controls can be automated."""
        automatable_keywords = ['monitoring', 'logging', 'scanning', 'detection', 'reporting', 
                               'validation', 'encryption', 'backup', 'audit']
    ...
    def _create_automation_roadmap(self, opportunities: List[Dict]) -> List[Dict]:
        ...
        phase1 = [o for o in opportunities if o['automation_score'] >= 0.7 and o['complexity'] <= 5]
        if phase1:
            roadmap.append({
                'phase': 'Quick Wins',
                'timeline': '1-3 months',
        ...
        phase2 = [o for o in opportunities if o['automation_score'] >= 0.7 and o['complexity'] > 5]
        if phase2:
            roadmap.append({

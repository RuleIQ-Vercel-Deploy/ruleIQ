...
class IQNeo4jIntegration:
    ...
    async def assess_compliance_risk(self, business_profile: Dict[str, Any]
        ) ->Dict[str, Any]:
        """
        ...
        async with self.driver.session() as session:
            result = await session.run(query, {'industry': business_profile
                .get('industry', 'general'), 'country': business_profile.
                get('country', 'Global'), 'revenue': business_profile.get(
                'annual_revenue', 0), 'employees': business_profile.get(
                'employee_count', 0)})
            async for record in result:
                ...
                regulation = {'id': record['regulation_id'], 'title':
                    record['title'], 'base_risk_score': base_risk,
                    'adjusted_risk_score': adjusted_risk,
                    'enforcement_frequency': record['enforcement_freq'],
                    'max_penalty': record['max_penalty'],
                    'automation_potential': record['automation_potential'] or
                    0.5, 'complexity': record['complexity'] or 5,
                    'timeline': record['timeline'], 'enforcement_count':
                    record['enforcement_count'], 'total_penalties': record[
                    'total_penalties'] or 0, 'suggested_controls': record[
                    'suggested_controls'], 'related_regulations': record[
                    'related_regulations'], 'evidence_templates': record[
                    'evidence_templates']}
                ...
                if adjusted_risk >= 8:
                    high_risk_count += 1
                if adjusted_risk >= 9:
                    critical_regulations.append({'title': record['title'],
                        'risk': adjusted_risk, 'max_penalty': record[
                        'max_penalty']})
        avg_risk = total_risk_score / len(applicable_regulations
            ) if applicable_regulations else 0
        ...
        return {'applicable_regulations': applicable_regulations,
            ...
            'average_risk_score': round(avg_risk, 2), 'high_risk_count':
            high_risk_count, 'critical_regulations': critical_regulations,
            'risk_level': risk_level, 'automation_potential': self.
            _calculate_automation_potential(applicable_regulations),
            'estimated_implementation_months': self._estimate_timeline(
            applicable_regulations), 'top_controls': self.
            _identify_top_controls(applicable_regulations)}

    async def get_compliance_roadmap(self, business_profile: Dict[str, Any]
        ) ->List[Dict[str, Any]]:
        """
        ...
        roadmap = []
        processed = set()
        for reg in sorted(regulations, key=lambda x: x[
            'adjusted_risk_score'], reverse=True):
            if reg['id'] in processed:
                continue
            deps = await self._get_regulation_dependencies(reg['id'])
            for dep in deps:
                if dep['id'] not in processed:
                    roadmap.append({'phase': 'Foundation', 'regulation_id':
                        dep['id'], 'title': dep['title'], 'reason':
                        f"Dependency for {reg['title']}", 'risk_score': dep
                        ['risk_score'], 'timeline': dep['timeline'],
                        'controls': dep['controls']})
                    processed.add(dep['id'])
            phase = self._determine_phase(reg['adjusted_risk_score'])
            roadmap.append({'phase': phase, 'regulation_id': reg['id'],
                'title': reg['title'], 'reason': self.
                _get_prioritization_reason(reg), 'risk_score': reg[
                'adjusted_risk_score'], 'timeline': reg['timeline'],
                'controls': reg['suggested_controls'][:5]})
            processed.add(reg['id'])
        return roadmap

    async def find_control_overlaps(self, regulation_ids: List[str]) ->Dict[
        str, Any]:
        """
        ...
        async with self.driver.session() as session:
            ...
            overlaps = []
            async for record in result:
                overlaps.append({'control': record['control_name'],
        ...
        efficiency_gain = (1 - overlapping_controls / total_controls
            ) * 100 if total_controls > 0 else 0
        return {'overlapping_controls': overlaps, 'total_unique_controls':
            total_controls, 'overlapping_control_count':
            overlapping_controls, 'efficiency_gain_percentage': round(
            efficiency_gain, 1), 'recommended_implementation_order': self.
            _optimize_control_order(overlaps)}

    async def analyze_enforcement_patterns(self, industry: str,
        ...
        async with self.driver.session() as session:
            ...
            async for record in result:
                patterns.append({'regulation': record['regulation_title'],
        ...
        return {'enforcement_patterns': patterns,
            'total_industry_penalties': total_industry_penalties,
            'trending_violations': trending, 'high_risk_areas': patterns[:5
            ] if patterns else [], 'recommendations': self.
            _generate_enforcement_recommendations(patterns)}

    async def get_automation_opportunities(self, regulation_ids: List[str]
        ) ->Dict[str, Any]:
        """
        ...
        async with self.driver.session() as session:
            ...
            async for record in result:
                ...
                opportunity = {'regulation_id': record['regulation_id'],
                    'title': record['title'], 'automation_score':
                    automation_score, 'complexity': record['complexity'] or
                    5, 'automatable_controls': self.
                    _identify_automatable_controls(record['controls']),
                    'automation_category': self._categorize_automation(
                    automation_score)}
                opportunities.append(opportunity)
                if automation_score >= 0.7:
                    high_automation.append(opportunity)
        avg_automation = total_automation_score / len(opportunities
        ...
        return {'automation_opportunities': opportunities,
            ...
            'estimated_efficiency_gain': self._estimate_efficiency_gain(
            opportunities), 'implementation_roadmap': self.
            _create_automation_roadmap(opportunities)}

    def _categorize_risk_level(self, avg_risk: float, high_risk_count: int
        ) ->str:
    ...
    def _calculate_automation_potential(self, regulations: List[Dict]) ->float:
        ...
        if not regulations:
            return 0
        scores = [r['automation_potential'] for r in regulations if r[
    ...
    def _identify_top_controls(self, regulations: List[Dict]) ->List[str]:
        ...
        sorted_controls = sorted(control_counts.items(), key=lambda x: x[1],
            reverse=True)
        return [control for control, _ in sorted_controls[:10]]

    async def _get_regulation_dependencies(self, regulation_id: str) ->List[
        Dict]:
        ...
        async with self.driver.session() as session:
            ...
            async for record in result:
                deps.append({'id': record['id'], 'title': record['title'],
                    'risk_score': record['risk_score'] or 5, 'timeline':
                    record['timeline'], 'controls': record['controls']})
        return deps
    ...
    def _get_prioritization_reason(self, regulation: Dict) ->str:
        ...
        if regulation['adjusted_risk_score'] >= 9:
            return (
                f"Critical risk (score: {regulation['adjusted_risk_score']}) with {regulation['enforcement_count']} enforcement cases"
                )
        elif regulation['enforcement_count'] > DEFAULT_RETRIES:
            return (
                f"High enforcement activity ({regulation['enforcement_count']} cases, ${regulation['total_penalties']:,.0f} in penalties)"
                )
        elif regulation['adjusted_risk_score'] >= 7:
            return (
                f"High risk regulation (score: {regulation['adjusted_risk_score']})"
                )
        else:
    ...
    def _optimize_control_order(self, overlaps: List[Dict]) ->List[Dict]:
        ...
        return sorted(overlaps, key=lambda x: (x['coverage_ratio'], x[
            'regulation_count']), reverse=True)[:10]
            ...
            for case in pattern.get('recent_cases', []):
                ...
                if violation:
                    violation_counts[violation] = violation_counts.get(
                        violation, 0) + 1
        sorted_violations = sorted(violation_counts.items(), key=lambda x:
            x[1], reverse=True)
        return [v[0] for v in sorted_violations[:5]]

    def _generate_enforcement_recommendations(self, patterns: List[Dict]
        ) ->List[str]:
        ...
        high_frequency = [p for p in patterns if p['enforcement_count'] >=
            DEFAULT_RETRIES]
        if high_frequency:
            recommendations.append(
    ...
    def _identify_automatable_controls(self, controls: List[str]) ->List[str]:
        ...
        automatable_keywords = ['monitoring', 'logging', 'scanning',
            'detection', 'reporting', 'validation', 'encryption', 'backup',
            'audit']
        automatable = []
        for control in controls:
            if any(keyword in control.lower() for keyword in
    ...
    def _estimate_efficiency_gain(self, opportunities: List[Dict]) ->Dict[
        str, Any]:
        """Estimate efficiency gain from automation."""
        high_auto = len([o for o in opportunities if o['automation_score'] >=
            0.7])
        medium_auto = len([o for o in opportunities if 0.4 <= o[
            'automation_score'] < 0.7])
        time_saved_hours = high_auto * 40 + medium_auto * 20
        cost_saved = time_saved_hours * 150
        return {'estimated_hours_saved_annually': time_saved_hours,
    ...
    def _create_automation_roadmap(self, opportunities: List[Dict]) ->List[Dict
        ...
        roadmap = []
        phase1 = [o for o in opportunities if o['automation_score'] >= 0.7 and
            o['complexity'] <= DEFAULT_RETRIES]
        if phase1:
            roadmap.append({'phase': 'Quick Wins', 'timeline': '1-3 months',
                'items': phase1[:5], 'expected_impact': 'High'})
        phase2 = [o for o in opportunities if o['automation_score'] >= 0.7 and
            o['complexity'] > DEFAULT_RETRIES]
        if phase2:
            roadmap.append({'phase': 'Strategic Automation', 'timeline':
                '3-6 months', 'items': phase2[:5], 'expected_impact': 'High'})
        phase3 = [o for o in opportunities if 0.4 <= o['automation_score'] <
            0.7]
        if phase3:
            roadmap.append({'phase': 'Incremental Improvements', 'timeline':
                '6-12 months', 'items': phase3[:5], 'expected_impact':
                'Medium'})
        return roadmap

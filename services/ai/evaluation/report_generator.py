...
class ReportGenerator:
    ...
    def generate_report(self, evaluation_result: Dict[str, Any], format:
        ...
        timestamp = evaluation_result.get('timestamp', datetime.now(
            timezone.utc).isoformat())
        report_name = f"evaluation_report_{timestamp.replace(':', '-')}"
        if format == 'html':
            return self._generate_html_report(evaluation_result,
                report_name, include_charts)
        elif format == 'json':
    ...
    def _generate_html_report(self, result: Dict[str, Any], name: str,
        ...
        html_content = template.render(result=result, charts=charts,
    ...
    def _generate_markdown_report(self, result: Dict[str, Any], name: str
        ) ->Path:
        ...
        if 'summary' in result:
            ...
            md_lines.append(
                f"- **Successful:** {summary.get('successful_tasks', 0)}")
        ...
        if 'comparison' in result:
            ...
            md_lines.append('| Metric | Baseline | Current | Change | Status |',
                )
            md_lines.append('|--------|----------|---------|--------|--------|',
                )
    ...
    def _generate_charts(self, metrics: Dict[str, Any], name: str) ->Dict[
        str, str]:
...
class TrendAnalyzer:
    ...
    def analyze_trends(self, metric_name: str, days_back: int=30) ->Dict[
        str, Any]:
        ...
        return {'metric': metric_name, 'period': f'{days_back} days',
            'data_points': len(df), 'current_value': float(df['value'].iloc
            [-1]), 'mean': float(mean), 'std': float(std), 'trend_slope':
            float(slope), 'trend_direction': 'improving' if slope > 0 else
            'declining', 'anomalies': anomalies.to_dict('records') if not
            anomalies.empty else [], 'history': df.to_dict('records')}

    def generate_trend_report(self, metrics: List[str], days_back: int=30
        ) ->Dict[str, Any]:
        """Generate trend report for multiple metrics."""
        report = {'timestamp': datetime.now(timezone.utc).isoformat(),
            'period': f'{days_back} days', 'metrics': {}}

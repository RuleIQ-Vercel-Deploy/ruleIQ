...
class TestCommonSchemas:
    ...
    def test_reg_citation_valid(self):
        ...
        citation = RegCitation(framework='GDPR', article='Article 32', url=
    ...
    def test_source_meta_valid(self):
        ...
        source = SourceMeta(origin='ico.org.uk', domain='uk', fetched_at=
            datetime.now(timezone.utc), trust_score=0.95)
    ...
    def test_source_meta_confidence_bounds(self):
        ...
        with pytest.raises(ValidationError):
            SourceMeta(origin='external', domain='test.com', fetched_at=
                datetime.now(timezone.utc), trust_score=1.5)
        with pytest.raises(ValidationError):
            SourceMeta(origin='external', domain='test.com', fetched_at=
                datetime.now(timezone.utc), trust_score=-0.1)
    ...
    def test_temporal_validity_date_order(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            TemporalValidity(effective_from=date(2025, 1, 1), effective_to=
                date(2024, 1, 1))
...
class TestComplianceScenario:
    ...
    def test_compliance_scenario_valid(self):
        ...
        scenario = ComplianceScenario(id='CS-001', triggers=['data_breach',
            'personal_data_exposure'], regulation_refs=[RegCitation(
            framework='GDPR', article='Article 33'), RegCitation(framework=
            'GDPR', article='Article 34')], expected_outcome=
            ExpectedOutcome(obligations=['notify_authority_72h',
            'notify_individuals'], risk_level='high',
            enforcement_likelihood=0.8), temporal=TemporalValidity(
            effective_from=date(2018, 5, 25)), version='0.1.0', source=
            SourceMeta(origin='external', domain='ico.org.uk', fetched_at=
            datetime.now(timezone.utc), trust_score=0.9))
    ...
    def test_compliance_scenario_required_fields(self):
        ...
        with pytest.raises(ValidationError):
            ComplianceScenario(id='CS-002', regulation_refs=[RegCitation(
                framework='GDPR', article='Article 5')], expected_outcome=
                ExpectedOutcome(obligations=['maintain_records'],
                risk_level='low'), temporal=TemporalValidity(effective_from
                =date(2018, 5, 25)), version='0.1.0', source=SourceMeta(
                origin='external', domain='test.com', fetched_at=datetime.
                now(timezone.utc)))

    def test_expected_outcome_risk_levels(self):
        """Test ExpectedOutcome risk level validation."""
        for level in ['low', 'medium', 'high', 'critical']:
            outcome = ExpectedOutcome(obligations=['test_obligation'],
                risk_level=level)
...
class TestEvidenceCase:
    ...
    def test_evidence_case_valid(self):
        ...
        evidence_case = EvidenceCase(id='EC-001', obligation_id=
            'OBL-GDPR-32', evidence_items=[EvidenceItem(type='policy',
            description='Data encryption policy', format='document',
            location='/policies/encryption.pdf'), EvidenceItem(type=
            'technical_control', description=
            'AES-256 encryption implementation', format='code', location=
            '/src/crypto/encryption.py')], framework_mapping=[FrameworkMap(
            framework='ISO27001', control_id='A.10.1.1', satisfaction_level
            =0.9)], version='0.1.0', source=SourceMeta(origin='external',
            domain='csrc.nist.gov', fetched_at=datetime.now(timezone.utc),
            trust_score=0.85))
    ...
    def test_evidence_item_types(self):
        ...
        valid_types = ['policy', 'technical_control', 'process',
            'audit_log', 'certification']
        for item_type in valid_types:
            item = EvidenceItem(type=item_type, description=
                'Test description', format='document')
            assert item.type == item_type
        item = EvidenceItem(type='invalid_type', description='Test', format
            ='document')
    ...
    def test_evidence_item_formats(self):
        ...
        for item_format in valid_formats:
            item = EvidenceItem(type='policy', description=
                'Test description', format=item_format)
            assert item.format == item_format
        item = EvidenceItem(type='policy', description='Test', format=
            'invalid_format')
...
class TestRegulatoryQAPair:
    ...
    def test_regulatory_qa_valid(self):
        ...
        qa_pair = RegulatoryQAPair(id='QA-001', question=
            'What is the data breach notification timeline under GDPR?',
            answer=
            "Organizations must notify the supervisory authority within 72 hours of becoming aware of a personal data breach, unless the breach is unlikely to result in a risk to individuals' rights and freedoms."
            , regulation_refs=[RegCitation(framework='GDPR', article=
            'Article 33', url=
            'https://eur-lex.europa.eu/legal-content/EN/TXT/?uri=CELEX:32016R0679#033'
            )], version='0.1.0', source=SourceMeta(origin='external',
            domain='ico.org.uk', fetched_at=datetime.now(timezone.utc),
            trust_score=0.9))
    ...
    def test_regulatory_qa_confidence_bounds(self):
        ...
        qa_pair = RegulatoryQAPair(id='QA-002', question='Test question?',
            answer='Test answer.', regulation_refs=[], version='0.1.0',
            source=SourceMeta(origin='external', domain='test.com',
            fetched_at=datetime.now(timezone.utc), trust_score=0.5))
    ...
    def test_regulatory_qa_empty_refs_allowed(self):
        ...
        qa_pair = RegulatoryQAPair(id='QA-003', question=
            'General compliance question?', answer=
            'General answer without specific regulation.', regulation_refs=
            [], version='0.1.0', source=SourceMeta(origin='external',
...
class TestSchemaIntegration:
    ...
    def test_all_schemas_json_serializable(self):
        ...
        source = SourceMeta(origin='external', domain='test.com',
            fetched_at=datetime.now(timezone.utc), trust_score=0.8)
        temporal = TemporalValidity(effective_from=date(2024, 1, 1))
        scenario = ComplianceScenario(id='CS-TEST', triggers=['test'],
            regulation_refs=[citation], expected_outcome=ExpectedOutcome(
            obligations=['test'], risk_level='low'), temporal=temporal,
            version='0.1.0', source=source)
    ...
    def test_version_field_required(self):
        ...
        with pytest.raises(ValidationError):
            ComplianceScenario(id='CS-NO-VER', triggers=['test'],
                regulation_refs=[], expected_outcome=ExpectedOutcome(
                obligations=['test'], risk_level='low'), temporal=
                TemporalValidity(effective_from=date(2024, 1, 1)), source=
                SourceMeta(origin='external', domain='test.com', fetched_at
                =datetime.now(timezone.utc)))
        with pytest.raises(ValidationError):
            EvidenceCase(id='EC-NO-VER', obligation_id='OBL-001',
                evidence_items=[], framework_mapping=[], source=SourceMeta(
                origin='external', domain='test.com', fetched_at=datetime.
                now(timezone.utc)))

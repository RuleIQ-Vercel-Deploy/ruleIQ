...
class TestComplianceScenario:
    ...
    def test_expected_outcome_risk_levels(self):
        ...
        # Risk level field accepts any string (no enum validation)
        outcome = ExpectedOutcome(
            obligations=["test"], risk_level="extreme"  # Schema accepts any string
        )
    ...
    def test_expected_outcome_enforcement_bounds(self):
        ...
        with pytest.raises(ValidationError):
            ExpectedOutcome(
                obligations=["test"],
                risk_level="low",
                enforcement_likelihood=1.5,  # > 1.0
            )
...
class TestEvidenceCase:
    ...
    def test_evidence_case_valid(self):
        ...
        evidence_case = EvidenceCase(
            ...
            framework_mapping=[
                FrameworkMap(
                    framework="ISO27001", control_id="A.10.1.1", satisfaction_level=0.9
                )
            ],
            version="0.1.0",
            source=SourceMeta(
    ...
    def test_evidence_item_types(self):
        ...
        for item_type in valid_types:
            item = EvidenceItem(
                type=item_type, description="Test description", format="document"
            )
            assert item.type == item_type

        # Type field accepts any string (no enum validation)
    ...
    def test_evidence_item_formats(self):
        ...
        for item_format in valid_formats:
            item = EvidenceItem(
                type="policy", description="Test description", format=item_format
            )
            assert item.format == item_format

        # Format field accepts any string (no enum validation)
    ...
    def test_framework_map_satisfaction_bounds(self):
        ...
        with pytest.raises(ValidationError):
            FrameworkMap(
                framework="SOC2", control_id="CC1.1", satisfaction_level=1.2  # > 1.0
            )

        with pytest.raises(ValidationError):
            FrameworkMap(
                framework="SOC2", control_id="CC1.1", satisfaction_level=-0.1  # < 0.0
            )
...
class TestRegulatoryQAPair:
    ...
    def test_regulatory_qa_confidence_bounds(self):
        ...
        # RegulatoryQAPair can be created with minimal required fields
        qa_pair = RegulatoryQAPair(
            id="QA-002",
    ...
    def test_regulatory_qa_empty_refs_allowed(self):
        ...
        qa_pair = RegulatoryQAPair(
            ...
            regulation_refs=[],  # Empty is allowed
            version="0.1.0",
            source=SourceMeta(
                origin="external",
                domain="test.com",
...
class TestSchemaIntegration:
    ...
    def test_all_schemas_json_serializable(self):
        ...
        # Create instances of all schemas
    ...
    def test_version_field_required(self):
        ...
        # ComplianceScenario without version
        with pytest.raises(ValidationError):
            ComplianceScenario(
                id="CS-NO-VER",
                triggers=["test"],

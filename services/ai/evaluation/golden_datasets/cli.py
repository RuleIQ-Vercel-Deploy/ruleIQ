...
from ..metrics import coverage_summary, dataset_quality_summary
    ...
    for dataset_type, data in datasets.items():
        if data:
            ...
            if data and hasattr(data[0], "source"):
                source_meta = {"domain": data[0].source.domain, "fetched_at": data[0].source.fetched_at}
                trust_scores[dataset_type] = external_validator.score_trustworthiness(data, source_meta)
    report = {
        "metadata": {
...
def main() -> None:
    ...
    parser.add_argument(
        "--root", type=Path, default=Path("data/golden_datasets"), help="Root directory for golden datasets"
    )
    parser.add_argument("--version", type=str, required=True, help="Dataset version (e.g., 0.1.0 or v0.1.0)")
    parser.add_argument("--outdir", type=Path, default=Path("artifacts"), help="Output directory for reports")

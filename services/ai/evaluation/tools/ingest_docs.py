...
class ManifestProcessor:
    ...
    def get_priority_documents(self, min_priority: int = 4) -> List[Dict[str, Any]]:
        ...
        logger.info(
            f"Found {len(priority_docs)} documents with priority >= {min_priority}"
        )
...
class GoldenDatasetBuilder:
    ...
    def ingest_document(self, golden_doc: GoldenDoc) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Created {len(chunks)} chunks for document {golden_doc.doc_id}"
            )
...
def main():
    ...
    for idx, doc_info in enumerate(priority_docs, 1):
        logger.info(
            f"Processing document {idx}/{len(priority_docs)}: {doc_info.get('title', 'Unknown')}"
        )
    ...
    logger.info(
        f"Successfully processed: {success_count}/{len(priority_docs)} documents"
    )

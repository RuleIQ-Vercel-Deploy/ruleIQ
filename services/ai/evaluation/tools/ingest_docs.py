...
from services.ai.evaluation.tools.ingestion import ChunkProcessor, DocumentProcessor, EmbeddingGenerator, GraphIngestion
...
class GoldenDatasetBuilder:
    ...
    def process_manifest_document(self, doc_info: Dict[str, Any]) -> Optional[GoldenDoc]:
        ...
        golden_doc = GoldenDoc(
            ...
            expected_outcomes=[{"category": category, "confidence": doc_info.get("priority", 3) / 5.0}],
        )
        return golden_doc
...
def main() -> Any:
    ...
    parser.add_argument(
        "--min-priority", type=int, default=4, help="Minimum priority for documents to ingest (default: 4)"
    )
    ...
    for idx, doc_info in enumerate(priority_docs, 1):
        logger.info("Processing document %s/%s: %s" % (idx, len(priority_docs), doc_info.get("title", "Unknown")))

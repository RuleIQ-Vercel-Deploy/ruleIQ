...
class RetryConfig:
    ...
    def __post_init__(self) ->None:
        if self.retryable_exceptions is None:
            self.retryable_exceptions = [ModelTimeoutException,
                ModelOverloadedException, AIServiceException]
...
class RetryHandler:
    ...
    async def retry_async(self, operation: Callable, *args, model_name: str
        ...
        for attempt in range(1, self.config.max_attempts + 1):
            attempt_info = RetryAttempt(attempt_number=attempt, model_name=
                model_name, delay=0.0 if attempt == 1 else self.
            ...
            try:
                if attempt > 1:
                    ...
                    self.logger.info(
                        'Retrying %s (attempt %s/%s) for model %s after %ss delay'
                         % (operation_name, attempt, self.config.
                        max_attempts, model_name, attempt_info.delay))
                ...
                if attempt > 1:
                    self.logger.info(
                        'Operation %s succeeded on attempt %s for model %s (duration: %ss)'
                         % (operation_name, attempt, model_name,
                ...
                self.logger.warning(
                    ...
                    (operation_name, model_name, attempt, self.config.
                    max_attempts, e))
        raise ModelRetryExhaustedException(model_name=model_name, attempts=
            self.config.max_attempts, last_error=str(last_exception),
            context={'operation': operation_name, 'retry_history': [{
            'attempt': a.attempt_number, 'delay': a.delay, 'duration': a.
    ...
    def get_retry_statistics(self) ->Dict[str, Any]: if not self.retry_history:
        ...
        return {'total_attempts': len(self.retry_history),
            'successful_attempts': len(successful_attempts),
            'failed_attempts': len(failed_attempts), 'success_rate': len(
            successful_attempts) / len(self.retry_history),
            'average_attempts_to_success': sum(a.attempt_number for a in
            successful_attempts) / len(successful_attempts) if
            successful_attempts else 0, 'common_exceptions': self.
            _get_common_exceptions(), 'average_delay': sum(a.delay for a in
    ...
    def decorator(func) ->Any:
        ...
        if asyncio.iscoroutinefunction(func):
            ...
            def sync_wrapper(*args, **kwargs) ->Any:
                ...
                handler = RetryHandler(config)
                return handler.retry_sync(func, *args, model_name=
                    model_name, operation_name=func.__name__, **kwargs)
            return sync_wrapper
    return decorator
...
AGGRESSIVE_RETRY_CONFIG = RetryConfig(max_attempts=5, base_delay=0.5,
    max_delay=30.0, strategy=RetryStrategy.EXPONENTIAL_BACKOFF)
CONSERVATIVE_RETRY_CONFIG = RetryConfig(max_attempts=2, base_delay=2.0,
    max_delay=10.0, strategy=RetryStrategy.LINEAR_BACKOFF)
...
def get_retry_handler(config_type: str='default') ->RetryHandler: global _default_retry_handler, _aggressive_retry_handler, _conservative_retry_handler
    if config_type == 'aggressive':
        ...
        if _conservative_retry_handler is None:
            _conservative_retry_handler = RetryHandler(

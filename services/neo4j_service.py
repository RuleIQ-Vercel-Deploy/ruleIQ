...
class Neo4jGraphRAGService:
    ...
    async def health_check(self) -> Dict[str, Any]:
        """Check Neo4j service health status"""
        try:
            # First check if driver is initialized
            if self.driver is None:
                return {
                    "status": "error",
                    "connected": False,
            ...
            is_connected = await self._verify_connection()
            
            if is_connected:
                # Get additional statistics if connected
                try:
                    ...
                    # Connection works but stats failed - still healthy
                    return {
                        "status": "healthy",
                        "connected": True,
                        "message": "Neo4j operational (stats unavailable)",
    ...
    async def execute_query(
        ...
        def _run_query() -> List[Dict[str, Any]]:
            ...
            with self.driver.session(database=self.database, default_access_mode=access_mode) as session:
    ...
    async def query_by_domain_and_jurisdiction(self, domain: str, jurisdiction: str) -> Dict[str, Any]:

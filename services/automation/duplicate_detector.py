...
class DuplicateDetector:
    ...
    def _generate_content_hash(evidence_data: Dict[str, Any]) -> str:
        ...
        try:
            ...
            return hashlib.sha256(json.dumps(content_to_hash, sort_keys=True, default=str).encode()).hexdigest()
    ...
    async def is_duplicate(
        ...
        try:
            ...
            logger.error("Database error while checking for duplicates for user %s: %s" % (user_id, e), exc_info=True)
    ...
    async def get_duplicate_statistics(db: AsyncSession, user_id: UUID, days: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error("Database error getting duplicate stats for user %s: %s" % (user_id, e), exc_info=True)
            raise DatabaseException(
                "Failed to retrieve duplicate statistics.",
            ) from e
        except Exception as e:
            logger.error("Unexpected error getting duplicate stats for user %s: %s" % (user_id, e), exc_info=True)

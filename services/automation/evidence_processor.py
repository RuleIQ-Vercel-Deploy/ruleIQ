...
REASONING: [brief explanation]"""
    ...
    def _fallback_classification(self, evidence: EvidenceItem) -> Dict[str, Any]:
        ...
        try:
            ...
            elif any(keyword in name_lower + desc_lower for keyword in ['training', 'certificate', 'course', 'education']):
    ...
    async def enrich_evidence_with_ai(self, evidence: EvidenceItem) -> EvidenceItem:
        ...
        try:
            ...
            if classification['confidence'] >= 60:
                ...
                if (not evidence.evidence_type or
                    evidence.evidence_type == 'unknown' or
                    classification['suggested_type'] != evidence.evidence_type):
                    evidence.evidence_type = classification['suggested_type']
                    logger.info(f"Updated evidence {evidence.id} type to {classification['suggested_type']} (confidence: {classification['confidence']}%)")
            ...
            logger.info(f"AI enrichment completed for evidence {evidence.id} with confidence {classification['confidence']}%")
            return evidence
    ...
    def process_evidence_with_ai(self, evidence: EvidenceItem) -> None:
        ...
        try:
            ...
            logger.error(f"Failed to process evidence {evidence.id} with AI preparation: {e}", exc_info=True)

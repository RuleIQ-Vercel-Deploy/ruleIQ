...
class EvidenceProcessor:
    ...
    def process_evidence(self, evidence: EvidenceItem) -> None:
        ...
        try:
            ...
            raw_data_for_hash = (
                json.loads(evidence.raw_data) if isinstance(evidence.raw_data, str) else evidence.raw_data
            )
            content_to_hash = {
            ...
            content_hash = hashlib.sha256(json.dumps(content_to_hash, sort_keys=True, default=str).encode()).hexdigest()
            ...
            logger.error("Failed to process evidence %s due to data error: %s" % (evidence.id, e), exc_info=True)
            metadata = evidence.metadata or {}
    ...
    async def get_processing_stats(self, user_id: UUID, days: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error("Database error while getting processing stats for user %s: %s" % (user_id, e), exc_info=True)
            raise DatabaseException("Failed to retrieve processing statistics.") from e
        except Exception as e:
            logger.error("Unexpected error getting processing stats for user %s: %s" % (user_id, e), exc_info=True)
...
REASONING: [brief explanation]"""
    ...
    def _parse_classification_response(self, response_text: str, evidence: EvidenceItem) -> Dict[str, Any]:
    ...
    def _fallback_classification(self, evidence: EvidenceItem) -> Dict[str, Any]:
        ...
        try:
            ...
            if any(keyword in name_lower + desc_lower for keyword in ["policy", "procedure", "standard", "guideline"]):
                suggested_type = "policy_document"
                controls = ["A.5.1.1", "A.5.1.2"]
            elif any(
                keyword in name_lower + desc_lower for keyword in ["training", "certificate", "course", "education"]
            ):
                suggested_type = "training_record"
                controls = ["A.7.2.2", "A.7.2.3"]
            elif any(keyword in name_lower + desc_lower for keyword in ["audit", "assessment", "review", "evaluation"]):
                suggested_type = "audit_report"
                controls = ["A.9.2.1", "A.9.2.2"]
            elif any(keyword in name_lower + desc_lower for keyword in ["config", "setting", "technical", "system"]):
                suggested_type = "technical_config"
                controls = ["A.12.6.1", "A.12.6.2"]
            elif any(keyword in name_lower + desc_lower for keyword in ["access", "permission", "user", "login"]):
                suggested_type = "access_control"
                controls = ["A.9.1.1", "A.9.1.2"]
            elif any(keyword in name_lower + desc_lower for keyword in ["incident", "breach", "security event"]):
                suggested_type = "incident_report"
                controls = ["A.16.1.1", "A.16.1.2"]
            elif any(keyword in name_lower + desc_lower for keyword in ["contract", "agreement", "dpa", "baa"]):
                suggested_type = "contract_agreement"
                controls = ["A.15.1.1", "A.15.1.2"]
            elif any(keyword in name_lower + desc_lower for keyword in ["certificate", "certification", "attestation"]):
                suggested_type = "certification"
                controls = ["A.18.1.1", "A.18.1.2"]
            elif any(keyword in name_lower + desc_lower for keyword in ["log", "monitoring", "alert", "event"]):
                suggested_type = "monitoring_log"
                controls = ["A.12.4.1", "A.12.4.2"]
            elif any(keyword in name_lower + desc_lower for keyword in ["risk", "threat", "vulnerability", "impact"]):
    ...
    async def enrich_evidence_with_ai(self, evidence: EvidenceItem) -> EvidenceItem:
        ...
        try:
            ...
            if classification["confidence"] >= MINUTE_SECONDS:
                if (
                    ...
                    logger.info(
                        ...
                        % (evidence.id, classification["suggested_type"], classification["confidence"])
                    )
            ...
            logger.error("Failed to enrich evidence %s with AI: %s" % (evidence.id, e), exc_info=True)
            return evidence
    ...
    def process_evidence_with_ai(self, evidence: EvidenceItem) -> None:
        ...
        try:
            ...
            logger.error("Failed to process evidence %s with AI preparation: %s" % (evidence.id, e), exc_info=True)
            self.process_evidence(evidence)

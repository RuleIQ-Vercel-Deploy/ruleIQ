...
RECOMMENDATIONS: [list recommendations]"""
    ...
    def _parse_quality_response(self, response_text: str) -> Dict[str, Any]:
        ...
        try:
            ...
            for line in lines:
                ...
                # Parse scores
                if ':' in line and any(dimension in line.upper() for dimension in
                                     ['COMPLETENESS', 'CLARITY', 'CURRENCY', 'VERIFIABILITY', 'RELEVANCE', 'SUFFICIENCY', 'OVERALL']):
                    parts = line.split(':', 1)
    ...
    def _fallback_quality_analysis(self, evidence: Optional[EvidenceItem]) -> Dict[str, Any]:
        ...
        try:
            if evidence is None:
                return {
                    'scores': {'completeness': 30, 'clarity': 30, 'currency': 30, 'verifiability': 30, 'relevance': 30, 'sufficiency': 30},
                    'overall_score': 30,
    ...
    def _combine_traditional_and_ai_scores(self, traditional_scores: Dict[str, float], ai_analysis: Dict[str, Any]) -> float:
    ...
    async def detect_semantic_duplicates(self, evidence: EvidenceItem, candidate_evidence: List[EvidenceItem],
        ...
        try:
            ...
            logger.error(f"Semantic duplicate detection failed for evidence {evidence.id}: {e}", exc_info=True)
            return []

    async def _analyze_semantic_similarity(self, model, content1: str, content2: str,
                                         evidence1: EvidenceItem, evidence2: EvidenceItem) -> Dict[str, Any]:
...
RECOMMENDATION: [action]"""
    ...
    async def batch_duplicate_detection(self, evidence_items: List[EvidenceItem],

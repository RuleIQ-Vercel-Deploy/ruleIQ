...
This is an automated security alert from RuleIQ.
        ...
        try:
            ...
            logger.info(f"Security alert sent to {user.email} for failed login attempts")
    ...
    async def send_password_change_notification(
        ...
        smtp_host = os.getenv("SMTP_HOST")
        if not smtp_host:
            logger.warning("Email configuration missing, skipping password change notification")
            return

...
class AgenticRAGSystem:
    ...
    def __init__(self) -> None:
        ...
        self.use_knowledge_graph = (
            os.getenv("USE_KNOWLEDGE_GRAPH", "false").lower() == "true"
        )
            ...
            with self.engine.connect() as conn:
                ...
                try:
                    ...
                    if existing_table:
                        ...
                        conn.execute(
                            text("DROP TABLE IF EXISTS documentation_chunks CASCADE")
                        )
    ...
    async def _store_documentation_chunk(
        ...
        try:
            ...
            try:
                ...
                logger.error(
                    f"Fallback also failed for chunk {chunk_id}: {fallback_error}"
                )
                raise
            ...
            for line in lines:
                if line.startswith("```"):
                    if in_code_block:
                        # End of code block
                        if (
                            current_code and len("\n".join(current_code)) >= 100
                        ):  # Substantial code
                            code_blocks.append(
                                {
                                    "code": "\n".join(current_code),
    ...
    async def _store_code_example(
        ...
        try:
            ...
            if result.data:
                ...
                logger.warning(
                    f"No data returned when storing code example {example_id}"
                )
            ...
            try:
                ...
                logger.error(
                    f"Fallback also failed for code example {example_id}: {fallback_error}"
                )
                raise
                ...
                return [
                    {
                        ...
                        "similarity": float(
                            row[5]
                        ),  # Using rank as similarity for consistency
                    }
                    for row in result.fetchall()
                ]
        except Exception as e:
    ...
    def _combine_search_results(
        ...
        combined_results = sorted(
            result_dict.values(), key=lambda x: x["similarity"], reverse=True
        )

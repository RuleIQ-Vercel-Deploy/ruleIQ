...
class AgenticRAGSystem:
        ...
        try:
            ...
            for i, chunk in enumerate(chunks):
                ...
                if self.use_contextual_embeddings:
                    contextual_content = await self._add_contextual_information(chunk["content"], content, framework)
                ...
                if self.use_agentic_rag and "code" in chunk["content"].lower():
                    await self._extract_and_store_code_examples(chunk["content"], framework, chunk_id)
    ...
    async def _generate_embedding(self, text: str) -> List[float]:
        ...
        try:
            if self.use_mistral_embeddings and self.mistral_client:
                response = self.mistral_client.embeddings.create(model=self.embedding_model, inputs=[text])
                return response.data[0].embedding
            elif self.openai_client:
                response = await self.openai_client.embeddings.create(model="text-embedding-3-small", input=text)
                return response.data[0].embedding
            else:
                ...
                if line.startswith("```"):
                    if in_code_block:
                        if current_code and len("\n".join(current_code)) >= DEFAULT_LIMIT:
                            code_blocks.append(
                                {"code": "\n".join(current_code), "language": current_language or "python"}
                            )
                        current_code = []
                        current_language = None
                        in_code_block = False
    ...
    async def _store_code_example(
        ...
        try:
            ...
            try:
                ...
                logger.error("Fallback also failed for code example %s: %s" % (example_id, fallback_error))
                raise

    async def query_documentation(
        self, query: str, source_filter: Optional[str] = None, query_type: str = "documentation", max_results: int = 5
    ) -> AgenticRAGResponse:
        """
        ...
        try:
            ...
            if query_type == "code_examples" and self.use_agentic_rag:
                results = await self._search_code_examples(query_embedding, query, source_filter, max_results)
            elif query_type == "hybrid" and self.use_hybrid_search:
                results = await self._hybrid_search(query_embedding, query, source_filter, max_results)
            else:
                results = await self._search_documentation(query_embedding, query, source_filter, max_results)
    ...
    async def _search_documentation(
        self, query_embedding: List[float], query: str, source_filter: Optional[str], max_results: int
    ) -> List[Dict[str, Any]]:
    ...
    async def _hybrid_search(
        ...
        try:
            vector_results = await self._search_documentation(query_embedding, query, source_filter, max_results * 2)
    ...
    async def _keyword_search(self, query: str, source_filter: Optional[str], max_results: int) -> List[Dict[str, Any]]:
    ...
    async def _generate_answer(self, query: str, results: List[Dict[str, Any]]) -> str:
        ...
        try:
            ...
            """
            response = await self.openai_client.chat.completions.create(
                model=self.llm_model, messages=[{"role": "user", "content": prompt}], max_tokens=1000, temperature=0.1
            )
            return response.choices[0].message.content.strip()
        except Exception as e:
    ...
    async def get_framework_statistics(self) -> Dict[str, Any]:
        ...
        try:
            with self.engine.connect() as conn:
                ...
                stats = {"documentation_chunks": {}, "code_examples": {}, "total_chunks": 0, "total_code_examples": 0}

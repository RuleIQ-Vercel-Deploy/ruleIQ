...
class EnforcementAPIClient:
    ...
    def _parse_fca_enforcement(self, data: Dict) -> Dict[str, Any]:
        ...
        for item in data.get('items', []):
            action = {
                'id': hashlib.md5(f"fca_{item.get('reference', '')}".encode()).hexdigest()[:16],
                'regulator': 'FCA',
                ...
                if response.status == 200:
                    ...
                    for notice in notices[:50]:  # Limit to 50 most recent
                        ...
                        if title and link:
                            ...
                            action = {
                                'id': hashlib.md5(link['href'].encode()).hexdigest()[:16],
                                'regulator': 'FCA',
                                'type': 'enforcement',
    ...
    def _parse_ico_enforcement(self, data: Any) -> Dict[str, Any]:
        ...
        for item in items:
            action = {
                'id': hashlib.md5(f"ico_{item.get('id', '')}".encode()).hexdigest()[:16],
                'regulator': 'ICO',
        ...
        try:
            ...
            async with self.session.get(url) as response:
                if response.status == 200:
                    ...
                    entries = soup.find_all(['article', 'div'], class_=['enforcement-item', 'search-result'])
                    ...
                    for entry in entries[:50]:
                        ...
                        if title:
                            ...
                            action = {
                                'id': hashlib.md5(f"ico_{title.text}".encode()).hexdigest()[:16],
                                'regulator': 'ICO',
    ...
    async def fetch_all_enforcement(self) -> Dict[str, Any]:
        ...
        tasks = [
            self.fetch_fca_enforcement(),
            self.fetch_ico_enforcement()
        ]
        ...
        for result in results:
            if isinstance(result, dict) and result.get('success'):
                ...
                logger.info(f"✅ Fetched {result.get('count')} actions from {result.get('source')}")
            else:
    ...
    def _extract_violation_type(self, text: str) -> str:
        ...
        violation_keywords = [
            'data breach', 'security breach', 'unauthorised access',
            'failure to report', 'misleading', 'market abuse',
            'money laundering', 'terrorist financing', 'sanctions breach',
            'unfair treatment', 'systems and controls', 'governance',
            'GDPR violation', 'consent', 'transparency', 'accountability'
        ]
    ...
    def _calculate_statistics(self, actions: List[Dict]) -> Dict[str, Any]:
        ...
        penalties = [
            action.get('penalty_amount', 0) 
            for action in actions 
            if isinstance(action.get('penalty_amount'), (int, float)) and action.get('penalty_amount', 0) > 0
        ]
        ...
        if penalties:
            ...
            stats['median_penalty'] = sorted_penalties[mid] if len(sorted_penalties) % 2 else (sorted_penalties[mid-1] + sorted_penalties[mid]) / 2
    ...
    async with EnforcementAPIClient() as client:
        ...
        if fca_data['success']:
            ...
            if fca_data['actions']:
                ...
                logger.info(f"  Sample: {sample.get('entity')} - £{sample.get('penalty_amount', 0):,.0f}")
        ...
        if all_data['success']:
            ...
            if stats['top_penalties']:
                ...
                for p in stats['top_penalties'][:3]:
                    logger.info(f"    - {p['entity']}: £{p['amount']:,.0f} ({p['regulator']})")

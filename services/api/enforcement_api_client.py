...
class EnforcementAPIClient:
                ...
                if response.status == HTTP_OK:
                    ...
                    for notice in notices[:50]:
                        ...
                        if title and link:
                            ...
                            action = {
                                "id": hashlib.md5(link["href"].encode(), usedforsecurity=False).hexdigest()[:16],
                                "regulator": "FCA",
                                "type": "enforcement",
    ...
    async def fetch_ico_enforcement(self, limit: int = 50) -> Dict[str, Any]:
        ...
        try:
            async with self.session.get(self.endpoints["ico"]["api"], params={"pageSize": limit}) as response:
                if response.status == HTTP_OK:
                    data = await response.json()
                ...
                if response.status == HTTP_OK:
                    ...
                    for entry in entries[:50]:
                        ...
                        if title:
                            ...
                            amount = self._extract_amount(text)
                            action = {
                                "id": hashlib.md5(f"ico_{title.text}".encode(), usedforsecurity=False).hexdigest()[:16],
                                "regulator": "ICO",
    ...
    async def fetch_all_enforcement(self) -> Dict[str, Any]:
        ...
        for result in results:
            if isinstance(result, dict) and result.get("success"):
                ...
                logger.info("✅ Fetched %s actions from %s" % (result.get("count"), result.get("source")))
            else:
    ...
    async with EnforcementAPIClient() as client:
        ...
        if fca_data["success"]:
            ...
            if fca_data["actions"]:
                ...
                logger.info("  Sample: %s - £%s" % (sample.get("entity"), sample.get("penalty_amount", 0)))

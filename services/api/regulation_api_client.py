...
class RegulationAPIClient:
    ...
    def __init__(self):
        ...
        self.api_endpoints = {
            ...
            "fca_handbook": {"base_url": "https://www.handbook.fca.org.uk", "api_path": "/api/handbook"},
            "ico": {"base_url": "https://ico.org.uk", "enforcement_path": "/action-weve-taken/enforcement/"},
            "eur_lex": {
        ...
        for section in penalty_sections:
            ...
            if any(keyword in section_text for keyword in ["penalty", "fine", "offence", "sanction", "imprisonment"]):
                money_matches = re.findall(
                    "£[\\d,]+(?:\\.\\d{2})?(?:\\s*(?:million|billion))?", section_text, re.IGNORECASE
                )
                for match in money_matches[:3]:
                    penalties.append({"type": "financial", "amount": match, "context": section_text[:200]})
                percent_matches = re.findall(
                    "(\\d+(?:\\.\\d+)?)\\s*%\\s*of\\s*(?:annual\\s*)?(?:global\\s*)?(?:turnover|revenue)", section_text
                )
                for match in percent_matches[:2]:
                    penalties.append(
                        {"type": "percentage", "amount": f"{match}% of turnover", "context": section_text[:200]}
                    )
    ...
    def _extract_dates_from_xml(self, root, namespaces) -> List[Dict[str, Any]]:
        ...
        if commencement:
            dates.append({"type": "commencement", "date": commencement, "description": "Regulation comes into force"})
        enactment = self._get_xml_text(root, ".//ukm:EnactmentDate", namespaces, "")
        if enactment:
            dates.append({"type": "enactment", "date": enactment, "description": "Date of enactment"})
        for element in root.findall(".//leg:P1", namespaces):
            text = "".join(element.itertext()).strip()
            date_patterns = [

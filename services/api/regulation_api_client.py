...
class RegulationAPIClient:
    ...
    def _parse_clml_xml(self, xml_content: str, url: str) -> Dict[str, Any]:
        ...
        try:
            ...
            doc_type = self._get_xml_text(
                root, ".//ukm:DocumentMainType", namespaces, ""
            )
    ...
    def _get_xml_text(
        self, element, xpath: str, namespaces: dict, default: str = ""
    ) -> str:
        ...
        for section in penalty_sections:
            ...
            if any(
                ...
                percent_matches = re.findall(
                    r"(\d+(?:\.\d+)?)\s*%\s*of\s*(?:annual\s*)?(?:global\s*)?(?:turnover|revenue)",
                    section_text,
                )
                ...
                if "imprison" in section_text:
                    prison_match = re.search(
                        r"(?:up to\s*)?(\d+)\s*(?:year|month)", section_text
                    )
                    if prison_match:
                        penalties.append(
                            {
    ...
    def _extract_dates_from_xml(self, root, namespaces) -> List[Dict[str, Any]]:
        ...
        commencement = self._get_xml_text(
            root, ".//ukm:CommencementDate", namespaces, ""
        )
        if commencement:
            dates.append(
                {
        ...
        for element in root.findall(".//leg:P1", namespaces):
            ...
            for pattern in date_patterns:
                ...
                for match in matches[:2]:  # Limit to 2 per pattern
                    dates.append(
                        {"type": "deadline", "date": match, "context": text[:150]}
                    )
                    ...
                    updates.append(
                        {
                            ...
                            "updated": self._get_xml_text(
                                entry, "atom:updated", namespaces
                            ),
                            "link": (
                                entry.find("atom:link", namespaces).get("href")

...
class RegulationAPIClient:
    ...
    def __init__(self):
        ...
        # Headers that work with legislation.gov.uk
        self.headers = {
        ...
        self.api_endpoints = {
            "uk_legislation": {
                ...
                "data_format": "/data.xml",  # CLML XML format
                "atom_feed": "/data.feed",
    ...
    async def fetch_uk_legislation(self, url: str) -> Dict[str, Any]:
        ...
        logger.info(f"Fetching UK legislation from: {api_url}")
        ...
        if cache_file.exists():
            logger.info(f"Using cached data for {api_url}")
            with open(cache_file, 'r') as f:
                return json.load(f)
        
        # Use synchronous requests in executor to avoid aiohttp issues
        ...
        try:
            ...
            response = await loop.run_in_executor(
                self.executor,
                lambda: requests.get(api_url, headers=self.headers, timeout=30)
            )
            ...
            if response.status_code == 200:
                ...
                logger.info(f"Received {len(xml_content)} characters of XML")
                ...
                logger.error(f"Failed to fetch {api_url}: {response.status_code}")
                logger.error(f"Error response: {response.text[:500]}")
                return {"success": False, "error": f"HTTP {response.status_code}: {response.text[:200]}"}
                    
        except Exception as e:
            logger.error(f"Error fetching {api_url}: {e}")
    ...
    def _parse_clml_xml(self, xml_content: str, url: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Extract requirements (obligations)
            ...
            logger.error(f"Error parsing CLML XML: {e}")
            return {
                "success": False,
    ...
    def _extract_requirements_from_xml(self, root, namespaces) -> List[Dict[str, Any]]:
        ...
        # Look for provisions that contain obligations
        # Common patterns: "shall", "must", "required to", etc.
        provisions = root.findall('.//leg:P1', namespaces) or root.findall('.//leg:P', namespaces)
        
        for provision in provisions[:50]:  # Limit to first 50
            text_content = ''.join(provision.itertext()).strip()
            
            # Check if this is a requirement
            if any(keyword in text_content.lower() for keyword in ['shall', 'must', 'required', 'obliged', 'duty']):
                # Get provision number
                prov_num = provision.get('id', '')
                
                requirements.append({
                    "id": prov_num,
                    "text": text_content[:500],  # Truncate long text
                    "type": "mandatory",
    ...
    def _extract_penalties_from_xml(self, root, namespaces) -> List[Dict[str, Any]]:
        ...
        # Look for sections about penalties, offences, sanctions
        ...
        for section in penalty_sections:
            ...
            if any(keyword in section_text for keyword in ['penalty', 'fine', 'offence', 'sanction', 'imprisonment']):
                # Extract monetary penalties
                money_matches = re.findall(r'£[\d,]+(?:\.\d{2})?(?:\s*(?:million|billion))?', section_text, re.IGNORECASE)
                ...
                # Extract percentage penalties
                percent_matches = re.findall(r'(\d+(?:\.\d+)?)\s*%\s*of\s*(?:annual\s*)?(?:global\s*)?(?:turnover|revenue)', section_text)
                ...
                if 'imprison' in section_text:
                    prison_match = re.search(r'(?:up to\s*)?(\d+)\s*(?:year|month)', section_text)
                    if prison_match:
                        penalties.append({
                            "type": "criminal",
    ...
    def _extract_enforcement_from_xml(self, root, namespaces) -> Dict[str, Any]:
        ...
        # Look for enforcement authorities
        for element in root.findall('.//leg:P1', namespaces):
            text = ''.join(element.itertext()).strip()
            
            # Common UK enforcement authorities
            authorities = ['Information Commissioner', 'FCA', 'Financial Conduct Authority', 
                          'Prudential Regulation Authority', 'PRA', 'Secretary of State',
                          'Ofcom', 'Competition and Markets Authority']
            ...
            # Enforcement powers
            if any(power in text.lower() for power in ['investigate', 'inspect', 'audit', 'enforce', 'prosecute']):
    ...
    def _extract_dates_from_xml(self, root, namespaces) -> List[Dict[str, Any]]:
        ...
        for element in root.findall('.//leg:P1', namespaces):
            ...
            date_patterns = [
                ...
                r'no later than\s+(\d{1,2}\s+\w+\s+\d{4})'
            ]
    ...
    async def fetch_fca_enforcement(self) -> List[Dict[str, Any]]:
        ...
        # FCA provides enforcement data through their API
        url = "https://www.fca.org.uk/news/search-results/enforcement"
        
        try:
            async with self.session.get(url) as response:
                if response.status == 200:
                    # Parse enforcement notices
                    # Note: Real implementation would need proper HTML parsing
                    content = await response.text()
                    
                    enforcements = []
                    # Extract enforcement data (simplified)
                    # In reality, would parse the HTML or use FCA's data API
                    
                    return enforcements
        except Exception as e:
            logger.error(f"Error fetching FCA enforcement: {e}")
            return []
    ...
    async def fetch_regulation_updates(self, since_date: str) -> List[Dict[str, Any]]:
        ...
        try:
            response = await loop.run_in_executor(
                self.executor,
                lambda: requests.get(feed_url, headers=self.headers, timeout=30)
            )
            ...
            if response.status_code == 200:
                ...
                for entry in entries[:20]:  # Latest 20 updates
                    updates.append({
                        "title": self._get_xml_text(entry, 'atom:title', namespaces),
                        "updated": self._get_xml_text(entry, 'atom:updated', namespaces),
                        "link": entry.find('atom:link', namespaces).get('href') if entry.find('atom:link', namespaces) is not None else '',
                        "summary": self._get_xml_text(entry, 'atom:summary', namespaces)
                    })
                    
        except Exception as e:
            logger.error(f"Error fetching regulation updates: {e}")
...
async def test_api_client():
    ...
    async with RegulationAPIClient() as client:
        # Test UK GDPR fetch
        test_urls = [
            "https://www.legislation.gov.uk/ukpga/2018/12/contents",  # UK Data Protection Act 2018
            "https://www.legislation.gov.uk/ukpga/2000/8/contents",  # Human Rights Act
            "https://www.legislation.gov.uk/uksi/2017/692/contents",  # Money Laundering Regulations
        ]
        
        for test_url in test_urls[:1]:  # Test first URL for now
            logger.info(f"Testing with {test_url}")
            result = await client.fetch_uk_legislation(test_url)
        
            if result['success']:
                logger.info(f"✅ Successfully fetched regulation")
                logger.info(f"  Title: {result['metadata']['title']}")
                logger.info(f"  Requirements found: {len(result['requirements'])}")
                logger.info(f"  Penalties found: {len(result['penalties'])}")
                logger.info(f"  Controls identified: {result['controls']}")
        ...
        logger.info(f"Found {len(updates)} recent regulation updates")

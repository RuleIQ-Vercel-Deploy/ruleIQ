...
class ReportGenerator:
    ...
    async def generate_report(
        self, user_id: UUID, business_profile_id: UUID, report_type: str, parameters: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
        """Generate a report based on type and parameters."""
        try:
            profile_res = await self.db.execute(
                select(BusinessProfile).where(
                    and_(BusinessProfile.id == business_profile_id, BusinessProfile.user_id == user_id)
                )
            )
            ...
            logger.info("Generating report '%s' for business profile %s" % (report_type, business_profile_id))
            return await builder(profile, parameters or {})
        except SQLAlchemyError as e:
            logger.error(
                "Database error during report generation for profile %s: %s" % (business_profile_id, e), exc_info=True
            )
            raise DatabaseException("A database error occurred while generating the report.") from e
        except BusinessLogicException as e:
            logger.warning(
                "Business logic error during report generation for profile %s: %s" % (business_profile_id, e)
            )
            raise
        except Exception as e:
            logger.error(
                "Unexpected error during report generation for profile %s: %s" % (business_profile_id, e), exc_info=True
            )
    ...
    async def _get_all_controls(self, framework_id: UUID) -> List[Dict]:
        ...
        try:
            ...
            logger.error("Failed to get controls for framework %s: %s" % (framework_id, e), exc_info=True)
    ...
    async def _get_linked_evidence(self, profile_id: UUID) -> List[EvidenceItem]:
        ...
        try:
            ...
            logger.error("Failed to get evidence for profile %s: %s" % (profile_id, e), exc_info=True)
        ...
        try:
            ...
            active_evidence_res = await self.db.execute(
                select(func.count(EvidenceItem.id)).where(
                    and_(EvidenceItem.business_profile_id == profile_id, EvidenceItem.status == "active")
                )
            )
            policies_res = await self.db.execute(
                select(func.count(GeneratedPolicy.id)).where(GeneratedPolicy.business_profil == profile_id)
            )
            ...
            logger.error("Failed to calculate key metrics for profile %s: %s" % (profile_id, e), exc_info=True)
    ...
    async def _generate_recommendations(self, profile_id: UUID) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.warning(
                "Could not generate recommendations for profile %s due to upstream error: %s" % (profile_id, e)
            )
            raise
        except Exception as e:
            logger.error(
                "Unexpected error generating recommendations for profile %s: %s" % (profile_id, e), exc_info=True
            )

...
class PDFGenerator:
    ...
    def _build_header(self, report_data: Dict) -> List:
        ...
        generated_date = datetime.fromisoformat(report_data['generated_at']).strftime('%B %d, %Y at %I:%M %p')
    ...
    def _build_executive_summary(self, report_data: Dict) -> List:
        ...
        if summary:
            ...
            framework_table = Table(framework_data, colWidths=[1.3*inch, 1*inch, 1.2*inch, 0.8*inch, 1*inch])
                ...
                for category, gap_list in categories.items():
                    for gap in gap_list:
                        ...
                        gap_data.append([
                            framework.upper(),
                            category,
                            Paragraph(gap.get('title', 'Unknown Gap'), self.styles['ReportBodyText']),
                            Paragraph(gap.get('severity', 'Medium').title(), 
                                    ParagraphStyle('temp', textColor=severity_color, fontSize=10, fontName='Helvetica-Bold')),
                            gap.get('remediation_effort', 'Unknown')
                        ])
            
            if len(gap_data) > 1:
                gap_table = Table(gap_data, colWidths=[1*inch, 1*inch, 2.8*inch, 0.8*inch, 0.8*inch])
                gap_table.setStyle(TableStyle([
                    ('BACKGROUND', (0, 0), (-1, 0), self.colors['secondary']),
                    ('TEXTCOLOR', (0, 0), (-1, 0), colors.whitesmoke),
                    ('ALIGN', (0, 0), (-1, -1), 'LEFT'),
                    ('ALIGN', (3, 0), (-1, -1), 'CENTER'),  # Center severity and effort columns
                    ('VALIGN', (0, 0), (-1, -1), 'TOP'),
                    ('FONTNAME', (0, 0), (-1, 0), 'Helvetica-Bold'),
                    ('FONTSIZE', (0, 0), (-1, 0), 10),
                    ('FONTSIZE', (0, 1), (-1, -1), 9),
    ...
    def _build_evidence_report(self, report_data: Dict) -> List:
        ...
        if evidence_summary:
            story.append(Paragraph("Evidence Collection Status by Framework", self.styles['SubsectionHeader']))
            
            evidence_data = [['Framework', 'Total Items', 'Completion %', 'Automation Opportunities']]
            ...
            evidence_table = Table(evidence_data, colWidths=[1.5*inch, 1*inch, 1.2*inch, 1.8*inch])
        ...
        if automation_opportunities:
            story.append(Paragraph("Priority Automation Opportunities", self.styles['SubsectionHeader']))
    ...
    def _build_audit_readiness(self, report_data: Dict) -> List:
        ...
        if readiness_scores:
            ...
            readiness_data = [['Framework', 'Overall Score', 'Evidence %', 'Policy %', 'Gaps Remaining']]
            ...
            readiness_table = Table(readiness_data, colWidths=[1.2*inch, 1*inch, 1*inch, 1*inch, 1*inch])
    ...
    def _build_compliance_status(self, report_data: Dict) -> List:
        ...
        if framework_status:
            ...
            status_table = Table(status_data, colWidths=[1.3*inch, 1*inch, 1.2*inch, 1.2*inch, 0.8*inch])

...
Remember: You are the Chief Compliance Officer. Think strategically, act decisively, and always base your decisions on verifiable evidence accessed through the GraphRAG Query Agent."""
    ...
    async def process_query(self, user_query: str, context: Optional[Dict[
        ...
        try:
            ...
            try:
                ...
                LIMIT 5
                """
                result = await self.neo4j.execute_query(query, {'query':
                    user_query[:50]})
            ...
            return {'status': 'success', 'timestamp': datetime.now(timezone
                .utc).isoformat(), 'summary': {'risk_posture': 'MEDIUM',
                'compliance_score': 0.7, 'top_gaps': [],
            ...
            return {'status': 'error', 'error': str(e), 'summary': {
                'risk_posture': 'UNKNOWN', 'compliance_score': 0.0,
    ...
    async def _perceive_node(self, state: IQAgentState) ->IQAgentState:
        ...
        try:
            coverage_analysis = await execute_compliance_query(QueryCategory
                .REGULATORY_COVERAGE, self.neo4j)
            gap_analysis = await execute_compliance_query(QueryCategory.
                COMPLIANCE_GAPS, self.neo4j)
            if 'regulations' in state.current_query.lower():
                regulation_codes = self._extract_regulation_codes(state.
                    current_query)
                if regulation_codes:
                    cross_jurisdictional = await execute_compliance_query(
                        QueryCategory.CROSS_JURISDICTIONAL, self.neo4j,
                        regulation_codes=regulation_codes)
                    state.graph_context['cross_jurisdictional'
                        ] = cross_jurisdictional.data
            state.graph_context.update({'coverage_analysis':
                coverage_analysis.data, 'coverage_metadata':
                coverage_analysis.metadata, 'compliance_gaps': gap_analysis
                .data, 'gap_metadata': gap_analysis.metadata,
                'perception_timestamp': datetime.now(timezone.utc).isoformat()}
                )
            state.compliance_posture = {'overall_coverage':
                coverage_analysis.metadata.get('overall_coverage', 0.0),
                'total_gaps': gap_analysis.metadata.get('total_gaps', 0),
                'critical_gaps': gap_analysis.metadata.get('critical_gaps',
                0), 'high_risk_gaps': gap_analysis.metadata.get(
                'high_risk_gaps', 0)}
            logger.info('IQ PERCEIVE: Found %s gaps, coverage: %s' % (state
                .compliance_posture['total_gaps'], state.compliance_posture
                ['overall_coverage']))
        except Exception as e:
    ...
    async def _plan_node(self, state: IQAgentState) ->IQAgentState:
        ...
        try:
            risk_convergence = await execute_compliance_query(QueryCategory
                .RISK_CONVERGENCE, self.neo4j)
            temporal_changes = await execute_compliance_query(QueryCategory
                .TEMPORAL_CHANGES, self.neo4j, lookback_months=6,
                forecast_months=3)
            gaps = state.graph_context.get('compliance_gaps', [])
            action_plan = []
            for gap in gaps[:10]:
                action = {'action_id': f"action_{gap['gap_id']}", 'type':
                    'IMPLEMENT_CONTROL', 'priority': gap['requirement'][
                    'risk_level'], 'target': gap['requirement']['title'],
                    'regulation': gap['regulation']['code'], 'risk_level':
                    gap['requirement']['risk_level'], 'severity_score': gap
                    .get('gap_severity_score', 0), 'cost_estimate': self.
                    _estimate_action_cost(gap), 'timeline': self.
                    _estimate_timeline(gap), 'graph_reference': gap['gap_id']}
                action_plan.append(action)
            action_plan.sort(key=lambda x: ({'critical': 1, 'high': 2,
                'medium': 3}.get(x['priority'], 4), -x['severity_score']))
            state.action_plan = action_plan
            state.risk_assessment = {'convergence_patterns': len(
                risk_convergence.data), 'recent_regulatory_changes':
                temporal_changes.metadata.get('recent_changes', 0),
                'overall_risk_level': self._calculate_overall_risk(state.
                compliance_posture), 'planning_horizon': '90_days'}
            state.graph_context.update({'risk_convergence':
                risk_convergence.data, 'temporal_changes': temporal_changes
                .data, 'planning_timestamp': datetime.now(timezone.utc).
                isoformat()})
            logger.info('IQ PLAN: Generated %s prioritized actions' % len(
                action_plan))
        except Exception as e:
        ...
        try:
            ...
            for action in state.action_plan[:3]:
                if await self._should_auto_execute(action):
                    ...
                    await self._store_execution_evidence(action,
                        execution_result)
                else:
            ...
            state.graph_context['actions_executed'] = len([a for a in
                executed_actions if a.get('status') == 'executed'])
    ...
    async def _learn_node(self, state: IQAgentState) ->IQAgentState:
        ...
        logger.info(
            'IQ LEARN: Analyzing enforcement patterns and updating knowledge')
        try:
            enforcement_learning = await execute_compliance_query(QueryCategory
                .ENFORCEMENT_LEARNING, self.neo4j)
            patterns = self._detect_compliance_patterns(state.graph_context,
                state.evidence_collected)
            effectiveness_updates = await self._update_control_effectiveness(
                state.evidence_collected)
            state.patterns_detected = patterns
            state.graph_context.update({'enforcement_insights':
                enforcement_learning.data, 'patterns_detected': patterns,
        ...
        try:
            for pattern in state.patterns_detected:
                await self.memory_manager.store_knowledge_graph_memory(
                    graph_query_result=pattern, query_category=
                    QueryCategory.REGULATORY_COVERAGE, importance_score=0.7)
            relevant_memories = (await self.memory_manager.
                retrieve_contextual_memories(query=state.current_query,
                context=state.graph_context, max_memories=5))
            state.memories_accessed = [m.id for m in relevant_memories.
                retrieved_memories]
            if state.step_count % 10 == 0:
                consolidation = (await self.memory_manager.
                    consolidate_compliance_knowledge())
                state.graph_context['knowledge_consolidation'] = consolidation
            state.graph_context.update({'memories_accessed': len(state.
                memories_accessed), 'memory_consolidation_due': state.
                step_count % 10 == 9, 'remember_timestamp': datetime.now(
                timezone.utc).isoformat()})
            logger.info('IQ REMEMBER: Accessed %s relevant memories' % len(
    ...
    async def _respond_node(self, state: IQAgentState) ->IQAgentState:
        ...
        try:
            llm_response = await self.llm.ainvoke([SystemMessage(content=
                self.system_prompt), HumanMessage(content=response_prompt)])
    ...
    def _format_response(self, state: IQAgentState) ->Dict[str, Any]:
        ...
        risk_posture = self._determine_risk_posture(state.
            compliance_posture, state.risk_assessment)
        return {'status': 'success', 'timestamp': datetime.now(timezone.utc
            ).isoformat(), 'summary': {'risk_posture': risk_posture,
            'compliance_score': state.compliance_posture.get(
            'overall_coverage', 0.0), 'top_gaps': [gap['requirement'][
            'title'] for gap in state.graph_context.get('compliance_gaps',
            [])[:3]], 'immediate_actions': [action['target'] for action in
            state.action_plan[:3]]}, 'artifacts': {'compliance_posture':
            state.compliance_posture, 'action_plan': state.action_plan,
            'risk_assessment': state.risk_assessment}, 'graph_context': {
            'nodes_traversed': len(state.graph_context.get(
            'coverage_analysis', [])), 'patterns_detected': state.
            patterns_detected, 'memories_accessed': state.memories_accessed,
            'learnings_applied': len(state.evidence_collected)}, 'evidence':
            {'controls_executed': len([e for e in state.evidence_collected if
            e.get('status') == 'executed']), 'evidence_stored': len(state.
            evidence_collected), 'metrics_updated': state.graph_context.get
            ('actions_executed', 0)}, 'next_actions': [{'action': action[
            'target'], 'priority': action['priority'].upper(), 'owner':
            'Compliance Team', 'graph_reference': action['graph_reference']
            } for action in state.action_plan[:5]], 'llm_response': state.
            messages[-1].content if state.messages else 'No response generated'
            }

    def _extract_regulation_codes(self, query: str) ->List[str]:
        """Extract regulation codes from query text"""
        common_codes = ['GDPR', '6AMLD', 'DORA', 'BSA', 'MLR2017',
            'DPA2018', 'MIFID2']
    ...
    def _estimate_action_cost(self, gap: Dict[str, Any]) ->float:
        ...
        base_cost = 5000.0
        risk_multiplier = {'critical': 3.0, 'high': 2.0, 'medium': 1.5,
            'low': 1.0}
    ...
    def _estimate_timeline(self, gap: Dict[str, Any]) ->str:
        ...
        risk_level = gap['requirement']['risk_level']
        timeline_map = {'critical': '30_days', 'high': '60_days', 'medium':
            '90_days', 'low': '180_days'}
        return timeline_map.get(risk_level, '90_days')

    def _calculate_overall_risk(self, compliance_posture: Dict[str, Any]
        ) ->str:
    ...
    async def _should_auto_execute(self, action: Dict[str, Any]) ->bool:
        ...
        return (risk_score < self.RISK_THRESHOLD and cost < self.
            AUTONOMY_BUDGET and action['priority'] in ['high', 'critical'])

    async def _execute_action(self, action: Dict[str, Any]) ->Dict[str, Any]:
        """Execute compliance action (placeholder)"""
        return {'action_id': action['action_id'], 'status': 'executed',
            'timestamp': datetime.now(timezone.utc).isoformat(), 'result':
            'success', 'evidence_ref': f"evidence_{action['action_id']}",
            'effectiveness': 0.85}

    async def _create_escalation(self, action: Dict[str, Any]) ->Dict[str, Any
        ]:
        """Create escalation for manual approval"""
        return {'action_id': action['action_id'], 'status': 'escalated',
            'timestamp': datetime.now(timezone.utc).isoformat(), 'reason':
            'Requires manual approval - high risk or cost',
            'approval_required': True}

    async def _store_execution_evidence(self, action: Dict[str, Any],
        result: Dict[str, Any]) ->None:
        """Store execution evidence in graph"""
        pass

    def _detect_compliance_patterns(self, graph_context: Dict[str, Any],
            ...
            for domain, count in domain_gaps.items():
                if count > MAX_RETRIES:
                    patterns.append({'pattern_type':
                        'HIGH_GAP_CONCENTRATION', 'domain': domain,
                        'gap_count': count, 'confidence': 0.8})
        return patterns

    async def _update_control_effectiveness(self, evidence: List[Dict[str,
        Any]]) ->List[Dict[str, Any]]:
        """Update control effectiveness based on execution results"""
        updates = []
        for evidence_item in evidence:
            if evidence_item.get('status') == 'executed':
                updates.append({'control_id': evidence_item['action_id'],
                    'effectiveness': evidence_item.get('effectiveness', 0.8
                    ), 'updated_at': evidence_item.get('timestamp',
...
"""
    ...
    def _determine_risk_posture(self, compliance_posture: Dict[str, Any],
    ...
    async def process_query_with_context(self, user_query: str,
        business_profile_id: Optional[str]=None, session_id: Optional[str]=
        None, context: Optional[Dict[str, Any]]=None) ->Dict[str, Any]:
        """
        ...
        try:
            business_context_str = json.dumps(business_context, indent=2
                ) if business_context else 'No business context available'
        except (TypeError, ValueError):
            business_context_str = str(business_context
                ) if business_context else 'No business context available'
        try:
            compliance_gaps_str = json.dumps(compliance_gaps, indent=2
                ) if compliance_gaps else 'No specific gaps identified'
        except (TypeError, ValueError):
            compliance_gaps_str = str(compliance_gaps
                ) if compliance_gaps else 'No specific gaps identified'
        prompt = f"""
        User Query: {user_query}
        ...
        try:
            ...
            logger.error('LLM invocation failed: %s' % e)
            llm_response = 'Unable to generate AI response at this time.'
        response = {'status': 'success', 'timestamp': datetime.now(timezone
            .utc).isoformat(), 'business_context': business_context,
            'summary': {'risk_posture': 'MEDIUM' if compliance_gaps else
            'LOW', 'compliance_score': 0.75 if not compliance_gaps else 0.5,
            'top_gaps': [gap.get('title', '') for gap in compliance_gaps[:3
            ]], 'immediate_actions': ['Review compliance gaps',
            'Update business profile', 'Implement missing controls']},
            'artifacts': {'compliance_gaps': compliance_gaps, 'neo4j_data':
            {'gaps_found': len(compliance_gaps), 'regulations_checked': [
            'GDPR'] if business_context else []}}, 'evidence': {
            'available_evidence': 0}, 'llm_response': llm_response}
        if self.has_postgres_access and business_profile_id:
            try:
    ...
    async def retrieve_business_context(self, business_profile_id: str) ->Dict[
        str, Any]:
        """
        ...
        try:
            profile_stmt = select(BusinessProfile).where(BusinessProfile.id ==
                business_profile_id)
            result = await self.postgres_session.execute(profile_stmt)
            profile = result.scalars().first()
            if not profile:
                return {'error': 'Business profile not found'}
            evidence_stmt = select(Evidence).where(Evidence.
                business_profile_id == business_profile_id).order_by(Evidence
                .created_at.desc()).limit(10)
            evidence_result = await self.postgres_session.execute(evidence_stmt
                )
            evidence_items = evidence_result.scalars().all()
            return {'profile': {'id': str(profile.id), 'company_name':
                profile.company_name, 'industry': profile.industry,
                'company_size': profile.company_size,
                'handles_personal_data': profile.handles_personal_data,
                'data_processing_activities': profile.
                data_processing_activities, 'created_at': profile.
                created_at.isoformat()}, 'evidence': [{'id': str(item.id),
                'title': item.title, 'description': item.description,
                'evidence_type': item.evidence_type, 'file_path': item.
                file_path, 'created_at': item.created_at.isoformat()} for
                item in evidence_items]}
        except Exception as e:
            logger.error('Error retrieving business context: %s' % e)
            return {'error': str(e)}

    async def retrieve_session_context(self, session_id: str) ->Optional[Dict
        [str, Any]]:
        """
        ...
        try:
            stmt = select(AssessmentSession).where(AssessmentSession.id ==
                session_id)
            result = await self.postgres_session.execute(stmt)
            session = result.scalars().first()
            if session:
                return {'session_id': str(session.id), 'questions_answered':
                    session.questions_answered, 'compliance_score': session
                    .compliance_score, 'risk_level': session.risk_level,
    ...
    async def assess_compliance_with_context(self, business_profile_id: str,
        ...
        assessment = {'status': 'success', 'business_context':
            business_context, 'compliance_assessment': {
            'applicable_regulations': regulations,
            'business_specific_risks': self._identify_business_risks(
            business_context), 'requirements_analysis': neo4j_result.get(
            'data', []), 'recommendations': self.
            _generate_contextualized_recommendations(business_context,
            neo4j_result.get('data', []))}}
        return assessment

    async def search_compliance_resources(self, query: str,
        include_evidence: bool=True, include_regulations: bool=True) ->Dict[
        str, Any]:
        """
        ...
        """
        results = {'query': query, 'regulations': [], 'evidence': [],
            'total_results': 0}
        ...
        if include_evidence and self.has_postgres_access:
            ...
            from database.models.evidence import Evidence
            stmt = select(Evidence.title, Evidence.description).where(or_(
                Evidence.title.ilike(f'%{query}%'), Evidence.description.
                ilike(f'%{query}%'))).limit(10)
            pg_result = await self.postgres_session.execute(stmt)
            evidence_items = pg_result.all()
            results['evidence'] = [{'title': item[0], 'relevance': 0.8} for
                item in evidence_items]
        results['total_results'] = sum(r['matches'] for r in results[
            'regulations']) + len(results['evidence'])
        return results
    ...
    async def _count_available_evidence(self, business_profile_id: str) ->int:
        ...
        from database.models.evidence import Evidence
        stmt = select(func.count(Evidence.id)).where(Evidence.
            business_profile_id == business_profile_id)
        result = await self.postgres_session.execute(stmt)
        return result.scalar() or 0

    def _identify_business_risks(self, business_context: Dict[str, Any]
        ) ->List[str]:
    ...
    def _generate_contextualized_recommendations(self, business_context:
        Dict[str, Any], requirements: List[Dict[str, Any]]) ->List[str]:
        """Generate recommendations based on business context."""
        recommendations = []
        if not business_context or 'profile' not in business_context:
            return [
                'Complete business profile for personalized recommendations']
        profile = business_context['profile']
...
async def create_iq_agent(neo4j_service: Neo4jGraphRAGService
    ...
    try:
        ...
        if graph_stats.get('total_nodes', 0) < 10:
            logger.info(
                'IQ Agent: Empty graph detected, attempting to initialize...')
            try:
                await initialize_compliance_graph()
                logger.info('IQ Agent: Compliance graph initialized')
            except Exception as init_error:
                logger.warning(
                    'IQ Agent: Could not initialize graph (read-only mode?): %s'
                     % init_error)

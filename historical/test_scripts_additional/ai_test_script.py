        ...
        if result.returncode == 0 and response_text:
            try:
                ...
                if status in ["healthy", "degraded"]:
                    results["tests"].append({
                        "test": "backend_health", "status": "pass",
                        "message": f"Backend is responding. Overall status: {status}", "details": response_json
                    })
                    ...
                    results["tests"].append({
                        "test": "backend_health", "status": "fail",
                ...
                print(f"❌ Backend Health: Invalid JSON response")
                return results
        else:
            results["tests"].append({
                "test": "backend_health", "status": "fail",
            ...
            print(f"❌ Backend Health: Failed with no response")
            return results
    except Exception as e:
        results["tests"].append({
            "test": "backend_health", "status": "error",
    ...
    try:
        ...
        if result.returncode == 0 and response_text:
            try:
                response_json = json.loads(response_text)
                results["tests"].append({
                    "test": "ai_help_endpoint", "status": "pass",
                ...
                results["tests"].append({
                    "test": "ai_help_endpoint", "status": "fail",
                    "message": "AI help endpoint responded with invalid JSON.",
                    "details": {"response": response_text}
                })
                print(f"❌ AI Help Endpoint: Invalid JSON response")
        else:
            results["tests"].append({
                "test": "ai_help_endpoint", "status": "fail",
            ...
            print(f"❌ AI Help Endpoint: Failed")
            
    except Exception as e:
        results["tests"].append({
            "test": "ai_help_endpoint", "status": "error",
...
def run_all_tests():
    ...
    passed_tests = sum(1 for phase in all_results["phases"] for test in phase["tests"] if test["status"] == "pass")
